...
#define INFMSG(X)                                                            \
  {                                                                          \
    ...
    if (MsgManager::infoStream())                                            \
      (*(MsgManager::infoStream())) << "INF " << timebuf << " [" << __FILE__ \
                                    << " " << __LINE__ << "] " << X << endl; \
  }
...
#define WRNMSG(X)                                                            \
  {                                                                          \
    ...
    if (MsgManager::warnStream())                                            \
      (*(MsgManager::warnStream())) << "WRN " << timebuf << " [" << __FILE__ \
                                    << " " << __LINE__ << "] " << X << endl; \
  }
...
#define ERRMSG(X)                                                             \
  {                                                                           \
    ...
    if (MsgManager::errorStream())                                            \
      (*(MsgManager::errorStream())) << "ERR " << timebuf << " [" << __FILE__ \
                                     << " " << __LINE__ << "] " << X << endl; \
  }
...
#define INFSTREAM(X)                                                  \
  {                                                                   \
    if (MsgManager::infoStream()) (*(MsgManager::infoStream())) << X; \
  }

// Just get a stream to the warning device, no extra junk
#define WRNSTREAM(X)                                                  \
  {                                                                   \
    if (MsgManager::warnStream()) (*(MsgManager::warnStream())) << X; \
  }

// Just get a stream to the error device, no extra junk
#define ERRSTREAM(X)                                                    \
  {                                                                     \
    if (MsgManager::errorStream()) (*(MsgManager::errorStream())) << X; \
  }
...
#define DBGSTREAM(X)                                                    \
  {                                                                     \
    if (MsgManager::debugStream()) (*(MsgManager::debugStream())) << X; \
  }
