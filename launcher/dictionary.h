...
void Dictionary<K, V>::clear() {
  ...
  while ((getSize() > (uint32)MIN_TABLE_SIZE) && (keepSize == FALSE)) shrink();
}
...
bit8 Dictionary<K, V>::remove(IN K &key, OUT V &value) {
...
#endif
  {
    ...
    table[offset] = temp;
    entries--;
    if (percent <= SHRINK_THRESHOLD) shrink();
    return (TRUE);
  }
  node = node->hashNext;
...
void Dictionary<K, V>::shrink(void) {
  ...
  if ((size <= (uint32)MIN_TABLE_SIZE) || (keepSize == TRUE)) return;
