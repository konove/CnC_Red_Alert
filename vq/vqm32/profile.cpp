  ...
  while (1) {
    ...
    if (anim_frame < (total_frames + scene_frames)) {
      ...
      if (!stricmp(ext, "PAL")) {
        ...
        sprintf(outbuf, "%s%s%s%04d.%s", rootdir, extdir, prefix, file_frame,
                ext);
        return (0);
      }
    }
...
long GetINIInt(char const* section, char const* entry, long deflt,
  ...
  GetINIString(section, entry, buffer, buffer, sizeof(buffer), fname);
...
long GetINIString(char const* section, char const* entry, char const* def,
  ...
  while (1) {
    ...
    if (!memicmp(entry, txt, len) && (isspace(txt[len]) || txt[len] == '=')) {
      fclose(fp);
