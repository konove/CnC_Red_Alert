...
void VQA_InitMono(VQAHandleP* vqap) {
  ...
  Mono_Set_Cursor((AUDIO_WX2 + AUDIO_WX1 - strlen(AUDIO_TITLE)) / 2, AUDIO_WY1);
...
void VQA_UpdateMono(VQAHandleP* vqap) {
  ...
  sprintf(
      txt, "%02ld:%02ld.%02ld", curtime / (VQA_TIMETICKS * VQA_TIMETICKS),
      curtime / VQA_TIMETICKS,
      ((curtime * 100L) / VQA_TIMETICKS) - ((curtime / VQA_TIMETICKS) * 100L));
  ...
  Mono_Set_Cursor(
      LOADER_WX1 + 9 + vqabuf->Audio.PlayPosition / config->HMIBufSize,
      LOADER_WY2 - 1);

  Mono_Print("P");

  Mono_Set_Cursor(LOADER_WX1 + 9 + vqabuf->Audio.AudBufPos / config->HMIBufSize,
                  LOADER_WY2 - 1);
