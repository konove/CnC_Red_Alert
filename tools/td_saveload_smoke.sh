...
# Usage: tools/td_saveload_smoke.sh [tdsdl] [scenario, default SCG01EA] [--factory]
set -euo pipefail

TDSDL=$(realpath "${1:-cmake-build-strict-ra-clang/src/td/tdsdl}")
SCENARIO=${2:-SCG01EA}
FIXTURE=()
if [[ ${3:-} == --factory ]]; then FIXTURE=(-FACTORYTEST); fi
...
filter() {
  sed -n 's/^.*] \(frame .*\)/\1/p' "$1" | awk '$2 > 60 && ($3 == "unit" || $3 == "factory" || $3 == "trigger")'
}
...
if diff -u continuous.positions loaded.positions > difference.log; then
  echo "OK: $(wc -l < loaded.positions) unit/factory/trigger states identical across save/load"
else
