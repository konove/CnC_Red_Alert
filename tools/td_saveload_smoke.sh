...
filter() {
  sed -n 's/^.*] \(frame .*\)/\1/p' "$1" | awk '$2 > 60 && ($3 == "unit" || $3 == "factory" || $3 == "trigger" || $3 == "teamtype" || $3 == "team" || $3 == "teamcount")'
}
...
if diff -u continuous.positions loaded.positions > difference.log; then
  echo "OK: $(wc -l < loaded.positions) unit/factory/trigger/team states identical across save/load"
else
