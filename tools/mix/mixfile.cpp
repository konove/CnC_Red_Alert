 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Calc_CRC -- Calculate the CRC value of a block of data. *
 *   compfunc -- Comparison function used by qsort(). * DataClass::DataClass --
 *Constructor for a data file object node.                          *
 *   DataClass::Process_Input -- Process the input file list and builds linked
 *records.        * DataClass::Process_Output -- Creates the output data file
 *from the component source files.* DataClass::~DataClass -- Destructor for the
 *data file object.                             *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
          ...
          */
          ...
          case '-':
            switch (toupper(arg[1])) {
              ...
              case 'E':
                if (DataClass::ExtCount >= sizeof(DataClass::ExtFrom) /
              ...
              case 'I':
                if (DataClass::AltPathCount >=
                    sizeof(DataClass::AltPath) /
                        sizeof(DataClass::AltPath[0])) {
                  ...
                  DataClass::AltPath[DataClass::AltPathCount++] =
                      strupr(strdup(dir));
                }
                break;
 ...
 *records.          *
 *                                                                                             *
 *    This routine will process the list of files in the input file.  It builds
 *a linked       * list of DataClass objects.  This routine is the initial
 *process of creating a packed     * data file. * *
 *                                                                                             *
 * INPUT:   infile   -- Pointer to input filename. *
 *                                                                                             *
 *          quiet    -- Process quietly? *
 *                                                                                             *
 *          paths    -- Should the path of the filename be considered as part of
 *its signature?*
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This constructs a data file object node and links it into the list of
 *object nodes.      * It performs a preliminary check on the existance of the
 *the source file.                 *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This is the destructor for the data file object.  It deallocates any
 *memory allocated    * and de-links it from the list of file objects. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    This is a support function that compares two file data objects against
 *their CRC         * values. *
 *                                                                                             *
...
/***********************************************************************************************
 * DataClass::Process_Output -- Creates the output data file from the component
 *source files.  *
 *                                                                                             *
 *    This is the final step in creation of the data output file.  It writes the
 *header        * block and then appends the appropriate files to the output
 *file.                         *
 *                                                                                             *
...
void DataClass::Process_Output(char const *outname) {
  ...
  if (Count) {
    ...
    */
    ...
    /*
    **	Now write the actual data -- one file at a time in the order that they
