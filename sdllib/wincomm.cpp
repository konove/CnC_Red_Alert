...
HANDLE WinModemClass::Serial_Port_Open(char *device_name, int baud, int parity,
  ...
  switch (parity) {
    case 'O':
      libsp_parity = SP_PARITY_ODD;
      break;
    case 'E':
      libsp_parity = SP_PARITY_EVEN;
      break;
    case 'N':
      libsp_parity = SP_PARITY_NONE;
      break;
    case 'S':
      libsp_parity = SP_PARITY_SPACE;
      break;
    case 'M':
      libsp_parity = SP_PARITY_MARK;
      break;
  }
  ...
  result = sp_set_flowcontrol(
      port, flowcontrol ? SP_FLOWCONTROL_RTSCTS : SP_FLOWCONTROL_NONE);
  if (result != SP_OK) {
    sp_close(port);
    sp_free_port(port);
    return NULL;
