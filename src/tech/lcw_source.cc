 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * LcwSource::Get -- Fetch data through the LCW processor. *
 *   LcwSource::LcwSource -- Constructor for LCW straw object. *
 *   LcwSource::~LcwSource -- Destructor for the LCW straw. *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 * LcwSource::LcwSource -- Constructor for LCW straw object. *
 *                                                                                             *
 *    This will initialize the LCW straw object. Whether the object is to
 ...
 *=============================================================================================*/
LcwSource::LcwSource(CodecMode control, ByteSource& source, int blocksize)
...
/***********************************************************************************************
 * LcwSource::Get -- Fetch data through the LCW processor. *
 *                                                                                             *
 ...
 * HISTORY: * 07/04/1996 JLB : Created. *
 *=============================================================================================*/
base::ssize LcwSource::Read(std::span<std::byte> buffer) {
  ...
  while (slen > 0) {
    ...
    */
    ...
    if (Control == CodecMode::kDecompress) {
      ...
      int incount = static_cast<int>(ChainedSource::Read(
      ...
      incount = static_cast<int>(
          ChainedSource::Read(WritableByteView(ptr, BlockHeader.CompCount)));
      ...
      BlockHeader.UncompCount = static_cast<uint16_t>(static_cast<int>(
          ChainedSource::Read(WritableByteView(Buffer.data(), BlockSize))));
