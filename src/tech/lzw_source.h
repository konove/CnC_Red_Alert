...
*desired.
*/
class LzwSource : public ChainedSource {
 public:
  // source must outlive this source.
  LzwSource(CodecMode control, ByteSource& source, int blocksize = 1024 * 8);
  ~LzwSource() override = default;
 ...
 public:
  LzwSource(const LzwSource&) = delete;
  LzwSource& operator=(const LzwSource&) = delete;
  LzwSource(LzwSource&&) = delete;
  LzwSource& operator=(LzwSource&&) = delete;
};
