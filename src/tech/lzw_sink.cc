 ...
 *   LzwSink::LzwSink -- Constructor for the LZW processor pipe. * LzwSink::Put
 *-- Send some data through the LZW processor pipe.                            *
 *   LzwSink::~LzwSink -- Deconstructor for the LZW pipe object. *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/***********************************************************************************************
 * LzwSink::LzwSink -- Constructor for the LZW processor pipe. *
 *                                                                                             *
 *    This will initialize the LzwSink object so that it is prepared for
 ...
 *=============================================================================================*/
LzwSink::LzwSink(CodecMode control, ByteSink& next, int blocksize)
    : ChainedSink(next),
      Control(control),
      BlockSize(blocksize),
...
/***********************************************************************************************
 * LzwSink::Put -- Send some data through the LZW processor pipe. *
 *                                                                                             *
 ...
 * HISTORY: * 07/04/1996 JLB : Created. *
 *=============================================================================================*/
bool LzwSink::Write(std::span<const std::byte> bytes) {
  ...
  if (Control == CodecMode::kDecompress) {
    while (slen > 0 && !corrupt_) {
      ...
      */
      if (BlockHeader.CompCount == 0xFFFF) {
        ...
        memmove(source_buffer_.data() + Counter, source, base::ToSize(len));
        ...
        */
        if (Counter == sizeof(BlockHeader)) {
          memmove(&BlockHeader, source_buffer_.data(), sizeof(BlockHeader));
          Counter = 0;
        ...
        */
        if (std::cmp_equal(Counter, BlockHeader.CompCount)) {
          ...
          const int produced = LZW_Uncompress(
              Buffer(source_buffer_.data(), BlockHeader.CompCount),
              Buffer(output_buffer_.data(), BlockSize + SafetyMargin));
    ...
    */
    if (Counter > 0) {
      ...
      memmove(source_buffer_.data() + Counter, source, base::ToSize(tocopy));
      source = (char*)source + tocopy;
      slen -= tocopy;
      Counter += tocopy;

      if (Counter == BlockSize) {
        const int len = LZW_Compress(
            Buffer(source_buffer_.data(), BlockSize),
            Buffer(output_buffer_.data(), BlockSize + SafetyMargin));
        ...
        ChainedSink::Write(ByteView(output_buffer_.data(), len));
        Counter = 0;
      }
    }
    ...
    */
    if (slen > 0) {
      memmove(source_buffer_.data(), source, base::ToSize(slen));
      Counter = slen;
    }
  }
...
/***********************************************************************************************
 * LzwSink::Flush -- Flushes any partially accumulated block. *
 *                                                                                             *
...
bool LzwSink::Flush() {
  if (Control == CodecMode::kDecompress) {
    ...
    */
    const int len =
        LZW_Compress(Buffer(source_buffer_.data(), Counter),
                     Buffer(output_buffer_.data(), BlockSize + SafetyMargin));
