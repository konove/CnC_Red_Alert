 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * LcwSink::Flush -- Flushes any partially accumulated block. *
 *   LcwSink::LcwSink -- Constructor for the LCW processor pipe. * LcwSink::Put
 *-- Send some data through the LCW processor pipe.                            *
 *   LcwSink::~LcwSink -- Deconstructor for the LCW pipe object. *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/***********************************************************************************************
 * LcwSink::LcwSink -- Constructor for the LCW processor pipe. *
 *                                                                                             *
 *    This will initialize the LcwSink object so that it is prepared for
 ...
 *=============================================================================================*/
LcwSink::LcwSink(CodecMode control, ByteSink& next, int blocksize)
    : ChainedSink(next),
      Control(control),
      BlockSize(blocksize),
...
/***********************************************************************************************
 * LcwSink::Put -- Send some data through the LCW processor pipe. *
 *                                                                                             *
 ...
 * HISTORY: * 07/04/1996 JLB : Created. *
 *=============================================================================================*/
bool LcwSink::Write(std::span<const std::byte> bytes) {
    ...
    */
    if (Counter > 0) {
      ...
      if (Counter == BlockSize) {
        ...
        ChainedSink::Write(ByteView(Buffer2.data(), len));
        Counter = 0;
      }
    }
...
/***********************************************************************************************
 * LcwSink::Flush -- Flushes any partially accumulated block. *
 *                                                                                             *
