...
*rates.
*/
class LcwSink : public ChainedSink {
 public:
  // next must outlive this sink.
  LcwSink(CodecMode control, ByteSink& next, int blocksize = 1024 * 8);
  ~LcwSink() override = default;
 ...
 public:
  LcwSink(const LcwSink&) = delete;
  LcwSink& operator=(const LcwSink&) = delete;
  LcwSink(LcwSink&&) = delete;
  LcwSink& operator=(LcwSink&&) = delete;
};
