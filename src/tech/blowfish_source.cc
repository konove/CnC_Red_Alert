 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * BlowfishSource::Get -- Fetch a block of data from the straw. *
 *   BlowfishSource::Key -- Submit a key to the Blowfish straw. *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/***********************************************************************************************
 * BlowfishSource::Get -- Fetch a block of data from the straw. *
 *                                                                                             *
 ...
 *=============================================================================================*/
base::ssize BlowfishSource::Read(std::span<std::byte> buffer) {
  ...
  */
  if (!BF.has_value()) {
    return ChainedSource::Read(buffer);
  }
  ...
  while (slen > 0) {
    ...
    */
    const int incount = static_cast<int>(
        ChainedSource::Read(WritableByteView(Buffer.data(), kBlockSize)));
    if (incount == 0) {
      break;
    }
 ...
 * HISTORY: * 07/03/1996 JLB : Created. *
 *=============================================================================================*/
void BlowfishSource::Key(const void* key, int length) {
  /*
