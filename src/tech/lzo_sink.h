...
*rates.
*/
class LzoSink : public ChainedSink {
 public:
  // next must outlive this sink.
  LzoSink(CodecMode control, ByteSink& next, int blocksize = 1024 * 8);
  ~LzoSink() override = default;
 ...
 public:
  LzoSink(const LzoSink&) = delete;
  LzoSink& operator=(const LzoSink&) = delete;
  LzoSink(LzoSink&&) = delete;
  LzoSink& operator=(LzoSink&&) = delete;
};
