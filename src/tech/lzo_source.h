...
*desired.
*/
class LzoSource : public ChainedSource {
 public:
  // source must outlive this source.
  LzoSource(CodecMode control, ByteSource& source, int blocksize = 1024 * 8);
  ~LzoSource() override = default;
 ...
 public:
  LzoSource(const LzoSource&) = delete;
  LzoSource& operator=(const LzoSource&) = delete;
  LzoSource(LzoSource&&) = delete;
  LzoSource& operator=(LzoSource&&) = delete;
};
