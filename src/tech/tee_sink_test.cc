...
TEST(TeePipeTest, CopiesAllInputAcrossWrites) {
  ...
  EXPECT_TRUE(tee.Write(std::as_bytes(std::span("FRAM").first(4))));
  EXPECT_TRUE(tee.Write(std::as_bytes(std::span("\0\1").first(2))));
...
TEST(TeePipeTest, CopyFailureDoesNotInterruptMainStream) {
  ...
  EXPECT_TRUE(tee.Write(std::as_bytes(std::span("12").first(2))));
  EXPECT_FALSE(tee.copy_ok());
...
TEST(TeePipeTest, DisabledCopyPreservesMainResult) {
  ...
  EXPECT_FALSE(tee.Write(std::as_bytes(std::span("FRAM").first(4))));
  EXPECT_FALSE(tee.ok());
  EXPECT_TRUE(tee.copy_ok());
