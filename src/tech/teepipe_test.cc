...
TEST(TeePipeTest, CopiesAllInputAcrossWrites) {
  ...
  BufferPipe main_sink(main_bytes.data(), 6);
  BufferPipe copy_sink(copy_bytes.data(), 6);
  TeePipe tee(main_sink, &copy_sink);
  EXPECT_EQ(tee.Put("FRAM", 4), 4);
  EXPECT_EQ(tee.Put("\0\1", 2), 2);
...
TEST(TeePipeTest, CopyFailureDoesNotInterruptMainStream) {
  ...
  BufferPipe main_sink(main_bytes.data(), 6);
  BufferPipe copy_sink(copy_bytes.data(), 2);
  TeePipe tee(main_sink, &copy_sink);
  EXPECT_EQ(tee.Put("FRAM", 4), 4);
  EXPECT_FALSE(tee.copy_ok());
  EXPECT_EQ(tee.Put("12", 2), 2);
  EXPECT_FALSE(tee.copy_ok());
...
TEST(TeePipeTest, DisabledCopyPreservesMainResult) {
  ...
  BufferPipe main_sink(main_bytes.data(), 2);
  TeePipe tee(main_sink, nullptr);
  EXPECT_EQ(tee.Put("FRAM", 4), 2);
  EXPECT_TRUE(tee.copy_ok());
