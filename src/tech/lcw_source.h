...
/*
**	This class handles LCW compression/decompression to the data stream that
...
*desired.
*/
class LcwSource : public ChainedSource {
 public:
  // source must outlive this source.
  LcwSource(CodecMode control, ByteSource& source, int blocksize = 1024 * 8);
  ~LcwSource() override = default;
  ...
  */
  ...
  /*
  **	LCW compression requires a safety margin when decompressing over itself.
 ...
 public:
  LcwSource(const LcwSource&) = delete;
  LcwSource& operator=(const LcwSource&) = delete;
  LcwSource(LcwSource&&) = delete;
  LcwSource& operator=(LcwSource&&) = delete;
};
