...
/* $Header:   F:\projects\c&c\vcs\code\edit.cpv   2.18   16 Oct 1995 16:48:16
 * JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * EditClass::Action -- Handles input events. *
 *   EditClass::Draw_Background -- Draw the background to the edit gadget. *
 *   EditClass::Draw_Me -- Draws the edit box and embedded text. *
 *   EditClass::Draw_Text -- Draws the edit gadget text. * EditClass::EditClass
 *-- Normal constructor for edit class object.                         *
 ...
 *                                                                                             *
 * INPUT:   id    -- The ID number for this edit object. This is the ID number
 *that will be    * returned by the Input() function when the <RETURN> key is
 *pressed if this * gadget has the keyboard input focus. *
 *                                                                                             *
 *          text  -- Referenct to the text buffer that the edit gadget will
 *modify as keyboard * input is processed. The value that this buffer contains
 *is the default    * text displayed. *
 *                                                                                             *
 *          maxlen-- The maximum size of the text buffer specified. This length
 *INCLUDES the   * trailing null character so a simple sizeof() function call
 *can be used.   *
 *                                                                                             *
 *          flags -- These are the text print control flags. It is used to
 *control how the     * text looks in the edit box. Use the normal TPF_???
 *flags.                 *
 *                                                                                             *
 *          x,y   -- The pixel coordinates of the upper left corner of the edit
 *gadget area.   *
 *                                                                                             *
 *          w,h   -- The pixel dimensions of the edit box. If either of these
 *are no provided, * or set to -1, then the dimension is determined from the
 *string itself.    *
 *                                                                                             *
 *          sytle -- This style flag parameter control what kind of characters
 *are allowed in  * the edit box. The initial string in the text buffer may
 *contain illegal   * characters, but they are NOT removed regardless of this
 *parameter.        *
 *                                                                                             *
 ...
 *                                                                                             *
 * INPUT:   text     -- Reference to the character array that this edit gadget
 *will be         * modifying. * max_len  -- The maximum size of the buffer that
 *will be modified.                  *
 *                                                                                             *
 ...
 * EditClass::Draw_Me -- Draws the edit box and embedded text. *
 *                                                                                             *
 *    This routine will render the edit box. This will show the box outline as
 *well as any     * text it may contain. *
 *                                                                                             *
 * INPUT:   forced   -- Should the edit box be drawn even if it thinks it
 *doesn't have to?     *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will handle all mouse and keyboard events directed at this
 *edit box         * gadget. For keyboard events, this will insert the
 *characters into the edit box.          *
 *                                                                                             *
 * INPUT:   flags -- The event flag that triggered this function call. *
 *                                                                                             *
 *          key   -- Reference to the keyboard/mouse event that triggered this
 *function call.  *
 *                                                                                             *
 ...
 * EditClass::Draw_Background -- Draw the background to the edit gadget. *
 *                                                                                             *
 *    This routine will redraw the edit gadget background. The overlaying text
 *is handled by   * a different routine. The mouse is guaranteed to be hidden
 ...
 *                                                                                             *
 *    This routine is called when the edit gadget text needs to be drawn. The
 *background has   * already been drawn by the time this function is called. The
 *mouse is guaranteed to be    * hidden as well. *
 *                                                                                             *
 ...
 * EditClass::Handle_Key -- Handles keyboard input to edit gadget. *
 *                                                                                             *
 *    This is the gruntwork routine that processes keyboard input to the edit
 *gadget. This     * routine will be called when keyboard input has been
 *detected and this gadget has the     * current focus. *
 *                                                                                             *
 * INPUT:   ascii -- The ASCII key code that was fetched from the keyboard
 *buffer.             *
 *                                                                                             *
 * OUTPUT:  bool; Should this keyboard input NOT cause the gadget ID number to
 *be returned     * from the controlling Input() routine? Typically, the return
 *value would be   * true unless the focus is lost due to the <RETURN> key being
 *pressed.         *
 *                                                                                             *
...
bool EditClass::Handle_Key(KeyASCIIType ascii) {
  switch (ascii) {
    ...
    *this *	key code might be passed to this routine if this routine has
    *been overridden *	and the key event was consumed.
    */
    case 0:
      break;
    ...
    */
    ...
    /*
    **	If the keyboard event was not a recognized special key, then examine to
    ...
    */
    default:
      ...
      */
      ...
      /*
      **	The character passed all legality checks, so add it to the edit
      *string *	and flag this gadget to be redrawn. The manual flag to redraw is
      *needed *	because the event flag has been cleared. This prevents the
