...
/* $Header:   F:\projects\c&c\vcs\code\aircraft.cpv   2.12   19 Jun 1995
 * 09:27:22   JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 *this location.    * AircraftClass::Cell_Seems_Ok -- Checks to see if a cell is
 *good to enter.                 * AircraftClass::Desired_Load_Dir -- Determines
 *where passengers should line up.            * AircraftClass::Draw_It --
 *Renders an aircraft object at the location specified.           *
 ...
 *   AircraftClass::In_Which_Layer -- Determine which render layer the aircraft
 *lies.          * AircraftClass::Init -- Initialize the aircraft system to an
 *empty state.                  * AircraftClass::Is_LZ_Clear -- Determines if
 *landing zone is free for landing.             * AircraftClass::Mark -- Flags
 *cells under the aircraft so that they will be redrawn.       *
 ...
 *repair center.  * AircraftClass::Mission_Guard -- Handles aircraft in guard
 *mode.                           * AircraftClass::Mission_Guard_Area -- Handles
 *the aircraft guard area logic.               * AircraftClass::Mission_Hunt --
 *Maintains hunt AI for the aircraft.                        *
 ...
 *overlaps.          * AircraftClass::Pip_Count -- Returns the number of
 *"objects" in aircraft.                  * AircraftClass::Player_Assign_Mission
 *-- Handles player input to assign a mission.         * AircraftClass::Pose_Dir
 *-- Fetches the natural landing facing.                            *
 *   AircraftClass::Process_Fly_To -- Handles state machine for flying to
 *destination.         * AircraftClass::Process_Landing -- Landing process state
 *machine handler.                  * AircraftClass::Process_Take_Off -- State
 *machine support for taking off.                  * AircraftClass::Read_INI --
 *Reads aircraft object data from an INI file.                   *
 *   AircraftClass::Rearm_Delay -- Returns the delay between shots for this
 *aircraft.          * AircraftClass::Receive_Message -- Handles receipt of
 *radio messages.                      * AircraftClass::Response_Attack -- Gives
 *audio response to attack order.                   *
 ...
 *   AircraftClass::What_Action -- Determines what action to perform. *
 *   AircraftClass::Write_INI -- Writes the current aircraft objects to an INI
 *file.           * AircraftClass::operator delete -- Deletes the aircraft
 *object.                            * AircraftClass::operator new -- Allocates
 *a new aircraft object from the pool              *
 ...
 * AircraftClass::As_Target -- Returns aircraft as a target number. *
 *                                                                                             *
 *    This routine will convert the aircraft into a target number. This target
 *number can      * then be assigned to a targeting or navigation computer. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
void AircraftClass::Draw_It(int x, int y, WindowNumberType window) {
  ...
  *Custom rotor *	art has a different rotor set for each facing. Rotor
  *shapes occur after the first *	32 shapes of the helicopter body.
  */
 ...
 *file.             *
 *                                                                                             *
 *    This routine is used to output the current list of aircraft objects to the
 *INI file      * buffer specified. It is typically used by the scenario editor.
 **
 *                                                                                             *
...
/***********************************************************************************************
 * AircraftClass::Mark -- Flags cells under the aircraft so that they will be
 *redrawn.         *
 *                                                                                             *
 *    This routine is used to flag the cells under the aircraft so that those
 *cells will       * be redrawn during the next map drawing process. This is a
 *necessary step whenever the    * aircraft moves or changes shape. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *lies.            *
 *                                                                                             *
 *    This routine is used to figure out which rendering layer the aircraft is
 *located in.     * It can be determined from the aircraft's height. The layer
 *value is used to handle the   * display sequence. Objects in lower layers
 *appear beneath objects in higher layers.       *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *destination.      *
 *                                                                                             *
 * WARNINGS:   Because the aircraft can be move at a fast speed, the distance to
 *target value  * will probably never be zero. The likely case will be that the
 ...
 *packet to be      * propogated to all connected machines. *
 *                                                                                             *
 * HISTORY: * 06/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 *object to be     * propogated to all connected machines. *
 *                                                                                             *
 * HISTORY: * 06/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 *game frame.   * This is controlled by net/modem propogation delay. *
 *                                                                                             *
 * HISTORY: * 06/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 * AircraftClass::Fire_Coord -- Calculates the point of origin for a bullet. *
 *                                                                                             *
 *    This routine is used to find the exact coordinate where the bullet should
 *appear if      * fired from this object. *
 *                                                                                             *
 * INPUT:   which -- Which weapon to consider. *
 *                                                                                             *
 * OUTPUT:  Returns with the coordinate of where the projectile will appear if
 *fired.          *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
RadioMessageType AircraftClass::Receive_Message(RadioClass* from,
  ...
  switch (message) {
    ...
    /*
    **	Something disasterous has happened to the object in contact with. Fall
    ...
    */
    case RADIO_CAN_LOAD:
      if (!In_Radio_Contact() && Class->IsTransporter &&
          How_Many() < Class->Max_Passengers() && from &&
 ...
 *is to avoid unneccessary    * mid-air collisions. *
 *                                                                                             *
 ...
 *aircraft.            *
 *                                                                                             *
 *    Aircraft have a faster rearm delay than their weapon would otherwise
 *indicate. This is   * necessary to give helicopters a combat edge while still
 *allowing them to share the       * weapon types used by ground units. *
 *                                                                                             *
 * INPUT:   second   -- Is this for the second shot? The second shot uses the
 *full rearm       * delay. The first shot, if part of a two shot weapon, is
 *given an       * abbreviated rearm time. *
 *                                                                                             *
 ...
 * AircraftClass::Threat_Range -- Returns with a range to scan for targets. *
 *                                                                                             *
 *    This routine returns with the distance to scan for targets according to
 *the type of      * search requested. The search type is typically the weapon
 *(or short) range and vicinity  * distances. This is used by Guard and Guard
 *Area missions. Aircraft never consider their  * weapon range in this
 *determination since they can fly so fast, weapon range is           *
 *    meaningless. *
 *                                                                                             *
 * INPUT:   control  -- The range control parameter; * -1 = range doesn't matter
 *-- return -1 for compatability reasons.      * 0  = short range scan * 1  =
 *long range scan                                                   *
 *                                                                                             *
