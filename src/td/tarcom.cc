...
/* $Header:   F:\projects\c&c\vcs\code\tarcom.cpv   2.17   16 Oct 1995 16:52:20
 * JOE_BOSTIC  $ */
/***********************************************************************************************
...
void TarComClass::AI() {
  ...
  if (Class->Primary != WEAPON_NONE) {
    ...
    *primary weapon.
    */
    const WeaponTypeClass* weapon = &Weapons.at(Class->Primary);
    int primary = 0;
    FireErrorType ok = Can_Fire(TarCom, 0);
    if ((ok != FIRE_OK) && (Can_Fire(TarCom, 1) == FIRE_OK)) {
      ok = FIRE_OK;
      primary = 1;
      weapon = &Weapons.at(Class->Secondary);
    }
        ...
        if (Class->IsLockTurret) {
          ...
          if (*this == UNIT_FTANK) {
            SecondaryFacing.Set_Desired(
                Facing_Dir(Dir_Facing(Direction(TarCom))));
          } else {
