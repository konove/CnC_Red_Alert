...
/* $Header:   F:\projects\c&c\vcs\code\sidebar.cpv   2.13   02 Aug 1995 17:03:22
 * JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * SidebarClass::Abandon_Production -- Stops production of the
 *object specified.             * SidebarClass::Activate -- Controls the sidebar
 *activation.                                * SidebarClass::Activate_Demolish
 *-- Controls the demolish button on the sidebar.           *
 *   SidebarClass::Activate_Repair -- Controls the repair button on the sidebar.
 ** SidebarClass::Activate_Upgrade -- Controls the upgrade button on the
 *sidebar.             * SidebarClass::Add -- Adds a game object to the sidebar
 *list.                              * SidebarClass::AI -- Handles player
 *clicking on sidebar area.                              * SidebarClass::Draw_It
 *-- Renders the sidebar display.                                     *
 ...
 *   SidebarClass::One_Time -- Handles the one time game initializations. *
 *   SidebarClass::Recalc -- Examines the sidebar data and updates it as
 *necessary.            * SidebarClass::Refresh_Cells -- Intercepts the refresh,
 *looking for sidebar controls.      * SidebarClass::SBGadgetClass::Action --
 *Special function that controls the mouse over the s* SidebarClass::Scroll --
 *Handles scrolling the sidebar object strip.                       *
 *   SidebarClass::Set_Current -- Sets a specified object that controls the
 *sidebar display.   * SidebarClass::SidebarClass -- Default constructor for the
 *sidebar.                        * SidebarClass::StripClass::Abandon_Produ --
 *Abandons production associated with sidebar.   *
 *   SidebarClass::StripClass::Activate -- Adds the strip buttons to the input
 *system.         * SidebarClass::StripClass::Add -- Add an object to the side
 *strip.                         * SidebarClass::StripClass::AI -- Input and AI
 *processing for the side strip.               *
 *   SidebarClass::StripClass::Deactivate -- Removes the side strip buttons from
 *the input syst* SidebarClass::StripClass::Draw_It -- Render the sidebar
 *display.                          * SidebarClass::StripClass::Factory_Link --
 *Links a factory to a sidebar button.            *
 *   SidebarClass::StripClass::Flag_To_Redra -- Flags the sidebar strip to be
 *redrawn.         * SidebarClass::StripClass::Get_Special_Cameo -- Fetches the
 *special event cameo shape.     * SidebarClass::StripClass::Init_Clear -- Sets
 *sidebar to a known (and deactivated) state   *
 *   SidebarClass::StripClass::Init_IO -- Adds buttons to the button list *
 *   SidebarClass::StripClass::Init_Theater -- Performs theater-specific
 *initialization        * SidebarClass::StripClass::One_Time -- Performs one
 *time actions necessary for the side str* SidebarClass::StripClass::Recalc --
 *Revalidates the current sidebar list of objects.      *
 *   SidebarClass::StripClass::Scroll -- Causes the side strip to scroll. *
 *   SidebarClass::StripClass::SelectClass:: -- Action function when buildable
 *cameo is selecte* SidebarClass::StripClass::SelectClass:: -- Assigns special
 *values to a buildable select bu*
 *   SidebarClass::StripClass::SelectClass::SelectClass -- Default constructor.
 ** SidebarClass::StripClass::StripClass -- Default constructor for the side
 *strip class.     * SidebarClass::Which_Column -- Determines which column a
 *given type should appear.         * sortfunc -- Utility routine that handles
 *'qsort' the strip buttons.                       *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
**	animation.
*/
unsigned char SidebarClass::StripClass::ClockTranslucentTable[(1 + 1) * 256];
...
** Shape data pointers
*/
std::span<const std::byte> SidebarClass::StripClass::LogoShapes;
std::span<const std::byte> SidebarClass::StripClass::ClockShapes;
std::span<const std::byte> SidebarClass::StripClass::SpecialShapes[3];

std::span<const std::byte> SidebarClass::SidebarShape1;
std::span<const std::byte> SidebarClass::SidebarShape2;

/***********************************************************************************************
 * SidebarClass::SidebarClass -- Default constructor for the sidebar. *
 *                                                                                             *
 *    Constructor for the sidebar handler. It basically sets up the sidebar to
 *the empty       * condition. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
SidebarClass::SidebarClass() {
  new (&base::At(Column, 0)) StripClass(InitClass());
  new (&base::At(Column, 1)) StripClass(InitClass());
}
 ...
 *sidebar display. It  * should only be called ONCE. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
void SidebarClass::One_Time() {
  ...
  */
  base::At(base::At(WindowList, static_cast<int>(WINDOW_SIDEBAR)), kWindowX) =
      (SideX + PowWidth) / 8;
  base::At(base::At(WindowList, static_cast<int>(WINDOW_SIDEBAR)), kWindowY) =
      SideY + 1 + TopHeight;
  base::At(base::At(WindowList, static_cast<int>(WINDOW_SIDEBAR)),
           kWindowWidth) = SideWidth / 8;
  base::At(base::At(WindowList, static_cast<int>(WINDOW_SIDEBAR)),
           kWindowHeight) =
  ...
  */
  ...
  base::At(Column, 0).X = SideX + PowWidth + spacing;
  base::At(Column, 0).Y = SideY + TopHeight + 1;
  base::At(Column, 1).X = base::At(Column, 0).X +
                          (StripClass::kStripWidth * factor) + spacing -
                          (factor - 1);
  base::At(Column, 1).Y = SideY + TopHeight + 1;

  base::At(Column, 0).One_Time(0);
  base::At(Column, 1).One_Time(1);
...
void SidebarClass::Init_Clear() {
  ...
  base::At(Column, 0).Init_Clear();
  base::At(Column, 1).Init_Clear();
 ...
 * SidebarClass::Init_Theater -- Performs theater-specific initialization *
 *                                                                                             *
 * INPUT:   theater  -- The theater that is being initialized. Sometimes this
 *has an effect on * the data that is loaded. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
...
/***********************************************************************************************
 * SidebarClass::Which_Column -- Determines which column a given type should
 *appear.           *
 *                                                                                             *
 *    Use this function to resolve what column the specified object type should
 *be placed      * into. *
 *                                                                                             *
 ...
 * SidebarClass::Factory_Link -- Links a factory to a sidebar strip. *
 *                                                                                             *
 *    This routine will link the specified factory to the sidebar strip. A
 *factory must be     * linked to the sidebar so that as the factory production
 *progresses, the sidebar will     * show the production progress. *
 *                                                                                             *
...
/***********************************************************************************************
 * SidebarClass::Refresh_Cells -- Intercepts the refresh, looking for sidebar
 *controls.        *
 *                                                                                             *
 *    This routine intercepts the Refresh_Cells call in order to see if the
 *sidebar needs      * to be refreshed as well. If the special code to refresh
 *the sidebar was found, it        * flags the sidebar to be redrawn and then
 *removes the code from the list.                 *
 *                                                                                             *
 * INPUT:   cell  -- The cell to base the refresh list on. *
 *                                                                                             *
 *          list  -- Pointer to the cell offset list that elaborates all the
 *cells that        * need to be flagged for redraw. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
 ...
 *=============================================================================================*/
void SidebarClass::Refresh_Cells(CELL cell, std::span<const int16_t> list) {
  if (list.front() == REFRESH_SIDEBAR) {
    IsSidebarToRedraw = true;
    base::At(Column, 0).IsToRedraw = true;
    base::At(Column, 1).IsToRedraw = true;
 ...
 * SidebarClass::Activate_Repair -- Controls the repair button on the sidebar. *
 *                                                                                             *
 *    Use this routine to turn the repair sidebar button on and off. Typically,
 *the button     * is enabled when the currently selected structure is friendly
 *and damaged.                *
 *                                                                                             *
 * INPUT:   control  -- The controls how the button is to be activated or
 *deactivated;         * 0  -- Turn button off. * 1  -- Turn button on. * -1 --
 *Toggle button state.                                             *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    Use this routine to turn the upgrade sidebar button on and off. Typically,
 *the button    * is enabled when the currently selected structure can be
 *upgraded and disabled otherwise. *
 *                                                                                             *
 * INPUT:   control  -- The controls how the button is to be activated or
 *deactivated;         * 0  -- Turn button off. * 1  -- Turn button on. * -1 --
 *Toggle button state.                                             *
...
/***********************************************************************************************
 * SidebarClass::Activate_Demolish -- Controls the demolish button on the
 *sidebar.             *
 *                                                                                             *
 *    Use this routine to turn the demolish/dismantle sidebar button on and off.
 *Typically,    * the button is enabled when a friendly building is selected and
 *disabled otherwise.       *
 *                                                                                             *
 * INPUT:   control  -- The controls how the button is to be activated or
 *deactivated;         * 0  -- Turn button off. * 1  -- Turn button on. * -1 --
 ...
 *                                                                                             *
 *    This routine is used to add a game object to the sidebar. Call this
 *routine when a       * factory type building is created. It handles the case
 *of adding an item that has already * been added -- it just ignores it. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is used to scroll the sidebar strip of objects. The strip
 *appears whenever  * a building is selected that can produce units. If the
 *number of units to produce is      * greater than what the sidebar can hold,
 *this routine is used to scroll the other object  * into view so they can be
 *selected.                                                       *
 *                                                                                             *
 * INPUT:   up -- Should the scroll be upwards? Upward scrolling reveals object
 *that are       * later in the list of objects. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine handles the processing necessary when the player clicks on
 *the sidebar.     * Typically, this is selection of the item to build. *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 10/28/94   JLB : Created. * 11/11/1994 JLB : Processes input
 *directly.                                                * 12/26/1994 JLB :
 *Uses factory manager class for construction handling.                    *
...
void SidebarClass::AI(KeyNumType& input, int x, int y) {
  ...
  */
  ...
  if (IsSidebarActive && !Debug_Map) {
    ...
    if (input == KN_UP) {
      redraw |= base::At(Column, 0).Scroll(true);
      redraw |= base::At(Column, 1).Scroll(true);
      input = KN_NONE;
    }
  }
  ...
  if (IsSidebarActive) {
    ...
    */
    ...
    if (redraw) {
      // IsToRedraw = true;
      base::At(Column, 0).Flag_To_Redraw();
      base::At(Column, 1).Flag_To_Redraw();
 ...
 *necessary.              *
 *                                                                                             *
 *    Occasionally a factory gets destroyed. This routine must be called in such
 *a case        * because it might be possible that sidebar object need to be
 *removed. This routine will   * examine all existing objects in the sidebar
 *class and if no possible factory can         * produce it, then it will be
 *removed.                                                     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This routine is exhaustive and thus time consuming. Only call it
 *when really    * necessary. Such as when a factory is destroyed rather than
 *when a non-factory   * is destroyed. *
 *                                                                                             *
...
void SidebarClass::Recalc() {
  ...
  redraw |= base::At(Column, 0).Recalc();
  redraw |= base::At(Column, 1).Recalc();
 ...
 *                                                                                             *
 *    Use this routine to turn the sidebar on or off. This routine handles
 *updating the        * necessary flags. *
 *                                                                                             *
 * INPUT:   control  -- Tells what to do with the sidebar according to the
 *following:          * 0 = Turn sidebar off. * 1 = Turn sidebar on. * -1=
...
bool SidebarClass::Activate(int control) {
  ...
  */
  if (IsSidebarActive != old) {
    /*
    **	If the sidebar is activated but was on the right side of the screen,
    ...
    */
    if (IsSidebarActive /*&& X*/) {
      ...
      Zoom.Zap();
      Add_A_Button(Zoom);
      base::At(Column, 0).Activate();
      base::At(Column, 1).Activate();
      Background.Zap();
      ...
      base::At(Column, 0).Deactivate();
      base::At(Column, 1).Deactivate();
      Remove_A_Button(RadarButton);
      Remove_A_Button(PowerButton);
    }

    /*
    **	Since the sidebar status has changed, update the map so that the
    *graphics *	will be rendered correctly.
    */
    Flag_To_Redraw(true);
  }
...
/***********************************************************************************************
 * SidebarClass::StripClass::One_Time -- Performs one time actions necessary for
 *the side stri *
 *                                                                                             *
 *    Call this routine ONCE at the beginning of the game. It handles retrieving
 *pointers to   * the shape files it needs for rendering. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
void SidebarClass::StripClass::One_Time(int /*unused*/) {
  ...
  for (int lp = 0; lp < 3; lp++) {
    ...
    base::At(SpecialShapes, lp) = MixArchive::RetrieveData(fullname);
  }
}

/***********************************************************************************************
 * SidebarClass::StripClass::Get_Special_Cameo -- Fetches the special event
 *cameo shape.       *
 *                                                                                             *
 *    This routine will return with a pointer to the cameo data for the special
 *objects that   * can appear on the sidebar (e.g., nuclear bomb). *
 *                                                                                             *
 * INPUT:   type  -- The special type to fetch the cameo imagery for. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the cameo imagery for the specified
 *special object.      *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *=============================================================================================*/
std::span<const std::byte> SidebarClass::StripClass::Get_Special_Cameo(
    int type) {
  return base::At(SpecialShapes, type);
}

/***********************************************************************************************
 * SidebarClass::StripClass::Init_Clear -- Sets sidebar to a known (and
 *deactivated) state     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *system.           *
 *                                                                                             *
 *    This routine will add the side strip buttons to the map's input system.
 *This routine     * should be called once when the sidebar activates. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   Never call this routine a second time without first calling
 *Deactivate().       *
 *                                                                                             *
 * HISTORY: * 01/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 *the input syste *
 *                                                                                             *
 *    Call this routine to remove all the buttons on the side strip from the
 *map's input       * system. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   Never call this routine unless the Activate() function was
 *prevously called.    *
 *                                                                                             *
 * HISTORY: * 01/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    This routine is called by qsort() in order to sort the sidebar buttons.
 *This sorting     * forces the sidebar buttons to always occur in the order
 *that they can be built in,       * rather than the order that they were added
 *to the sidebar list.                          *
 *                                                                                             *
 ...
 *                                                                                             *
 * OUTPUT:  Returns <0 if the first object can be produced before the second. It
 *returns       * >0 if the reverse is true. It returns exactly 0 if the
 *production scneario for     * both objects is the same. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 * INPUT:   object   -- Pointer to the object type that can be built and is to
 *be added to     * the side strip. *
 *                                                                                             *
 * OUTPUT:  bool; Was the object successfully added to the side strip? Failure
 *could be the    * result of running out of room in the side strip array or the
 *object might    * already be in the list. *
 *                                                                                             *
 * WARNINGS:   none. *
 *                                                                                             *
 * HISTORY: * 12/31/1994 JLB : Created. *
 ...
 * SidebarClass::StripClass::Scroll -- Causes the side strip to scroll. *
 *                                                                                             *
 *    Use this routine to flag the side strip to scroll. The direction scrolled
 *is controlled  * by the parameter. Scrolling is merely initiated by this
 *routine. Subsequent calls to     * the AI function and the Draw_It function
 *are required to properly give the appearence    * of scrolling. *
 *                                                                                             *
 * INPUT:   bool; Should the side strip scroll UP? If it is to scroll down then
 *pass false.    *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/31/1994 JLB : Created. * 07/29/1995 JLB : Simplified scrolling
 *logic.                                              *
 *=============================================================================================*/
bool SidebarClass::StripClass::Scroll(bool up) {
  ...
  *It looks like *	the "window" to the object list is moving up even though
 ...
 *redrawn.           *
 *                                                                                             *
 *    This utility routine is called when something changes on the sidebar and
 *it must be      * reflected the next time drawing is performed. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    The side strip AI processing is performed by this function. This function
 *not only       * checks for player input, but also handles any graphic logic
 *updating necessary as a      * result of flashing or construction animation. *
 *                                                                                             *
 * INPUT:   input -- The player input code. *
 *                                                                                             *
 *          x,y   -- Mouse coordinate to use. *
 *                                                                                             *
 * OUTPUT:  bool; Did the AI detect that it will need a rendering change? If
 *this routine      * returns true, then the Draw_It function should be called
...
bool SidebarClass::StripClass::AI(KeyNumType& input, int /*unused*/,
  ...
  */
  if (!IsScrolling && Scroller) {
    if (BuildableCount <= kMaxVisible) {
      ...
      /*
      **	Top of list is moving toward lower ordered entries in the object
      *list. It looks like *	the "window" to the object list is moving up
      *even though the actual object images are *	scrolling downward.
      */
      if (Scroller < 0) {
        if (!TopIndex) {
          Scroller = 0;
  ...
  */
  ...
  /*
  **	Handle any flashing logic. Flashing occurs when the player selects an
  ...
  */
  ...
  /*
  **	If any of the logic determined that this side strip needs to be redrawn,
  *then *	set the redraw flag for this side strip.
  */
  if (redraw) {
    Flag_To_Redraw();
  }
 ...
 *                                                                                             *
 *    Use this routine to render the sidebar display. It checks to see if it
 *needs to be       * redrawn and only redraw if necessary. If the "complete"
 *parameter is true, then it       * will force redraw the entire strip. *
 *                                                                                             *
 * INPUT:   complete -- Should the redraw be forced? A force redraw will ignore
 *the redraw     * flag. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
  ...
  if (IsToRedraw || complete) {
    ...
    */
    for (int i = 0; i < kMaxVisible + (IsScrolling ? 1 : 0); i++) {
      ...
      int shapenum = 0;
      std::span<const unsigned char> remapper;
      ...
      */
      ...
      /*
      **	Fetch the shape number for the object type located at this
      ...
      */
        ...
        CC_Draw_Shape(
            ...
            x -
                (base::At(
                 ...
                 8) +
                LeftEdgeOffset,
            y - base::At(base::At(WindowList, static_cast<int>(WINDOW_SIDEBAR)),
                         kWindowY),
            WINDOW_SIDEBAR,
      ...
      /*
      **	Draw the overlapping clock shape if this is object is being
      *constructed. *	If the object is completed, then display "Ready" with no
      *clock shape.
      */
      if (production) {
        if (completed) {
...
/***********************************************************************************************
 * SidebarClass::StripClass::Recalc -- Revalidates the current sidebar list of
 *objects.        *
 *                                                                                             *
 *    This routine will revalidate all the buildable objects in the sidebar.
 *This routine      * comes in handy when a factory has been destroyed, and the
 *sidebar needs to reflect any   * change that this requires. It checks every
 *object to see if there is a factory available * that could produce it. If none
 *can be found, then the object is removed from the         * sidebar. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
bool SidebarClass::StripClass::Recalc() {
  ...
  /*
  **	Sweep through all objects listed in the sidebar. If any of those object
  *can *	not be created -- even in theory -- then they must be removed
  *form the sidebar and *	any current production must be abandoned.
  */
  bool redraw = false;
  for (int index = 0; index < BuildableCount; index++) {
    ...
    if (!ok) {
      /*
      **	If there was something in production, then abandon it before
      *deleting the *	factory manager.
      */
      ...
      */
      if (BuildableCount > 1 && index < BuildableCount - 1) {
        base::MoveBytes(std::as_writable_bytes(base::Suffix(Buildables, index)),
                        std::as_bytes(base::Suffix(Buildables, index + 1)),
                        sizeof(base::At(Buildables, 0)) *
                            base::ToSize(BuildableCount - index - 1));
      }
      TopIndex = 0;
      IsToRedraw = true;
      redraw = true;
...
#ifdef NEVER
  /*
  **	If there are no more buildable objects to display, make the sidebar go
  *away.
  */
  if (!BuildableCount) {
 ...
 * SidebarClass::StripClass::SelectClass::SelectClass -- Default constructor. *
 *                                                                                             *
 *    This is the default constructor for the button that controls the buildable
 *cameos on     * the sidebar strip. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   The coordinates are set to zero by this routine. They must be set
 *to the        * correct values before this button will function. *
 *                                                                                             *
 * HISTORY: * 01/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 *buildable select but *
 *                                                                                             *
 *    Use this routine to set custom buildable vars for this particular select
 *button. It      * uses this information to properly know what buildable object
 *to start or stop production * on. *
 *                                                                                             *
 ...
 *cameo is selected *
 *                                                                                             *
 *    This function is called when the buildable icon (cameo) is clicked on. It
 *handles        * starting and stopping production as indicated. *
 *                                                                                             *
...
bool SidebarClass::StripClass::SelectClass::Action(unsigned flags,
  ...
  **	the mouse is currently addressing. This doesn't mean that the factory
  *number *	fetched is actually producing the indicated object, merely that
          ...
          if (factory->Is_Building()) {
            ...
            */
            if (factory->Has_Completed()) {
              ...
              /*
              **	The factory must have been in a suspended state. Resume
...
/***********************************************************************************************
 * SidebarClass::SBGadgetClass::Action -- Special function that controls the
 *mouse over the si *
 *                                                                                             *
 *    This routine is called whenever the mouse is over the sidebar. It makes
 *sure that the    * mouse is always the normal shape while over the sidebar. *
 *                                                                                             *
...
/***********************************************************************************************
 * SidebarClass::StripClass::Factory_Link -- Links a factory to a sidebar
 *button.              *
 *                                                                                             *
 *    This routine will link the specified factory to this sidebar strip. The
 *exact button to  * link to is determined from the object type and id
 *specified. A linked button is one that * will show appropriate construction
 *animation (clock shape) that matches the state of     * the factory. *
 *                                                                                             *
 ...
 *                                                                                             *
 * OUTPUT:  Was the factory successfully attached? Failure would indicate that
 *there is no     * object of the specified type and sub-type in the sidebar
 *list.                     *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    This routine is used to abandon production of the object specified. The
 *factory will     * be completely disabled by this call. *
 *                                                                                             *
 * INPUT:   type     -- The object type that is to be abandoned. The sub-type is
 *not needed    * since it is presumed there can be only one type in production
 *at any   * one time. *
 *                                                                                             *
...
/***********************************************************************************************
 * SidebarClass::StripClass::Abandon_Produ -- Abandons production associated
 *with sidebar.     *
 *                                                                                             *
 *    Production of the object associated with this sidebar is abandoned when
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/18/1995 JLB : Created. * 08/06/1995 JLB : More intelligent
 *abandon logic for multiple factories.                   *
 *=============================================================================================*/
