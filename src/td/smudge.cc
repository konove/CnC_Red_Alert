...
/* $Header:   F:\projects\c&c\vcs\code\smudge.cpv   2.18   16 Oct 1995 16:52:06
 * JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * SmudgeClass::Init -- Initialize the smudge tracking system. *
 *   SmudgeClass::SmudgeClass -- Constructor for smudge objects. *
 *   SmudgeClass::operator delete -- Deletes the smudge from the tracking
 *system.              * SmudgeClass::operator new -- Creator of smudge objects.
 ** SmudgeClass::Mark -- Marks a smudge down on the map. * SmudgeClass::Read_INI
 *-- Reads smudge data from an INI file.                              *
 ...
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to a newly allocated smudge object. If one
 *couldn't be      * found, then NULL is returned. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 * SmudgeClass::SmudgeClass -- Constructor for smudge objects. *
 *                                                                                             *
 *    This is the typical constructor for smudge objects. If the position to
 *place the         * smudge is not given, then the smudge will be initialized
 *in a limbo state. If the        * smudge is placed on the map, then this
 *operation causes the smudge object itself to be   * deleted and special map
 *values updated to reflect the presence of a smudge.              *
 *                                                                                             *
 * INPUT:   type  -- The type of smudge to construct. *
 *                                                                                             *
 *          pos   -- The position to place the smudge. If -1, then the smudge is
 *initialized   * into a limbo state. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
 ...
 *                                                                                             *
 *    This routine is used during the scenario clearing process to initialize
 *the smudge       * object tracking system to a null state. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *supported.              *
 *                                                                                             *
 * OUTPUT:  bool; Was the smudge marked successfully? Failure occurs if the
 *smudge isn't       * marked DOWN. *
 *                                                                                             *
 * WARNINGS:   The smudge object is DELETED by this routine. *
 *                                                                                             *
 * HISTORY: * 09/22/1994 JLB : Created. * 12/23/1994 JLB : Checks low level
 ...
 *                                                                                             *
 *    This routine is used by the scenario loader to read the smudge data in an
 *INI file and   * create the appropriate smudge objects on the map. *
 *                                                                                             *
 ...
 * SmudgeClass::Write_INI -- Writes the smudge data to an INI file. *
 *                                                                                             *
 *    This routine is used by the scenario editor to write the smudge data to an
 *INI file.     *
 *                                                                                             *
 * INPUT:   buffer   -- Pointer to the INI file staging buffer. *
 *                                                                                             *
 ...
 * SmudgeClass::Disown -- Disowns (removes) a building bib piece. *
 *                                                                                             *
 *    This routine is used when a building is removed from the game. If there
 *was any bib      * attached, this routine will be called to disown the cells
 *and remove the bib artwork.    *
 *                                                                                             *
 * INPUT:   cell  -- The origin cell for this bib removal. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This is actually working on a temporary bib object. It is created
 *for the sole  * purpose of calling this routine. It will be deleted
