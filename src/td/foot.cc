...
/* $Header:   F:\projects\c&c\vcs\code\foot.cpv   2.17   16 Oct 1995 16:51:42
 * JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 * FootClass::~FootClass -- Default destructor for foot class objects. *
 *                                                                                             *
 *    At this level of the destruction process, the house record of the number
 *of units        * currently in inventory is decremented to reflect this units
 *destruction.                 *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *targetting will cause it to attack       * the target it selects. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * FootClass::Mission_Timed_Hunt -- This is the AI process for multiplayer
 *computer units.     *
 *                                                                                             *
 * For multiplayer games, the computer AI can't just blitz the human players;
 *the humans       * need a little time to set up their base, or whatever.  This
 *state just waits for            * a certain period of time, then goes into
 *hunt mode.                                         *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
int FootClass::Mission_Timed_Hunt() {
  ...
  if (!House->IsHuman) {
    ...
    */
    ...
    /*
    ** Jump into HUNT mode on a random die roll; the computer units will
    *periodically
 ...
 *if the object is teathered.    * In this case (presumes offloading to the
 ...
 * FootClass::Assign_Mission -- Assign mission to foot class object. *
 *                                                                                             *
 *    When a new mission is assigned, any precalculated path should be
 *truncated. This is      * in anticipation that the new mission will result in
 *a change of path.                    *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    When an object of FootClass type is limboed, then it must be removed from
 *any team       * it may be a member of. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *          source      -- The purpitrator of the damage. By knowing who caused
...
ResultType FootClass::Take_Damage(int& damage, int distance,
  ...
  if (result != RESULT_NONE && Team) {
    ...
    if (result != RESULT_DESTROYED) {
      /*
      **	Determine if the target that is current being attacked has a
      ...
      */
      if (source && source->IsActive && !House->Is_Ally(source) &&
          ...
          (Mission == MISSION_AMBUSH || Mission == MISSION_GUARD ||
           ...
           Mission == MISSION_ATTACK || Mission == MISSION_TIMED_HUNT)) {
        /*
        **	Assign the source of the damage as the new target. This occurs
        *for the computer *	controled units. For the player, this only
        *occurs if the source of the damage *	is within range.
        */
        if (!House->IsHuman) {
          /*
...
/***********************************************************************************************
 * FootClass::Active_Click_With -- Intiates attack or move according to target
 *clicked on.     *
 *                                                                                             *
...
void FootClass::Per_Cell_Process(bool center) {
  ...
  */
  ...
  /*
  **	If adjacent to an enemy building that has the ability to reveal a
  *stealth tank, *	then shimmer the cloaked object.
  */
  if (Cloak == CLOAKED) {
 ...
 *want to overide		* TARGET		navcom  - the new
 *navigation point to overide	*
 *                                                                         *
 * OUTPUT:		none *
 *                                                                         *
...
int FootClass::Mission_Enter() {
  ...
  */
  if (!In_Radio_Contact()) {
    ...
    /*
    **	Since radio contact exists with the transport, maintain a dialogue so
    *that *	the transport can give proper instructions to the passenger.
    */
 ...
 *=============================================================================================*/
void FootClass::Assign_Destination(TARGET target) { NavCom = target; }
 ...
 *other blockage types.      * The value returned will indicatd the most severe
