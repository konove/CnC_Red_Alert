...
/* $Header:   F:\projects\c&c\vcs\code\map.cpv   2.17   16 Oct 1995 16:52:22
 * JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MapClass::Cell_Distance -- Determines the distance between two
 *cells.                     * MapClass::Cell_Region -- Determines the region
 *from a specified cell number.              * MapClass::Cell_Threat -- Gets a
 *houses threat value for a cell                            *
 *   MapClass::Close_Object -- Finds a clickable close object to the specified
 *coordinate.     * MapClass::In_Radar -- Is specified cell in the radar map? *
 *   MapClass::Init -- clears all cells * MapClass::Logic -- Handles map related
 *logic functions.                                   * MapClass::One_Time --
 *Performs special one time initializations for the map.              *
 ...
 *                                                                                             *
 *    This routine will return with the calculated "straight line" * distance
 *between the two cells specified. It uses the dragon strike * method of
 ...
 *          cell2    -- Second cell. *
 *                                                                                             *
 * OUTPUT:  Returns with the "cell" distance between the two cells * specified.
 **
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 04/29/1994 JLB : Created. * 04/30/1994 JLB : Converted to member
 *function.                                            *
 *=============================================================================================*/
  ...
  */
  for (int y = 0; y < MapCellHeight - 1; y++) {
    for (int x = 0; x < MapCellWidth; x++) {
      value +=
          (*this)[(MapCellY + y) * MAP_CELL_W + (MapCellX + x)].Tiberium_Adjust(
              true);
    }
  }
  return value;
 ...
 * MapClass::Read_Binary -- reads the map's binary image file *
 *                                                                                             *
 * INPUT: * root      root filename for scenario * crc       ptr to CRC value to
 *update                                                   *
 *                                                                                             *
 ...
 * WARNINGS: * none. *
 *                                                                                             *
 * HISTORY: * 11/14/1994 BR : Created. * 01/08/1995 JLB : Fixup any obsolete
 *icons detected.                                       *
 *=============================================================================================*/
#ifdef DEMO
...
#endif
{
  ...
  */
  ...
  for (i = 0; i < MAP_CELL_TOTAL; i++) {
    ...
    /*
    **	Verify that the template type actually contains the template number
    *specified. If *	an illegal icon was specified, then replace it with
    *clear terrain.
    */
 ...
 * MapClass::Write_Binary -- writes the map's binary image file *
 *                                                                                             *
 * INPUT: * root      root filename for scenario *
 *                                                                                             *
 * OUTPUT: * 1 = success, 0 = failure *
 *                                                                                             *
 * WARNINGS: * none. *
