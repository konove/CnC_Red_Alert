...
/* $Header:   F:\projects\c&c\vcs\code\dial8.cpv   2.18   16 Oct 1995 16:51:32
 * JOE_BOSTIC  $ */
/***************************************************************************
...
Dial8Class::Dial8Class(int id, int x, int y, int w, int h, DirType dir)
  ...
  */
  base::At(base::At(FacePoint, 0), 0) = FaceX;
  base::At(base::At(FacePoint, 0), 1) = FaceY - (h * 8 / 2 / 10);

  base::At(base::At(FacePoint, 1), 0) = FaceX + (w * 7 * 8 / 2 / 100);
  base::At(base::At(FacePoint, 1), 1) = FaceY - (h * 7 * 8 / 2 / 100);

  base::At(base::At(FacePoint, 2), 0) = FaceX + (w * 8 / 2 / 10);
  base::At(base::At(FacePoint, 2), 1) = FaceY;

  base::At(base::At(FacePoint, 3), 0) = FaceX + (w * 7 * 8 / 2 / 100);
  base::At(base::At(FacePoint, 3), 1) = FaceY + (h * 7 * 8 / 2 / 100);

  base::At(base::At(FacePoint, 4), 0) = FaceX;
  base::At(base::At(FacePoint, 4), 1) = FaceY + (h * 8 / 2 / 10);

  base::At(base::At(FacePoint, 5), 0) = FaceX - (w * 7 * 8 / 2 / 100);
  base::At(base::At(FacePoint, 5), 1) = FaceY + (h * 7 * 8 / 2 / 100);

  base::At(base::At(FacePoint, 6), 0) = FaceX - (w * 8 / 2 / 10);
  base::At(base::At(FacePoint, 6), 1) = FaceY;

  base::At(base::At(FacePoint, 7), 0) = FaceX - (w * 7 * 8 / 2 / 100);
  base::At(base::At(FacePoint, 7), 1) = FaceY - (h * 7 * 8 / 2 / 100);

  base::At(base::At(FaceLine, 0), 0) = FaceX;
  base::At(base::At(FaceLine, 0), 1) = FaceY - (h * 6 / 2 / 10);

  base::At(base::At(FaceLine, 1), 0) = FaceX + (w * 7 * 6 / 2 / 100);
  base::At(base::At(FaceLine, 1), 1) = FaceY - (h * 7 * 6 / 2 / 100);

  base::At(base::At(FaceLine, 2), 0) = FaceX + (w * 6 / 2 / 10);
  base::At(base::At(FaceLine, 2), 1) = FaceY;

  base::At(base::At(FaceLine, 3), 0) = FaceX + (w * 7 * 6 / 2 / 100);
  base::At(base::At(FaceLine, 3), 1) = FaceY + (h * 7 * 6 / 2 / 100);

  base::At(base::At(FaceLine, 4), 0) = FaceX;
  base::At(base::At(FaceLine, 4), 1) = FaceY + (h * 6 / 2 / 10);

  base::At(base::At(FaceLine, 5), 0) = FaceX - (w * 7 * 6 / 2 / 100);
  base::At(base::At(FaceLine, 5), 1) = FaceY + (h * 7 * 6 / 2 / 100);

  base::At(base::At(FaceLine, 6), 0) = FaceX - (w * 6 / 2 / 10);
  base::At(base::At(FaceLine, 6), 1) = FaceY;

  base::At(base::At(FaceLine, 7), 0) = FaceX - (w * 7 * 6 / 2 / 100);
  base::At(base::At(FaceLine, 7), 1) = FaceY - (h * 7 * 6 / 2 / 100);
}
