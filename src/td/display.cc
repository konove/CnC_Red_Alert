 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * DisplayClass::AI -- Handles the maintenance tasks for the map
 *display.                    * DisplayClass::Calculated_Cell -- Fetch a map
 *cell based on specified method.              * DisplayClass::Cell_Object --
 *Determines what has been clicked on.                         *
 *   DisplayClass::Cell_Shadow   -- Determine what shadow icon to use for the
 *cell.            * DisplayClass::Click_Cell_Calc -- Determines cell from
 *screen X & Y.                       * DisplayClass::Coord_To_Pixel --
 *Determines X and Y pixel coordinates.                     *
 ...
 *   DisplayClass::Flag_To_Redraw -- Flags the display so that it will be
 *redrawn as soon as poss* DisplayClass::Get_Occupy_Dimensions -- computes width
 *& height of the given occupy list   * DisplayClass::Init_Clear -- Clears the
 *display to a known state.                          * DisplayClass::Init_IO --
 *Creates the map's button list                                    *
 ...
 *COORDINATE                            * DisplayClass::Read_INI -- Reads map
 *control data from INI file.                           *
 ...
 *cursor.         * DisplayClass::Set_View_Dimensions -- Sets the tactical
 *display screen coordinates.        * DisplayClass::Submit -- Adds a game
 *object to the map rendering system.                   *
 ...
 *list for specified text string.   * DisplayClass::Write_INI -- Writes map data
 *into INI file.                                 *
 ...
 *objects          * DisplayClass::Prev_Object -- Searches for the previous
 *object on the map.                 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 *          width -- The width of the tactical display (in pixels). If this
 *parameter is       * omitted, then the width will be as wide as the screen
 *will allow.         *
 *                                                                                             *
 *          height-- The height of the tactial display (in pixels). If this
 *parameter is       * omitted, then the width wil be as wide as the screen will
 *allow.          *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
    ...
    for (FacingType facing = FACING_N; facing < FACING_COUNT; facing++) {
      ...
      */
      ...
      if (base && base->What_Am_I() == RTTI_BUILDING &&
          base->House->Class->House == PendingHouse) {
        return true;
      }
    }
  }
 ...
 *                                                                                             *
 *    This routine is used to write the map control data into the INI * file.
 *The scenario editor uses this when creating the scenario * startup file. *
 *                                                                                             *
  ...
  */
  for (CELL cell = 0; cell < MAP_CELL_TOTAL; cell++) {
    if ((*this)[cell].IsTrigger) {
      ...
      **	Save entry.
      */
      WWWritePrivateProfileString("CellTriggers", entry, trig->Get_Name(),
                                  buffer);
    }
  }
}
 ...
 *to work with display heirarchy.                                  * 12/24/1994
    ...
    if (Lepton_To_Pixel(Coord_X(DesiredTacticalCoord)) !=
      ...
      */
      if (!forced && (oldw != Lepton_To_Pixel(TacLeptonWidth) ||
        ...
        Set_Cursor_Pos(-1);
        /*
        ** If hid page is in video memory then we may nned to blit from the seen
        *page to
      ...
      */
      ...
      if (!forced) {
        ...
        **
        */
        const int startx = -Lepton_To_Pixel(Coord_XLepton(TacticalCoord));
        const int starty = -Lepton_To_Pixel(Coord_YLepton(TacticalCoord));
        oldw -= 24;
        oldh -= 24;
        ...
        if (abs(oldx) < 0x25 && abs(oldy) < 0x25) {
          ...
          *copied.
          */
          const int tactical_x = -Lepton_To_Pixel(Coord_XLepton(TacticalCoord));
...
CELL DisplayClass::Calculated_Cell(SourceType dir, HousesType house) {
  ...
  while (cell == 0) {
    ...
    */
    switch (dir) {
      /*
      **	Looks for the northern most straight path shipping lane and
      *returns *	the cell of one of the ends.
      */
      ...
      */
      ...
      /*
      **	Dramatic entry point is somewhere on the visible screen as
      *defined *	by the current tactical map position.
      */
      case SOURCE_VISIBLE:
        cell = XY_Cell(Coord_XCell(TacticalCoord) +
      ...
      */
      case SOURCE_BEACH: {
        ...
        /*
        **	Fixup entry list so that it doesn't come close to blocking
      ...
      case SOURCE_OCEAN:
        cell = XY_Cell(Random_Pick(0, MapCellWidth - 2) + MapCellX,
    ...
    */
    if (!shadow) {
      ...
      */
      ...
      //!((TechnoClass *)object)->IsOwnedByPlayer && ((TechnoClass
      ...
      */
      ...
      if ((object->Is_Techno() &&
        ...
        switch (object->What_Am_I()) {
          ...
          case RTTI_BUILDING:
            if (*dynamic_cast<BuildingClass*>(object) != STRUCT_MISSION) {
...
void DisplayClass::Compute_Start_Pos() {
  ...
  */
  ...
  /*
  **	Since the TacticalCell (starting cell) represents the screen's
  *upper-left *	corner, adjust for that.
  */
