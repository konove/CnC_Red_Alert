...
/* $Header:   F:\projects\c&c\vcs\code\bdata.cpv   2.17   16 Oct 1995 16:50:08
 * JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 *building types.   * BuildingTypeClass::Create_And_Place -- Creates and places
 *a building object onto the map. * BuildingTypeClass::Create_One_Of -- Creates
 *a building of this type.                      * BuildingTypeClass::Dimensions
 *-- Fetches the pixel dimensions of the building.            *
 ...
 *building.           * BuildingTypeClass::Legal_Placement -- Determines if
 *building can be legally placed at pos.* BuildingTypeClass::Max_Pips --
 *Determines the maximum pips to display.                    *
 ...
 *   BuildingTypeClass::Who_Can_Build_Me -- Determines which factory can create
 *the building.  * BuildingTypeClass::Width -- Determines width of bulding in
 *icons.                         *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
BuildingTypeClass::BuildingTypeClass(
    ...
    BSizeType size, std::span<const int16_t> exitlist,
    std::span<const int16_t> sizelist,
  ...
  for (StructType sindex = STRUCT_WEAP; sindex < STRUCT_COUNT; sindex++) {
    ...
    */
    if (building.IsBuildable) {
      ...
      building.Set_Cameo_Data(MixArchive::RetrieveData(fullname));
    }
    ...
    */
    fullname = std::filesystem::path(building.IniName)
                   ...
                   .string();
    building.Set_Image_Data(MixArchive::RetrieveData(fullname));
  }
...
/***********************************************************************************************
 * BuildingTypeClass::Legal_Placement -- Determines if building can be legally
 *placed at pos.  *
 *                                                                                             *
 *    This routine is used to determine if a building can be legally * placed at
 *the specified position. Buildings can only be placed on * unoccupied rock
 *terrain.                                                                 *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/15/1991 JLB : Created. * 04/26/1992 JLB : Concrete and scenario
 *init adjustment.                                   * 05/06/1992 JLB : Good,
 *Bad, and Adjacent checking added.                                  *
...
bool BuildingTypeClass::Legal_Placement(CELL pos) const {
  std::span<const int16_t> offset{};  // Pointer to cell offset list.
...
#ifdef NEVER
  /*
  **	Concrete has special checking performed to determine legal placement.
  *Concrete *	can legally be placed if there is any cell that would be
  *affected by the concrete *	placement. Unlike other buildings, only one cell
  *needs to be effective in order *	to flag legal placement for the entire
  *"structure".
  */
  ...
  *of *	obstacles. If this check passes for all foundation squares, only then
  *does the *	routine return that it is legal to place.
  */
 ...
 *the building.    *
 *                                                                                             *
 *    Use this routine to determine which building is available to build the
 *building. If      * there are no suitable buildings, then NULL is returned.
 *Typical use of this function is  * to maintain the construction list sidebar.
 **
 *                                                                                             *
 * INPUT:   intheory -- If true, then it merely checks for a building of the
 *proper ownership  * when determining if construction is allowed. It doesn't
 *consider the   * possibility that the construction building is currently busy
 *or not.   *
 *                                                                                             *
 ...
 *                                                                                             *
 *          house    -- The owner of the building to be built. Only construction
 *buildings of  * the same ownership are allowed to build. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the construction object (building) that
 *can build        * the building type. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 11/29/1994 JLB : Created. * 01/02/1995 JLB : Scans in reverse
 *order so that later buildings are biased.               *
 *=============================================================================================*/
...
std::span<const int16_t> BuildingTypeClass::Occupy_List(bool placement) const {
  ...
  if ((placement && Bib_And_Offset(bib, cell)) ||
    ...
    */
    src = OccupyList;
    while (!src.empty() && src.front() != REFRESH_EOL) {
      base::ConsumeFront(dest) = base::ConsumeFront(src);
    }
    dest.front() = REFRESH_EOL;
 ...
 * BuildingTypeClass::Repair_Cost -- Determines cost per "step" of repair. *
 *                                                                                             *
 *    Use this routine to determine how much it will cost to repair the building
 *one           * step. A step is defined as the number of hit points returned
 *from the Repair_Step()      * function. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * BuildingTypeClass::Repair_Step -- Determines the repair step rate. *
 *                                                                                             *
 *    This routine will determine how many strength points get healed for each
 *"step". The     * cost to repair one step is determine from the Repair_Cost()
 *function.                    *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *=============================================================================================*/
int BuildingTypeClass::Max_Pips() const { return Bound(Capacity / 100, 0, 10); }

/***********************************************************************************************
 * BuildingTypeClass::Full_Name -- Fetches the full name text number. *
 *                                                                                             *
 *    This routine will fetch the full name of this building (expressed as a
 *text number).     * If special civilian names are enabled, then the civilian
 *buildings will show their true  * name rather than "civilian building". *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
