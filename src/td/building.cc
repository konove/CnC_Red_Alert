...
/* $Header:   F:\projects\c&c\vcs\code\building.cpv   2.13   02 Aug 1995
 * 17:00:14   JOE_BOSTIC  $ */
/***********************************************************************************************
      ...
      if (Transmit_Message(RADIO_NEED_TO_MOVE, from) == RADIO_ROGER) {
        if (*this == STRUCT_HELIPAD) {
          ...
          if (*this == STRUCT_REPAIR) {
            ...
            param =
                ::As_Target(Coord_Cell(Adjacent_Cell(Center_Coord(), DIR_SW)));
          }
        }
      ...
      */
      if (Strength <= 1) {
        ...
        if (*this == STRUCT_WEAP) {
          ...
          /*
          **	Special render stage for silos. The stage is dependant on the
 ...
 * BuildingClass::Fire_At -- Fires weapon at specified target. *
 *                                                                                             *
 *    This routine does the actual firing of a projectile from the * building
 *toward the specified target. Prior to calling this * routine, the building
 *must have rotated into position and acquired                       * a
 *suitable target. *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the projectile just launched. This * may
 *come in handy if additional adjustments to the projectile * are required. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 04/16/1994 JLB : Created. *
 *=============================================================================================*/
...
void BuildingClass::AI() {
  ...
  */
  ...
  if ((!Class->IsTurretEquipped && *this != STRUCT_OBELISK) ||
  ...
  */
  if (!House->IsHuman && Mission != MISSION_CONSTRUCTION &&
  ...
  */
  ...
  /*
  **	If the building was in a recoil state (as it would be just as it fires),
  *then *	restore the building.
  */
  if (IsInRecoilState) {
...
bool BuildingClass::Unlimbo(COORDINATE coord, DirType dir) {
  ...
  if (*this == STRUCT_ROAD) {
    if (Can_Enter_Cell(Coord_Cell(coord), FACING_NONE) == MOVE_OK) {
      ObjectClass* o =
          OverlayTypeClass::As_Reference(OVERLAY_ROAD).Create_One_Of(House);
  ...
  if (this != source) {
    ...
    */
    ...
    if (source && res != RESULT_NONE) {
      ...
      */
      if (*this != STRUCT_SAM && !House->Is_Ally(source) &&
 ...
 *ground for      * buildings that are euipped with weapons. *
 *                                                                                             *
 ...
 *object is in a     * teathered condition. This condition will be automatically
...
/***********************************************************************************************
 * BuildingClass::Fire_Coord -- Calculates the coordinate that projectile would
 *appear.        *
 *                                                                                             *
 *    This routine is used to determine where a projectile would appear if this
 *building       * were to fire. The location usually depends on the current
 *rotation setting of the        * turret or else on the direction of the target
 *that will be fired upon.                   *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with coordinate that the projectile should appear at if the
 *building       * were to fire. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/24/1994 JLB : Created. *
...
TARGET BuildingClass::Greatest_Threat(ThreatType threat) const {
  ...
  if (*this != STRUCT_SAM) {
    threat = threat |
             (THREAT_INFANTRY | THREAT_BOATS | THREAT_VEHICLES | THREAT_RANGE);
 ...
 *indefinately    * until explicitly stopped. *
 *                                                                                             *
 * INPUT:   bstate   -- The animation state to initiate. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   The buliding graphic state will reflect the first stage of this
 ...
 * BuildingClass::Write_INI -- Writes all building data to an INI file. *
 *                                                                                             *
 *    This routine is used to write the buildings into an INI file. It is
 *necessary for the    * scenario editor save game option. *
 *                                                                                             *
 *    INI entry format: * Housename, Typename, Strength, Cell, Facing,
 *Triggername                               *
 *                                                                                             *
...
/***********************************************************************************************
 * BuildingClass::Refund_Amount -- Fetches the refund amount if building is
 *sold.              *
 *                                                                                             *
 *    This routine will return the amount of money to be refunded to the
 *building's owner      * if the building is sold. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
void BuildingClass::Detach_All(bool all) {
  ...
  */
  if (House) {
    ...
    **	detmermine if any object that is being produced can still be produced.
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 08/06/1995 JLB : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * BuildingClass::Passes_Proximity_Check -- Determines if building placement is
 *near friendly sq*
 *                                                                                             *
 ...
 *by the function.                                          *
 *                                                                                             *
 * INPUT:   homecell -- The cell that the building would like to be placed down
 *at.            *
 *                                                                                             *
