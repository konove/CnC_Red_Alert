 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * ThemeClass::AI -- Process the theme engine and restart songs. *
 ...
 ** ThemeClass::Theme_File_Name -- Constructs a filename for the specified
 *theme.             * ThemeClass::Track_Length -- Calculates the length of the
 *song (in seconds).               *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
ThemeClass::ThemeControl ThemeClass::_themes[THEME_COUNT] = {
    ...
    {"FAC2226M", TXT_THEME_FAC2, 0, 328, true, false, true, kHouseFlagSoviet},
    {"HELL226M", TXT_THEME_HELL, 0, 375, true, false, true, kHouseFlagAllies},
    {"RUN1226M", TXT_THEME_RUN1, 0, 312, true, false, true, kHouseFlagSoviet},
    {"SMSH226M", TXT_THEME_SMSH, 0, 272, true, false, true, kHouseFlagAllies},
    {"TREN226M", TXT_THEME_TREN, 0, 312, true, false, true, kHouseFlagSoviet},
    {"WORK226M", TXT_THEME_WORK, 0, 277, true, false, true, kHouseFlagAllies},
    {"AWAIT", TXT_THEME_AWAIT, 0, 259, true, false, true, kHouseFlagAllies},
    {"DENSE_R", TXT_THEME_DENSE_R, 0, 294, true, false, true, kHouseFlagAllies},
    {"FOGGER1A", TXT_THEME_FOGGER1A, 0, 297, true, false, true,
     kHouseFlagAllies},
    {"MUD1A", TXT_THEME_MUD1A, 0, 280, true, false, true, kHouseFlagAllies},
    {"RADIO2", TXT_THEME_RADIO2, 0, 237, true, false, true, kHouseFlagAllies},
    {"ROLLOUT", TXT_THEME_ROLLOUT, 0, 227, true, false, true, kHouseFlagAllies},
    {"SNAKE", TXT_THEME_SNAKE, 0, 277, true, false, true, kHouseFlagAllies},
    {"TERMINAT", TXT_THEME_TERMINAT, 0, 310, true, false, true,
     kHouseFlagAllies},
    {"TWIN", TXT_THEME_TWIN, 0, 229, true, false, true, kHouseFlagAllies},
    {"VECTOR1A", TXT_THEME_VECTOR1A, 0, 252, true, false, true,
     kHouseFlagAllies},
    {"MAP", TXT_THEME_MAP, 0, 63, false, true, true, kHouseFlagNone},
    {"SCORE", TXT_THEME_SCORE, 0, 106, false, true, true, kHouseFlagNone},
    {"INTRO", TXT_THEME_INTRO, 0, 205, false, true, true, kHouseFlagNone},
    {"CREDITS", TXT_THEME_CREDITS, 0, 163, false, true, true, kHouseFlagNone},
    ...
    {"CHAOS2", TXT_THEME_CHAOS2, 0, 250, true, false, true,
     kHouseFlagSoviet | kHouseFlagSpain},
    {"SHUT_IT", TXT_THEME_SHUT_IT, 0, 261, true, false, true, kHouseFlagAllies},
    {"TWINMIX1", TXT_THEME_TWINMIX1, 0, 222, true, false, true,
     kHouseFlagSoviet | kHouseFlagSpain},
    {"UNDER3", TXT_THEME_UNDER3, 0, 246, true, false, true, kHouseFlagAllies},
    {"VR2", TXT_THEME_VR2, 0, 255, true, false, true,
     kHouseFlagSoviet | kHouseFlagSpain},
    {"BOG", TXT_THEME_BOG, 0, 212, true, false, true, kHouseFlagAllies},
    {"FLOAT_V2", TXT_THEME_FLOAT_V2, 0, 274, true, false, true,
     kHouseFlagSoviet | kHouseFlagSpain},
    {"GLOOM", TXT_THEME_GLOOM, 0, 236, true, false, true, kHouseFlagAllies},
    {"GRNDWIRE", TXT_THEME_GRNDWIRE, 0, 228, true, false, true,
     kHouseFlagSoviet | kHouseFlagSpain},
    {"RPT", TXT_THEME_RPT, 0, 275, true, false, true, kHouseFlagAllies},
 ...
 * ThemeClass::Base_Name -- Fetches the base filename for the theme specified. *
 *                                                                                             *
 *    This routine is used to retrieve a pointer to the base filename for the
 *theme            * specified. *
 *                                                                                             *
 * INPUT:   theme -- The theme number to convert into a base filename. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the base filename for the theme specified.
 *If the        * theme number is invalid, then a pointer to "No Theme" is
 *returned instead.         *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 * ThemeClass::Full_Name -- Retrieves the full score name. *
 *                                                                                             *
 *    This routine will fetch and return with a pointer to the full name of the
 *theme          * specified. *
 *                                                                                             *
 * INPUT:   theme -- The theme to fetch the full name for. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the full name for this score. This pointer
 *may point to  * EMS memory. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 01/16/1995 JLB : Created. *
 ...
 *                                                                                             *
 *    This is a maintenance function that will restart an appropriate theme if
 *the current one * has finished. This routine should be called frequently. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * ThemeClass::Next_Song -- Calculates the next song number to play. *
 *                                                                                             *
 *    use this routine to figure out what song number to play. It examines the
 *option settings * for repeat and shuffle so that it can return the correct
 *value.                          *
 *                                                                                             *
 * INPUT:   theme -- The origin (last) index. The new value is related to this
 *for all but     * the shuffling method of play. *
 *                                                                                             *
 ...
 * ThemeClass::Queue_Song -- Queues the song to the play queue. *
 *                                                                                             *
 *    This routine will cause the current song to fade and the specified song to
 *start. This   * is the normal and friendly method of changing the current
 *song.                          *
 *                                                                                             *
 * INPUT:   theme -- The song to start playing. If -1 is passed in, then just
 *the current song.* is faded. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
...
void ThemeClass::Queue_Song(ThemeType theme) {
  ...
  *valid, then *	set the queued theme accordingly.
  */
 ...
 * ThemeClass::Play_Song -- Starts the specified song play NOW. *
 *                                                                                             *
 *    This routine is used to start the specified theme playing right now. If
 *there is already * a theme playing, it is cut short so that this one may
 *start.                             *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   This cuts off any current song in a abrupt manner. Only use this
 ...
 **
 *                                                                                             *
 *    This routine will construct (into a static buffer) a filename that matches
 *the theme     * number specified. This constructed filename is returned as a
 *pointer. The filename will  * remain valid until the next call to this
 *routine.                                        *
 *                                                                                             *
 * INPUT:   theme -- The theme number to convert to a filename. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the constructed filename for the specified
 *theme number. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Use this routine to calculate the length of the song. The length is
 *determined by        * reading the header of the song and dividing the sample
 *rate into the sample length.      *
 *                                                                                             *
 * INPUT:   theme -- The song number to examine to find its length. *
 *                                                                                             *
 * OUTPUT:  Returns with the length of the specified theme. This length is in
 *the form of      * seconds. *
 *                                                                                             *
 * WARNINGS:   This routine goes to disk to fetch this information. Don't call
 *frivolously.    *
 *                                                                                             *
 * HISTORY: * 01/16/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 * ThemeClass::Stop -- Stops the current theme from playing. *
 *                                                                                             *
 *    Use this routine to stop the current theme. After this routine is called,
 ...
 * ThemeClass::Is_Allowed -- Checks to see if the specified theme is legal. *
 *                                                                                             *
 *    Use this routine to determine if a theme is allowed to be played. A theme
 *is not allowed * if the scenario is too early for that score, or the score
 *only is allowed in special     * cases. *
 *                                                                                             *
 ...
 * ThemeClass::From_Name -- Determines theme number from specified name. *
 *                                                                                             *
 *    Use this routine to convert a name (either the base filename of the theme,
 *or a partial  * substring of the full name) into the matching ThemeType value.
 *Typical use of this is    * when parsing the INI file for theme control
 *values.                                      *
 *                                                                                             *
 * INPUT:   name  -- Pointer to base filename of theme or a partial substring of
 *the full      * theme name. *
 *                                                                                             *
 * OUTPUT:  Returns with the matching theme number. If no match could be found,
 *then           * THEME_NONE is returned. *
 *                                                                                             *
 * WARNINGS:   If a filename is specified the comparison is case insensitive.
 *When scanning    * the full theme name, the comparison is case sensitive. *
 *                                                                                             *
 * HISTORY: * 05/29/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 * ThemeClass::Scan -- Scans all scores for availability. *
 *                                                                                             *
 *    This routine should be called whenever a score mixfile is registered. It
 *will scan       * to see if any score is unavailable. If this is the case,
 *then the score will be so       * flagged in order not to appear on the play
 *list. This condition is likely to occur       * when expansion mission disks
 *contain a different score mix than the release version.     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * ThemeClass::Set_Theme_Data -- Set the theme data for scenario and owner. *
 *                                                                                             *
 *    This is an override function used to set a particular theme's initial
 *scenario and       * owner values. Typically, the rules control file will be
 *the source of calling this       * routine. *
 *                                                                                             *
 * INPUT:   theme    -- The theme to set these override values for. *
 *                                                                                             *
 *          scenario -- The first scenario when this theme becomes available on
 *the play list. *
 *                                                                                             *
 *          owners   -- A bitfield representing the owners allowed to play this
 *song.          *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
