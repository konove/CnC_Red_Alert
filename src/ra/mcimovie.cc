...
bool MCIMovie::Open(const char* name, const char* device) {
  ...
  rc = mciSendCommand(0, MCI_OPEN, MCI_WAIT | MCI_OPEN_TYPE | MCI_OPEN_ELEMENT,
                      (DWORD)&mOpenParm);
  ...
  rc = mciSendCommand(mDeviceID, MCI_BREAK,
                      MCI_WAIT | MCI_BREAK_HWND | MCI_BREAK_KEY,
                      (DWORD)&breakParm);
...
bool MCIMovie::SizeDestination() {
  ...
  rc = mciSendCommand(mDeviceID, MCI_PUT,
                      MCI_WAIT | MCI_DGV_RECT | MCI_DGV_PUT_DESTINATION,
                      (DWORD)&putParm);
...
bool MCIMovie::AttachWindow(HWND window) {
  ...
  rc = mciSendCommand(mDeviceID, MCI_WINDOW,
                      MCI_WAIT | MCI_DGV_WINDOW_HWND | MCI_DGV_WINDOW_STATE,
                      (DWORD)&winParm);
