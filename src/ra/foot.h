...
class FootClass : public TechnoClass {
 public:
  /*
  **	This flag controls whether a range limiting effect should be in place.
  *If *	true, then target scanning will be limited to the range of the object
  **	regardless of what was requested from the target scanning logic. This
  *value *	is used for ships so that they won't permanently stick on a an
  *attack mission *	for a target they can never get within range of. This
  *value will toggle when *	a path cannot be generated and the target is not
  ...
  *than *	the more efficient "ring scan". This situation occurs when a
  ...
  *this *	operation begins, certain processes must occur. During these
  ...
  *flag is *	true. A short range destination is either the next cell or the
  *end of the *	current "curvy" track. An object that is driving is not allowed
  *to do anything *	else until it reaches its destination. The exception is
  ...
  *be *	set to true. This handles the unusual case of an object disembarking
  *from the *	hover lander yet not necessarily tethered but still located in
  ...
  *with 0 = stop *	and 255 = full speed.
  */
  int Speed;
  ...
  */
  ...
  /*
  **
  **	This is the desired destination of the unit. The unit will attempt to
  *head *	toward this target (avoiding intervening obstacles).
  */
  TARGET NavCom;
  ...
  **	quickly process the team when this object is the source of the change.
  ...
  *This *	is used to quickly process each team member when the team class
  *is the source *	of the change. An example would be if the team decided
  ...
  **	This path list is limited in size, so it might require several
  ...
  **	unit it reaches the point of complete failure. When that event occurs,
  *the *	unit will realize that it cannot get to its specified
  *destination and will *	try to perform some other action instead.
  */
  ...
  */
  Timer<FrameTickSource> BaseAttackTimer;
  ...
  */
  ...
  TARGET Greatest_Threat(ThreatType method) override;  // const;
...
#endif  // CNC_RED_ALERT_RA_FOOT_H_
