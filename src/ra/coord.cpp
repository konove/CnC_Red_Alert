 ...
 *point number.                 * Coord_Cell -- Convert a coordinate into a cell
 *number.                                    * Coord_Move -- Moves a coordinate
 *an arbitrary direction for an arbitrary distance         * Coord_Scatter --
 *Determines a random coordinate from an anchor point.                     *
 *   Coord_Spillage_List -- Calculate a spillage list for the dirty rectangle
 *specified.       * Coord_Spillage_List -- Determines the offset list for cell
 *spillage/occupation.           * Distance -- Determines the cell distance
 *between two cells.                               * Distance -- Determines the
 *lepton distance between two coordinates.                       * Distance --
 *Fetch distance between two target values. * Fixed_To_Cardinal -- Converts a
 *fixed point number into a cardinal number.                * Normal_Move_Point
 *-- Moves point with tilt compensation.                                  *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 *    This routine will convert the specified coordinate value into a cell
 *number. This is     * useful to determine the map index number into the cell
 *array that corresponds to a       * particular coordinate. *
 *                                                                                             *
 * INPUT:   coord -- The coordinate to convert into a cell number. *
 *                                                                                             *
 * OUTPUT:  Returns with the cell number that corresponds to the coordinate
 *specified.         *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will determine the lepton distance between the two specified
 *target         * values. *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   Be sure that the targets are legal before calling this routine.
...
short const* Coord_Spillage_List(COORDINATE coord, int maxsize) {
  ...
  //;	10 = above axis
  //;	11 = undefined
  static signed char const _SpillTable[16] = {8, 6, 2, -1, 0,  7,  1,  -1,
                                              4, 5, 3, -1, -1, -1, -1, -1};
  int index = 0;
  int x, y;
 ...
 *specified.         *
 *                                                                                             *
 *    Given a center coordinate and a dirty rectangle, calcuate a cell offset
 *list for         * determining such things as overlap and redraw logic.
 *Optionally, the center cell         * location will not be part of the list. *
 *                                                                                             *
 * INPUT:   coord -- The center coordinate that the dirty rectangle is based off
 *of.           *
 *                                                                                             *
 *          rect  -- Reference to the dirty rectangle. *
 *                                                                                             *
 *          nocenter -- If true, then the center cell offset will not be part of
 *the spillage  * list returned. This is handy when the center cell is known to
 *be       * processed by some other method and it can be safely and efficiently
 ** ignored by the list generated.                                         *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the spillage list that corresponds to the
 *data           * specified. This is a pointer to a static buffer and as such
 *it will only be valid  * until the next time that this routine is called. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
short const* Coord_Spillage_List(COORDINATE coord, Rect const& rect,
  ...
  /*
  **	Add the rectangle values to the coordinate in order to normalize the
  *start and end *	corners of the rectangle. The values are now absolute to
  *the real game world rather *	than relative to the coordinate.
  */
  ...
  /*
  **	Determine the upper left and lower right cell indexes. This is a simple
  *conversion from *	their lepton counterpart. These cells values are used to
  *form the bounding box for the *	map offset list.
  */
  ...
  /*
  **	Generate the spillage list by counting off the rows and colums of the
  *cells *	that are affected. This is easy since the upper left and lower
  *right corner cells *	are known.
  */
  int count = 0;
 ...
 *                                                                                             *
 *    This routine will move the point in the direction and distance specified
 *but it will     * take into account the tilt of the playing field. Typical use
 *of this routine is to       * determine positioning as it relates to the
 *playfield. Turrets are a good example of      * this. *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/19/1995 JLB : Created. *
 *=============================================================================================*/
