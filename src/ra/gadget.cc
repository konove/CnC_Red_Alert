 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * GadgetClass::Action -- Base action for gadget. *
 ...
 *   GadgetClass::Enable -- Enables the gadget. * GadgetClass::Extract_Gadget --
 *Sweeps through the gadget chain to find gadget specified.  *
 ...
 *   GadgetClass::Has_Focus -- Checks if this object currently has the keyboard
 *focus.         * GadgetClass::Remove -- Removes the specified gadget from the
 *list.                        * GadgetClass::Set_Focus -- Sets focus to this
 *gadget.                                      * GadgetClass::Sticky_Process --
 *Handles the sticky flag processing.                        *
 *   GadgetClass::~GadgetClass -- Destructor for gadget object. *
 *   GadgetClass::Is_List_To_Redraw -- tells if any gadget in the list needs
 *redrawing         * GadgetClass::GadgetClass -- Constructor for the gadget
 *object.                            * GadgetClass::Set_Position -- Set the
 *coordinate position of this gadget.                  *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
*/
static RemapControlType DefaultColorScheme = {15, 0, {}, {}, 0, 0,
                                              0,  0, 0,  0,  0, 0};
RemapControlType* GadgetClass::ColorScheme = &DefaultColorScheme;

/***********************************************************************************************
 * GadgetClass::GadgetClass -- Constructor for gadget object. *
 *                                                                                             *
 *    This is the normal constructor for gadget objects. A gadget object is only
 *concerned     * with the region on the screen to considered "its own" as well
 *as the flags that tell     * what mouse action should be recognized when the
 *mouse is over this screen area.          *
 *                                                                                             *
 * INPUT:   x,y   -- Coordinates (in pixels) of the upper left hand corner of
 *the region that  * will be "owned" by this gadget. *
 *                                                                                             *
 *          w,h   -- Width and height (in pixels) of this gadget's region. *
 *                                                                                             *
 *          flags -- The flags (mouse conditions) that will cause this gadget's
 ...
 *this gadget   * if this gadget currently has the focus. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
bool GadgetClass::Clicked_On(KeyNumType& key, unsigned flags, int mousex,
  ...
  */
  ...
  /*
  **	If keyboard input should be processed by this "gadget" and keyboard
  *input is *	detected, then always call the action function. It is up to the
  *action function *	in this case to either ignore the keyboard input or not.
  **
  **	For mouse actions, check to see if the mouse is in the region of the
 ...
 *                                                                                             *
 *    This function enables the gadget. An enabled gadget will be processed for
 *input          * purposes. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * GadgetClass::Disable -- Disables the gadget from input processing. *
 *                                                                                             *
 *    This routine will disable the gadget. A disabled gadget might be rendered,
 *but is        * ignored for input processing. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    Use this routine if an individual gadget needs to be removed from the list
 *of gadgets.   *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  bool; Was the specified gadget found and removed? A false indicates
 *that the       * gadget wasn't in the list. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 01/15/1995 JLB : Created. *
 ...
 * GadgetClass::Get_Next -- Returns a pointer to the next gadget in the chain. *
 *                                                                                             *
 *    This returns with the next gadget's pointer. It is identical to the base
 *Get_Next()      * function, but returns a pointer to a GadgetClass object. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * GadgetClass::Get_Prev -- Fetches a pointer to the previous gadget. *
 *                                                                                             *
 *    This routine will return the previous gadget in the list. It is identical
 *to the base    * function Get_Prev, but returns a pointer to a GadgetClass
 *object.                        *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * GadgetClass::Delete_List -- Deletes all gadget objects in list. *
 *                                                                                             *
 *    This function will delete all gadgets in the list. It is the counterpart
 *to the          * Create_One_Of functions. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   Any references to these gadget become invalidated by this
 *routine.              *
 *                                                                                             *
 * HISTORY: * 01/15/1995 JLB : Created. *
 *=============================================================================================*/
void GadgetClass::Delete_List() {
  ...
  */
  ...
  /*
  **	First delete all the gadgets following the first one. The reason the
  *first one *	is kept around is that sometimes deleting one gadget will result
  *in related gadgets *	in the same list also being deleted. The first gadget
  *will always contain the *	correct gadget pointer.
  */
  while (g) {
    g->Clear_Focus();
 ...
 *                                                                                             *
 *    This handles the base level action that a gadget performs when a
 *qualifying input event  * is detected. This sets the redraw flag and returns
 *true (to stop further processing).    * If no qualifying input event was
 *detected, but this routine was called anyway, then      * don't perform any
 *action. The call to this routine, in that case, must have been forced  * for
 *some other reason. *
 *                                                                                             *
 * INPUT:   flag  -- The input event bits that qualify for this gadget. A NULL
 *indicates that  * no qualifying event occurred. *
 *                                                                                             *
...
bool GadgetClass::Action(unsigned flags, KeyNumType& /*unused*/) {
  ...
  *make sure that *	any sticky flags are cleared up.
  */
  if (flags) {
    IsToRepaint = true;
 ...
 * GadgetClass::Draw_Me -- Gadget redraw action (flag control). *
 *                                                                                             *
 *    At this level, there is no actual rendering taking place with the call to
 *Draw_Me, but   * the IsToRepaint flag must be cleared. Derived objects will
 *call this routine and if it   * returns true, they will perform their custom
 *rendering.                                  *
 *                                                                                             *
 ...
 * GadgetClass::Draw_All -- Forces all gadgets in list to be redrawn. *
 *                                                                                             *
 *    Use this function to cause all gadget in the list to be redrawn regardless
 *of the state  * of the IsToRepaint flag. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
KeyNumType GadgetClass::Input() {
  ...
  *MouseQ... *	globals rather than their normal Mouse... globals. This is
  *because we need to *	know the position of the mouse at the exact instant when
  ...
  */
  if (!key) {
    ...
    *mouse *	buttons. As a side note, if we determine that the mouse button
  ...
  if (StuckOn) {
    ...
    */
    if (Focused && flags & KEYBOARD) {
      ...
      /*
      **	Sweep through all the buttons in the chain and pass the current
      *button state *	and keyboard input data to them. These routines will
      *detect whether they should *	perform some action and return a flag to
      *this effect. They also have the option *	of changing the key value so
...
/***********************************************************************************************
 * GadgetClass::Extract_Gadget -- Sweeps through the gadget chain to find gadget
 *specified.    *
 *                                                                                             *
 *    This examines the gadget list looking for on that has the same ID as
 *specified. If that  * gadget was found, then a pointer to it is returned.
 *Since only ControlClass gadgets      * or ones derived from it can have an ID
 *value, we know that the returned pointer is at    * least of the ControlClass
 *type.                                                          *
 *                                                                                             *
 * INPUT:   id -- The ID number to scan for. Zero is not a legal ID number and
 *if passed in,   * a NULL will always be returned. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the ControlClass gadget that has the
 *matching ID number. * If no matching gadget was found, then NULL is returned.
 **
 *                                                                                             *
 * WARNINGS:   If there happens to be more than one gadget with a matching ID,
 *this routine    * will return a pointer to the first one only. *
 *                                                                                             *
 ...
 * GadgetClass::Flag_To_Redraw -- Flags this gadget to be redrawn. *
 *                                                                                             *
 *    Use this routine to flag the gadget to be redrawn. A gadget so flagged
 *will have its     * Draw_Me function called at the next available opportunity.
 *Usually, this is the next     * time the Input() function is called. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    This function examines the event flags and handles any "sticky" processing
 *required.     * Sticky processing is when the button is flagged with the
 *"IsSticky" bit and it will      * be processed to the exclusion of all other
 *gadgets while the mouse button is held        * down. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This will set the focus to this gadget regardless of any current focus
 *setting. If there * is another gadget that has focus, it will have its focus
 *cleared before this gadget will * get the focus. A focused gadget is one that
 *has all keyboard input routed to it.         *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * GadgetClass::Clear_Focus -- Clears the focus if this gadget has it. *
 *                                                                                             *
 *    Use this function to clear the focus for the gadget. If the gadget doesn't
 *currently     * have focus, then this routine will do nothing. For added
 *functionality, overload this    * virtual function so that gadget specific
 *actions may be take when focus is lost.         *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *focus.           *
 *                                                                                             *
 *    If this object has the keyboard focus, then this routine will return true.
 *When the      * gadget has keyboard focus, all keyboard events get routed to
 *the gadget.                 *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine helps with moving a gadget's location. It will set the
 *gadgets upper        * left corner to the pixel location specified. *
 *                                                                                             *
 * INPUT:   x,y   -- The X and Y position to put the gadget's upper left corner
 *to.            *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
