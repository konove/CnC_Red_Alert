 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MapClass::Base_Region -- Finds the owner and base zone for
 *specified cell.                * MapClass::Cell_Region -- Determines the
 *region from a specified cell number.              * MapClass::Cell_Threat --
 *Gets a houses threat value for a cell                            *
 *   MapClass::Close_Object -- Finds a clickable close object to the specified
 *coordinate.     * MapClass::Destroy_Bridge_At -- Destroyes the bridge at
 *location specified.                * MapClass::Detach -- Remove specified
 *object from map references.                          * MapClass::In_Radar --
 *Is specified cell in the radar map?                                 *
 *   MapClass::Init -- clears all cells * MapClass::Intact_Bridge_Count --
 *Determine the number of intact bridges.                  * MapClass::Logic --
 *Handles map related logic functions.                                   *
 *   MapClass::Nearby_Location -- Finds a generally clear location near a
 *specified cell.      * MapClass::One_Time -- Performs special one time
 *initializations for the map.              * MapClass::Overlap_Down -- computes
 *& marks object's overlap cells                         * MapClass::Overlap_Up
 *-- Computes & clears object's overlap cells                          *
 *   MapClass::Overpass -- Performs any final cleanup to a freshly constructed
 *map.            * MapClass::Pick_Up -- Removes specified object from the map.
 ...
 *   MapClass::Set_Map_Dimensions -- Initialize the map. * MapClass::Sight_From
 *-- Mark as visible the cells within a specified radius.              *
 *   MapClass::Validate -- validates every cell on the map *
 *   MapClass::Write_Binary -- Pipes the map template data to the destination
 *specified.       * MapClass::Zone_Reset -- Resets all zone numbers to match
 *the map.                         * MapClass::Zone_Span -- Flood fills the
 *specified zone from the cell origin.               *
 ...
 **
 *                                                                                             *
 *    This routine is used by the game initialization function in order to
 *perform any one     * time initializations required for the map. This includes
 *allocation of the map and       * setting up its default dimensions. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 * This routine is used by Init_Clear to set the cells to a known state; it's
 *also used by     * the Load_Game routine to init all cells before loading a
 *set of cells from disk, so it      * needs to be called separately from the
 *other Init_xxx() routines.                           *
 *                                                                                             *
 * INPUT: * none. *
 *                                                                                             *
 * OUTPUT: * none. *
 ...
 *                                                                                             *
 *    This routine is used to set the legal limits and position of the * map as
 *it relates to the overall map array. Typically, this is * called by the
 *scenario loading code.                                                     *
 *                                                                                             *
 * INPUT:   x,y   -- The X and Y coordinate of the "upper left" corner * of the
 *map.                                                               *
 *                                                                                             *
 ...
 *    Typically, as a unit moves or is deployed, this routine will be * called.
 *Since it deals with MANY cells, it needs to be extremely * fast. *
 *                                                                                             *
 * INPUT:   cell     -- The coordinate that the sighting originates from. *
 *                                                                                             *
 *          sightrange-- The distance in cells that sighting extends. *
 *                                                                                             *
 *          incremental-- Is this an incremental sighting. In other * words, has
 *this function been called before where                      * the center
 *coordinate is no more than one cell                         * distant from the
 *last time?                                            *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/19/1992 JLB : Created. * 03/08/1994 JLB : Updated to use sight
 ...
 *    Typically, as a gap generator refreshes (when Encroach_Shadow() is called)
 *this routine's* called. Since it deals with MANY cells, it needs to be
 *extremely                         * fast. *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 11/10/1995 BWG : Created. *
 *=============================================================================================*/
 ...
 *    called. Since it deals with MANY cells, it needs to be extremely * fast. *
 *                                                                                             *
...
#ifdef OBSOLETE
  /*
  **	The objects on the map need to perform a manual look operation if they
  *happen *	to have their sight range overlap the gap radius.
  */
  if (!house->IsPlayerControl) {
    //	if (house != PlayerPtr) {
  ...
  for (int index = 0; index < Units.Count(); index++) {
    ...
    if (unit && !unit->IsInLimbo && unit->House == PlayerPtr) {
      ...
      if (dist <= unit->Class->SightRange +
                      Rule.GapShroudRadius /*gap generator sightrange*/) {
        unit->Look();
      }
    }
  }
...
/***********************************************************************************************
 * MapClass::UnJam_From -- Remove jamming on the cells within a specified
 *radius.              *
 *                                                                                             *
 *    This routine is used to jam the cells around a specific location. *
 *    Typically, as a gap generator structure is created, this routine will be *
 ...
 *                                                                                             *
 *    This determines if the specified cell can be within the navigable * bounds
 *of the map. Technically, this means, any cell that can be * scanned by radar.
 *If a cell returns false from this function, then                       * the
 *player could never move to or pass over this cell. *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 10/07/1992 JLB : Created. * 04/30/1994 JLB : Converted to member
 *function.                                            * 05/01/1994 JLB :
 *Speeded up.                                                              *
 *=============================================================================================*/
 ...
 * MapClass::Place_Down -- Places the specified object onto the map. *
 *                                                                                             *
 *    This routine is used to place an object onto the map. It updates the
 *"occupier" of the   * cells that this object covers. The cells are determined
 *from the Occupy_List function    * provided by the object. Only one cell can
 *have an occupier and this routine is the only  * place that sets this
 *condition.                                                          *
 *                                                                                             *
 ...
 *                                                                                             *
 *    The object specified is removed from the map by this routine. This will
 *remove the       * occupation flag for all the cells that the object covers.
 *The cells that are covered     * are determined from the Occupy_List function.
 **
 *                                                                                             *
 ...
 *                                                                                             *
 * This routine is just like Place_Down, but it doesn't mark the cell's
 *Occupier.              * This routine is used to implement MARK_OVERLAP_DOWN,
 *which is useful for changing           * an object's render size, but not its
 *logical size (ie when it's selected or an              * animation is attached
 *to it).                                                               *
 *                                                                                             *
 * INPUT: * cell -- The cell to base object overlap around. * object   -- The
 *object to place onto the map.                                            *
 *                                                                                             *
 * OUTPUT: * none. *
 *                                                                                             *
 * WARNINGS: * none. *
 ...
 * This routine is used to implement MARK_OVERLAP_UP, which is useful for
 *changing             * an object's render size, but not its logical size (ie
 *when it's selected or an              * animation is attached to it). *
 *                                                                                             *
 * INPUT: * cell -- The cell to base object overlap around. * object   -- The
 *object to place onto the map.                                            *
 *                                                                                             *
 ...
 *map.              *
 *                                                                                             *
 *    This routine will clean up anything necessary with the presumption that
 *the map has      * been freshly created. Such things to clean up include
 *various tiberium concentrations.   *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/19/1994 JLB : Created. * 02/13/1995 JLB : Returns total
 ...
 *specified.         *
 *                                                                                             *
 *    This stores the template data from the map to the output pipe specified.
 *The template    * data consists of the template type number and template icon
 *number for every cell on     * the map. The output is organized in such a way
 *so as to get maximum compression.         *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 * INPUT:   straw -- Reference to the straw that will supply the map template
 *data.            *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/11/1995 JLB : Created. * 07/09/1995 JLB : Handles two
    ...
    if (In_Radar(cell)) {
      ...
      */
      if (ptr->Can_Tiberium_Grow()) {
        /*
        **	Either replace an existing recorded cell value or add the new
 ...
 * MapClass::Place_Random_Crate -- Places a crate at random location on map. *
 *                                                                                             *
 *    This routine will place a crate at a random location on the map. This
 *routine will only  * make a limited number of attempts to place and if
 *unsuccessful, it will not place any.   *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 * OUTPUT:  bool; Was a crate found at the location specified and was it
 *removed?              *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *coordinate.       *
 *                                                                                             *
 *    This routine is used by the mouse input processing code to find a
 *clickable object       * close to coordinate specified. This is for targeting
 *as well as selection determination. *
 *                                                                                             *
 * INPUT:   coord -- The coordinate to scan for close object from. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to an object that is nearby the specified
 *coordinate.       *
 *                                                                                             *
 * WARNINGS:   There could be a cloaked object at the location, but it won't be
 *considered     * if it is not owned by the player. *
 *                                                                                             *
...
ObjectClass* MapClass::Close_Object(COORDINATE coord) const {
  ...
  */
  ...
  /*
  **	Only return the object if it is within 1/4 cell distance from the
 ...
 *                                                                                             *
 *    This routine will rescan the map and fill in the zone values for each of
 *the cells.      * All cells that are contiguous are given the same zone
 *number.                            *
 *                                                                                             *
 * INPUT:   method   -- The method to recalculate the zones upon. If 1 then
 *recalc non         * crushable zone. If 2 then recalc crushable zone. If 3,
 *then            * recalc both zones. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This is a time consuming routine. Call it as infrequently as
 *possible. It must  * be called whenever something that would affect
 *contiguousness occurs. Example:  * when a bridge is built or destroyed. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is used to fill a zone value into the map. The map is "flood
 *filled" from   * the cell specified. All adjacent (8 connected) and generally
 *passable terrain cells are  * given the zone number specified. This routine
 *checks for legality before filling         * occurs. The routine is safe to
 *call even if the legality of the cell is unknown at the   * time of the call.
 **
 *                                                                                             *
 ...
 *                                                                                             *
 * HISTORY: * 09/25/1995 JLB : Created. * 10/05/1996 JLB : Examines crushable
 *walls.                                                *
 *=============================================================================================*/
 ...
 *specified cell.        *
 *                                                                                             *
 *    This routine is used to find a location that probably will be ok to move
 *to that is      * located as close as possible to the specified cell. The
 *computer uses this when it has   * determined the ideal location for an
 *object, but then needs to give a valid movement     * destination to a unit. *
 *                                                                                             *
 * INPUT:   cell  -- The cell that scanning should radiate out from. *
 *                                                                                             *
 *          zone  -- The zone that must be matched to find a legal location
 *(value of -1 means * any zone will do). *
 *                                                                                             *
 *                                                                                             *
 *          check -- The type of zone to check against. Only valid if a zone
 *value is given.   *
 *                                                                                             *
 * OUTPUT:  Returns with the cell that is generally clear (legal to move to)
 *that is close     * to the specified cell. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 10/05/1995 JLB : Created. *
 *=============================================================================================*/
CELL MapClass::Nearby_Location(CELL cell, SpeedType speed, int zone,
 ...
 *                                                                                             *
 *    This routine is used to determine what base the specified cell is close to
 *and what      * zone of that base the cell lies in. This routine is
 *particularly useful in letting the   * computer know when the player targets a
 *destination near a computer's base.              *
 *                                                                                             *
 * INPUT:   cell     -- The cell that is to be checked. *
 *                                                                                             *
 *          house    -- Reference to the house type number. This value will be
 *set if a base   * was found nearby the specified cell. *
 *                                                                                             *
 *          zone     -- The zone that the cell is located in IF the cell is near
 *a base.       *
 *                                                                                             *
 *                                                                                             *
 * OUTPUT:  Was a base near the specified cell found? If not, then the 'house'
 *and 'zone'      * reference values are left in an undefined state and the
 ...
 *                                                                                             *
 *    This routine will take the object (represented by a target value) and
 *remove all         * references to it from the map. Typically, this is used to
 ...
 *                                                                                             *
 *    This will examine the entire map and return the number of bridges that are
 *intact. An    * intact bridge is one that units can travel over. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * MapClass::Pick_Random_Location -- Picks a random location on the map. *
 *                                                                                             *
 *    This routine will pick a random location on the map. It performs no
 *legality checking    * other than forcing the cell to be on the map proper. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * MapClass::Shroud_The_Map -- cover the whole map in darkness (usually from
 *blackout crate)	  *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
