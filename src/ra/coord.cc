...
// Fixed-point multiply: tables store values scaled by 127, so >> 7 converts
// back. calcy negates because screen Y increases downward.
int calcx(const signed short v, const short distance) {
  return (v * distance) >> 7;
}
int calcy(const signed short v, const short distance) {
  return -((v * distance) >> 7);
}

void Move_Point(short& x, short& y, const DirType dir,
                const unsigned short distance) {
  static constexpr signed char CosTable[256] = {
  ...
  static constexpr signed char SinTable[256] = {
      127,  126,  126,  126,  126,  126,  125,  125,  124,  123,  123,
      122,  121,  120,  119,  118,  117,  116,  114,  113,  112,  110,
      108,  107,  105,  103,  102,  100,  98,   96,   94,   91,   89,
      87,   85,   82,   80,   78,   75,   73,   70,   67,   65,   62,
...
void Normal_Move_Point(short& x, short& y, const DirType dir,
  ...
  x += calcx(CosTable[dir], distance);
  // Halve Y to compensate for the 2:1 isometric perspective.
