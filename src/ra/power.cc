 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * PowerClass::AI -- Process the power bar logic. *
 ...
 *   PowerClass::PowerButtonClass::Action -- Handles the mouse over the power
 *bar area.        * PowerClass::PowerClass -- Default constructor for the power
 *bar class.                    * PowerClass::Refresh_Cells -- Intercepts the
 *redraw logic to see if sidebar to redraw too. * PowerClass::Power_Height --
 *Given a value figure where it falls on bar                    *
 ...
 *                                                                                             *
 *    This is the default constructor for the power bar class. It doesn't really
 *do anything.  *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *=============================================================================================*/
PowerClass::PowerClass() : FlashTimer(0) {}

/***********************************************************************************************
 * PowerClass::Init_Clear -- Clears all the power bar variables. *
 *                                                                                             *
 *    This routine is called in preparation for the start of a scenario. The
 *power bar is      * initialized into the null state by this routine. As soon
 *as the scenario starts, the     * power bar will rise to reflect the actual
 ...
 *                                                                                             *
 * INPUT:   complete -- Should the power bar be redrawn even if it isn't
 *specifically flagged  * to do so? *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
 ...
 *                                                                                             *
 *    Use this routine to process the power bar logic. This consists of
 *animation effects.     *
 *                                                                                             *
 * INPUT:   input -- The player input value to be consumed or ignored as
 *appropriate.          *
 *                                                                                             *
...
/***********************************************************************************************
 * PowerClass::Refresh_Cells -- Intercepts the redraw logic to see if sidebar to
 *redraw too.   *
 *                                                                                             *
 *    This routine will examine a refresh list request and determine if the
 *sidebar would be   * affect. If so, it will flag the sidebar to be redrawn. *
 *                                                                                             *
 * INPUT:   cell  -- The cell that the offset list is base on. *
 *                                                                                             *
 *          list  -- The list of cell offset used to flag for redraw. If the
 *special sidebar   * affecting cell magic offset number is detected, the
 *sidebar is flagged    * for redraw and the magic offset is removed. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
...
/***********************************************************************************************
 * PowerClass::PowerButtonClass::Action -- Handles the mouse over the power bar
 *area.          *
 *                                                                                             *
 *    This routine handles input on the power bar area. Since no input is used
 *for the power   * bar, this routine just pops up appropriate help text for the
 *power bar.                  *
 *                                                                                             *
 ...
 * PowerClass::Flash_Power -- Flag the power bar to flash. *
 *                                                                                             *
 *    This will cause the power bar to display with a flash so as to draw
 *attention to         * itself. Typical use of this effect is when power is
 *low.                                 *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
