 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * AircraftClass::AI -- Processes the normal non-graphic AI for the
 *aircraft.                * AircraftClass::Active_Click_With -- Handles
 *clicking over specified cell.                 *
 ...
 *   AircraftClass::Can_Enter_Cell -- Determines if the aircraft can land at
 *this location.    * AircraftClass::Can_Fire -- Checks to see if the aircraft
 *can fire.                        * AircraftClass::Cell_Seems_Ok -- Checks to
 *see if a cell is good to enter.                 *
 *   AircraftClass::Desired_Load_Dir -- Determines where passengers should line
 *up.            * AircraftClass::Draw_It -- Renders an aircraft object at the
 *location specified.           * AircraftClass::Draw_Rotors -- Draw rotor
 *blades on the aircraft.                          *
 *   AircraftClass::Edge_Of_World_AI -- Detect if aircraft has exited the map. *
 *   AircraftClass::Enter_Idle_Mode -- Gives the aircraft an appropriate
 *mission.              * AircraftClass::Exit_Object -- Unloads passenger from
 *aircraft.                            * AircraftClass::Fire_At -- Handles
 *firing a projectile from an aircraft.                   *
 *   AircraftClass::Fire_Direction -- Determines the direction of fire. *
 *   AircraftClass::Good_Fire_Location -- Searches for and finds a good spot to
 *fire from.     * AircraftClass::Good_LZ -- Locates a good spot ot land. *
 *   AircraftClass::In_Which_Layer -- Calculates the display layer of the
 *aircraft.            * AircraftClass::Init -- Initialize the aircraft system
 *to an empty state.                  * AircraftClass::Is_LZ_Clear -- Determines
 *if landing zone is free for landing.             *
 *   AircraftClass::Landing_Takeoff_AI -- Handle aircraft take off and landing
 *processing.     * AircraftClass::Look -- Aircraft will look if they are on the
 *ground always.               * AircraftClass::Mission_Attack -- Handles the
 *attack mission for aircraft.                 * AircraftClass::Mission_Enter --
 *Control aircraft to fly to the helipad or repair center.  *
 ...
 *   AircraftClass::Mission_Retreat -- Handles the aircraft logic for leaving
 *the battlefield. * AircraftClass::Mission_Unload -- Handles unloading cargo. *
 *   AircraftClass::Movement_AI -- Handles aircraft physical movement logic. *
 *   AircraftClass::New_LZ -- Find a good landing zone. *
 *   AircraftClass::Overlap_List -- Returns with list of cells the aircraft
 *overlaps.          * AircraftClass::Paradrop_Cargo -- Drop a passenger by
 *parachute.                           * AircraftClass::Per_Cell_Process --
 *Handle the aircraft per cell process.                  *
 *   AircraftClass::Pip_Count -- Returns the number of "objects" in aircraft. *
 *   AircraftClass::Player_Assign_Mission -- Handles player input to assign a
 *mission.         * AircraftClass::Pose_Dir -- Fetches the natural landing
 *facing.                            * AircraftClass::Process_Fly_To -- Handles
 *state machine for flying to destination.         *
 ...
 *   AircraftClass::Shape_Number -- Fetch the shape number to use for the
 *aircraft.            * AircraftClass::Sort_Y -- Figures the sorting
 *coordinate.                                  * AircraftClass::Take_Damage --
 *Applies damage to the aircraft.                             *
 ...
 *   AircraftClass::operator new -- Allocates a new aircraft object from the
 *pool              * AircraftClass::~AircraftClass -- Destructor for aircraft
 *object.                          * _Counts_As_Civ_Evac -- Is the specified
 *object a candidate for civilian evac logic?       *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *logic?         *
 *                                                                                             *
 *    Examines the specified object to see if it qualifies to be a civilian
 *evacuation. This   * can only occur if it is a civilian (or Tanya) and the
 *special evacuation flag has been   * set in the scenario control structure. *
 *                                                                                             *
 * INPUT:   candidate   -- Candidate object to examine for civilian evacuation
 *legality.       *
 *                                                                                             *
 * OUTPUT:  bool; Is the specified object considered a civilian that must be
 *auto-evacuated?   *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
static bool Counts_As_Civ_Evac(const ObjectClass* candidate) {
  ...
  */
  const auto* inf = dynamic_cast<const InfantryClass*>(candidate);

  /*
  **	Certain infantry types will always be considered a civilian evacuation
  *candidate. These *	include the special one-time infantry that appear in
  *some missions.
  */
 ...
 * AircraftClass::operator delete -- Deletes the aircraft object. *
 *                                                                                             *
 *    This routine will return the aircraft object back to the free aircraft
 *object pool.      *
 *                                                                                             *
 ...
 * AircraftClass::AircraftClass -- The constructor for aircraft objects. *
 *                                                                                             *
 *    This routine is the constructor for aircraft objects. An aircraft object
 *can be          * created and possibly placed into the game system by this
 *routine.                        *
 *                                                                                             *
 ...
 * AircraftClass::Unlimbo -- Removes an aircraft from the limbo state. *
 *                                                                                             *
 *    This routine is used to transition the aircraft from the limbo to the non
 *limbo state.   * It occurs when the aircraft is placed on the map for whatever
 *reason. When it is         * unlimboed, only then will normal game processing
 *recognize it.                           *
 *                                                                                             *
 ...
 *aircraft.              *
 *                                                                                             *
 *    This will determine what shape number to use for the aircraft in its
 *current state.      * The shape number can be used for drawing or determine
 ...
 *                                                                                             *
 * WARNINGS:   Some aircraft, particularly helicopters, require other shapes
 *attached to it.   *
 *                                                                                             *
...
/***********************************************************************************************
 * AircraftClass::Draw_It -- Renders an aircraft object at the location
 *specified.             *
 *                                                                                             *
 *    This routine is used to display the aircraft object at the coordinates
 *specified.        * The tactical map display uses this routine for all
 *aircraft rendering.                   *
 *                                                                                             *
...
void AircraftClass::Draw_It(int x, int y, WindowNumberType window) const {
  ...
  **	rotation value accordingly. A rotation of DIR_N means no rotation at
  *all.
  */
 ...
 *                                                                                             *
 *    This routine will draw rotor blades on the aircraft. It is presumed that
 *the aircraft    * has already been drawn at the X and Y pixel coordinates
 *specified.                       *
 *                                                                                             *
...
void AircraftClass::Draw_Rotors(int x, int y, WindowNumberType window) const {
  ...
  */
  ...
  if (*this == AIRCRAFT_TRANSPORT) {
    const base::EnumArray<FacingType, int> _stretch = {8, 9, 10, 9,
                                                       8, 9, 10, 9};
 ...
 * AircraftClass::Read_INI -- Reads aircraft object data from an INI file. *
 *                                                                                             *
 *    This routine is used to read the aircraft object data from the INI file
 *buffer           * specified. This is used by the scenario loader code to
 *interpret the INI file and        * create the specified objects therein. *
 *                                                                                             *
 ...
 * AircraftClass::Mission_Hunt -- Maintains hunt AI for the aircraft. *
 *                                                                                             *
 *    Hunt AI consists of finding a target and attacking it. If there is no
 *target assigned    * and this unit doesn't automatically hunt for more
 *targets, then it will change           * mission to a more passive (land and
 *await further orders) type.                          *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
int AircraftClass::Mission_Hunt() {
  ...
  if (Class->IsFixedWing) {
    ...
    constexpr int kFlyToTarget = 2;
    constexpr int kDropBombs = 3;
    constexpr int kRegroup = 4;
    switch (Status) {
      /*
 ...
 *                                                                                             *
 *    This handles the non-graphic AI processing for the aircraft. This usually
 *entails        * maintenance and other AI functions. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
void AircraftClass::AI() {
  ...
  */
  ...
  /*
  **	Perform any base class AI processing. If during this process, the
  *aircraft was *	destroyed, then detect this and bail from this AI
  *routine early.
  */
  FootClass::AI();
  if (!IsActive) {
  ...
  **	performance hit, but there is no other choice. The cells under an
  *aircraft *	do not know if there is an aircraft above it. Thus, it cannot
  *flag the *	aircraft to redraw. As a consequence, all aircraft must redraw.
  */
  ...
  *appropriate. As the aircraft transitions *	between flying level and ground
  ...
  */
  ...
  /*
  **	Always flag the map draw process to occur if there is an aircraft in the
  ...
  */
  ...
  /*
  **	When aircraft leave the edge of the map, they might get destroyed. This
  *occurs if the *	aircraft is a non-player produced unit and it has
 ...
 *overlaps.            *
 *                                                                                             *
 *    When aircraft are flying, they can overlap quite a number of cells. These
 *cells can      * be determined from the coordinate where the aircraft is
 *centered and the size of the     * aircraft's shape. Landed aircraft are a
 *special case and are usually much smaller        * than when flying. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to a cell offset list that specifies all
 *cells that         * the aircraft overlaps given the aircraft's current state.
 **
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is used to clear out the aircraft allocation system. It is
 *called in        * preparation for a scenario load or save game load. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    This function is used to handle finding, heading toward, landing, and
 *unloading the      * cargo from the aircraft. Once unloading of cargo has
 *occurred, then the aircraft follows * a different mission. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns the number of game ticks to delay before calling this
 *function again.      *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    This routine examines the landing zone (as specified by the target
 *parameter) in order   * to determine if it is free to be landed upon. Call
 *this routine when it is necessary     * to double check this. Typically this
 *occurs right before a helicopter lands and also     * when determining the
...
bool AircraftClass::Is_LZ_Clear(TARGET target) const {
  ...
  *location *	legal if the occupying object is in radio contact with the
 ...
 *                                                                                             *
 *    This routine is used to determine the coordinate to use for sorting the
 *aircraft. This   * sorting value is used when the aircraft is on the ground.
 *At that time the aircraft      * must be rendered in proper relationship to
 *the other ground objects.                     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the coordinate to use when sorting the aircraft with
...
/***********************************************************************************************
 * AircraftClass::Mission_Retreat -- Handles the aircraft logic for leaving the
 *battlefield.   *
 *                                                                                             *
 *    This mission will be followed when the aircraft decides that it is time to
 *leave the     * battle. Typically, this occurs when a loaner transport has
 *dropped off its load or when  * an attack air vehicle has expended its
 *ordinance.                                        *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the number of game ticks to delay before calling this
 *routine again.  *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
int AircraftClass::Mission_Retreat() {
  ...
  if (Class->IsFixedWing) {
    if (Class->IsFixedWing && Height < kFlightLevel) {
      Height += 1;
      return 3;
    }
  ...
  switch (Status) {
    ...
    */
    case kFaceMapEdge:
      ...
      /*
      **	Take advantage of the fact that the source map edge enumerations
      *happen to *	occur in a clockwise order and are the first four
      *enumerations of the map *	edge default for the house. If this
      *value is masked and then shifted, a *	normalized direction value
 ...
 *                                                                                             *
 *    This routine is called when the aircraft is to unload a passenger. The
 *passenger must    * be able to move under its own power. Typical situation is
 *when a transport helicopter    * is to unload an infantry unit. *
 *                                                                                             *
 * INPUT:   unit  -- Pointer to the unit that is to be unloaded from this
 *aircraft.            *
 *                                                                                             *
 * OUTPUT:  bool; Was the unit unloaded successfully? *
 *                                                                                             *
 * WARNINGS:   The unload process is merely started by this routine. Radio
 *contact is          * established with the unloading unit and when the unit is
 *clear of the aircraft  * the radio contact will be broken and then the
 *aircraft is free to pursue        * other. *
 *                                                                                             *
  ...
  */
  for (const auto direction : kUnloadPriority) {
    ...
    if ((unit->Can_Enter_Cell(adjacent) == MOVE_OK) &&
        unit->Unlimbo(Coord, Facing_Dir(direction))) {
 ...
 *                                                                                             *
 *    Call this routine when a passenger needs to be dropped off by parachute.
 *One passenger   * is offloaded by a call to this routine. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the delay time that it is safe to wait before
 *processing any further  * paradrop actions. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 07/26/1996 JLB : Created. *
 ...
 *                                                                                             *
 *    Sometimes, aircraft firing needs special handling. Example: for napalm
 *bombs, the        * bomb travels forward at nearly the speed of the delivery
 *aircraft, not necessarily the   * default speed defined in the BulletTypeClass
 *structure.                                  *
 *                                                                                             *
 * INPUT:   target   -- The target that the projectile is heading for. *
 *                                                                                             *
 *          which    -- Which weapon to use in the attack. 0=primary,
 *1=secondary.             *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the bullet that was created as a result of
 *this attack.  *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
BulletClass* AircraftClass::Fire_At(TARGET target, int which) {
  ...
  */
  if (bullet && bullet->Class->IsDropping) {
 ...
 *                                                                                             *
 *    This routine is used to apply damage to the specified aircraft. This is
 *where any        * special crash animation will be initiated. *
 *                                                                                             *
 * INPUT:   damage   -- Reference to the damage that will be applied to the
 *aircraft.          * This value will be filled in with the actual damage that
 *was           * applied. *
 *                                                                                             *
 *          distance -- Distance from the source of the explosion to this
 *aircraft.            *
 *                                                                                             *
 *          warhead  -- The warhead type that the damage occurs from. *
 *                                                                                             *
 *          source   -- Pointer to the originator of the damage. This can be
 *used so that      * proper "thank you" can be delivered. *
 *                                                                                             *
...
ResultType AircraftClass::Take_Damage(int& damage, int distance,
  ...
  */
  ...
  /*
  **	Special action is performed if the aircraft is killed -- the cargo is
  *destroyed *	as well.
  */
  switch (res) {
 ...
 * AircraftClass::Mission_Move -- Handles movement mission. *
 *                                                                                             *
 *    This state machine routine is used when an aircraft (usually helicopter)
 *is to move      * from one location to another. It will handle any necessary
 *take off and landing this     * may require. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the number of game frames that should elapse before
...
int AircraftClass::Mission_Move() {
  ...
  if (Class->IsFixedWing) {
    constexpr int kTakeOff = 0;
    constexpr int kFlyTowardTarget = 1;
        ...
        */
        if (Process_Take_Off()) {
          ...
          */
          ...
          Status = kFlyTowardTarget;
        }
        return 1;

      case kFlyTowardTarget: {
 ...
 **
 *                                                                                             *
 *    Use this routine when the mission for the aircraft is in doubt. This
 *routine will find   * an appropriate mission for the aircraft and dispatch it.
 **
 *                                                                                             *
  ...
  if (Class->IsFixedWing) {
    if (In_Which_Layer() == LAYER_GROUND) {
      ...
      */
      if (((IsALoaner && House->IsHuman) || (!House->IsHuman && !Ammo)) &&
          (Team.Is_Valid() && Team->Has_Entered_Map())) {
        Team->Remove(this);
      }
          ...
          if (building != nullptr &&
            ...
            if (Class->IsFixedWing) {
              Status = 0;  // BG - reset the mission status to avoid landing on
                           // the ground next to the airstrip
              if (IsLanding) {
                Process_Take_Off();
              }
            }
    ...
    if (In_Which_Layer() == LAYER_GROUND) {
      if (IsALoaner) {
        if (Is_Something_Attached()) {
          /*
          **	In the case of a computer controlled helicopter that hold
          *passengers, *	don't unload when landing. Wait for specific
          *instructions from the *	controlling team.
          */
          if (Team.Is_Valid()) {
            //					if (Team.Is_Valid() &&
            //! House->IsHuman) {
...
/***********************************************************************************************
 * AircraftClass::Process_Fly_To -- Handles state machine for flying to
 *destination.           *
 *                                                                                             *
 *    This support routine is used when the helicopter is to fly to the
 *destination. It can    * optionally slow the helicopter down as it approaches
 *the destination.                    *
 *                                                                                             *
 * INPUT:   slowdown -- Should the aircraft be slowed down when it approaches
 *the dest?        *
 *                                                                                             *
 * OUTPUT:  Returns with the distance remaining between the aircraft and the
 *destination.      *
 *                                                                                             *
 * WARNINGS:   Because the aircraft can move at a fast speed, the distance to
 *target value     * will probably never be zero. The likely case will be that
 *the aircraft          * overshoots the target. *
 *                                                                                             *
 * HISTORY: * 06/14/1995 JLB : Created. * 03/05/1996 JLB : Specifies destination
 *target value.                                      *
 *=============================================================================================*/
 ...
 * AircraftClass::Active_Click_With -- Handles clicking over specified object. *
 *                                                                                             *
 *    This routine is used when the player clicks over the speicifed object. It
 *will assign    * the appropriate mission to the aircraft. *
 *                                                                                             *
 * INPUT:   action   -- The action that was nominally determined by the
 *What_Action function.  *
 *                                                                                             *
 *          object   -- The object over which the mouse was clicked. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This routine will alter the game sequence and causes an event
 *packet to be      * propagated to all connected machines. *
 *                                                                                             *
 * HISTORY: * 06/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    This routine is used when the player clicks the mouse of the specified
 *cell. It will     * assign the appropriate mission to the aircraft. *
 *                                                                                             *
 ...
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This routine will affect the game sequence and causes an event
 *object to be     * propagated to all connected machines. *
 *                                                                                             *
 * HISTORY: * 06/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 *mission.           *
 *                                                                                             *
 *    This routine is called as a result of player input with the intent to
 *change the         * mission of the aircraft. *
 *                                                                                             *
 * INPUT:   mission  -- The mission requested of the aircraft. *
 *                                                                                             *
 *          target   -- The value to assign to the aircraft's targeting
 *computer.              *
 *                                                                                             *
 *          dest.    -- The value to assign to the aircraft's navigation
 *computer.             *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   The mission specified will be executed at an indeterminate future
 *game frame.   * This is controlled by net/modem propagation delay. *
 *                                                                                             *
 * HISTORY: * 06/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 * AircraftClass::What_Action -- Determines what action to perform. *
 *                                                                                             *
 *    This routine will determine what action would occur if the mouse were
 *clicked over the   * cell specified. The display system calls this routine to
 *determine what mouse shape      * to use. *
 *                                                                                             *
 * INPUT:   cell  -- The cell over which the mouse is currently positioned. *
 *                                                                                             *
 * OUTPUT:  Returns with the action that will be performed if the mouse were
 *clicked at the    * specified cell location. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 06/19/1995 JLB : Created. *
 ...
 *                                                                                             *
 *    Use this routine to get the desired facing the aircraft should assume when
 *landing.      *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the normal default facing the aircraft should have when
 *landed.       *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is the state machine that handles the attack mission for
 *aircraft. It will  * handling homing in on and firing on the target in the
 *aircraft's targeting computer.     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the number of game ticks to pass before this routine
 ...
 * AircraftClass::New_LZ -- Find a good landing zone. *
 *                                                                                             *
 *    Use this routine to locate a good landing zone that is nearby the location
 *specified.    * By using this routine it is possible to assign the same
 *landing zone to several          * aircraft and they will land nearby without
 *conflict.                                     *
 *                                                                                             *
 * INPUT:   oldlz -- Target value of desired landing zone (usually a cell target
 *value).       *
 *                                                                                             *
 * OUTPUT:  Returns with the new good landing zone. It might be the same value
 *passed in.      *
 *                                                                                             *
 * WARNINGS:   The landing zone might be a goodly distance away from the ideal
 *if there is     * extensive blocking terrain in the vicinity. *
 *                                                                                             *
 * HISTORY: * 06/19/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 * AircraftClass::Receive_Message -- Handles receipt of radio messages. *
 *                                                                                             *
 *    This routine receives all radio messages directed at this aircraft. It is
 *used to handle * all inter-object coordination. Typically, this would be for
 *transport helicopters and    * other complex landing operations required of
 *helicopters.                                *
 *                                                                                             *
 ...
 *                                                                                             *
 *          param    -- An optional parameter that may be used to transfer
 *additional          * data. *
 *                                                                                             *
...
RadioMessageType AircraftClass::Receive_Message(RadioClass* from,
  ...
  switch (message) {
    ...
    /*
    **	Something disastrous has happened to the object in contact with. Fall
    *back *	and regroup. This means that any landing process is immediately
    *aborted.
    */
    case RADIO_RUN_AWAY:
      if (IsLanding) {
    ...
    *be given *	to the impatient unit.
    */
    case RADIO_DOCKING:
 ...
 *up.              *
 *                                                                                             *
 *    This routine is used by the transport helicopter to determine the location
 *where the     * infantry passengers should line up before loading. *
 *                                                                                             *
 * INPUT:   object   -- The object that is trying to load up on this transport.
 **
 *                                                                                             *
 *                   -- Reference to the cell that the passengers should move to
 *before the    * actual load process may begin. *
 *                                                                                             *
 * OUTPUT:  Returns with the direction that the helicopter should face for the
 *load operation. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 * AircraftClass::Process_Take_Off -- State machine support for taking off. *
 *                                                                                             *
 *    This routine is used by the main game state machine processor. This
 *utility routine      * handles a helicopter as it transitions from landed to
 *flying state.                      *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * AircraftClass::Process_Landing -- Landing process state machine handler. *
 *                                                                                             *
 *    This is a support routine that is called by the main state machine
 *routines. This        * routine is responsible for handling the helicopter as
 *it transitions from flight to      * landing. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * AircraftClass::Can_Enter_Cell -- Determines if the aircraft can land at this
 *location.      *
 *                                                                                             *
 *    This routine is used when the passability of a cell needs to be
 *determined. This is      * necessary when scanning for a location that the
 *aircraft can land.                       *
 *                                                                                             *
 * INPUT:   cell  -- The cell location to check for landing. *
 *                                                                                             *
 * OUTPUT:  Returns a value indicating if the cell is a legal landing spot or
 *not.             *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *fire from.       *
 *                                                                                             *
 *    Given the specified target, this routine will locate a good spot for the
 *aircraft to     * fire at the target. *
 *                                                                                             *
 * INPUT:   target   -- The target that is desired to be attacked. *
 *                                                                                             *
 * OUTPUT:  Returns with the target location of the place that firing should be
 *made from.     *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 06/12/1995 JLB : Created. * 06/14/1995 JLB : Finer resolution on
 *ring scan.                                           * 11/02/1996 JLB : Bias
 ...
 *                                                                                             *
 *    This routine examines the navigation computers of other aircraft in order
 *to see if the  * specified cell is safe to fly to. The intent of this routine
 *is to avoid unnecessary     * mid-air collisions. *
 *                                                                                             *
 * INPUT:   cell     -- The cell to examine for clear airspace. *
 *                                                                                             *
 *          strict   -- Should the scan consider the aircraft, that is making
 *this check, a    * blocking aircraft. Typically, the aircraft itself is not
 *considered    * a blockage -- an aircraft can always exist where it is
 *currently       * located. A strict check is useful for helicopters that need
 *to move    * around at the slightest provocation. *
 *                                                                                             *
 ...
 * AircraftClass::Pip_Count -- Returns the number of "objects" in aircraft. *
 *                                                                                             *
 *    This routine is used by the render logic to draw the little container
 *"pips". This       * corresponds to the number of passengers for a transport
 *helicopter or the number of      * shots remaining for an attack helicopter. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * AircraftClass::Mission_Enter -- Control aircraft to fly to the helipad or
 *repair center.    *
 *                                                                                             *
 *    This routine is used when the aircraft needs to fly for either rearming or
 *repairing.    * It tries to establish contact with the support building. Once
 *contact is established     * the ground controller takes care of commanding
 *the aircraft.                             *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
int AircraftClass::Mission_Enter() {
  ...
  constexpr int kDownwind = 4;
  constexpr int kCrosswind = 5;
  constexpr int kTravel = 6;
  constexpr int kLanding = 7;

  /*
  **	Verify that it has a valid NavCom. If it doesn't then request one from
  *the *	building this building is trying to land upon. If that fails,
  *then enter *	idle mode.
  */
  if ((!Target_Legal(NavCom) && In_Which_Layer() != LAYER_GROUND) &&
      (Transmit_Message(RADIO_DOCKING) != RADIO_ROGER)) {
    Enter_Idle_Mode();
    return 1;
  }
  ...
  switch (Status) {
    case kInitial:
      if (Height < kFlightLevel || IsLanding) {
        Status = kTakeoff;
      } else {
        Status = kAltitude;
      }
 ...
 *                                                                                             *
 *    This routine is used when helicopters need a place to land, but there are
 *no obvious     * spots (i.e., helipad) available. It will try to land near a
 *friendly helipad or friendly * building if there are no helipads anywhere. In
 *the event that there are no friendly      * buildings anywhere on the map,
 *then just land right where it is flying.                  *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the target location where this aircraft should land.
 *This value may   * not be a clear cell, but the normal landing logic will
 ...
 * AircraftClass::Set_Speed -- Sets the speed for the aircraft. *
 *                                                                                             *
 *    This routine will set the speed for the aircraft. The speed is specified
 *as a fraction   * of full speed. *
 *                                                                                             *
 * INPUT:   speed -- The fixed point fractional speed setting. 0x00 is stopped,
 *0xFF is full   * speed. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
...
void AircraftClass::Set_Speed(int speed) {
  ...
  const MPHType sp = static_cast<MPHType>(std::min(
      static_cast<int>(Class->MaxSpeed) * SpeedBias * House->AirspeedBias,
 ...
 * AircraftClass::Fire_Direction -- Determines the direction of fire. *
 *                                                                                             *
 *    This routine will determine what direction a projectile would take if it
 *were fired      * from the aircraft. This is the direction that the aircraft's
 *body is facing.             *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This is the destructor for aircraft. It will limbo the aircraft if it
 *isn't already      * and also removes the aircraft from any team it may be
 *attached to.                       *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * AircraftClass::Scatter -- Causes the aircraft to move away a bit. *
 *                                                                                             *
 *    This routine will cause the aircraft to move away from its current
 *location and then     * enter some idle mode. Typically this is called when
 *the aircraft is attacked while on    * the ground. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * AircraftClass::Mission_Guard -- Handles aircraft in guard mode. *
 *                                                                                             *
 *    Aircraft don't like to be in guard mode if in flight. If this situation is
 *detected,     * then figure out what the aircraft should be doing and go do
 *it.                          *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *routine again. *
 *                                                                                             *
 * WARNINGS:   This routine typically calls the normal guard logic for ground
 *units.           *
 *                                                                                             *
...
int AircraftClass::Mission_Guard() {
  ...
  */
  ...
  /*
  **	Computer controlled helicopters will defend themselves by bouncing
  *around *	and looking for a free helipad.
  */
  ...
  /*
  **	Perform a special check to hunt for harvesters that are outside of the
 ...
 * AircraftClass::Mission_Guard_Area -- Handles the aircraft guard area logic. *
 *                                                                                             *
 *    This routine handles area guard logic for aircraft. Aircraft require
 *special handling    * for this mode since they are to guard area only if they
 *are in a position to do so.      * Otherwise they just defend themselves. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the number of game frames to delay before calling this
 ...
 *                                                                                             *
 *    This routine is used to determine if the aircraft can fire its weapon at
 *the target      * specified. If it cannot, then the reason why is returned. *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 03/05/1996 JLB : Created. * 07/11/1996 JLB : Fixed for camera
 *carrying aircraft.                                      *
 *=============================================================================================*/
FireErrorType AircraftClass::Can_Fire(TARGET target, int which) const {
  ...
  **	paradrop or 'throw out' the cargo. This is always allowed if the terrain
 ...
 *processing.       *
 *                                                                                             *
 *    This routine handles the tricky maneuver of taking off and landing. The
 *process of       * landing is not entirely safe and thus the aircraft may be
 ...
 *                                                                                             *
 * WARNINGS:   Only call this routine once per aircraft per game logic loop. Be
 *sure to        * examine the return value and if true, abort all further
 *processing of this      * aircraft since it is now dead. *
 *                                                                                             *
  ...
  if (Is_Door_Closed() && (IsLanding || IsTakingOff)) {
    ...
    if (IsLanding) {
      ...
      if (Height <= 0) {
        ...
        /*
        **	If the NavCom now equals the destination, then clear out the
        *NavCom.
        */
    ...
    */
    if (layer != In_Which_Layer()) {
      /*
      **	When the aircraft is about to enter the ground layer, perform on
      *last *	check to see if it is legal to enter that location. If not, then
      **	start the take off process. Let the normal logic handle this
      **	change of plans.
      */
      bool ok = true;
      if ((In_Which_Layer() == LAYER_GROUND && !IsTakingOff &&
           !Class->IsFixedWing) &&
      ...
      if (ok) {
        ...
        *ground object. *	This aspect is controlled by the Place_Down and
        *Pick_Up functions.
        */
        if (In_Which_Layer() == LAYER_GROUND) {
          ...
          */
          if (In_Radio_Contact() && Target_Legal(NavCom) &&
              NavCom != Contact_With_Whom()->As_Target()) {
            Transmit_Message(RADIO_OVER_OUT);
          }
        }
      }
 ...
 * AircraftClass::Edge_Of_World_AI -- Detect if aircraft has exited the map. *
 *                                                                                             *
 *    Certain aircraft will be eliminated when they leave the edge of the world
 *presumably     * after completing their mission. An exception is for aircraft
 *that have been newly        * created as reinforcements and have not yet
 ...
 *                                                                                             *
 * WARNINGS:   Be sure to call this routine only once per aircraft per game
 *logic loop. If     * the return value is true, then abort any further
 *processing of this aircraft    * since it has been eliminated. *
 *                                                                                             *
 * HISTORY: * 07/29/1996 JLB : Created. *
 *=============================================================================================*/
...
#ifdef OLD
        /*
        **	Transport planes that leave can only be because they carry
        *purchased *	equipment and must be have their cost refunded.
        */
 ...
 * AircraftClass::Movement_AI -- Handles aircraft physical movement logic. *
 *                                                                                             *
 *    This routine manages the aircraft movement across the map. If any movement
 *occurred, the * aircraft will be flagged to be redrawn. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * AircraftClass::Rotation_AI -- Handle aircraft body and flight rotation. *
 *                                                                                             *
 *    This will process the aircraft visible body and flight model rotation
 *operations. If     * any rotation occurred, the aircraft will be flagged to be
 *redrawn.                       *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This is a seldom used function since its only purpose is to be called when
 *an aircraft   * lands on the ground. *
 *                                                                                             *
...
/***********************************************************************************************
 * AircraftClass::Assign_Destination -- Assigns movement destination to the
 *object.            *
 *                                                                                             *
 *    This routine is called when the object needs to have a new movement
 *destination          * assigned.  Aircraft have their own version of this
 *routine because a fixed-wing plane	  * trying to land will behave poorly if
 *given a new destinatio while it's landing.			  * *
 *                                                                                             *
...
void AircraftClass::Assign_Destination(TARGET dest) {
  ...
  if (Target_Legal(dest) && Class->IsFixedWing &&
    ...
    //	if (Target_Legal(dest) /*&& Class->IsFixedWing*/ && (IsLanding ||
 ...
 *aircraft.              *
 *                                                                                             *
 *    This examines the aircraft to determine what display layer it should be
 *located          * in. Fixed wing aircraft must always be in the top layer if
 *they are flying even though   * they may be low to the ground. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Aircraft perform a look operation according to their sight range. If the
 *aircraft is     * on the ground, then it will look a distance of one cell
 *regardless of what its           * specified sight range is. *
 *                                                                                             *
