...
When a guest changes house, he tells the host, who informs everyone. A house
change can't be denied. When a guest changes color, he asks the host for the new
color, and assumes the request will go through. If it doesn't, the host sends
the guest a messages setting him back to the old color, otherwise, everyone gets
the new color for the guest. When the host changes a game param, the change will
be noticed within PARAMREFRESHWAIT milliseconds and then transmitted to all
guests. All player specific information is stored within the players list
(IconListClass), and nowhere else. Though this storage method got a bit
inelegant, it gets the job done. All "informs" (messages sent from host to
guest) except color changes (WOL_GAMEOPT_INFCOLOR) and player "accept" status
changes (WOL_GAMEOPT_INFACCEPT) are assigned a ID, which
increments on each send. Color informs are: a) not a condition for removing a
guest's "accept" status, and b) sent out to individual guests in certain cases,
and would thus complicate the ID tracking scheme. When a guest "accepts" the
game setup (with a WOL_GAMEOPT_REQACCEPT), the latest received ID is included in
the message. If the host receives a WOL_GAMEOPT_REQACCEPT with an out-of-date
ID, the accept is ignored. Though the guest sets himself to "accepted" when he
sends the WOL_GAMEOPT_REQACCEPT, if it gets ignored by the host it is because a
new inform is already on its way, which will reset the guest's own status to
"not accepted" when it arrives. Host clears all accepteds when params are
changed, player joins or leaves, or house change message arrives. Though a
WOL_GAMEOPT_INFACCEPT is sent out when a guest accepts, there is no
corresponding "unaccept all" message sent out at this point, as all guests will
naturally clear their recorded accept status for everyone when the change
arrives there. Guest clears own accepted when an inform from the server other
than color change arrives, player joins or leaves, or own house is directly
changed. When all players are "accepted", and the host says "start", a
WOL_GAMEOPT_INFSTART is sent to all guests and the host goes into "waiting to
start" mode. If a house change or other arrives in this phase, host clears
waiting mode and (naturally) marks guests unaccepted and sends out the house
change to the guests. (The same applies to player joins/leaves.) While in this
mode, the host user is locked out of making any changes to game params. If a
change does come in during this phase, a WOL_GAMEOPT_INFCANCELSTART is sent to
the guests, and the waiting mode cancelled. When a guest receives
WOL_GAMEOPT_INFSTART: If he does not see himself as accepted, it is because a
change has just been sent to the host, or some other event occurred that the
host will learn about, so the guest can simply ignore the WOL_GAMEOPT_INFSTART.
If the guest does see himself as accepted when WOL_GAMEOPT_INFSTART arrives, he
responds with WOL_GAMEOPT_REQSTART and goes into "waiting to start" mode. When
in this mode, the user is locked out of making any changes. Note that the host
is not really asking for any sort of "confirmation that it's ok" to start from
the guest here. (Hence no "cancel" response is possible.) It's really just a way
of telling the guests not to make any more changes to the setup; it's like we're
making sure we've "flushed the queue" of game changes. If a guest receives a
WOL_GAMEOPT_INFCANCELSTART, he cancels out of "waiting to start" mode. Note that
while in the waiting mode, normal processing of messages is allowed to take
place - the user is simply blocked from doing anything. Presumably (hopefully)
this phase will last less than a second. When the host receives a
WOL_GAMEOPT_REQSTART from everyone, he tells everyone to start with a
WOL_GAMEOPT_INFGO. Because there is a chance of color changes being out of sync,
all player colors are sent to all guests at this point. There is no chance of
other data being out of sync.
*/
...
//				debugprint( "Illegal GameInfoCurrent value." );
  ...
  while (bProcess) {
    ...
    if (bHostSayGo) {
      ...
      //			debugprint( "Host about to exit game setup
      dialog for game.\n" ); if( !ExitGameChannel() ) ResultReturn =
                                                ...
                                                // cancel setup.
    {
      ...
      bProcess = false;
      ResultReturn =
          RESULT_WOLGSUP_LOGOUT;  //	As if the user logged himself out.
    }
    ...
    if (display) {
      ...
      */
      ...
      if (display >= REDRAW_BACKGROUND) {
        ...
        if (pDropListHouse->IsDropped) pDropListHouse->Collapse();
        ...
        // iGameInfoSpacingY, scheme, TBLACK, TPF_TYPE );
      }
    ...
    switch (input) {
        ...
        if (!bHost) {
                          ...
                          // accepting, anyway.
          {
            ...
            if (SetPlayerAccepted(
                    pWO->szMyName,
                    true))  //	Else we didn't find ourself in list!
            {
              //							debugprint(
              //"Sending accept.\n" ); 	Tell host we accept.
              char szSend[20];
      ...
      case (KN_ESC):
        if (pDropListHouse->IsDropped) bRetractHouseDropDown = true;
        break;
...
//***********************************************************************************************
void WOL_GameSetupDialog::SetPlayerColor(const char* szName,
...
//***********************************************************************************************
bool WOL_GameSetupDialog::SetPlayerAccepted(const char* szName,
                                            bool bAccepted) {
...
//***********************************************************************************************
bool WOL_GameSetupDialog::SetPlayerReadyToGo(const char* szName,
                                             const char* szReadyState) {
  ...
  if (!bHost) {
    ...
    switch (opt) {
      ...
      case WOL_GAMEOPT_INFPARAMS:
        ...
        if (!AcceptParams(szInform)) bLeaveDueToRulesMismatchTrigger = true;
        SetSpecialControlStates();
...
bool operator==(const GAMEPARAMS& gp1, const GAMEPARAMS& gp2) {
  return gp1.GPacket == gp2.GPacket &&
         gp1.bAftermathUnits == gp2.bAftermathUnits &&
...
(char*)gp1.ScenarioInfo.ShortFileName ); else
...
(char*)gp1.ScenarioInfo.FileDigest ); else
...
PlayerColorType PlayerColorTypeOf(RemapControlType* pColorRemap) {
  for (PlayerColorType pcolor = PCOLOR_FIRST; pcolor < PCOLOR_COUNT; pcolor++) {
    if (&ColorRemaps[pcolor] == pColorRemap) return pcolor;
  }
  return PCOLOR_NONE;
}
...
//***********************************************************************************************
bool WOL_GameSetupDialog::InformAboutPlayerColor(const char* szName,
                                                 PlayerColorType Color,
                                                 User* pUserPriv) {
...
//***********************************************************************************************
bool WOL_GameSetupDialog::InformAboutPlayerHouse(const char* szName,
                                                 HousesType House,
                                                 User* pUserPriv) {
...
//***********************************************************************************************
bool WOL_GameSetupDialog::InformAboutPlayerAccept(const char* szName,
                                                  User* pUserPriv) {
...
bool WOL_GameSetupDialog::bAllGuestsAccept() {
  for (int i = 0; i < pILPlayers->Count(); i++) {
    if (!pWO->bItemMarkedAccepted(i)) return false;
  }
  return true;
}
...
//***********************************************************************************************
void WOL_GameSetupDialog::GuestIsReadyToPlay(const char* szName,
                                             const char* szReadyState) {
...
void WOL_GameSetupDialog::HostSaysGo() {
  ...
  /*	...Method changed. It appears that my channelleave may appear to guests
     before the "go" privategameopt. For this reason, I'll wait until I receive
     a copy of the "go" message sent to myself, before proceeding.

          pWO->pChat->PumpMessages();		//	Flush the send out.
          //	(ajw - Note: An apparent bug in wolapi means that this pump does
     not necessarily flush the go gameopts.
          //	This is ok in practice, because the host hits pumps later that
     will flush them.)

          //	Pretend we just received szSend, and processed it like a guest
     would. char* szFakeGoMessage = szSend + 3;
    ...
    if (strcmp(szPlayerName, pWO->szMyName) != 0 &&
      ...
      /*
                              else
                              {
                                      NetNumType netxxx;
                                      NetNodeType nodexxx;
                                      pPlayerNew->Address.Get_Address( netxxx,
      nodexxx );
      //				debugprint( "Player ip
      %i.%i.%i.%i.%i.%i\n", nodexxx[0], nodexxx[1], nodexxx[2], nodexxx[3],
      nodexxx[4], nodexxx[5] );
                              }
      */
