 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * VesselTypeClass::As_Reference -- Converts a vessel type into a
 *VesselTypeClass reference. * VesselTypeClass::Create_And_Place -- Creates a
 *vessel and places it at location.          * VesselTypeClass::Create_One_Of --
 *Creates a vessel object that matches this vessel type.  *
 *   VesselTypeClass::Dimensions -- Fetches the pixel width and height of this
 *vessel type.    * VesselTypeClass::Display -- Displays a generic
 *representation of this vessel type.        * VesselTypeClass::From_Name --
 *Converts a name into a vessel type.                         *
 *   VesselTypeClass::Init_Heap -- Initialize the vessel heap. *
 *   VesselTypeClass::One_Time -- Performs one time initialization for vessel
 *types.           * VesselTypeClass::Overlap_List -- Figures the overlap list
 *for the vessel type.            * VesselTypeClass::Prep_For_Add -- Adds vessel
 *types to the scenario editor object list.    * VesselTypeClass::Turret_Adjust
 *-- Adjust turret offset according to facing specified.     *
 *   VesselTypeClass::VesselTypeClass -- Constructor for naval vessel types. *
 *   VesselTypeClass::Who_Can_Build_Me -- Fetches pointer to available factory
 *for this vessel.* VesselTypeClass::operator delete -- Returns a vessel type
 *object back to the memory pool. * VesselTypeClass::operator new -- Allocate a
 *vessel type object from the special memory poo*
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 *    This is the constructor for the vessel static data. Each vessels is assign
 *a specific    * variation. This class elaborates what the variation actually
 *is.                         *
 *                                                                                             *
...
/***********************************************************************************************
 * VesselTypeClass::operator new -- Allocate a vessel type object from the
 *special memory pool *
 *                                                                                             *
 ...
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the allocated vessel type class object. If
 *memory in the * special heap has been exhaused, then NULL will be returned. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * VesselTypeClass::operator delete -- Returns a vessel type object back to the
 *memory pool.   *
 *                                                                                             *
 *    This will return a previously allocated vessel object back to the special
 *pool from      * whence it was originally allocated. *
 *                                                                                             *
 ...
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   Only call this routine once and do so before processing the
 *rules.ini file.     *
 *                                                                                             *
 * HISTORY: * 07/09/1996 JLB : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * VesselTypeClass::As_Reference -- Converts a vessel type into a
 *VesselTypeClass reference.   *
 *                                                                                             *
 *    This routine will fetch a reference to the vessel type that corresponds to
 *the vessel    * type specified. *
 *                                                                                             *
 * INPUT:   type  -- The vessel type number to convert. *
 *                                                                                             *
 * OUTPUT:  Returns with a reference to the vessel type class that corresponds
...
#ifdef NEVER
/***********************************************************************************************
 * VesselTypeClass::Who_Can_Build_Me -- Fetches pointer to available factory for
 *this vessel.  *
 *                                                                                             *
 *    Use this routine to fetch a pointer to the vessel factory that can build
 *this vessel     * type. *
 *                                                                                             *
 * INPUT:   intheory -- If true, then this indicates that if the factory is
 *currently          * busy doing other things, this won't make in ineligible
 *for searching. * Typical use of this is by the sidebar logic which needs only
 *to know   * if theoretical production is allowed. *
 *                                                                                             *
 *          legal    -- If true, then the buildings are checked for specific
 *legality when     * being scanned. For building placement, this is usually
 *false, for      * sidebar button adding, this is usually true. *
 *                                                                                             *
 *          house    -- The owner of the unit to be produced. This has an effect
 *of legality.  *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the factory (building) that can produce
 *this vessel type.*
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * VesselTypeClass::Display -- Displays a generic representation of this vessel
 *type.          *
 *                                                                                             *
 *    This routine is used by the scenario editor to display a representation of
 *this          * vessel type in the object placement dialog. *
 *                                                                                             *
 * INPUT:   x,y      -- Pixel coordinate to render the center of this vessel
 *type to.          *
 *                                                                                             *
 *          window   -- The window to clip the shape to. The pixel coordinates
 *are relative    * to this window. *
 *                                                                                             *
 *          house    -- The owner of the vessel. This is used to give the vessel
 *its color.    *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
...
/***********************************************************************************************
 * VesselTypeClass::Prep_For_Add -- Adds vessel types to the scenario editor
 *object list.      *
 *                                                                                             *
 *    This routine is called when the scenario editor needs to obtain a list of
 *the            * vessel object that can be placed down. It will submit all the
 *vessel types that can      * be placed down. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *vessel type.    *
 *                                                                                             *
 *    This routine is called when the type of vessel is known (by way of a
 *VesselTypeClass)    * and a corresponding vessel object needs to be created. *
 *                                                                                             *
 * INPUT:   house -- Pointer to the owner that this vessel will be assigned to.
 **
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the vessel object created. If no vessel
 *could be         * created, then NULL is returned. *
 *                                                                                             *
 * WARNINGS:   The vessel is created in a limbo state. It must first be placed
 *down upon       * the map before it starts to function. *
 *                                                                                             *
 * HISTORY: * 03/20/1996 JLB : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * VesselTypeClass::Create_And_Place -- Creates a vessel and places it at
 *location.            *
 *                                                                                             *
 ...
 *                                                                                             *
 * OUTPUT:  bool; Was the vessel successfully created and placed down upon the
 *map?            *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *vessel type.      *
 *                                                                                             *
 *    This routine is used to fetch the width and height of this vessel type.
 *These dimensions * are not specific to any particular facing. Rather, they are
 *only for the generic vessel  * size. *
 *                                                                                             *
 * INPUT:   width, height  -- Reference to the integers that are to be
 *initialized with the    * pixel width and height of this vessel type. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
 ...
 *types.             *
 *                                                                                             *
 *    This routine will load in the vessel shape data. It should be called only
...
/***********************************************************************************************
 * VesselTypeClass::Turret_Adjust -- Adjust turret offset according to facing
 *specified.       *
 *                                                                                             *
 *    This routine will determine the pixel adjustment necessary for a turret.
 *The direction   * specified is what the vessel body is facing. *
 *                                                                                             *
 * INPUT:   dir   -- The presumed direction of the body facing for the vessel. *
 *                                                                                             *
 *          x,y   -- The center pixel position for the vessel. These values
 *should be          * adjusted (they are references) to match the adjusted
 *offset for the       * turret. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
...
/***********************************************************************************************
 * VesselTypeClass::Overlap_List -- Figures the overlap list for the vessel
 *type.              *
 *                                                                                             *
 *    This routine will return the overlap list for a vessel that is sitting
 *still in the      * center of a cell. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the overlap list that this vessel would
 *use.             *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Use this routine to convert an ASCII version of a vessel type into the
 *corresponding     * VesselType id value. Typical use of this would be to parse
 *the INI file.                 *
 *                                                                                             *
 * INPUT:   name  -- Pointer to the ASCII name to be converted into a vessel
 *type.             *
 *                                                                                             *
 * OUTPUT:  Returns with the vessel type number that matches the string
 *specified.             *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * VesselTypeClass::Max_Pips -- Fetches the maximum pips allowed for this
 *vessel.              *
 *                                                                                             *
 *    This routine will determine the number of pips (maximum) allowed for this
 *unit type.     * Typically, this is the number of passengers allowed.
 **
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the maximum number of pips allowed for this vessel
 *type.              *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
