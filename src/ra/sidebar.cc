 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * SidebarClass::AI -- Handles player clicking on sidebar area. *
 *   SidebarClass::Abandon_Production -- Stops production of the object
 *specified.             * SidebarClass::Activate -- Controls the sidebar
 ...
 *list.                              * SidebarClass::Draw_It -- Renders the
 *sidebar display.                                     *
 ...
 *sidebar.                        * SidebarClass::SidebarClass -- This is the no
 *initialization constructor for the sidebar.  * SidebarClass::StripClass::AI --
 *Input and AI processing for the side strip.               *
 *   SidebarClass::StripClass::Abandon_Produ -- Abandons production associated
 *with sidebar.   * SidebarClass::StripClass::Activate -- Adds the strip buttons
 *to the input system.         * SidebarClass::StripClass::Add -- Add an object
 *to the side strip.                         *
 ...
 *given type should appear.         * SidebarClass::Zoom_Mode_Control -- Handles
 *the zoom mode toggle operation.                *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
std::span<const std::byte> SidebarClass::SidebarBottomShape = {};
...
base::EnumArray<SpecialWeaponType, std::span<const std::byte>>
...
SidebarClass::SidebarClass() {
  ...
  */
  base::At(WindowList[static_cast<int>(WINDOW_SIDEBAR)], kWindowX) = kSideX + 8;
  ...
  */
  ...
  base::At(Column, 0).X = kColumnOneX * 2;
  base::At(Column, 0).Y = kColumnOneY * 2;
  base::At(Column, 1).X = kColumnTwoX * 2;
  base::At(Column, 1).Y = kColumnTwoY * 2;
}
...
void SidebarClass::One_Time() {
  ...
  */
  base::At(WindowList[static_cast<int>(WINDOW_SIDEBAR)], kWindowY) -= 2;
  ...
  *dependant)
  */
  if (SidebarShape.empty()) {
    SidebarShape = MixArchive::RetrieveData("SIDEBAR.SHP");
  }
}
...
void SidebarClass::Init_IO() {
  ...
  */
  if (!MapEditorActive) {
    ...
    Repair.Set_Shape(MixArchive::RetrieveData("REPAIR.SHP"));
    ...
    Upgrade.Set_Shape(MixArchive::RetrieveData("SELL.SHP"));
...
/***********************************************************************************************
 * SidebarClass::Reload_Sidebar -- Loads appropriate sidebar shapes depending on
 *house			  *
 *                                                                                             *
 * INPUT:  none
 **
...
bool SidebarClass::Scroll(bool up, int column) {
  ...
  if (base::At(Column, column).Scroll(up)) {
    // No need to redraw the whole sidebar juts because we scrolled a strip is
...
void SidebarClass::StripClass::One_Time(int /*unused*/) {
  ...
  */
  ClockShapes = MixArchive::RetrieveData("CLOCK.SHP");
...
void SidebarClass::StripClass::Init_IO(int id) {
  ...
  base::At(UpButton, ID).X = X + (kUpXOffset * 2);
  base::At(UpButton, ID).Y = Y + (kUpYOffset * 2);

  base::At(UpButton, ID).Set_Shape(MixArchive::RetrieveData("STRIPUP.SHP"));

  base::At(DownButton, ID).IsSticky = true;
  base::At(DownButton, ID).ID = static_cast<unsigned>(kButtonDown + id);
  base::At(DownButton, ID).X = X + (kDownXOffset * 2);
  base::At(DownButton, ID).Y = Y + (kDownYOffset * 2);

  /*
  ** Buttons are in a slightly different position in the new sidebar
  */
  base::At(UpButton, ID).Y--;
  base::At(DownButton, ID).Y--;

  base::At(DownButton, ID).Set_Shape(MixArchive::RetrieveData("STRIPDN.SHP"));
 ...
 *are required to properly give the appearance    * of scrolling. *
 *                                                                                             *
      ...
      if (static_cast<unsigned>(index) < static_cast<unsigned>(BuildableCount)) {
        ...
        if (base::At(Buildables, index).BuildableType != RTTI_SPECIAL) {
          ...
          if (obj != nullptr) {
            ...
            */
            if (base::At(Buildables, index).BuildableType ==
                RTTI_INFANTRYTYPE) {
              remapper = {};
            }
          ...
          spc = static_cast<SpecialWeaponType>(
              base::At(Buildables, index).BuildableID);
 ...
 *                                                                                             *
 *    This is the function that is called when the map button is pressed. It
 *will toggle       * between the different modes that the radar map can assume.
 **
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
