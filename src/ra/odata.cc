 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * OverlayTypeClass::As_Reference -- Fetch a reference to the
 *overlay type specified.        * OverlayTypeClass::Coord_Fixup -- Adjust the
 *coord to be legal for assignment.             *
 *   OverlayTypeClass::Create_And_Place -- Creates and places a overlay object
 *on the map.     * OverlayTypeClass::Create_One_Of -- Creates an object of this
 *overlay type.                * OverlayTypeClass::Display -- Displays a generic
 *representation of overlay.                * OverlayTypeClass::Draw_It -- Draws
 *the overlay image at location specified.               *
 ...
 *data.         * OverlayTypeClass::OverlayTypeClass -- Constructor for overlay
 *type objects.               * OverlayTypeClass::Prep_For_Add -- Prepares to
 *add overlay to scenario.                    * OverlayTypeClass::Radar_Icon --
 *Gets a pointer to the radar icons                         *
 *   OverlayTypeClass::operator delete -- Returns an overlay type object back to
 *the pool.     * OverlayTypeClass::operator new -- Allocate an overlay type
 *class object from pool.        *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/***********************************************************************************************
 * OverlayTypeClass::operator new -- Allocate an overlay type class object from
 *pool.          *
 *                                                                                             *
 *    This will allocate an overlay type class object from the special memory
 *pool that is     * for that purpose. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the overlay type class object allocated.
 *If there is     * insufficient memory to fulfill the request, then NULL is
 *returned.                 *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *the pool.       *
 *                                                                                             *
 *    This will return a previously allcoated overaly type object to the special
 *memory        * pool that it was allocated from. *
 *                                                                                             *
 * INPUT:   pointer  -- Pointer to the overlay type class object to return the
 *memory pool.    *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 * OverlayTypeClass::Init_Heap -- Initialize the overlay type class heap. *
 *                                                                                             *
 *    This will initialize the overlay type heap by pre-allocated all overlay
 ...
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   It should be called once and before the rules.ini file is
 *processed.            *
 *                                                                                             *
 * HISTORY: * 07/09/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 *data.           *
 *                                                                                             *
 *    This routine should be called once when the game first starts. It will
 *establish         * pointers to the graphic data of the overlay objects. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * OverlayTypeClass::From_Name -- Determine overlay from ASCII name. *
 *                                                                                             *
 *    This routine is used to determine the overlay number given only * an ASCII
 *representation. The scenario loader uses this routine * to construct the map
 *from the INI control file.                                          *
 *                                                                                             *
 * INPUT:   name  -- Pointer to the ASCII name of the overlay. *
 *                                                                                             *
 * OUTPUT:  Returns with the overlay number. If the name had no match, * then
 *returns with OVERLAY_NONE. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 * OverlayTypeClass::Occupy_List -- Determines occupation list. *
 *                                                                                             *
 *    This routine is used to examine the overlay map and build an * occupation
 *list. This list is used to render a overlay cursor as * well as placement of
 *icon numbers.                                                       *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is used to display a generic view of the overlay * object.
...
void OverlayTypeClass::Display(int x, int y, WindowNumberType window,
  ...
  if (Get_Image_Data() != NULL) {
    ...
    if (Type == OVERLAY_GEMS1 || Type == OVERLAY_GEMS2 ||
        Type == OVERLAY_GEMS3 || Type == OVERLAY_GEMS4) {
      frame = 2;
    }
 ...
 *                                                                                             *
 *    This routine prepares a list of overlay objects so that the * scenario
 *editor can use this list to display a dialog box. The * selection of a overlay
 *object will allow its placement upon the                          * map. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *the map.       *
 *                                                                                             *
 *    This support routine is used by the scenario editor to add a overlay
 ...
 *                                                                                             *
 *    This routine will create an object of this type. For certain overlay
 *objects, such       * as walls, it is actually created as a building. The
 *"building" wall is converted into    * a overlay at the moment of placing down
 *on the map.                                      *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the appropriate object for this overlay
 *type.            *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will draw the overlay shape at the coordinates specified. It
 *is presumed    * that all the underlying layers have already been rendered by
 *the time this routine is    * called. *
 *                                                                                             *
 * INPUT:   x, y  -- Coordinate (upper left) of cell where overlay image is to
 *be drawn.       *
 *                                                                                             *
 *          data  -- Cell specific data that controls the imagery of the
 *overlay.              *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 * OverlayTypeClass::Init -- Initialize the overlay graphic data per theater. *
 *                                                                                             *
 *    This routine will update the overlay graphic data according to the theater
 *specified.    * It is typically called when the scenario is first loaded
 *(theater change).               *
 *                                                                                             *
...
/***********************************************************************************************
 * OverlayTypeClass::As_Reference -- Fetch a reference to the overlay type
 *specified.          *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   Be sure that the overlay type specified is legal. Illegal type
 ...
 **
 *                                                                                             *
 *    This will adjust the coordinate specified so that it will be of legal
 *format to          * assign as the coordinate of an overlay. Overlays are
 *always relative to the upper left   * corner of the cell, so this routine
 *drops the fractional cell components.                *
 *                                                                                             *
