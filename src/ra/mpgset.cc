...
MPGSettings::MPGSettings(FileClass& file) : mDeviceName(nullptr) {
  ...
  if (ini.Load(file)) {
    ini.Get_String("MovieSettings", "Device", "Default", buffer,
                   sizeof(buffer));
  }
...
void MPGSettings::Dialog() {
  ...
  TextButtonClass testbtn(BUTTON_TEST, "Test", TPF_BUTTON, d_test_x, d_test_y,
                          d_test_w, d_test_h);
  ...
  ListClass method(BUTTON_METHOD, d_method_x, d_method_y, d_method_w,
                   d_method_h, TPF_TEXT, MFCD::Retrieve("BTN-UP.SHP"),
