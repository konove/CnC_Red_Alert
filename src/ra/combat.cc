 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Combat_Anim -- Determines explosion animation to play. *
 *   Explosion_Damage -- Inflict an explosion damage affect. * Modify_Damage --
 ...
 *                                                                                             *
 *    This routine is the core of combat tactics. It implements the * affect
 *various armor types have against various weapon types. By * careful
 *exploitation of this table, tactical advantage can be * obtained. *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 04/16/1994 JLB : Created. * 04/17/1994 JLB : Always does a minimum
 *of damage.                                         * 01/01/1995 JLB : Takes
 *into account distance from damage source.                          *
...
int Modify_Damage(int damage, WarheadType warhead, ArmorType armor,
  ...
  *if the heal *	effect is close enough.
  */
  if (damage < 0) {
    if (distance < 0x008) {
 ...
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This routine can consume some time and will affect the AI * of
 *nearby enemy units (possibly).                                               *
 *                                                                                             *
 * HISTORY: * 08/16/1991 JLB : Created. * 11/30/1991 JLB : Uses coordinate
 *system.                                                  * 12/27/1991 JLB :
 *Radius of explosion damage effect.                                       *
 *   04/13/1994 JLB : Streamlined. * 04/16/1994 JLB : Warhead damage type
 *modifier.                                            * 04/17/1994 JLB :
 *Cleaned up.                                                              *
 *   06/20/1994 JLB : Uses object pointers to distribute damage. * 06/20/1994
 *JLB : Source is a pointer. * 06/18/1996 JLB : Strength could be negative for
 *healing effects.                          *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    This routine is called when a projectile impacts. This routine will
 ...
 *                                                                                             *
 *          land     -- The land type that this explosion is over. Sometimes,
 *this makes       * a difference (especially over water). *
 *                                                                                             *
 * OUTPUT:  Returns with the animation to play. If no animation is to be played,
 *then          * ANIM_NONE is returned. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/19/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 * Wide_Area_Damage -- Apply wide area damage to the map. *
 *                                                                                             *
 *    This routine will apply damage to a very wide area on the map. The damage
 *will be        * spread out from the coordinate specified by the radius
 *specified. The amount of damage   * will attenuate according to the distance
 *from center.                                    *
 *                                                                                             *
 * INPUT:   coord    -- The coordinate that the explosion damage will center
 *about.            *
 *                                                                                             *
