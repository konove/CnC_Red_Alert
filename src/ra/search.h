 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * IndexClass<T>::Add_Index -- Add element to index tracking
 *system.                         * IndexClass<T>::Clear -- Clear index handler
 *to empty state.                               * IndexClass<T>::Count -- Fetch
 *the number of index entries recorded.                       *
 *   IndexClass<T>::Fetch_Index -- Fetch data from specified index. *
 *   IndexClass<T>::Increase_Table_Size -- Increase the internal index table
 *capacity.         * IndexClass<T>::IndexClass -- Constructor for index
 *handler.                               * IndexClass<T>::Invalidate_Archive --
 *Invalidate the archive pointer.                      *
 *   IndexClass<T>::Is_Archive_Same -- Checks to see if archive pointer is same
 *as index.      * IndexClass<T>::Is_Present -- Checks for presense of index
 *entry.                          * IndexClass<T>::Remove_Index -- Find matching
 *index and remove it from system.             * IndexClass<T>::Search_For_Node
 *-- Perform a search for the specified node ID              *
 *   IndexClass<T>::Set_Archive -- Records the node pointer into the archive. *
 *   IndexClass<T>::Sort_Nodes -- Sorts nodes in preparation for a binary
 *search.              * IndexClass<T>::~IndexClass -- Destructor for index
 *handler object.                        * compfunc -- Support function for
 *bsearch and bsort.                                       *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
*indexing. The regular binary sort and search *	function are used for speedy
*index retreival. Typical use of this would be to index pointers to *	normal
*data objects, but it can be used to hold the data objects themselves. Keep in
*mind that *	the data object "T" is copied around by this routine in the
*internal tables so not only must *	it have a valid copy constructor, it
*must also be efficient. The internal algorithm will *	create an arbitrary
*number of default constructed objects of type "T". Make sure it has a
**	default constructor that is effecient. The constructor need not perform
*any actual *	initialization since this class has prior knowledge about the
*legality of these temporary *	objects and doesn't use them until after the
*copy constructor is used to initialize them.
*/
...
class IndexClass {
 ...
 private:
  /*
  **	This node object is used to keep track of the connection between the
  *data *	object and the index identifier number.
  */
  struct NodeElement {
 ...
 *                                                                                             *
 *    This routine will free all internal resources and bring the index handler
 *into a         * known empty state. After this routine, the index handler is
 *free to be reused.           *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *capacity.           *
 *                                                                                             *
 *    This helper function will increase the capacity of the internal index
 *table without      * performing any alterations to the index data. Use this
 *routine prior to adding a new     * element if the existing capacity is
 *insufficient.                                        *
 *                                                                                             *
 * INPUT:   amount   -- The number of index element spaces to add to its
 *capacity.             *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This will return the quantity of index entries that have been recored by
 *this index      * handler. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will scan for the specified index entry. If it was found,
 ...
 * IndexClass<T>::Fetch_Index -- Fetch data from specified index. *
 *                                                                                             *
 *    This routine will find the specified index and return the data value
 *associated with it. *
 *                                                                                             *
 * INPUT:   id -- The index ID to search for. *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   This routine presumes that the index exists. If it doesn't exist,
 *then the      * default constructed object "T" is returned instead. To avoid
 *this problem,      * always verfiy the existance of the index by calling
 *Is_Present() first.         *
 *                                                                                             *
 * HISTORY: * 11/02/1996 JLB : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * IndexClass<T>::Is_Archive_Same -- Checks to see if archive pointer is same as
 *index.        *
 *                                                                                             *
 *    This routine compares the specified index ID with the previously recorded
 *index archive  * pointer in order to determine if they match. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will set the archive pointer to an invalid state. This must
 *be performed    * if ever the archive pointer would become illegal (such as
 *when the element it refers to  * is deleted). *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * IndexClass<T>::Set_Archive -- Records the node pointer into the archive. *
 *                                                                                             *
 *    This routine records the specified node pointer. Use this routine when
 *there is a        * good chance that the specified node will be requested in
 ...
 *                                                                                             *
 *    This routine will record the index information into this index manager
 *object. It        * associates the index number with the data specified. The
 *data is copied to an internal   * storage location. *
 *                                                                                             *
 * INPUT:   id    -- The ID number to associate with the data. *
 *                                                                                             *
 *          data  -- The data to store. *
 *                                                                                             *
 * OUTPUT:  bool; Was the element added without error? Failure indicates that
 *RAM has been     * exhausted. *
 *                                                                                             *
 * WARNINGS:   The data is COPIED to internal storage. This means that the data
 *object must    * have a functional and efficient copy constructor and
 *assignment operator.       *
 *                                                                                             *
 * HISTORY: * 11/02/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 **
 *                                                                                             *
 *    This will scan through the previously added index elements and if a match
 *was found, it  * will be removed. *
 *                                                                                             *
...
bool IndexClass<T>::Remove_Index(int id) {
  ...
  **	downward to fill the vacated location. We cannot use memcpy here because
  *the type *	object may not support raw copies. C++ defines the assignment
  *operator to deal *	with this, so that is what we use.
  */
  if (found_index != -1) {
 ...
 *                                                                                             *
 *    This compare function presumes that its parameters are pointing to
 *NodeElements and that * the first "int" in the node is the index ID number to
 ...
 *                                                                                             *
 * WARNINGS:   This is highly dependant upon the layout of the NodeElement
 *structure.          *
 *                                                                                             *
 * HISTORY: * 11/02/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 **
 *                                                                                             *
 *    This routine will perform a binary search on the previously recorded index
 *values and    * if a match was found, it will return a pointer to the
 *NodeElement.                       *
 *                                                                                             *
 * INPUT:   id -- The index ID to search for. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the NodeElement that matches the index ID
 *specified. If  * no matching index could be found, then NULL is returned. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 11/02/1996 JLB : Created. *
 *=============================================================================================*/
template <class T>
const IndexClass<T>::NodeElement* IndexClass<T>::Search_For_Node(int id) const {
  /*
  ...
  */
  ...
  /*
  **	If the list has not yet been sorted, then do so now. Binary searching
