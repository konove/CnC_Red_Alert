 ...
 *                                                                                             *
 *    This is the default constructor for FootClass objects. It sets the foot
 *class values to  * their default starting settings. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    This routine is used to set a unit's velocity control structure. * The
 *game will then process the unit's movement during the momentum * physics
 *calculation. *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/07/1992 JLB : Created. * 09/24/1993 JLB : Revised for faster
 *speed.                                                * 04/02/1994 JLB :
 *Revised for new system.                                                  *
 *   04/15/1994 JLB : Converted to member function. * 07/21/1994 JLB :
 *Simplified.                                                              *
 *=============================================================================================*/
 ...
 * FootClass::Mark -- Unit interface to map rendering system. *
 *                                                                                             *
 *    This routine is the interface function for units as they relate to * the
 *map rendering system. Whenever a unit's imagery changes, this * function is
 *called.                                                                      *
 *                                                                                             *
 * INPUT:   mark  -- Type of image change (MARK_UP, _DOWN, _CHANGE) * MARK_UP --
 *Unit is removed.                                                    *
 *             MARK_CHANGE -- Unit alters image but doesn't move. * MARK_DOWN --
 *Unit is overlaid onto existing icons.                              *
 *                                                                                             *
 * OUTPUT:  bool; Did the marking operation succeed? Failure could be the result
 *of marking    * down when it is already down, or visa versa. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/14/1991 JLB : Created. * 04/15/1994 JLB : Converted to member
 *function.                                            * 12/23/1994 JLB :
 ...
 *                                                                                             *
 *    This is a common routine used by both infantry and other ground travelling
 *units. It     * will fill in the unit's basic path to the NavCom destination.
 **
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  bool; Was a path found? A failure to find a path means either the
 *target cannot    * be found or the terrain prohibits the unit's movement. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
  ...
  if (Target_Legal(NavCom)) {
    ...
    */
    ...
    if (!skip_path) {
      ...
      *keep trying *	with greater determination until either a complete
      ...
      */
      for (;;) {
        ...
        **	level if the severity can be increased. If not, then consider
 ...
 *destination.              *
 *                                                                                             *
 *    This simple AI script handles moving the vehicle to its desired
 *destination. Since       * simple movement is handled directly by the engine,
 *this routine merely waits until       * the unit has reached its destination,
 *and then causes the unit to enter idle mode.       *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * FootClass::Mission_Capture -- Handles the capture mission. *
 *                                                                                             *
 *    Capture missions are nearly the same as normal movement missions. The only
 *difference    * is that the final destination is handled in a special way so
 *that it is not marked as    * impassable. This allows the object (usually
 *infantry) the ability to walk onto the       * object and thus capture it. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Units that are performing stationary guard duty use this AI process. They
 *will sit       * still and target any enemies that get within range. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the delay before calling this routine again. *
...
int FootClass::Mission_Guard() {
  ...
  if (What_Am_I() == RTTI_INFANTRY) {
    /*
    **	If this is a bomber type infantry and the current target is a building,
 ...
 *object.              *
 *                                                                                             *
 *    This is the counterpart routine to the Start_Driver function. It clears
 *the driving      * status flags and destination coordinate record. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * FootClass::Start_Driver -- This starts the driver heading to the destination
 *desired.       *
 *                                                                                             *
 *    Before a unit can move it must be started by this routine. This routine
 *handles          * reserving the cell and setting the driving flag. *
 *                                                                                             *
 * INPUT:   headto   -- The coordinate of the immediate drive destination. This
 *is one cell    * away from the unit's current location. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will determine the sort coordinate for foot class object.
 *This coordinate   * is usually the coordinate of the object. The exception is
 *if the object is tethered.     * In this case (presumes offloading to the
 *north), the sorting coordinate is adjusted      * so that the object will be
 *drawn on top of the transport unit.                           *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine clears the units' navigation computer in preparation for
 *removal from the   * game. This is probably called as a result of unit
 *destruction in combat. Clearing the    * navigation computer ensures that the
 *normal AI process won't start it moving again while * the object is undergoing
...
/***********************************************************************************************
 * FootClass::Approach_Target -- Sets the navigation computer to approach target
 *object.       *
 *                                                                                             *
 *    This routine will set the navigation computer to approach the target
 *indicated by the    * targeting computer. It is through this function that the
 *unit nears the target so        * that weapon firing may occur. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/31/1994 JLB : Created. * 12/13/1994 JLB : Made part of
 *TechnoClass.                                                * 12/22/1994 JLB :
    ...
    */
    ...
    if (!Target_Legal(NavCom) && (!In_Range(TarCom, primary) || !IsLocked)) {
      ...
      */
      ...
      /*
      **	Sweep through the cells between the target and the unit, looking
      *for *	a cell that the unit can enter but which is also within weapon
      *range *	of the target. If after a reasonable search, no appropriate cell
      *could *	be found, then the target will be assigned as the movement
      ...
      */
      ...
      /*
      **	If a suitable intermediate location was found, then head toward
      *it. *	Otherwise, head toward the enemy unit directly.
      */
      if (found) {
 ...
 *weapon range.     *
 *                                                                                             *
 *    This mission routine causes the unit to scan for targets out to twice its
 *weapon range   * from the home point. If a target was found, then it will be
 *attacked. The unit will      * chase the target until it gets up to to its
 *weapon range from the home position.         * In that case, it will return to
 *home position and start scanning for another target.     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * FootClass::Unlimbo -- Unlimbos object and performs special fixups. *
 *                                                                                             *
 *    This routine will make sure that the home position for the foot class
 *object gets        * reset. This is necessary since the home position may
 *change depending on the unit's      * transition between limbo and non-limbo
 *condition.                                        *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine intercepts the damage assigned to this object and if this
 *object is         * a member of a team, it informs the team that the damage
 *has occurred. The team may       * change it's priority or action based on
 *this event.                                      *
 *                                                                                             *
 * INPUT:   damage      -- The damage points inflicted on the unit. *
 *                                                                                             *
 *          distance    -- The distance from the point of damage to the unit
 *itself.           *
 *                                                                                             *
 *          warhead     -- The type of damage that is inflicted. *
 *                                                                                             *
 *          source      -- The perpetrator of the damage. By knowing who caused
 *the damage,    * the team know's who to "get even with". *
 *                                                                                             *
...
ResultType FootClass::Take_Damage(int& damage, int distance,
  ...
  if (result != RESULT_NONE && Team) {
    ...
    if (result != RESULT_DESTROYED && result != RESULT_NONE) {
      /*
      **	Determine if the target that is currently being attacked has a
      *weapon that can *	do harm to a ground based unit. This information
      *is needed so that an appropriate *	response will occur when damage
      *is taken.
      */
      //			bool tweap = false;
      //			if (As_Techno(TarCom)) {
      ...
      */
      ...
      /*
      **	If this object is not part of a team and it can retaliate for
...
/***********************************************************************************************
 * FootClass::Active_Click_With -- Initiates attack or move according to target
 *clicked on.    *
 *                                                                                             *
 *    At this level, the object is known to have the ability to attack or move
 *to the          * target specified (in theory). Perform the attack or move as
 *indicated.                   *
 *                                                                                             *
  ...
  switch (action) {
    ...
    case ACTION_MOVE:
      if (Can_Player_Move()) {
        ...
        /*
        **	If the destination object is not the same zone, then pick a
        *nearby location.
        */
...
/***********************************************************************************************
 * FootClass::Active_Click_With -- Performs action as a result of left mouse
 *click.            *
 *                                                                                             *
 *    This routine performs the action requested when the left mouse button was
 *clicked over   * a cell. Typically, this is just a move command. *
 *                                                                                             *
 ...
 *condition.             *
 *                                                                                             *
 *    This routine is called as this object moves from cell to cell. When the
 *center of the    * cell is reached, check to see if any trigger should be
 *sprung. For moving units, reduce  * the path to the distance to the target.
 *This forces path recalculation in an effort to   * avoid units passing each
 *other.                                                          *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/08/1995 JLB : Created. * 07/08/1995 JLB : Handles generic enter
 ...
 *					TARGET	   tarcom  - the new target we
 *want to override		* TARGET		navcom  - the new
 *navigation point to override	*
 *                                                                         *
 * OUTPUT:		none *
 *                                                                         *
...
/***********************************************************************************************
 * FootClass::Receive_Message -- Movement related radio messages are handled
 *here.             *
 *                                                                                             *
 *    This routine handles radio message that are related to movement. These are
 *used for      * complex coordinated maneuvers. *
 *                                                                                             *
 * INPUT:   from     -- Pointer to the originator of this radio message. *
 *                                                                                             *
 *          message  -- The radio message that is being received. *
 *                                                                                             *
 *          param    -- The optional parameter (could be a movement
 *destination).              *
 *                                                                                             *
 * OUTPUT:  Returns with the radio response appropriate to the message received.
 *Usually the   * response is RADIO_ROGER. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
RadioMessageType FootClass::Receive_Message(RadioClass* from,
  ...
  switch (message) {
    ...
    */
    ...
    /*
    **	Something bad has happened to the object in contact with. Abort any
    *coordinated *	activity with this object. Basically, ... run away! Run
    *away!
    */
    case RADIO_RUN_AWAY:
      if (In_Radio_Contact()) {
    ...
    /*
    **	Checks to see if this unit needs to move somewhere. If it is already in
    ...
    *occurs *	for passengers and when vehicles need to be repaired.
    */
 ...
 * FootClass::Mission_Enter -- Enter (cooperatively) mission handler. *
 *                                                                                             *
 *    This mission handler will cooperatively coordinate the object to maneuver
 *into the       * object it is in radio contact with. This is used by infantry
 *when they wish to load      * into an APC as well as by vehicles when they
 *wish to enter a repair facility.            *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns the number of game ticks before this routine should be
 *called again.       *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
int FootClass::Mission_Enter() {
  ...
  */
  if (contact != nullptr) {
    ...
    *again later.
    */
    if (Transmit_Message(RADIO_DOCKING, contact) != RADIO_ROGER &&
        !IsTethered) {
 ...
 *                                                                                             *
 *    This routine will assign the specified target to the navigation computer.
 *No legality    * checks are performed. *
 *                                                                                             *
...
void FootClass::Assign_Destination(TARGET target) {
  ...
  /*
  **	Presume that the easiest path is tried first. As the findpath proceeds,
  *when *	a failure occurs, this threshhold will be increased until path
  *failure *	cannot be prevent. At this point, all movement should cease.
  */
  PathThreshhold = MOVE_CLOAK;
}
 ...
 *                                                                                             *
 *    This routine will remove this object from the game system. This routine is
 *called when   * this object is about to be deleted. All other objects should
 *no longer reference this    * object in that case. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    This routine is called when the house determines that it should attack the
 *specified     * target. This routine will determine if it can attack the
 *target specified and if so,     * the amount of power it can throw at it. This
 *returned power value is used to allow       * intelligent distribution of
 *retaliation.                                                 *
 *                                                                                             *
 * INPUT:   target   -- The target that this object just might be assigned to
 *attack and thus  * how much power it can bring to bear should be returned. *
 *                                                                                             *
 * OUTPUT:  Returns with the amount of power that this object can bring to bear
 *against the    * potential target specified. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 07/08/1995 JLB : Created. *
 ...
 *                                                                                             *
 *    This routine is called when a unit (infantry, vehicle, or aircraft) is
 *destroyed.        *
 *                                                                                             *
 ...
 * FootClass::Greatest_Threat -- Fetches the greatest threat to this object. *
 *                                                                                             *
 *    This routine will return with the greatest threat (best target) for this
 *object. For     * movable ground object, they won't automatically return ANY
 *target if this object is      * cloaked. Otherwise, cloaking is relatively
 *useless.                                      *
 *                                                                                             *
 * INPUT:   method   -- The request method (bit flags) to use when scanning for
 *a target.      *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 07/08/1995 JLB : Created. * 07/10/1996 JLB : Handles scan range
 *limitation.                                           *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    This routine will detach the specified target from the tracking systems of
 *this object.  * It will be removed from the navigation computer and any queued
 *mission record.           *
 *                                                                                             *
 * INPUT:   target   -- The target to be removed from this object. *
 *                                                                                             *
 *          all      -- Is the unit really about to be eliminated? If this is
 *true then even   * friendly contact (i.e., radio) must be eliminated. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 07/18/1995 JLB : Created. * 07/24/1996 JLB : Removes target from
 *NavQueue list.                                       *
 *=============================================================================================*/
void FootClass::Detach(TARGET target, bool all) {
  ...
  /*
  **	If the navigation computer is assigned to the target, then the
 ...
 **
 *                                                                                             *
 *    This routine is called when a packet/package/bail of Tiberium needs to be
 *offloaded      * from the object. This function is overridden for those
 *objects that can contain          * Tiberium. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the number of credits offloaded from the object. *
 *                                                                                             *
 * WARNINGS:   This routine must be called multiple times in order to completely
 *offload the   * Tiberium. When this routine return 0, all Tiberium has been
 *offloaded.          *
 *                                                                                             *
 ...
 *specified.          *
 *                                                                                             *
 *    This routine examines the specified cell to see if the object can enter
 *it. This         * function is to be overridden for objects that could have
 *the possibility of not being    * allowed to enter the cell. Typical objects
 *at the FootClass level always return          * MOVE_OK. *
 *                                                                                             *
 * INPUT:   cell     -- The cell to examine. *
 *                                                                                             *
 *          facing   -- The direction that this cell might be entered from. *
 *                                                                                             *
 * OUTPUT:  Returns with the move check result type. This will be MOVE_OK if
 *there is not      * blockage. There are various other values that represent
 *other blockage types.      * The value returned will indicated the most severe
 *reason why entry into the cell   * is blocked. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 07/19/1995 JLB : Created. *
 ...
 * FootClass::Can_Demolish -- Checks to see if this object can be sold back. *
 *                                                                                             *
 *    This routine determines if it is legal to sell the object back. A foot
 *class object can  * only be sold back if it is sitting on a repair bay. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * FootClass::Sell_Back -- Causes this object to be sold back. *
 *                                                                                             *
 *    When an object is sold back, a certain amount of money is refunded to the
 *owner and then * the object is removed from the game system. *
 *                                                                                             *
 * INPUT:   control  -- The action to perform. The only supported action is "1",
 *which means   * to sell back. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
 ...
 *shortly.            *
 *                                                                                             *
 *    This routine comes in handy when determining where a travelling object
 *will be at        * when considering the amount of time it would take for a
 *normal unit to travel one cell.  * Using this information, an intelligent
 *"approach target" logic can be employed.          *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * FootClass::Adjust_Dest -- Adjust candidate movement cell to account for
 *formation.          *
 *                                                                                             *
 *    This routine modify the specified cell if the unit is part of a formation.
 *The           * adjustment will take into consideration the formation relative
 *offset from the           * (presumed) center cell specified. *
 *                                                                                             *
 * INPUT:   cell  -- The cell to presume as the desired center point of the
 *formation.         *
 *                                                                                             *
 * OUTPUT:  Returns with the cell that should be used as the actual destination.
 *If this       * object is part of a formation, then the cell location will be
 ...
 * FootClass::Handle_Navigation_List -- Processes the navigation queue. *
 *                                                                                             *
 *    This routine will process the navigation queue. If the queue is present
 *and valid and    * there is currently no navigation target assigned to this
 *object, then the first entry    * of the queue will be assigned. The remaining
 *entries will move down. If the queue is     * to be processed as a circular
 *list, then the first entry is appended to the end.         *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
  ...
  if (!Target_Legal(NavCom)) {
    ...
    */
    if (Target_Legal(target)) {
      ...
      *the *	target value from the first part to the end of the queue.
      */
      if (IsNavQueueLoop) {
        for (int index = 0; index < std::ssize(NavQueue); index++) {
...
/***********************************************************************************************
 * FootClass::Queue_Navigation_List -- Add a target to the objects navigation
 *list.            *
 *                                                                                             *
 *    This routine will append the destination target to the object's NavQueue
 *list. After     * doing so, if the object is not doing anything important,
 *then it will be started on      * that destination. This is functionally the
 *same as Assign_Destination, but it stores     * the target to the NavQueue
 *first.                                                        *
 *                                                                                             *
 ...
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   The queue is of finite size and any queue requests that would
 *exceed that size  * are ignored. If there are no queue entries pending and the
 *unit is not          * otherwise occupied, then the queue target might be
 *carried directly into the    * NavCom. *
 *                                                                                             *
 * HISTORY: * 07/18/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    This routine will clear out any values in the navigation queue. This is
 *the preferred    * way of aborting a navigation queue for a unit. If the unit
 *is already travelling, it     * won't be interrupted by this routine. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This will clear the navigation list but not the navigation
 *computer. Thus a     * unit will still travel to its current immediate
 *destination.                    *
 *                                                                                             *
 ...
 *and the game.   *
 *                                                                                             *
 *    This routine will determine if this object has permission to leave the map
 *and thus      * leave the game. Typical objects with this permission are
 *transports used to drop of      * reinforcements. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  bool; Does this object have permission to travel off the map edge
 *and leave the    * game? *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
bool FootClass::Is_Allowed_To_Leave_Map() const {
  ...
  *objects can never *	leave the map unless they are part of a team that gives
...
/***********************************************************************************************
 * FootClass::Is_Recruitable -- Determine if this object is recruitable as a
 *team members.     *
 *                                                                                             *
 *    This will examine this object to determine if it is suitable as a team
 *recruit. Some     * objects are disqualified if they are otherwise premptively
 *occupied.                     *
 *                                                                                             *
...
bool FootClass::Is_Recruitable(const HouseClass* house) const {
  ...
  */
  ...
  /*
  **	If the object is not a playing member of the game, then don't consider
  *it available.
  */
  if (IsInLimbo) {
    return false;
  ...
  */
  ...
  /*
  **	If it is currently in a mission the precludes recruitment into a team,
 ...
 *                                                                                             *
 *    This basically just sees if this object is within weapon range of the
 *target and if      * so, it will stop movement so that firing may commence.
 *This prevents the occasional      * case of an attacker driving right up to
 *the defender before firing.                      *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
/***********************************************************************************************
 * FootClass::Is_On_Priority_Mission -- Checks to see if this object should be
 *given priority. *
 *                                                                                             *
 *    Some objects are on an important mission that must succeed. If the object
 *is on such     * a mission, then it will be more aggressive in its movement
 *action.                       *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * FootClass::Mission_Retreat -- Handle reatreat from map mission for mobile
 *objects.          *
 *                                                                                             *
 *    This will try to make this mobile object leave the map. It does this by
 *assigning a      * movement destination that is located off the edge of the
 *map.                            *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
    ...
    */
    case FIND_EDGE:
      if (Target_Legal(NavCom)) {
        ...
        /*
        **	If this is part of a team, then pick the edge where the team as
        *likely *	entered from.
        */
        ...
        /*
        **	If an edge hasn't been found, then try to find one that is not
