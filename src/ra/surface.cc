...
Surface::Surface(int w, int h, Buffer const* buffer, int pitch)
  ...
  */
  if (buffer != nullptr) {
    ...
    /*
    **	Reduce the dimensions if the buffer is not big enough. This size
    *bounding is only *	possible if the buffer size is known. Otherwise, presume
    ...
    */
    ...
    /*
    **	A new buffer without existing memory specified, will allocate and manage
    *its *	own memory for the surface.
    */
