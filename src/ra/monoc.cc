 ...
 * MonoClass::Draw_Box -- Draws a box using the IBM linedraw characters. *
 *                                                                                             *
 *    Use this routine to draw a box to the monochrome screen. The IBM line draw
 *characters    * are used to give the it a fancy appearance. There are several
 ...
 *                                                                                             *
 *          attrib   -- The text attribute to use when drawing the box outline
 *characters.     *
 *                                                                                             *
 *          thick    -- The thickness style to use. Examine the BoxStyleType
 *enum for          * elaboration on the supported styles. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   The interior of the box is NOT cleared by this routine. It is
 *advised that this * area be cleared before the box is drawn. *
 *                                                                                             *
 * HISTORY: * 10/17/1994 JLB : Created. *
 *=============================================================================================*/
...
void MonoClass::Fill_Attrib(int x, int y, int w, int h, MonoAttribute attrib) {
  if (!w || !h || static_cast<unsigned>(x) >= SubW || static_cast<unsigned>(h) >= SubH ||
      static_cast<unsigned>(x) + w > SubW || static_cast<unsigned>(y) + h > SubH)
    return;
  ...
  while (*text) {
    ...
    */
    switch (cell.Character) {
      ...
      **	cursor goes off the bottom of the display, the display is
...
/***********************************************************************************************
 * MonoClass::operator = -- Handles making one mono object have the same imagery
 *as another.   *
 *                                                                                             *
 *    The assignment operator will handle copying the imagery from one
 *monochrome object to    * another. Use this routine in to make two monochrome
 *class objects visually identical.    *
 *                                                                                             *
...
/************************************************************************************
**	This is the set of C wrapper functions that access the MonoClass support
*routines. *	Since the C interface doesn't have the ability to write to
*non-visible pages, it *	will just blast the output to whichever mono
*page is currently visible. If there is *	no mono class object that is
*visible, then one will be created -- BUT NOT FREED. *	Typically, this is ok,
*since the C interface will create only one MonoClass object *	and the system
*supports up to 8.
*/
 ...
 *                                                                                             *
 *    This routine will print formatted text (with parameters) to the visible
 *monochrome page  * at whatever the current cursor location is. *
 *                                                                                             *
 * INPUT:   string   -- The string to use as the main text and formatting
 *control string.      *
 *                                                                                             *
 *          ...      -- Any additional parameters required by the formatting
 *string.           *
 *                                                                                             *
 * OUTPUT:  Returns with the number of characters written to the display. *
 *                                                                                             *
 * WARNINGS:   The total size of the formatted text must not exceed 256
 *characters.            *
 *                                                                                             *
 * HISTORY: * 06/04/1996 JLB : Created. *
 *=============================================================================================*/
...
void Mono_Text_Print(void const* text, int x, int y, int attrib) {
  if (MonoClass::Is_Enabled()) {
    ...
    mono->Text_Print(static_cast<const char*>(text), x, y, static_cast<MonoClass::MonoAttribute>(attrib));
  }
}
 ...
 *                                                                                             *
 *    Use this routine to draw a rectangle to the monochrome screen. The
 *dimensions, attribute,* and line style are controlled by parameters. *
 *                                                                                             *
...
void Mono_Draw_Rect(int x, int y, int w, int h, int attrib, int thick) {
  if (MonoClass::Is_Enabled()) {
    ...
    mono->Draw_Box(x, y, w, h, static_cast<MonoClass::MonoAttribute>(attrib),
                   static_cast<MonoClass::BoxStyleType>(thick));
  }
}
 ...
 * Mono_X -- Fetches the X cursor position for current visible mono page. *
 *                                                                                             *
 *    Use this routine to get the current cursor X position. This only applies
 *to the          * currently visible monochrome page. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * Mono_Y -- Fetches the Y cursor position for current mono page. *
 *                                                                                             *
 *    This routine will fetch the current Y cursor position for the monochrome
 *page.           *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
