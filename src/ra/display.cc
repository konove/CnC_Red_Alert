...
void DisplayClass::Draw_It(bool forced) {
  ...
  if (IsToRedraw || forced) {
        ...
        for (y = starty;
          ...
          for (x = startx;
            ...
            if (x <= oldx || x >= oldx + oldw || y <= oldy ||
                y >= oldy + oldh) {
              CELL c = Click_Cell_Calc(
                  Bound(x, 0, Lepton_To_Pixel(TacLeptonWidth) - 1) + TacPixelX,
                  Bound(y, 0, Lepton_To_Pixel(TacLeptonHeight) - 1) +
                      TacPixelY);
    ...
    */
    HidPage.Blit(HidPage, 0, HidPage.Get_Height() - 1, 0, HidPage.Get_Height(),
                 HidPage.Get_Width(), 1, false);
#endif
...
int DisplayClass::TacticalClass::Action(unsigned flags, KeyNumType& key) {
  ...
  */
  ...
  bool edge = y == 0 || x == 0 || x == SeenBuff.Get_Width() - 1 ||
