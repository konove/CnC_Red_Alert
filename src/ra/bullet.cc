 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * BulletClass::AI -- Logic processing for bullet. *
 *   BulletClass::BulletClass -- Bullet constructor. *
 *   BulletClass::Bullet_Explodes -- Performs bullet explosion logic. *
 *   BulletClass::Detach -- Removes specified target from this bullet's
 *targeting system.      * BulletClass::Draw_It -- Displays the bullet at
 *location specified.                        * BulletClass::In_Which_Layer --
 *Fetches the layer that the bullet resides in.              * BulletClass::Init
 *-- Clears the bullets array for scenario preparation.                   *
 ...
 *   BulletClass::Shape_Number -- Fetches the shape number for the bullet
 *object.              * BulletClass::Sort_Y -- Sort coordinate for bullet
 *rendering.                              * BulletClass::Target_Coord -- Fetches
 *coordinate to use when firing on this object.        * BulletClass::Unlimbo --
 *Transitions a bullet object into the game render/logic system.    *
 *   BulletClass::delete -- Bullet memory delete. * BulletClass::new --
 *Allocates memory for bullet object.                                   *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/02/1994 JLB : Created. * 06/20/1994 JLB : Firer is a base class
 *pointer.                                           * 12/10/1994 JLB : Auto
 ...
 * BulletClass::~BulletClass -- Destructor for bullet objects. *
 *                                                                                             *
 *    The bullet destructor must detect if a dog has been attached to this
 *bullet. If so,      * then the attached dog must be unlimboed back onto the
 *map. This operation is necessary   * because, unlike other objects, the dog
 *flies with the bullet it fires.                   *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * BulletClass::Occupy_List -- Determines the bullet occupation list. *
 *                                                                                             *
 *    This function will determine the cell occupation list and return a pointer
 *to it. Most   * bullets are small and the list is usually short, but on
 *occasion, it can be a list that  * rivals the size of regular vehicles. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the cell offset list that covers all the
 *cells a bullet  * is over. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
std::span<const int16_t> BulletClass::Occupy_List(bool /*placement*/) const {
  ...
  */
  ...
  /*
  **	Flying units need a special adjustment to the spillage list to take into
 ...
 * BulletClass::Mark -- Performs related map refreshing under bullet. *
 *                                                                                             *
 *    This routine marks the objects under the bullet so that they will * be
 *redrawn. This is necessary as the bullet moves -- objects under * its path
 *need to be restored. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *    Primarily this is motion, fuse tracking, and detonation logic. Call * this
 *routine no more than once per bullet per game tick. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
void BulletClass::AI() {
  ...
  */
  ...
  /*
  **	Homing projectiles constantly change facing to face toward the target
  ...
  switch (Physics(coord, PrimaryFacing)) {
    ...
    */
    case IMPACT_NORMAL:
      ...
      /*
      **	See if the bullet should be forced to explode now in spite of
      *what *	the fuse would otherwise indicate. Maybe the bullet hit a wall?
      */
      if (!forced) {
...
int BulletClass::Shape_Number() const {
  ...
  if (!Class->IsFaceless) {
    shapenum = base::At(UnitClass::BodyShape, Dir_To_32(PrimaryFacing));
  }
 ...
 *                                                                                             *
 *    This routine will zero out the bullet tracking list and object array in
 *preparation for  * the start of a new scenario. All bullets cease to exists
 ...
 *system.        *
 *                                                                                             *
 *    When an object is removed from the game system, it must be removed all
 *targeting and     * tracking systems as well. This routine is used to remove
 *the specified object from the   * bullet. If the object isn't part of this
 *bullet's tracking system, then no action is     * performed. *
 *                                                                                             *
...
/***********************************************************************************************
 * BulletClass::Unlimbo -- Transitions a bullet object into the game
 *render/logic system.      *
 *                                                                                             *
 *    This routine is used to take a bullet object that is in limbo and
 *transition it to the   * game system. A bullet object so transitioned, will be
 *drawn and logic processing         * performed. In effect, it comes into
 *existence.                                           *
 *                                                                                             *
...
bool BulletClass::Unlimbo(COORDINATE coord, DirType dir) {
  ...
  *already *	set direction and strength if the "punt" values were passed in.
  *This allows a bullet *	to be setup prior to being launched.
  */
  if (!Class->IsHigh) {
    Height = 0;
  }
  if (ObjectClass::Unlimbo(coord)) {
    ...
    /*
    **	Homing projectiles (missiles) do NOT override facing. They just fire in
    *the *	direction specified and let the chips fall where they may.
    */
    ...
    /*
    **	Possibly adjust the target if this projectile is inaccurate. This occurs
    *whenever *	certain weapons are trained upon targets they were never
    *designed to attack. Example: when *	turrets or anti-tank missiles
    *are fired at infantry. Indirect *	fire is inherently inaccurate.
    */
    if (IsInaccurate || Class->IsInaccurate ||
        ((Is_Target_Cell(TarCom) || Is_Target_Infantry(TarCom)) &&
         (Warhead == WARHEAD_AP || Class->IsFueled))) {
      /*
      **	Inaccuracy for low velocity or homing projectiles manifests
      *itself as a standard *	Circular Error of Probability (CEP) algorithm.
      *High speed projectiles usually *	just overshoot the target by extending
    ...
    **	number of game frames it would take for the projectile to reach the
    *target.
    */
    int range = 0xFF;
    if (!Class->IsDropping) {
    ...
    *a *	vertical component for the projectile launch. This is crudely
    *simulated *	by biasing ground speed according to target distance and
    *then giving *	enough vertical velocity to keep the projectile airborne
    *for the *	desired amount of time. The mathematically correct solution
    *would be to *	calculate launch angle (given fixed projectile velocity)
    *and then derive *	the vertical and horizontal components. That solution
    *would require a *	square root and an arcsine lookup table. OUCH!
    */
    Riser = 0;
    if (Class->IsArcing) {
      IsFalling = true;
...
/***********************************************************************************************
 * BulletClass::Target_Coord -- Fetches coordinate to use when firing on this
 *object.          *
 *                                                                                             *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the coordinate that should be used when firing at the
 *object.         *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This will return the coordinate to use when sorting this bullet in the
 *display list.     * Typically, this only occurs for bullets in the ground
 *layer. Since bullets are to be     * seen a bit more than the normal sorting
 *order would otherwise imply, bias the sort       * value such that bullets
 *will tend to be drawn on top of the objects.                     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the coordinate to use when sorting this bullet in the
 *display list.   *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    This examines the bullet to determine what rendering layer it should be
 *in. The          * normal logic applies unless this is a torpedo. A torpedo is
 *always in the surface        * layer. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * BulletClass::Is_Forced_To_Explode -- Checks if bullet should explode NOW. *
 *                                                                                             *
 *    This routine will examine the bullet and where it is travelling in order
 *to determine    * if it should prematurely explode. Typical of this would be
 *when a bullet hits a wall     * or a torpedo hits a ship -- regardless of
 *where the projectile was originally aimed.     *
 *                                                                                             *
 * INPUT:   coord -- The new coordinate to place the bullet at presuming it is
 *forced to       * explode and a modification of the bullet's coordinate is
 *needed.          * Otherwise, the coordinate is not modified. *
 *                                                                                             *
 ...
 * BulletClass::Bullet_Explodes -- Performs bullet explosion logic. *
 *                                                                                             *
 *    This handles the exploding bullet action. It will generate the animation
 *and the         * damage as necessary. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
