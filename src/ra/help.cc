 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * HelpClass::Draw_Help -- Display the help message (if necessary).
 ** HelpClass::HelpClass -- Default constructor for the help processor. *
 *   HelpClass::Help_AI -- Handles the help text logic. * HelpClass::Help_Text
 *-- Assigns text as the current help text.                            *
 *   HelpClass::Init_Clear -- Sets help system to a known state. *
 *   HelpClass::Overlap_List -- Returns with offset list for cells under help
 *text.            * HelpClass::Scroll_Map -- Makes sure scrolling doesn't leave
 *text shards.                  * HelpClass::Set_Cost -- Initiates the second
 *line of help text showing item cost.          *
 ...
 * HelpClass::HelpClass -- Default constructor for the help processor. *
 *                                                                                             *
 *    The help processor is initialized by this routine. It merely sets up the
 *help engine     * to the default state. The default state will not display any
 *help text. Call the         * Help_Text() function to enable help processing.
 **
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *text.              *
 *                                                                                             *
 *    Use this routine to fetch an offset list for the cells under the text
 *displayed. If      * there is no text displayed, then the list will consist of
 *just the terminator code.      *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the offset list for the help text overlap.
 *The offset    * list is based on the tactical map upper left corner cell. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 11/18/1994 JLB : Created. *
 ...
 *                                                                                             *
 *    This routine handles tracking the mouse position to see if the mouse
 *remains stationary  * for the required amount of time. If the time requirement
 *has been met, then it flags     * the help system to display the help text the
 *next time the Draw_Help() function is       * called. *
 *                                                                                             *
 ...
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This routine must be called once and only once per game frame (15
 *times per     * second). *
 *                                                                                             *
  ...
  if (CountDownTimer && !HelpText && Text != TXT_NONE) {
    ...
    */
    if (!IsRight && (X != x || Y != y)) {
      ...
      /*
      **	If the delay has expired, then the text must be drawn. Build the
      *help text *	overlay list at this time. Better to do it now, when we
      *KNOW it is needed, then *	to do it earlier when it might not be
      *needed.
      */
      Set_Text(Text);
    }
 ...
 *                                                                                             *
 *    Use this routine to change the help text that will pop up if the cursor
 *isn't moved      * for the help delay duration. Call this routine as often as
 *desired.                      *
 *                                                                                             *
...
void HelpClass::Help_Text(int text, int x, int y, int /*color*/, bool quick) {
  if (text != Text) {
    ...
    */
    ...
    /*
    **	Record the position of the mouse. This recorded position will be used to
    *determine *	if the mouse has moved. A moving mouse prevents the help
 ...
 *                                                                                             *
 *    This function will print the help text if it thinks it should. The timer
 *and text        * message can control whether this occurs. If there is no help
 *text or the countdown timer * has not expired, then no text will be printed. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * HelpClass::Set_Text -- Determines the overlap list and draw coordinates. *
 *                                                                                             *
 *    This routine is used to build the overlap list -- used for icon
 *refreshing. It also      * determines if the text can fit on the screen and
 *makes adjustments so that it will.      *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 11/18/1994 JLB : Created. * 12/11/1994 JLB : Won't draw past
 *tactical map edges.                                      *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    This routine intercepts the map scrolling request and then makes sure that
 *if, in fact,  * the map is going to scroll, then reset and erase the help text
 *so that it doesn't        * mess up the display. *
 *                                                                                             *
 * INPUT:   facing   -- The direction to scroll (unused by this routine). *
 *                                                                                             *
 *          really   -- If the scroll is actually going to occur, rather than
 *just be examined * for legality, then this parameter will be true. If this
 *parameter is   * true, then the help text is reset. *
 *                                                                                             *
...
/***********************************************************************************************
 * HelpClass::Set_Cost -- Initiates the second line of help text showing item
 *cost.            *
 *                                                                                             *
 *    Use this routine after the Help_Text() function to activate the second
 *line. The second  * line displays a cost. Typically, this is used by the
 *sidebar to display the cost of the  * specified item. *
 *                                                                                             *
 * INPUT:   cost  -- The cost to associate with this help text. If this value is
 *zero, then    * no second line is displayed, so don't pass in zero. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 01/09/1995 JLB : Created. *
 *=============================================================================================*/
 ...
 * HelpClass::Set_Tactical_Position -- Sets the position of the tactical map. *
 *                                                                                             *
 *    This routine will set the position of the tactical map. At this class
 *level, it merely   * makes sure that the help text disappears when this
 *happens. The lower level classes      * actually change the map's position. *
 *                                                                                             *
 * INPUT:   coord -- The new coordinate to make the upper left corner of the
 *visible display.  *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
