 ...
 *                                                                         *
 *-------------------------------------------------------------------------*
 * Functions: * IPXManagerClass::IPXManagerClass -- class constructor *
...
int IPXManagerClass::Init() {
  ...
  IPXConnClass::Configure(Socket, ConnectionNum, ListenECB, SendECB,
                          FirstHeaderBuf, SendHeader, FirstDataBuf, SendBuf,
                          Handler, PacketLen);
 ...
 *                                                                         *
 * INPUT:                                                                  *
 *		id				application-specific numerical
 *ID for this connection		* node			ptr to
 *IPXNodeIDType (name & address)
...
int IPXManagerClass::Get_Private_Message(std::span<std::byte> buf, int* buflen,
  ...
  //------------------------------------------------------------------------
  //	Scan all connections for a received packet, starting with
  //'CurConnection'
  //------------------------------------------------------------------------
  for (int i = 0; i < NumConnections; i++) {
    //.....................................................................
    ...
    //.....................................................................
    const int rc = base::At(Connection, CurConnection)->Get_Packet(buf, buflen);
    const int c_id = base::At(Connection, CurConnection)->ID;
 ...
 ** The socket number is byte-swapped, since IPX requires socket ID's * to be
 *stored high/low.
 **
 *                                                                         *
 * HISTORY:                                                                *
...
int32_t IPXManagerClass::Response_Time() {
  ...
  for (int i = 0; i < NumConnections; i++) {
    maxresp =
        std::max(base::At(Connection, i)->Queue->Avg_Response_Time(), maxresp);
  }
  return maxresp;
}
 ...
 *                                                                         *
 * INPUT:                                                                  *
 *		index				connection index to configure
 *(-1 = Global Channel)	* type_offset		ID's byte offset into packet
 ** type_size		size of ID, in bytes; 0 if none
 ** names				ptr to array of names; use ID as an
 *index into this	* namestart		numerical value of 1st name in
 *the array					* namecount		# in the
...
void IPXManagerClass::Configure_Debug(int index, int type_offset, int type_size,
  ...
  if (index == -1) {
    GlobalChannel->Queue->Configure_Debug(type_offset, type_size, names,
                                          namestart, namecount);
  } else if (base::At(Connection, index)) {
    base::At(Connection, index)
        ->Queue->Configure_Debug(type_offset, type_size, names, namestart,
                                 namecount);
  }
...
void IPXManagerClass::Mono_Debug_Print(int index, int refresh) {
  ...
  // to exist.
  if (index == -1 || base::At(Connection, index)) {
  ...
  if (index == -1) {
    ...
    Mono_Printf("%d  ",
                base::At(Connection, index)->Queue->Avg_Response_Time());
  }
