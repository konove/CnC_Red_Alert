 ...
 *of the thumb in "slider units".              * SliderClass::Set_Value -- Sets
...
SliderClass::SliderClass(unsigned id, int x, int y, int w, int h,
  ...
  if (!BelongToList) {
    PlusGadget = new ShapeButtonClass(id, MixArchive::RetrieveData("BTN-PLUS.SHP"),
                                      X + Width + 2, Y);
    MinusGadget = new ShapeButtonClass(id, MixArchive::RetrieveData("BTN-MINS.SHP"),
                                       X - 6, Y);
 ...
 *                                                                                             *
 *    This routine is called when a qualifying input event has occurred. This
