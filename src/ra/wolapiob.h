...
#define CHAT_USER_SQUELCHED \
  0x0004  //	Will theoretically be added to the api and implemented.

#define LOBBYPASSWORD \
  "not_a_valid_password"  // Password removed per Security review - LFeenanEA,
                          // 26th February 2025
...
class WolapiObject {
 public:
  ...
  void PrintMessage(const char* szText,
  ...
  HRESULT AttemptLogin(const char* szName, const char* szPass,
                       bool bPassIsMangled);
  void Logout();
  bool UpdateChannels(int iChannelType, CHANNELFILTER ChannelFilter,
                      bool bAutoping);
  void OnChannelList();
  void ListChannels();
  ...
  bool OnEnteringChatChannel(const char* szChannelName, bool bICreatedChannel,
                             int iLobby);
  ...
  void SetOptions(bool bEnableFind, bool bEnablePage, bool bLangFilterOn,
                  bool bShowAllGames);
