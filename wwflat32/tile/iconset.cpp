...
void* cdecl Load_Icon_Set(char const* filename, void* iconsetptr,
                          long buffsize);
...
/*
** This is the control structure at the start of a loaded icon set.  It must
*match
** the structure in WWLIB.I!  This structure MUST be a multiple of 16 bytes
*long.
*/
typedef struct {
  ...
  if (fh != ERROR) {
    ...
    if (id == FORM_ICON) {
      ...
      */
      ...
      bytespericon = ((((int)sinf.Width) << 3) * (((int)sinf.Height) << 3) *
                      (int)sinf.Bitplanes) >>
                     3;
      ...
      /*
      **	Allocate the icon buffer if one isn't provided.  First try EMS
      *and *	then try conventional RAM.
      */
      allocated = FALSE;
      if (!iconsetptr) {
      ...
      if (iconsetptr &&
        ...
        */
        ...
        /*
        **	Limit buffer to only the size needed.  This is done AFTER
        *loading of the *	raw icon data because it might have been
        *compressed and thus need any *	extra space to perform an overlapped
        *decompression.
        */
        ...
        /*
        **	Let the graphic overlay know of the icon data.  This could
        *involve *	translation and other data manipulations.
        */
