...
typedef struct {
  ...
  **	occur.  This is necessary since both interrupt and regular code can
  ...
  */
  ...
  /*
  **	This is the original sample pointer. It is used to control the sample
  *based on *	pointer rather than handle. The handle method is necessary when
  *more than one *	sample could be playing simultaneously. The pointer
  *method is necessary when *	the dealing with a sample that may have stopped
  *behind the programmer's back and *	this occurance is not otherwise
  *determinable.  It is also used in
...
int Decompress_Frame_Lock();
int Decompress_Frame_Unlock();
int sosCODEC_Lock();
int sosCODEC_Unlock();
void __GETDS();
}
