          ...
          if (st->DontTouch) {
            ...
            st->DataLength =
                Sample_Copy(st, &st->Source, &st->Remainder, &st->QueueBuffer,
                            &st->QueueSize, st->Buffer[st->Index],
                            SFX_MINI_STAGE_BUFFER_SIZE, st->Compression,
    ...
    for (index = 0; index < LockedData.MaxSamples; index++) {
      ...
      */
      if (st->Active && st->Reducer && st->Volume) {
        ...
        if (st->IsScore) {
          sosDIGISetSampleVolume(LockedData.DigiHandle, st->Handle,
                                 (st->Volume >> 8) * LockedData.ScoreVolume);
        } else {
          sosDIGISetSampleVolume(LockedData.DigiHandle, st->Handle,
                                 (st->Volume >> 8) * LockedData.SoundVolume);
        }
      }
      st++;
    }
 ...
 * DigiCallback -- Low level double buffering handler. *
 *                                                                                             *
 *    This routine is called in an interrupt to handle the double * buffering of
 *digital audio.  This routine is the interface between                       *
 *    the buffers maintained by Sound_Callback() and the HMI driver * itself. *
 *                                                                                             *
  ...
  if (driverhandle == LockedData.DigiHandle) {
    switch (callsource) {
      ...
      */
      ...
      /*
      **	The sample is finished processing, but not necessarily finished
      *playing.
      */
      case _SAMPLE_PROCESSED:
        if (st->DataLength && st->Active) {
          ...
          if (st->Remainder || st->QueueBuffer || st->Callback ||
              st->FilePending) {
            st->Service = TRUE;
            LockedData.ServiceSomething = TRUE;
          }
 ...
 *memory to					* long size     - how big an
 *area to set								*
 *                                                                         *
 * OUTPUT:		none *
 *                                                                         *
