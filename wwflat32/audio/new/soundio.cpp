 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * File_Callback -- called to fill queue buffer for streaming
 *sample     						  * Stream_Sample_Volume
 *-- generic streaming sample playback init                            *
 ...
 *   File_Stream_Sample_Volume -- Streams a sample directly from a file
 *w/volume.              * Sound_Callback -- Audio driver callback function. *
 *   Load_Sample -- Loads a digitized sample into RAM. * Load_Sample_Into_Buffer
 *-- Loads a digitized sample into a buffer.                        *
 *   Free_Sample -- Frees a previously loaded digitized sample. * Sound_End --
 *Uninitializes the sound driver.                                              *
 *   Stop_Sample -- Stops any currently playing sampled sound. * Sample_Status
 *-- Queries the current playing sample status (if any).                      *
...
/*
**	This define (if present) enables the simple HMI hardware initialization
*process. *	The process does not do auto detection, but rather takes the
*value directly from *	the setup program and uses that as the sound card
*number.  The only "detection" it *	does is to recognized the presence of
*the card and fetch its settings.
*/
 ...
 *- which sample buffer to put info in				* VOID **buffer
 *- the buffer pointer to load data into				* LONG
 **size		- the amount to load
 **
 *                                                                         *
    ...
    if (ptr) {
      ...
      */
      if (st->FilePending < (StreamLowImpact
        ...
        /*
        **	After filling all pending buffers, check to see if the queue
        *buffer *	is empty.  If so, then assign the first available
        *pending buffer to the *	queue.
        */
        st->DontTouch = TRUE;
  ...
  if (st->FilePendingSize < LockedData.StreamBufferSize || index == maxnum) {
    ...
    */
    if (!st->FilePending) {
      ...
      /*
      **	The QueueBuffer counts as an already played block so remove it
      *from the total.
...
int File_Stream_Sample_Vol(char const* filename, int volume,
  ...
  if (LockedData.DigiHandle != -1 && filename && Find_File(filename)) {
    ...
    */
    if ((unsigned)(handle = Get_Free_Sample_Handle(0xFF)) >=
        LockedData.MaxSamples) {
      return (-1);
    }
          ...
          if (st->Reducer && !st->Volume) {
            ...
            */
            if ((!st->QueueBuffer ||
                ...
                st->Callback) {
              if (!st->Callback(index, &st->Odd, &st->QueueBuffer,
                                &st->QueueSize)) {
                st->Callback = NULL;
              }
            }
...
BOOL Sound_Init(int sfx, int score, int sample, RateType rate,
                int bits_per_sample, int max_samples, int reverse_channels) {
  return Audio_Init(sample, -1, -1, -1, rate, bits_per_sample, max_samples,
                    reverse_channels);
}
  ...
  */
  ...
  while (sample) {
    ...
    */
    ...
    error = 0;
    error = sosDIGIInitDriver((int)sample, &DigiHardware, &init,
                              &LockedData.DigiHandle);
    if (error) {
    ...
    */
    DigiBuffer =
        Alloc(2048 + (((LONG)LockedData.MaxSamples * 2) *
                      (LONG)(SFX_MINI_STAGE_BUFFER_SIZE + SONARC_MARGIN)),
              (MemoryFlagType)(MEM_NORMAL | MEM_CLEAR | MEM_LOCK));
    for (index = 0; index < LockedData.MaxSamples; index++) {
      LockedData.SampleTracker[index].Buffer[0] = Add_Long_To_Pointer(
          DigiBuffer, ((LONG)index * 2) *
                          (LONG)(SFX_MINI_STAGE_BUFFER_SIZE + SONARC_MARGIN));
      LockedData.SampleTracker[index].Buffer[1] = Add_Long_To_Pointer(
          DigiBuffer, ((LONG)index * 2 + 1) *
                          (LONG)(SFX_MINI_STAGE_BUFFER_SIZE + SONARC_MARGIN));
  ...
  if (LockedData.DigiHandle != -1 && (unsigned)handle < LockedData.MaxSamples) {
    if (LockedData.SampleTracker[handle].Active ||
      ...
      if (!LockedData.SampleTracker[handle].IsScore) {
        DPMI_Unlock(LockedData.SampleTracker[handle].Original,
                    LockedData.SampleTracker[handle].OriginalSize);
      ...
      */
      if (!LockedData.SampleTracker[handle].Loading) {
        sosDIGIStopSample(LockedData.DigiHandle,
...
BOOL Sample_Status(int handle) {
  ...
  ** that it is.
  */
  return (!sosDIGISampleDone(LockedData.DigiHandle,
                             LockedData.SampleTracker[handle].Handle));
}
...
int Get_Free_Sample_Handle(int priority) {
  ...
  */
  ...
  if (LockedData.SampleTracker[id].Original &&
    ...
    DPMI_Unlock(LockedData.SampleTracker[id].Original,
                LockedData.SampleTracker[id].OriginalSize);
...
int Play_Sample_Handle(void const* sample, int priority, int volume,
  ...
  */
  ...
  if (SFX_MINI_STAGE_BUFFER_SIZE ==
      Sample_Copy(st, &st->Source, &st->Remainder, &st->QueueBuffer,
                  &st->QueueSize, st->Buffer[0], SFX_MINI_STAGE_BUFFER_SIZE,
                  st->Compression, &st->Trailer[0], &st->TrailerLen)) {
    st->DataLength =
        Sample_Copy(st, &st->Source, &st->Remainder, &st->QueueBuffer,
                    &st->QueueSize, st->Buffer[1], SFX_MINI_STAGE_BUFFER_SIZE,
                    st->Compression, &st->Trailer[0], &st->TrailerLen);
    st->Index = 1;
  } else {
    st->Index = 0;
  ...
  */
  if (RawHeader.Rate != LockedData.Rate) {
    ...
    start.dwSamplePitchAdd = (long)MAKE_LONG(
        (short)result.quot,
        (short)(((long)result.rem * 0x10000L) / LockedData.Rate));
 ...
 **
 *                                                                         *
 * OUTPUT:		long   				 - length of the sample
 *in ticks (60/sec) *
 *                                                                         *
 * HISTORY:                                                                *
