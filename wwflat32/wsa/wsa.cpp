...
typedef struct {
  ...
  // EXTRA_BYTES_ANIMATE_NOT_KNOW_ABOUT
  WORD file_handle;
  ULONG anim_mem_size;
} SysAnimHeaderType;
...
// THESE BYTES ON NOW FOR IT TO WORK.
...
VOID* cdecl Open_Animation(BYTE const* file_name, BYTE* user_buffer,
  ...
  if (user_flags & WSA_OPEN_DIRECT) {
    ...
    target_buffer_size =
        (ULONG)file_header.pixel_width * file_header.pixel_height;
  }
  ...
  // largest_frame_size BEFORE SAVING IT TO THE FILE.  THIS MEANS I HAVE TO ADD
  // THESE BYTES ON NOW FOR IT TO WORK.
  delta_buffer_size = (ULONG)file_header.largest_frame_size +
                      EXTRA_BYTES_ANIMATE_NOT_KNOW_ABOUT;
 ...
 * INPUT:		VOID * to the animation that we are processing *
 *                                                                         *
 * OUTPUT:		WORD True if the animation has a set palette.  False if
