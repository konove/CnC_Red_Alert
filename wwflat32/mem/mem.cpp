...
/*******************************************************************************
** A allocated block may have one of three meanings in the Time field.  The
*first
** is the time stamp of the last time it was referenced.  The other two values
** are defined below.  MEM_BLOCK_IN_USE means that it will never be returned as
*the
** oldest since there is no valid time stamp.  LOCKED_BLOCK has the same meaning
*as
...
** WARNING: If these values change to anything else, logic will need to be
*changed
...
/***************************************************************************
 * MEM_IN_USE -- Makes it so a block will never be returned as oldest * By
 *marking a memory block in use, the memory system will never return* it as the
 *oldest memory block.  It still can be moved in the Cleanup * code. *
 *                                                                         *
 * INPUT:   node  -- Pointer to memory block returned from Mem_Find.       *
 *                                                                         *
 * OUTPUT:  none                                                           *
 ...
 *                                                                         *
 * WARNINGS: pointer to node must be one that Mem_Alloc or * Mem_Find returned.
 ***
 *                                                                         *
 * HISTORY:                                                                *
