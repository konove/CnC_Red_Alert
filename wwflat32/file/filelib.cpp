...
/***************************************************************************
 * Read_File_With_Recovery -- read the same file on another directory if an
 *error    * occurs.                                                *
 *                                                                         *
 *                                                                         *
 * INPUT:                                                                  *
...
LONG cdecl Read_File_With_Recovery(WORD handle, VOID* buf, UWORD bytes) {
  ...
  do {
    ...
    // open the same file
    newhandle =
        Open_File(FileHandleTable[handle].Name, FileHandleTable[handle].Mode);
...
#if DEBUGPRINT
  Mono_Print(file_name);
  Mono_Print(":OPENERROR  ");
#endif
  ...
  if ((filedata->Flag & (FILEF_RESIDENT | FILEF_PRELOAD)) && !filedata->Ptr) {
    ...
    */
    if (filesize > Mem_Pool_Size(FileCacheHeap)) {
      ...
      filedata->Flag &=
          ~(FILEF_PRELOAD | FILEF_KEEP | FILEF_RESIDENT | FILEF_FLUSH);
