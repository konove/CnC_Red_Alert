...
int cdecl Open_File(char const* file_name, int mode) {
  ...
  */
  ...
  for (file_handle = 0; file_handle < TABLE_MAX;
       file_handle++, filehandletable++) {
  ...
  */
  ...
  if (filedata->Ptr && index != ERROR) {
    ...
    */
    if (index != ERROR && (filedata->Flag & FILEF_PACKED) && !immediate &&
        (mode & WRITE) == 0) {
      /*
      ...
      */
      ...
      /*
      **	Process the packed file header.  Update the start offset for
      *every *	file ellaborated in the packed file. *	Exception:  When a file
      *is specified in the packed file and an *
      ...
      */
      ...
      if (FileData == FileDataPtr &&
        ...
        Mono_Printf("Processing packed file '%s'\r",
                    FileDataPtr[filedata->Disk].Name);
#endif
        ...
        while (offset) {
          ...
          */
          ...
          if (i != ERROR) {
            cur = &FileDataPtr[i];
            if ((cur->Flag & FILEF_PACKED) &&
                cur->Disk == filehandletable->Index) {
...
long cdecl Read_File(int handle, VOID* buf, unsigned long bytes) {
  ...
  */
  if (fileindex != ERROR) {
    ...
    if (filedata->Size) {
      bytes = MIN((unsigned long)filedata->Size - FileHandleTable[handle].Pos,
                  bytes);
    }
  }
      ...
      while (bytes) {
        ...
        */
        if (Hard_Error_Occured) {
          number = Read_File_With_Recovery(
              handle, buf, (unsigned int)MIN(bytes, IO_CHUNK_SIZE));
          doshandle = FileHandleTable[handle].Handle;
        }
...
#if (DEBUGPRINT)
      Mono_Printf("Resident read '%s' %08lx[%ld].%ld\r", filedata->Name,
                  (long)filedata->Start, FileHandleTable[handle].Pos, bytes);
#endif
  ...
  */
  ...
  if (!doshandle) {
    ...
    */
    ...
    FileHandleTable[handle].Pos =
        MIN(FileHandleTable[handle].Pos, filedata->Size - 1);
  ...
  while (!Find_File(file_name)) {
    ...
    */
    do {
      ...
      // Search for the volume ID so that the disk information get read in
      // again.
