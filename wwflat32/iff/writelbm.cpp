...
/* $Header: g:/library/wwlib32/file/rcs/writelbm.cpp 1.1 1994/04/20 14:38:57
 * scott_bowen Exp $ */
/***************************************************************************
...
PUBLIC BOOL Write_LBM_File(WORD lbmhandle, BufferClass &buff, WORD bitplanes,
  ...
  Write_File(lbmhandle, "FORM????ILBM",
             12L);  // First 12 bytes of all .lbm files
 ...
 *         BYTE * palette -- pointer to paletter information * WORD bitplanes --
 *used to figure out size of palette            *
 *                                                                         *
...
PRIVATE LONG Write_CMAP(WORD lbmhandle, UBYTE *palette, WORD bitplanes) {
  ...
  return (((colors << 1) + colors) +
          8L);  // total size of CMAP 8 = "CMAP" + WORD (size)
}
...
PRIVATE LONG Write_BODY(WORD lbmhandle, BufferClass &buff, WORD bitplanes) {
  ...
  for (line = 0; line < 200; line++) {
    ...
    for (plane = 0; plane < bitplanes; plane++) {
      ...
      bodysize +=
          Write_Row(lbmhandle, buffer);  // write to to the BODY in the LBM
...
This is a definition of a
    DPII.LBM file.Below this definition are differences in DPIIe.LMB files
        .

    Created by : Scott K.Bowen Nov 18,
    1991

            Start with
                .LBM to read definition :
            ...
            CHUNKS
                ...
                BITPLANES
                    ->(word)  // number of bit planes used
    ...
    ? ?
      ?
      ? CHUNKSIZE
                ->(long)  // size of chunk not including header or size.
            CMAP->"CMAP" +
            CHUNKSIZE +
            PALETTE_INFO COMPRESS_METHOD
                ->(byte)  // compression method used
            CONTEXT->WIDTH
            + HEIGHT + POSX + POSY + #_BITPLANES + MASKING + COMPRESS_METHOD
            + PAD + TRANS_COL + XASPECT + YASPECT + PAGEWIDTH +
            PAGEHEIGHT CRNG
                ->  // we do not use
            ...
            FILESIZE
                ->(long)  // size of file minus (sizeof(FORM) + sizeof(FILESIZE)

            GREEN
                ->(byte)  // green color gun value
            ...
            NUM_COLORS
                ->  // number of colors used depending on format

            PAGE_WIDTH
                ->(word)  // width of page
            PAGE_HEIGHT
                ->(word)  // height of page
            PALETTE_INFO->(RED + GREEN + BLUE) @NUM_COLORS PAD
                ->(byte)  // not used.  used as a padding
    ...
    ? ? ? ?->  // other possibilities
               ...
               FILESIZE + "PBM " +
               CHUNKS DPPV
                   ->DPPS  // uses dpps instead of dppv
               CHUNKS->+
               TINY  // add these to old definition
