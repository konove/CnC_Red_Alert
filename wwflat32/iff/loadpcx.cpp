...
GraphicBufferClass* Read_PCX_File(char* name, BYTE* Palette, void* Buff,
                                  long Size) {
  unsigned i, j;
  unsigned rle;
  unsigned color;
 ...
 *                                                                         *
 * INPUT: name is a NULL terminated string of the fromat [xxxx.pcx]        *
 *			 Buff is a pointer to a BufferClass the will hold the
 *pcx file 	* at location pointd by Buffer;
 ** palette is optional, if palette != NULL the the color palette of * the pcx
 *file will be place in the memory block pointed	   * by palette.
 **
 *                                                                         *
 * OUTPUT: on succes a pointer to a GraphicBufferClass cointaining the     *
 *         pcx file, NULL othewise.                                        *
...
GraphicBufferClass* Read_PCX_File(char* name, BufferClass& Buff,
                                  char* palette) {
  return Read_PCX_File(name, palette, (void*)Buff.Get_Buffer(),
                       Buff.Get_Size());
}
