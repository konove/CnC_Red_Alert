 ...
 * VesselClass::VesselClass -- Constructor for vessel class objects. *
 *                                                                                             *
 *    This is the normal constructor for vessel class objects. It will set up a
 *vessel that    * is valid excepting that it won't be placed on the map. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    The destructor will destroy the vessel and ensure that it is properly
 *removed from the   * game engine. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    This routine is used to allocate a block of memory from the vessel heap.
 *If there is     * no more space in the heap, then this routine will return
 *NULL.                           *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This overloaded delete operator will return the vessel's memory back to
 *the pool of      * memory used for vessel allocation. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will return with a reference to the static class data for
 *this vessel.      *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a reference to the class data structure associated with
 *this vessel.  *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *specified.       *
 *                                                                                             *
 *    This routine is used by find path and other movement logic to determine if
 *this          * vessel can enter the cell specified. *
 *                                                                                             *
 * INPUT:   cell  -- The cell to check this vessel against. *
 *                                                                                             *
 * OUTPUT:  Returns with the movement restriction associated with movement into
 *this object.   *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the shape number to use for the ship's body when
 *drawing.             *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Draws the vessel on the tactical display. This routine is called by the
 *map rendering    * process to display the vessel. *
 *                                                                                             *
 * INPUT:   x,y   -- The pixel coordinate to draw this vessel at. *
 *                                                                                             *
 *          window-- The window to base clipping and coordinates upon when
 *drawing.            *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    This routine will fetch the overlap list for this vessel type. It takes
 *into             * consideration any movement the vessel may be doing. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is called once for each vessel object during each main game
 *loop. All       * normal AI processing is handled here. This includes
 *dispatching and maintaining any      * processing that is specific to vessel
 *objects.                                           *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
void VesselClass::AI(void) {
  ...
  /*
  **	HACK ALERT:
  **	If the ship finds itself in a hunt order but it has no weapons, then
  *tell it *	to sail off the map instead.
  */
 ...
 *                                                                                             *
 *    This routine is called when the vessel travels one cell. It handles any
 *processes that   * must occur on a per-cell basis. *
 *                                                                                             *
...
/***********************************************************************************************
 * VesselClass::What_Action -- Determines what action would occur if clicked on
 *object.        *
 *                                                                                             *
...
/***********************************************************************************************
 * VesselClass::Active_Click_With -- Intercepts the active click to see if
...
/***********************************************************************************************
 * VesselClass::Active_Click_With -- Performs specified action on specified
 *cell.              *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is called to apply damage to this vessel. The amount and type
 *of damage     * to apply is passed as parameters. This routine could end up
 *destroying the vessel.       *
 *                                                                                             *
 * INPUT:   damage   -- Reference to the amount of damage to apply to this
 *vessel. The damage  * value will be adjusted so that the actual damage applied
 *will be       * stored into this variable for possible subsequent examination.
 **
 *                                                                                             *
 *          distance -- The distance from the center of the damage to the vessel
 *itself.       *
 *                                                                                             *
 *          warhead  -- The warhead type of damage to apply. *
 *                                                                                             *
 *          source   -- The perpetrator of this damage. Knowing who was
 *responsible allows     * retaliation logic. *
 *                                                                                             *
 *          forced   -- Is this damage forced upon the vessel by some
 *supernatural means?      *
 *                                                                                             *
 * OUTPUT:  Returns with the result of the damage applied. This enumeration
 *indicates the      * general effect of the damage. Examine this return value
 *to see if the vessel       * has been destroyed. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is used to determine if this vessel can fire its weapon at
 *the target       * specified. *
 *                                                                                             *
 * INPUT:   target   -- The target candidate to determine if firing upon is
 *valid.             *
 *                                                                                             *
 *          which    -- Which weapon to use when considering the candidate as a
 *potential      * target. *
 *                                                                                             *
 * OUTPUT:  Returns with the fire error type. This enum indicates if the vessel
 *and fire. If   * it can't fire, then the enum indicates why. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/13/1996 JLB : Created. *
 ...
 **
 *                                                                                             *
 *    This routine is called to determine the coordinate that a fired projectile
 *will          * originate from. *
 *                                                                                             *
 * INPUT:   which -- Which weapon is this query directed at? *
 *                                                                                             *
 * OUTPUT:  Returns with the coordinate where a projectile would appear if it
 *were fired.      *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is used to clear out the vessel heap. It is called whenever a
 *scenario is   * being initialized prior to scenario or saved game loading. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *for the vessel *
 *                                                                                             *
 *    This routine is used by ships to determine what target they should go
 *after.             *
 *                                                                                             *
 * INPUT:   threat   -- The threat type that this ship should go after (as
 *determined by the   * team mission or general self defense principles). *
 *                                                                                             *
...
/***********************************************************************************************
 * VesselClass::Enter_Idle_Mode -- Causes the vessel to enter its default idle
 *mode.           *
 *                                                                                             *
 *    This routine is called when the vessel is finished with what it is doing,
 *but the next   * action is not known. This routine will determine what is the
 *appropriate course of       * action for this vessel and then start it doing
 *that.                                     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This is the handler function for when a vessel receives a radio * message.
 *Typical use of this is when a unit unloads from a lst * class so that clearing
 *of the transport is successful.                                   *
 *                                                                                             *
  ...
  switch (message) {
    ...
    */
    case RADIO_DOCKING:
      ...
      **
      */
      if (Class->Max_Passengers() > 0 && *this == VESSEL_TRANSPORT &&
...
/***********************************************************************************************
 * VesselClass::Desired_Load_Dir -- Determines the best cell and facing for
 *loading.           *
 *                                                                                             *
 ...
 * VesselClass::Assign_Destination -- Assign a destination for this vessel. *
 *                                                                                             *
 *    This routine is called when a destination is to be assigned to this
 *vessel.              *
 *                                                                                             *
 ...
 * VesselClass::Mission_Retreat -- Perform the retreat mission. *
 *                                                                                             *
 *    This will cause the vessel to run away from the battlefield. It searches
 *for an escape   * map edge according to the reinforcement edge specified in
 *the house.                     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Asking this question is part of the recloak process. If the answer is no,
 *then           * recloaking is postponed. This facilitates keeping submarines
 *visible for longer than     * they otherwise would be. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This will read and create all vessels specified in the INI database. This
 *routine is     * called when the scenario starts. *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    This routine is used to add the vessel data (needed for scenario start) to
 *the INI       * database specified. If there was any preexisting vessel data
 *in the database, it will    * be cleared *
 *                                                                                             *
 * INPUT:   ini   -- Reference to the ini database to store the vessel data
 *into.              *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
...
/***********************************************************************************************
 * VesselClass::Start_Driver -- Starts the vessel by reserving the location it
 *is moving to.   *
 *                                                                                             *
 *    This routine is called when the vessel starts moving. It will reserve the
 *destination    * cell so that it won't be occupied by another vessel as this
 *one is travelling.           *
 *                                                                                             *
 ...
 *clicked over the    * cell specified.
 **
 *                                                                                             *
...
ActionType VesselClass::What_Action(CELL cell) const {
  ...
  if (action == ACTION_NOMOVE && Class->PrimaryWeapon != NULL &&
 ...
 **
 *                                                                                             *
 *    Any turret or body rotation for this vessel will be handled by this
 *routine.             *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    This routine will process firing logic for the vessel. It includes
 *searching for targets * and performing any adjustments necessary to bring the
 *target to bear.                    *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *world.          *
 *                                                                                             *
 *    In addition to detecting the edge of world case, this routine will delete
 *the vessel     * if it occurs. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  bool; Was the vessel deleted by this routine? *
 *                                                                                             *
 * WARNINGS:   Be sure to examine the return value and if true, abort any
 *further processing   * for this vessel since it has been deleted. This routine
 *should be called once   * per vessel per game logic loop. *
 *                                                                                             *
 * HISTORY: * 07/29/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    When a vessel repairs, it does so 'by itself' and not under direct control
 *of another    * object. This self repair logic is processed here. Upon repair
 *completion of money        * exhuastion, the repair process will terminate. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    This routine is the auto-fire logic for the ship. It will check * to see
 *if we're an aircraft carrier, and if so, launch one of our * aircraft.  If
 *we're not an aircraft carrier, it lets the higher-level                    *
