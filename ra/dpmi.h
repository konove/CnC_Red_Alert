...
/* $Header:   F:\projects\c&c0\vcs\code\dpmi.h_v   4.43   05 Jul 1996 17:58:40
 * JOE_BOSTIC  $ */
/***********************************************************************************************
...
class DOSSegmentClass {
  /*
  **	This is the selector/segment value. In real mode it is the segment, in
  *protected *	mode it is the selector (also 16 bits). This value is moved into
  *DS or ES when *	accessing memory.
  ...
  *By using *	these, it is possible to create very small code that uses a
  *segment and *	offset without damaging the DS register. These are
  ...
  *in *	low DOS memory. In real mode, this is a simple segment assignment and
  *the size *	is always 64K regardless of what is specified. In protected
  *mode, the segment *	is used to update the selector and the size can be any
  *length.
  ...
  */
  ...
  /*
  **	These routines will move the data to/from regular memory and the
  ...
  */
  ...
  /*
  **	These routines move data around between sections of segmented
  *(descriptor) memory. *	Typically, this is used when accessing DOS
  *memory in protected mode or when dealing *	with hard memory areas such as
  *the screen.
  */
  static void Copy(DOSSegmentClass &src, int soffset, DOSSegmentClass &dest,
                   int doffset, int size);
  static void Swap(DOSSegmentClass &src, int soffset, DOSSegmentClass &dest,
                   int doffset, int size);
};
...
inline void DOSSegmentClass::Copy(DOSSegmentClass &src, int soffset,
                                  DOSSegmentClass &dest, int doffset,
                                  int size) {
  memmove((void *)(dest.Selector + doffset), (void *)(src.Selector + soffset),
          (unsigned)size);
}
