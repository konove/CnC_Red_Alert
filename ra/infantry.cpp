 ...
 *                                                                                             *
 *    This is the constructor used when creating an infantry unit. All values
 *are required     * except for facing and position. If these are absent, then
 *the infantry is created in     * a state of limbo -- not placed upon the map.
 **
 *                                                                                             *
 * INPUT:   see below... *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This is the default destructor for infantry type units. It will put the
 *infantry into    * a limbo state if it isn't already in that state and the
...
/***********************************************************************************************
 * InfantryClass::operator delete -- Returns the infantry object back to the
 *free pool         *
 *                                                                                             *
 ...
 * InfantryClass::Take_Damage -- Applies damage to the infantry unit. *
 *                                                                                             *
 *    This routine applies the damage specified to the infantry object. It is
 *possible that    * this routine will DESTROY the infantry unit in the process.
 **
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   Since the infantry unit could be destroyed by this routine, be
 *sure to check    * for this in the code that follows the call to
 *Take_Damage().                    *
 *                                                                                             *
 * HISTORY: * 09/08/1994 JLB : Created. * 11/22/1994 JLB : Shares base damage
 *handler for techno objects.                           * 03/31/1995 JLB :
 *Revenge factor.                                                          *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    This will determine the shape number to use for this infantry soldier. The
 *shape number  * is relative to the shape file associated with this infantry
 *unit.                        *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the shape number for this infantry object to be used
 *when drawing.    *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * InfantryClass::Per_Cell_Process -- Handles special operations that occur once
 *per cell.     *
 *                                                                                             *
 *    This routine will handle any special operations that need to be performed
 *once each      * cell travelled. This includes radioing a transport that it is
 *now clear and the          * transport is free to leave. *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/08/1994 JLB : Created. * 03/01/1995 JLB : Capture building
...
void InfantryClass::Per_Cell_Process(PCPType why) {
  ...
  if (why == PCP_END) {
    ...
    *trying to *	capture, then capture it.
    */
        ...
        if (*this == INFANTRY_RENOVATOR) {
          ...
          */
          ...
          if (*this == INFANTRY_SPY) {
            ...
            if (*this ==
                INFANTRY_THIEF) {  // Thief just raided a storage facility
              tech->House->IsThieved = true;
    ...
    */
    ...
    /*
    **	If the infantry unit is entering a cell that contains the building it is
    *trying to *	sabotage, then sabotage it.
    */
    ...
    */
    ...
    /*
    **	When the infantry reaches the center of the cell, it may begin a new
    *mission.
    */
    if (MissionQueue == MISSION_NONE && !Target_Legal(NavCom) &&
 ...
 *computer.              *
 *                                                                                             *
 *    This is a support routine that removes the target specified from any
 *targeting or        * navigation computers. When a target is destroyed or
 *removed from the game system,        * the target must be removed from any
 *tracking systems of the other units. This routine    * handles removal for
 *infantry units.                                                      *
 *                                                                                             *
 * INPUT:   target   -- The target to remove from the infantry unit's tracking
 *systems.        *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will force the infantry object system into its empty initial
 *state. It      * is called when the scenario needs to be cleared in
 *preparation for a scenario load.      *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *destination.             *
 *                                                                                             *
 *    This routine updates the infantry's navigation computer so that the
 *infantry will        * travel to the destination target specified. *
 *                                                                                             *
...
void InfantryClass::Assign_Destination(TARGET target) {
  ...
  */
  if (House->IsHuman && Target_Legal(target) && NavCom == target && IsProne &&
 ...
 * InfantryClass::Assign_Target -- Gives the infantry a combat target. *
 *                                                                                             *
 *    This routine will update the infantry's targeting computer so that it will
 *try to        * attack the target specified. This might result in it moving to
 *be within range and thus  * also cause adjustment of the navigation computer.
 **
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/08/1994 JLB : Created. * 06/30/1995 JLB : Tries to capture
 *target if possible.                                     *
 *=============================================================================================*/
void InfantryClass::Assign_Target(TARGET target) {
  ...
  /*
  **	If this is an infantry that can only capture, then also assign its
 ...
 **
 *                                                                                             *
 *    This routine is used to handle the non-graphic AI processing the infantry
 *requires.      * Call this routine ONCE per game frame. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
void InfantryClass::AI(void) {
  ...
  *but the *	stage class isn't set, then abort the firing flag.
  */
 ...
 *cell specified.   *
 *                                                                                             *
 *    This routine is used to examine the cell specified and determine if the
 *infantry is      * allowed to enter it. It is used by the path finding
 *algorithm.                           *
 *                                                                                             *
      ...
      */
      if (House->Is_Ally(obj) || ScenarioInit) {
        switch (obj->What_Am_I()) {
          ...
          *blockage. *	This depends on whether the unit is currently moving or
          *not.
          */
          case RTTI_UNIT:
 ...
 *but doesn't occ*
 *                                                                                             *
 *    This is a rendering support routine that will return a pointer to a list
 *of cell offsets * that specify the cells the infantry unit is currently
 *overlapping (graphic wise) but     * is not considered to occupy. This list is
 *used to update the map display.                *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns a pointer to an offset list for cells that the unit overlaps
 *but doesn't   * occupy. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Determines if the infantry unit can fire on the target. If it can't fire,
 *then the       * reason why is returned. *
 *                                                                                             *
 * INPUT:   target   -- The target to determine if the infantry can fire upon. *
 *                                                                                             *
 * OUTPUT:  Returns the fire error type that indicates if the infantry can fire
 *and if it      * can't, why not. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/01/1994 JLB : Created. * 06/27/1995 JLB : Flame thrower can
 ...
 *routine.       *
 *                                                                                             *
 *    Use this routine when the infantry unit as accomplished its task and needs
 *to find       * something to do. The default behavior is to enter some idle
 *state such as guarding.      *
 *                                                                                             *
 ...
 * InfantryClass::Random_Animate -- Randomly animate the infantry (maybe) *
 *                                                                                             *
 *    This routine is the random animator initiator for infantry units. This
 *routine should    * be called regularly. On occasion, it will cause the
 *infantry to go into an idle          * animation. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/01/1994 JLB : Created. * 12/13/1994 JLB : Does random facing
...
bool InfantryClass::Random_Animate(void) {
  ...
  if (Is_Ready_To_Random_Animate()) {
    ...
    **		"When in darkness or in doubt, run in circles, scream, and
    *shout!"
    */
 ...
 *                                                                                             *
 *    This routine is used when the infantry should scatter to a nearby cell.
 *Scattering       * occurs as an occasional consequence of being fired upon. It
 *is one of the features       * that makes infantry so "charming". *
 *                                                                                             *
 ...
 *should be made.     *
 *                                                                                             *
 *          nokidding-- The scatter should affect the player's infantry even if
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/24/1994 JLB : Created. * 12/12/1994 JLB : Flame thrower
 *infantry always scatter.                                   * 08/02/1996 JLB :
 ...
 **
 *                                                                                             *
 *    This starts the infantry into a choreographed animation sequence. These
 *sequences can    * be as simple as standing up or lying down, but can also be
 *complex, such as dying or     * performing some idle animation. *
 *                                                                                             *
 * INPUT:   todo     -- The choreographed sequence to start. *
 *                                                                                             *
 *          force    -- Force starting this animation even if the current
 *animation is flagged * as uninterruptible. This is necessary for death
 *animations.            *
 *                                                                                             *
...
bool InfantryClass::Do_Action(DoType todo, bool force) {
  ...
  if (todo != Doing &&
    ...
    /*
    **	Kludge to make sure that if infantry is in the dying animation, it isn't
 ...
 * InfantryClass::Stop_Driver -- Stops the infantry from moving any further. *
 *                                                                                             *
 *    This is used to stop the infantry from animating in movement. This
 *function will stop    * the infantry moving and revert it to either a prone or
 *standing.                         *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * InfantryClass::Start_Driver -- Handles giving immediate destination and move
 *orders.        *
 *                                                                                             *
 *    Use this routine to being the infantry moving toward the destination
 *specified. The      * destination is first checked to see if there is a free
 *spot available. Then the infantry * reserves that spot and begins movement
 *toward it.                                        *
 *                                                                                             *
 * INPUT:   headto   -- The coordinate location desired for the infantry to head
 *to.           *
 *                                                                                             *
 * OUTPUT:  bool; Was the infantry successfully started on its journey? Failure
 *may be because * the specified destination could not contain the infantry
 *unit.               *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/21/1994 JLB : Created. * 05/14/1995 JLB : Tries to move to
 *closest spot possible.                                  * 05/15/1995 JLB :
 ...
 *                                                                                             *
 *    This routine will clean up the infantry occupation bits (as necessary) as
 *well as stop   * the infantry movement process when it gets limboed. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Use this routine when the infantry unit wishes to fire a projectile. This
 *routine        * will launch the projectile and perform any other necessary
 *infantry specific operations. *
 *                                                                                             *
 ...
 * InfantryClass::Unlimbo -- Unlimbo infantry unit in legal sub-location. *
 *                                                                                             *
 *    This will attempt to unlimbo the infantry unit at the designated
 *coordinate, but will    * ensure that the coordinate is a legal subposition. *
 *                                                                                             *
...
bool InfantryClass::Unlimbo(COORDINATE coord, DirType facing) {
  ...
  */
  ...
  if (FootClass::Unlimbo(coord, facing)) {
    ...
    *player unless *	it actually appears in a cell mapped by the player.
    */
...
/***********************************************************************************************
 * InfantryClass::Greatest_Threat -- Determines greatest threat (target) for
 *infantry unit.    *
 *                                                                                             *
 *    This routine intercepts the Greatest_Threat request and adds the
 *appropriate target      * types to search for. For regular infantry, this
 *consists of all the ground types. For    * rocket launching infantry, this
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 01/01/1995 JLB : Created. * 09/28/1995 JLB : Engineers try to
 *recapture buildings first.                              *
 *=============================================================================================*/
TARGET InfantryClass::Greatest_Threat(ThreatType threat)  // const
{
  ...
  *threat. All others *	are ignored. If there is a building that needs to be
  *recaptured and it is nearby *	then automatically head toward it to
  *recapture it.
  */
  ...
  */
  ...
  /*
  **	Organic weapon types don't consider anything but infantry to be a
  *threat. Such *	weapon types would be the dog jaw and the medic first
  *aid kit.
  */
  if (Is_Weapon_Equipped() && Class->PrimaryWeapon->WarheadPtr->IsOrganic) {
    threat = threat &
             ~(THREAT_BUILDINGS | THREAT_VEHICLES | THREAT_BOATS | THREAT_AIR);
  }
  ...
  */
  ...
  /*
  **	If this is a bomber type, then allow buildings to be considered a
  *threat.
  */
...
/***********************************************************************************************
 * InfantryClass::Response_Select -- Plays infantry audio response due to being
 *selected.      *
 *                                                                                             *
 *    This routine handles playing an audio response as a result of the player
 *selecting the   * infantry unit. This occurs prior to giving it an order and
 *may not be followed by any    * order at all. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    When the infantry is given the order to move, this routine handles the
 *audio response    * generated by the infantry unit. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *order.            *
 *                                                                                             *
 *    When the player gives an infantry unit the order to attack, this routine
 *handles         * the audio response by that unit. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *check.             *
 *                                                                                             *
 *    This routine checks to see if the infantry unit can capture the specified
 *object rather  * than merely attacking it. If this is the case, then
 *ACTION_CAPTURE will be returned.     *
 *                                                                                             *
 * INPUT:   object   -- The object that the mouse is currently over. *
 *                                                                                             *
 * OUTPUT:  Returns the action that will be performed if the mouse were clicked
 ...
 *infantry soldier.     *
 *                                                                                             *
 *    This routine is called when the player clicks over an object while this
 *infantry soldier * is selected. Capture attempts are prohibited if the
 *infantry cannot capture. The         * command might respond if told to
 *sabotage something.                                     *
 *                                                                                             *
 ...
 * InfantryClass::Full_Name -- Fetches the full name of the infantry unit. *
 *                                                                                             *
 *    This routine will return with the full name (as a text number) for this
 *infantry         * unit. Typically, this is the normal name, but in cases of
 *civilian type survivors from   * a building explosion, it might be a
 *technician instead. In such a case, the special      * technician name number
 *is returned instead.                                              *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *handling.             *
 *                                                                                             *
 *    This routine intercepts the normal attack mission and if an engineer is
 *detected and the * target is a building, then the engineer will be
 *automatically assigned the capture       * mission. In other cases, the normal
 *attack logic will proceed.                           *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 08/07/1995 JLB : Created. * 04/15/1996 BWG : Engineers can only
...
/***********************************************************************************************
 * InfantryClass::What_Action -- Determines what action to perform for the cell
 *specified.     *
 *                                                                                             *
 *    This routine will determine what action to perform if the mouse was
 *clicked on the cell  * specified. This is just a courier function since the
 *lower level classes actually        * perform the work. The need for this
 *routine at this level is due to the existence of     * a similarly named
 *function at this level as well.  C++ namespace rules require this      *
 *    function courier to be in place or an error will result. *
 *                                                                                             *
 * INPUT:   cell  -- The cell that the mouse might be clicked upon. *
 *                                                                                             *
 * OUTPUT:  Returns with the action that would be given to this infantry unit if
 *the mouse     * were clicked at the cell specified. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 * InfantryClass::Class_Of -- Returns the class reference for this object. *
 *                                                                                             *
 *    This routine will return a reference to the infantry type class object
 *that describes    * this infantry's characteristics. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a reference to the InfantryTypeClass object associated
 *with this      * infantry object. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/21/1995 JLB : Created. *
 ...
 *routine.                                                                     *
 *                                                                                             *
 *    INI entry format: * Housename, Typename, Strength, Cellnum,
 *CellSublocation, Missionname,                  * Facingnum, Triggername *
 *                                                                                             *
        ...
        if (infantry != nullptr) {
          ...
          */
          ...
          if (infantry->Unlimbo(coord, dir)) {
            ...
            /*
            **	If the infantry could not be unlimboed, then this is a big
            *error. *	Delete the infantry.
            */
            delete infantry;
          }
        }
 ...
 *                                                                                             *
 * INPUT:   ini   -- Reference to the INI database to store the infantry data
 *to.              *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    Use this routine to handle the fear logic for this infantry. It will
 *slowly increase     * the bravery of the infantry as well as cause it to stand
 *up or lie down as appropriate.  * It will even handle the special fraidy cat
 *logic for civilian infantry.                  *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
  ...
  if (Fear > 0) {
    ...
    */
    if (IsProne) {
      ...
      /*
      **	Drop to the ground if anxious. Don't drop to the ground while
 ...
 * InfantryClass::Edge_Of_World_AI -- Detects when infantry has left the map. *
 *                                                                                             *
 *    This routine will detect when the infantry has left the edge of the world
 ...
 *                                                                                             *
 * WARNINGS:   Be sure the check the return value and if true, abort any further
 *processing    * for this infantry unit. *
 *                                                                                             *
 * HISTORY: * 07/29/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    This will examine the infantry and determine what firing action is
 *required. It will     * search for targets, starting firing animations, and
 *launch bullets as necessary.         *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    Infantry can be in one of many different animation sequences. At the
 *conclusion of each  * sequence, the infantry will quite likely transition to a
 *new animation state. This       * routine handles detecting when that
 *trasition should occur and starting the infantry     * into its new state. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
/***********************************************************************************************
 * InfantryClass::Movement_AI -- This routine handles all infantry movement
 *logic.             *
 *                                                                                             *
 *    It examines the infantry state and determines what movement action should
 *be initiated   * or processed. It handles the actual movement of the infantry
 *as well as any path finding * or infantry startup logic. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
  ...
  if (!IsFiring && !IsFalling && Doing != DO_DOG_MAUL) {
    if (!IsDriving) {
      ...
      */
      ...
      /*
      **	Double check to make sure it doesn't have a movement destination
      ...
      */
              ...
              if (Distance(NavCom) < Rule.CloseEnoughDistance && !IsTethered) {
                ...
                */
                if (TryTryAgain) {
                  ...
                  **	could not be found. In such a case, processing should
                  *stop *	or else the game will bog down with repeated
                  *path failures. *	Only perform the abort of the target is
                  *in a different zone.
                  */
                  if ((!IsZoneCheat ||
      ...
      /*
      **	The infantry knows where it should be headed, so head there.
      *Check *	to see if the infantry is "close enough" to the desired location
      *that *	it should just consider itself to have arrived. In this case,
      *force *	the infantry to the destination location and mark this path step
      **	as complete.
      */
      Mark(MARK_UP);
 ...
 *unit.             *
 *                                                                                             *
 *    The image data for the infantry differs from normal if this is a spy. A
 *spy always       * appears like a minigunner to the non-owning players. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the image data to use for this infantry
 *soldier.         *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *perform an idle  *
 *                                                                                             *
 *    This routine will examine this infantry and determine if it is allowed and
 *ready to      * perform an idle animation. The conditions under which idle
 *animations can be performed   * are restrictive. Hence this routine. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
bool InfantryClass::Is_Ready_To_Random_Animate(void) const {
  ...
  */
  ...
  /*
  **	While the infantry is in the air (such as when paradropping), it won't
  ...
  */
  ...
  /*
  **	When the infantry is walking or otherwise engauged in travel, it won't
  *idle animate.
  */
  if (IsDriving) {
    return (false);
  ...
  /*
  **	When prone, idle animations cannot occur. This is primarily because
  ...
  */
  ...
  /*
  **	Only if the infantry is in guard or ready stance is idle animations
  *allowed. This is *	because the idle animations start and end with these
  *frames.
  */
 ...
 *                                                                                             *
 *    This routine will paradrop this soldier at the location specified. It will
 *cause the     * soldier to hunt if controlled by the computer and to guard if
 *controlledy by the         * human. *
 *                                                                                             *
