 ...
 * resends, since the receive queue may receive a lot more packets than * we
 *send out.  So, re-sends cannot be detected; the application must be	*
 ...
 *								packets until
 *Service_Receive_Queue is called; 		* the parent classes ACK the
 *packet when it's 			* received.
 ** Get_Packet:			extracts the product ID from the header;
 ** doesn't care about reading packets in order			* Send
 *is capable of broadcasting the packet, or sending	* to a specific address
 **
 *																									*
