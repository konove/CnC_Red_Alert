...
int Read_Game_Options(char *name) {
  ...
  UnitBuildPenalty =
      WWGetPrivateProfileInt("Options", "BuildRate", 100, buffer);
  ...
  */
  WWGetPrivateProfileString(
      "Options", "Scenario", "SCM01EA.INI", Session.Options.ScenarioDescription,
    ...
    */
    if (Session.Options.ScenarioIndex == -1) {
      ...
      */
      ...
      // if (Session.Options.ScenarioIndex == -1)
      //	WWDebugString ("RA95I - Session.Options.ScenarioIndex is still
      //-1\n");
    }
  }
  ...
  Session.MaxAhead = WChatMaxAhead =
      WWGetPrivateProfileInt("Timing", "MaxAhead", 9, buffer);
  Session.FrameSendRate = WChatSendRate =
