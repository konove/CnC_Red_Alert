 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * UnitTypeClass::As_Reference -- Fetches a reference to the unit
 *type class specified.      * UnitTypeClass::Create_And_Place -- Creates and
 *places a unit object onto the map.         * UnitTypeClass::Create_One_Of --
 *Creates a unit in limbo.                                  *
 ...
 *objects.      * UnitTypeClass::Prep_For_Add -- Prepares scenario editor to add
 *unit.                      * UnitTypeClass::Read_INI -- Fetch the unit type
 *data from the INI database.                * UnitTypeClass::Turret_Adjust --
 *Turret adjustment routine for MLRS and MSAM units.        *
 *   UnitTypeClass::UnitTypeClass -- Constructor for unit types. *
 *   UnitTypeClass::operator delete -- Return a unit type class object back to
 *the pool.       * UnitTypeClass::operator new -- Allocates an object from the
 *unit type class heap.         *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
*/
const int UnitTypeClass::Harvester_Dump_List[22] = {
 ...
 * UnitTypeClass::UnitTypeClass -- Constructor for unit types. *
 *                                                                                             *
 *    This is the constructor for the unit types. It is used to initialize the
 *unit type class * structure. The unit type class is used to control the
 *behavior of the various types      * of units in the game. This constructor is
 *called for every unique unit type as it        * exists in the array of unit
 *types.                                                       *
 *                                                                                             *
...
/***********************************************************************************************
 * UnitTypeClass::operator new -- Allocates an object from the unit type class
 *heap.           *
 *                                                                                             *
 *    Use this routine to allocate a unit type class object from the special
 *heap that is      * maintained for this purpose. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the newly allocated unit type class
 *object. If there is  * no more room to allocate another unit type class
 ...
 *pool.         *
 *                                                                                             *
 *    This will return a previously allocated unit to the memory pool from
 *whence it came.     *
 *                                                                                             *
 * INPUT:   pointer  -- A Pointer to the unit type class object to return to the
 *memory pool.  *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 * UnitTypeClass::Init_Heap -- Initialize the unit type class heap. *
 *                                                                                             *
 *    This initializes the unit type class heap by pre-allocated all the known
 *unit types.     *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   Only call this once and call it before processing the rules.ini
 *file.           *
 *                                                                                             *
 * HISTORY: * 07/09/1996 JLB : Created. *
 *=============================================================================================*/
void UnitTypeClass::Init_Heap(void) {
  /*
  **	These unit type class objects must be allocated in the exact order that
  *they *	are specified in the UnitType enumeration. This is necessary
 ...
 * UnitTypeClass::Display -- Displays a generic unit shape. *
 *                                                                                             *
 *    This routine displays a generic representation of a unit of this * type.
 *Typically, it is used when adding objects to the game map. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is used to prepare the generic object adder dialog * box so
 *that it will be able to add a unit object. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *objects.        *
 *                                                                                             *
 *    This routine is used to perform the action necessary only once for the
 *unit type class.  * It loads unit shapes and brain files.   This routine
...
/***********************************************************************************************
 * UnitTypeClass::Create_And_Place -- Creates and places a unit object onto the
 *map.           *
 *                                                                                             *
 *    This routine is used by the scenario editor to create and place a unit
 *object of this    * type onto the map. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This function creates a unit of this type and keeps it in limbo. A pointer
 *to the        * created unit object is returned. It is presumed that this
 *object will later be           * unlimboed at the correct time and place. *
 *                                                                                             *
 * INPUT:   house -- Pointer to the house that is to own the unit. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the created unit object. If the unit
 *object              * could not be created, then NULL is returned. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * UnitTypeClass::As_Reference -- Fetches a reference to the unit type class
 *specified.        *
 *                                                                                             *
 *    Use this routine to return a reference to the UnitTypeClass object as
 *indicated by       * the unit type number specified. *
 *                                                                                             *
 * INPUT:   type  -- The unit type number to convert into a UnitTypeClass object
 *reference.    *
 *                                                                                             *
 ...
 * UnitTypeClass::Dimensions -- Determines the unit's pixel dimensions. *
 *                                                                                             *
 *    This routine will fill in the width and height for this unit type. This
 *width and        * height are used to render the selection rectangle and the
 *positioning of the health      * bargraph. *
 *                                                                                             *
 ...
 *unit type.     * Typically, this is the number of passengers allowed, but for
 *harvesters, it is the       * number of credits it holds divided by 100. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * UnitTypeClass::Turret_Adjust -- Turret adjustment routine for MLRS and MSAM
 *units.          *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will find the section in the INI database for this unit type
 *object and     * then fill in the override values specified. *
 *                                                                                             *
 * INPUT:   ini   -- Reference to the INI database that will be examined. *
 *                                                                                             *
 * OUTPUT:  bool; Was the section for this unit found in the database and the
 *data extracted?  *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
bool UnitTypeClass::Read_INI(CCINIClass &ini) {
  if (TechnoTypeClass::Read_INI(ini)) {
    ...
    /*
    **	If this unit can drive over walls, then mark it as recognizing the
    *crusher zone.
    */
