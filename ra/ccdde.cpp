 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Overview: * C&C's interface to the DDE class *
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 *                                                                                             *
 * Functions: * DDE_Callback -- DDE server callback function *
 * DDEServerClass::DDEServerClass -- class constructor * DDEServerClass::Enable
 *-- Enables the DDE callback                                          *
 * DDEServerClass::Disable -- Disables the DDE callback *
 * DDEServerClass::~DDEServerClass -- class destructor * DDESC::Callback --
 *callback function. Called from the DDE_Callback wrapper function         *
 * DDESC::Get_MPlayer_Game_Info -- returns a pointer to the multiplayer setup
 *info from wchat  * DDESC::Delete_MPlayer_Game_Info -- clears out multi player
 *game setup info                  * DDESC::Time_Since_Heartbeat -- returns the
 *time in ticks since the last heartbeat from wchat*
 *                                                                                             *
...
extern void WWDebugString(char const *string);
 ...
 *                                                                                             *
 * INPUT:    ptr to data from client * length of data *
 *                                                                                             *
 * OUTPUT:   Nothing *
 *                                                                                             *
 * WARNINGS: None *
...
/***********************************************************************************************
 * DDESC::Callback -- callback function. Called from the DDE_Callback wrapper
 *function         *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 * INPUT:    data from DDE client * length of data *
 *                                                                                             *
 * OUTPUT:   Nothing *
 *                                                                                             *
...
BOOL DDEServerClass::Callback(unsigned char *data, long length) {
  ...
  */
  if (length < 0) {
    ...
    /*
    ** Packet must be at least the length of the packet type & size fields to be
    *valid
    */
    ...
    */
    switch (packet_type) {
      /*
      ** This is a packet with the info required for starting a new internet
      *game. This is really just C&CSPAWN.INI sent from WChat instead of read
      *from disk.
      */
      case DDE_PACKET_START_MPLAYER_GAME:
        ...
        *(MPlayerGameInfo + actual_length) =
            0;  // Terminator in case we treat it as a string
...
/***********************************************************************************************
 * DDESC::Time_Since_Heartbeat -- returns the time in ticks since the last
 *heartbeat from wchat*
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 * INPUT:    Nothing *
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    ptr to the data to send * length of data * packet type identifier *
 *                                                                                             *
