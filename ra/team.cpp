 ...
 * _Is_It_Breathing -- Checks to see if unit is an active team member. *
 *                                                                                             *
 *    A unit could be a team member, but not be active. Such a case would occur
 *when a         * reinforcement team is inside a transport. It could also occur
 *if a unit is in the        * process of dying. Call this routine to ensure
 *that the specified unit is a will and      * able participant in the team. *
 *                                                                                             *
 * INPUT:   object   -- Pointer to the unit/infantry/aircraft that is to be
 *checked.           *
 *                                                                                             *
 * OUTPUT:  bool; Is the specified unit active and able to be given commands by
 *the team?      *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
static inline bool _Is_It_Breathing(FootClass const *object) {
  ...
  *However, if the *	scenario init flag is on, then it is probably a
  *reinforcement issue or scenario *	creation situation. In such a case, the
 ...
 **
 *                                                                                             *
 *    Use this routine to determine if the specified unit is an active
 *participant of the      * team. When a unit is first recruited to the team, it
 *must travel to the team's location  * before it can become an active player.
 *Call this routine to determine if the specified   * unit can be considered an
 *active player.                                                 *
 *                                                                                             *
 * INPUT:   object   -- Pointer to the object that is to be checked to see if it
 *is an         * active player. *
 *                                                                                             *
 * OUTPUT:  bool; Is the specified unit an active, living, initiated member of
 *the team?       *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
static inline bool _Is_It_Playing(FootClass const *object) {
  ...
  *participants of the *	team. This results in the team members that are
  *racing to regroup with the team (i.e., *	not initiated), will continue to
 ...
 *                                                                                             *
 *    This routine clears out the team object array in preparation for starting
 *a new          * scenario. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the freshly allocated team object. If an
 *allocation      * could not be made, then NULL is returned. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is called when a team object is destroyed. It handles
 *updating the total    * number count for this team object type. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
TeamClass::~TeamClass(void) {
  if (GameActive && Class.Is_Valid()) {
    ...
    /*
    **	When the team dies, any trigger associated with it, dies as well. This
    *will only occur *	if there are no other objects linked to this trigger.
    *Only player reinforcement *	members that broke off of the team
    *earlier will have this occur.
    */
    if (Trigger.Is_Valid()) {
 ...
 *                                                                                             *
 *    General purpose team logic is handled by this routine. It should be called
 *once per      * active team per game tick. This routine handles recruitment
 *and assigning orders to      * member units. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
void TeamClass::AI(void) {
  ...
  */
  ...
  /*
  **	If this team senses that its composition has been altered, then it
  ...
  if (IsAltered) {
    ...
    */
    ...
    if (Total) {
      ...
      /*
      **	Reinforceable teams will revert (or snap out of) the under
      *strength *	mode when the members transition the magic 1/3 strength
      *threshold.
      */
      if (Class->IsReinforcable) {
        ...
        /*
        **	Teams that are not flagged as reinforceable are never considered
        *under *	strength if the team has already started its main
        *mission. This *	ensures that once the team has started, it won't
      ...
      *destroyed *	when there are no team members left. This team was
      ...
      */
      if (IsHasBeen || Session.Type != GAME_NORMAL) {
        ...
        *triggers to see if one that *	depends on this team leaving the map
        *should be sprung.
        */
        if (IsLeaveMap) {
  ...
  */
    ...
    if (Total) {
      ...
      */
      ...
      for (int index = 0; index < Buildings.Count(); index++) {
        ...
        if (b != NULL && !b->IsInLimbo && b->House == House &&
            b->Class->PrimaryWeapon == NULL) {
  ...
  /*
  **	Flag this team into action when it gets to full strength. Human owned
  *teams are only *	used for reinforcement purposes -- always consider them
  *at full strength.
  */
 ...
 *                                                                                             *
 *    Use this routine to add the specified object to the team. The object is
 *checked to make  * sure that it can be assigned to the team. If it can't, then
 *the object will be left      * alone and false will be returned. *
 *                                                                                             *
 * INPUT:   obj      -- Pointer to the object that is to be assigned to this
 *team.             *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/29/1994 JLB : Created. * 01/02/1995 JLB : Initiation flag
...
bool TeamClass::Add(FootClass *obj) {
  ...
  *part of *	another team, then it must be removed from that team first.
  */
...
/***********************************************************************************************
 * TeamClass::Can_Add -- Determines if the specified object can be added to
 *team.              *
 *                                                                                             *
 *    This routine will examine the team and determine if the specified object
 *can be          * properly added to this team. This is a security check to
 *filter out those objects that   * should not be added because of conflicting
 *priorities or other restrictions.             *
 *                                                                                             *
 * INPUT:   obj      -- Pointer to the candidate object that is being checked
 *for legal        * adding to this team. *
 *                                                                                             *
 *          typeindex-- The class index number (according to the team type's
 *class array) that * the candidate object is classified as. The routine
 *processes much      * faster if you can provide this information, but if you
 *don't, the      * routine will figure it out. *
 *                                                                                             *
...
bool TeamClass::Can_Add(FootClass *obj, int &typeindex) const {
  ...
  */
  ...
  /*
  **	The object must be active, a member of this house. A special
  ...
  */
  ...
  /*
  **	Aircraft that have no ammo for their weapons cannot be recruited into a
  *team.
  */
  if (obj->What_Am_I() == RTTI_AIRCRAFT &&
      obj->Techno_Type_Class()->PrimaryWeapon != NULL && !obj->Ammo) {
    return (false);
  }
 ...
 *                                                                                             *
 *    Use this routine to remove an object from a team. Objects removed from the
 *team are      * then available to be recruited by other teams, or even by the
 *same team at a later time. *
 *                                                                                             *
 * INPUT:   obj      -- Pointer to the object that is to be removed from this
 *team.            *
 *                                                                                             *
 *          typeindex-- Optional index of where this object type is specified in
 *the type      * type class. This parameter can be omitted. It only serves to
 *make      * the removal process faster. *
 *                                                                                             *
...
bool TeamClass::Remove(FootClass *obj, int typeindex) {
  ...
  */
  ...
  /*
  **	Detach the common trigger for this team type. Only current and active
  *members of the *	team have that trigger attached. The exception is for
  *player team members that *	get removed from a reinforcement team.
  */
  ...
  /*
  **	If the proper team index was not provided, then find it in the type type
  *class. The *	team type class will not be set if the appropriate type could
  *not be found *	for this object. This indicates that the object was
  *illegally added. Continue to *	process however, since removing this
  ...
  **	ensure that at least one remaining member is still initiated. If not,
  ...
  */
  ...
  /*
  **	Must record that the team composition has changed. At the next
  *opportunity, *	the team members will be counted and appropriate AI
  *adjustments made.
  */
  IsAltered = JustAltered = true;
  return (true);
...
/***********************************************************************************************
 * TeamClass::Recruit -- Attempts to recruit members to the team for the given
 *index ID.       *
 *                                                                                             *
 *    This routine will take the given index ID and scan for available objects
 *of that type    * to recruit to the team. Recruiting will continue until that
 *object type has either       * been exhausted or if the team's requirement for
 *that type has been filled.               *
 *                                                                                             *
 * INPUT:   typeindex   -- The index for the object type to recruit. The index
 *is used to      * look into the type type's array of object types that make up
 *this   * team.                                                               *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/29/1994 JLB : Created. * 04/10/1995 JLB : Scans for units too.
 **
 *=============================================================================================*/
 ...
 * TeamClass::Detach -- Removes specified target from team tracking. *
 *                                                                                             *
 *    When a target object is about to be removed from the game (e.g., it was
 *killed), then    * any team that is looking at that target must abort from
 *that target.                     *
 *                                                                                             *
 * INPUT:   target   -- The target object that is going to be removed from the
 *game.           *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Use this routine to calculate the "center" location of the team. This is
 *the average     * position of all members of the team. Using this center value
 *it is possible to tell      * if a team member is too far away and where to
 *head to in order to group up.              *
 *                                                                                             *
 * INPUT:   center   -- Average center target location of the team. Only
 *initiated members     * will be considered. *
 *                                                                                             *
 *          close_member--Location (as target) of the unit that is closest to
 *the team's       * target. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
  ...
  */
  if (Class->MissionList[CurrentMission].Mission == TMISSION_HOUND_DOG) {
    ...
    */
    ...
    /*
    **	Set the center location as actually the friendly object that is closest.
    *If there *	is no friendly object, then don't set any center location at
    *all.
    */
    if (closest) {
    ...
    *their *	location. Only team members that are active will be considered.
    *Also keep *	track of the team member that is closest to the team's
    *target.
    */
    ...
    /*
    **	If there were any qualifying members, then the team's center point can
    *be *	determined.
    */
    if (quantity) {
      x /= quantity;
      ...
      /*
      **	If the center location is impassable, then just pick the
      *location of
 ...
 **
 *                                                                                             *
 *    This routine is used when a team member takes damage. Usually the team
 *will react in     * some fashion to the attack. This reaction can range from
 *running away to assigning this  * new target as the team's target. *
 *                                                                                             *
      ...
      if (source && !Is_A_Member(source) && Member &&
        ...
        if (Target != source->As_Target()) {
          ...
          */
          ...
          /*
          **	Don't change target to aggressor if the aggressor cannot
          *normally be attacked.
          */
          if (source->What_Am_I() == RTTI_AIRCRAFT ||
              (source->What_Am_I() == RTTI_VESSEL &&
               (Member->What_Am_I() == RTTI_UNIT ||
                Member->What_Am_I() == RTTI_INFANTRY))) {
            return;
          }
 ...
 * TeamClass::Coordinate_Attack -- Handles coordinating a team attack. *
 *                                                                                             *
 *    This function is called when the team knows what it should attack. This
 *routine will     * give the necessary orders to the members of the team. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
void TeamClass::Coordinate_Attack(void) {
  ...
  ** the target is invalid. This only applies to non-aircraft teams. An aircraft
 ...
 * TeamClass::Coordinate_Regroup -- Handles team idling (regrouping). *
 *                                                                                             *
 *    This routine is called when the team must delay at its current location.
 *Team members    * are grouped together by this function. It is called when the
 *team needs to sit and       * wait. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
    ...
    if (_Is_It_Playing(unit)) {
      if (unit->Distance(Zone) > Rule.StrayDistance &&
        ...
        if (!Target_Legal(unit->NavCom)) {
          // TCTCTC
          //				if (!Target_Legal(unit->NavCom) ||
          //::Distance(unit->NavCom, Zone) > Rule.StrayDistance) {
 ...
 * TeamClass::Coordinate_Do -- Handles the team performing specified mission. *
 *                                                                                             *
 *    This will assign the specified mission to the team. If there are team
 *members that are   * too far away from the center of the team, then they will
 *be told to move to the team's   * location. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This only works if the special mission the team members are to
 *perform does not * require extra parameters. The ATTACK and MOVE missions are
 *particularly bad.    *
 *                                                                                             *
 ...
 * TeamClass::Coordinate_Move -- Handles team movement coordination. *
 *                                                                                             *
 *    This routine is called when the team must move to a new location. Movement
 *and grouping  * commands associated with this task are initiated here. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
void TeamClass::Coordinate_Move(void) {
  ...
  if (Target_Legal(Target)) {
    if (!Lagging_Units()) {
      while (unit != NULL) {
        ...
        **	the team should continue NOT qualify as fully reaching the
        *desired *	location.
        */
  ...
  */
  ...
  /*
  **	If all the team members are close enough to the desired destination,
 ...
 *                                                                                             *
 *    This routine will examine the team and find any lagging units. The units
 *are then        * ordered to catch up to the team member that is closest to
 *the team's destination. This   * routine will not do anything unless lagging
 *members are suspected. This fact is          * indicated by setting the
 *IsLagging flag. The flag is set by some outside agent.          *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   Be sure to set IsLagging for the team if a lagging member is
 *suspected.         *
 *                                                                                             *
    ...
    if (_Is_It_Playing(unit)) {
      ...
      */
      if (unit->Distance(ClosestMember) > stray) {
        ...
        if (!Target_Legal(unit->NavCom)) {
          //				if (!Target_Legal(unit->NavCom) ||
          //::Distance(unit->NavCom, ClosestMember) > Rule.StrayDistance) {
          unit->Assign_Mission(MISSION_MOVE);
    ...
    if (_Is_It_Playing(unit)) {
      ...
      */
      ...
      if (unit->Is_Something_Attached() ||
        ...
        */
        if (Map[unit->Center_Coord()].Cell_Building() == NULL &&
        ...
        /*
        **	A loaner transport should vacate the map when all transported
        *objects *	have been offloaded.
        */
        if (unit->IsALoaner) {
          Remove(unit);
 ...
 *                                                                                             *
 *    This routine tells all non-transport units in the team to climb onto the
 *transport in the* team.  Note the transport must be a member of this team. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
int TeamClass::TMission_Load(void) {
  ...
  */
  ...
  /*
  **	In the case of no transport available, then consider the mission
  *complete *	since it can never complete otherwise.
  */
  if (trans == NULL) {
    IsNextMission = true;
    return (1);
  ...
  **	it is in the process of loading. During this time, don't bother to
 ...
 * TeamClass::Coordinate_Conscript -- Gives orders to new recruit. *
 *                                                                                             *
 *    This routine will give the movement orders to the conscript so that it
 *will group        * with the other members of the team. *
 *                                                                                             *
 * INPUT:   unit  -- Pointer to the conscript unit. *
 *                                                                                             *
 * OUTPUT:  bool; Is the unit still scurrying to reach the team's current
 *location?            *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    This routine is used to see if the team is leaving the map. A team that is
 *leaving the   * map gives implicit permission for its members to leave the
 *map.                          *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * TeamClass::Has_Entered_Map -- Determines if the entire team has entered the
 *map.            *
 *                                                                                             *
 *    This will examine all team members and only if all of them have entered
 *the map, will    * it return true. This routine is used to recognize the case
 *of a team that has been       * generated off map and one that has already
 *entered game play. This knowledge can lead    * to more intelligent behavior
 *regarding team and member disposition.                      *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *range.          *
 *                                                                                             *
 *    This routine is used when one of the team members cannot get within range
 *of the team's  * target. In such a case, the team must be assigned a new
 *target and all members of that   * team must recognize that a restricted
 *target scan is required. This is done by clearing  * out the team's target so
 *that it will be forced to search for a new one. Also, since the * members are
 *flagged for short scanning, whichever team member is picked to scan for a    *
 ...
 * TeamClass::TMission_Formation -- Process team formation change command. *
 *                                                                                             *
 *    This routine will change the team's formation to that specified in the
 *team command      * parameter. It is presumed that the team will have further
 *movement orders so that the    * formation can serve some purpose. Merely
 *changing the formation doesn't alter the        * member's location. The team
 *must be given a movement order before team member            * repositioning
 *will occur.                                                                *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the time to delay before further team actions should
 *occur.           *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This will tell the team to attack the quarry specified in the team
 *command. If the team  * already has a target, this it is presumed that this
 *target take precidence and it won't  * be changed. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the delay before the next team logic operation should
 *occur.          *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 * TeamClass::TMission_Spy -- Perform the team spy mission. *
 *                                                                                             *
 *    This will give the team a spy mission to the location specified. It is
 *presumed that     * the location of the team mission actually resides under
 *the building to be spied. If     * no building exists at the location, then
 *the spy operation is presumed to be a mere      * move operation. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the delay before the next team logic operation should
 ...
 * TeamClass::TMission_Follow -- Perform the "follow friendlies" team command. *
 *                                                                                             *
 *    This will cause the team members to search out and follow the nearest
 *friendly object.   *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * TeamClass::TMission_Loop -- Causes the team mission processor to jump to new
 *location.      *
 *                                                                                             *
 *    This is equivalent to a jump or goto command. It will alter the team
 *command processing  * such that it will continue processing at the command
 *number specified.                   *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * TeamClass::TMission_Invulnerable -- Makes the entire team invulnerable for a
 *period of time *
 *                                                                                             *
 *    This is a team mission that simulates the Iron Curtain device. It will
 *make all team     * members invlunerable for a temporary period of time. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the time delay before the next team logic operation
 *should occur.     *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is used by the team to set a global variable but otherwise
 *perform no       * visible effect on the team. By using this routine,
 *sophisticated trigger dependencies    * can be implemented. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the delay before the next team logic operation should
 ...
 **
 *                                                                                             *
 *    A patrolling team will move to the designated waypoint, but along the way
 *it will        * periodically scan for nearby enemies. If an enemy is found,
 *the patrol mission turns     * into an attack mission until the target is
 *destroyed -- after which it resumes its       * patrol duties. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the delay before the next call to this routine is
 *needed.             *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
int TeamClass::TMission_Patrol(void) {
  ...
  */
  ...
  /*
  **	If the mission target looks like it should be attacked, then do so,
  *otherwise *	treat it as a movement destination.
  */
 ...
 **
 *                                                                                             *
 *    This will scan through the team members looking for one that is suitable
 *as a leader     * type. A team can sometimes contain limboed or unarmed
 *members. These members are not     * suitable for leadership roles. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
FootClass *TeamClass::Fetch_A_Leader(void) const {
  ...
  */
  ...
  /*
  **	If no suitable leader was found, then just return with the first
  *conveniently *	accessable team member. This presumes that some member
  *is better than no member *	at all.
  */
  if (leader == NULL) {
    leader = Member;
  }
