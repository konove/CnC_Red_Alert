 ...
 *objects.                 * AircraftTypeClass::As_Reference -- Given an
 *aircraft type, find the matching type object. *
 *   AircraftTypeClass::Create_And_Place -- Creates and places aircraft using
 *normal game syste* AircraftTypeClass::Create_One_Of -- Creates an aircraft
 *object of the appropriate type.   * AircraftTypeClass::Dimensions -- Fetches
 *the graphic dimensions of the aircraft type.     * AircraftTypeClass::Display
 *-- Displays a generic version of the aircraft type.            *
 *   AircraftTypeClass::From_Name -- Converts an ASCII name into an aircraft
 *type number.      * AircraftTypeClass::Init_Heap -- Initialize the aircraft
 *type class heap.                  * AircraftTypeClass::Max_Pips -- Fetches the
 *maximum number of pips allowed.                *
 ...
 *Determines the overlap list for a landed aircraft.     *
 *   AircraftTypeClass::Prep_For_Add -- Prepares the scenario editor for adding
 *an aircraft obj* AircraftTypeClass::operator delete -- Returns aircraft type
 *to special memory pool.       * AircraftTypeClass::operator new -- Allocates
 *an aircraft type object from special pool.   *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/***********************************************************************************************
 * AircraftTypeClass::operator new -- Allocates an aircraft type object from
 *special pool.     *
 *                                                                                             *
 *    This will allocate an aircraft type class object from the memory pool of
 *that purpose.   *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * AircraftTypeClass::operator delete -- Returns aircraft type to special memory
 *pool.         *
 *                                                                                             *
 *    This will return the aircraft type class object back to the special memory
 ...
 *                                                                                             *
 *    This will initialize the aircraft type class heap by pre-allocating all
 *known aircraft   * types. It should be called once and before the rules.ini
 *file is processed.              *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * AircraftTypeClass::As_Reference -- Given an aircraft type, find the matching
 *type object.   *
 *                                                                                             *
 *    This routine is used to fetch a reference to the aircraft type class
 *object that matches * the aircraft type specified. *
 *                                                                                             *
 * INPUT:   aircraft -- The aircraft type to fetch a reference to the type class
 *object of.    *
 *                                                                                             *
 * OUTPUT:  Returns with a reference to the type class object of this aircraft
 *type.           *
 *                                                                                             *
 * WARNINGS:   Be sure that the aircraft type specified is legal. Illegal values
