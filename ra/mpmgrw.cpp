...
FARPROC MPGetProcAddress(HMODULE hModule, LPCSTR lpProcName) {
  ...
  if (!ret) {
    char msg[255];
    sprintf(msg, "Unable to load function %s from %s", lpProcName,
            "RA95MP.DLL");
...
MPlayerManClass::MPlayerManClass(void) : ConnManClass() {
  ...
  if (lib != 0) {
    MPlayerManCreate =
        (MPlayerInit_Type)MPGetProcAddress(lib, "MPlayerManCreate");
    MPlayerManDestroy =
        (MPlayerDestroy_Type)MPGetProcAddress(lib, "MPlayerManDestroy");
    Send_Private_Message_DLL = (Send_Private_Message_Type)MPGetProcAddress(
        lib, "Send_Private_Message");
    Get_Private_Message_DLL =
        (Get_Private_Message_Type)MPGetProcAddress(lib, "Get_Private_Message");
    Send_Global_Message_DLL =
        (Send_Global_Message_Type)MPGetProcAddress(lib, "Send_Global_Message");
    Get_Global_Message_DLL =
        (Get_Global_Message_Type)MPGetProcAddress(lib, "Get_Global_Message");
    Create_Connection_DLL =
        (Create_Connection_Type)MPGetProcAddress(lib, "Create_Connection");
    Delete_Connection_DLL =
        (Delete_Connection_Type)MPGetProcAddress(lib, "Delete_Connection");
    Connection_Name_DLL =
        (Connection_Name_Type)MPGetProcAddress(lib, "Connection_Name");
    Connection_Address_DLL =
        (Connection_Address_Type)MPGetProcAddress(lib, "Connection_Address");
    Num_Connections_DLL =
        (Num_Connections_Type)MPGetProcAddress(lib, "Num_Connections");
    Connection_ID_DLL =
        (Connection_ID_Type)MPGetProcAddress(lib, "Connection_ID");
    Connection_Index_DLL =
        (Connection_Index_Type)MPGetProcAddress(lib, "Connection_Index");
    Init_DLL = (Init_Type)MPGetProcAddress(lib, "Init");
    Find_Num_Connections_DLL = (Find_Num_Connections_Type)MPGetProcAddress(
        lib, "Find_Num_Connections");
  } else {
...
void MPlayerManClass::Set_Timing(unsigned long /*retrydelta*/,
