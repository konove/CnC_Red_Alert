...
bool IPXInterfaceClass::Get_Network_Card_Address(int card_number,
  ...
  ** The network number will be needed later for broadcasts as the net number
  *ff,ff,ff,ff
  ** doesn't work under NT.
  **
  ** Note: Due to a bug in Win95s implementation of Winsock, only the netnum &
  *nodenum
  ** values are correctly returned. NT returns all expected values. ST - 7/31/97
  *0:57AM
  */
...
bool IPXInterfaceClass::Open_Socket(SOCKET socketnum) {
  ...
  */
  ...
  /*
  ** Get the network card address. This is needed so we can bind the socket to
  *the net card.
  */
  ...
  */
  ...
  /*
  ** The SO_BROADCAST option allows broadcasting on this socket. This shouldn't
  *be needed
  ** except for the bug in the Win95 implementation of Winsock which causes
  *broadcasts to
  ...
  */
  if (setsockopt(Socket, SOL_SOCKET, SO_BROADCAST, (char *)&optval,
    ...
    sprintf(
        out,
        "TS: Failed to set IPX socket option SO_BROADCAST - error code %d.\n",
        GetLastError());
    OutputDebugString(out);
    assert(false);
  }
  ...
  */
  ...
  if (err == INVALID_SOCKET) {
    ...
    sprintf(
        out,
        "TS: Failed to set IPX protocol option IPX_PTYPE - error code %d.\n",
        GetLastError());
    OutputDebugString(out);
  ...
  */
  ...
  if (err == INVALID_SOCKET) {
    char out[128];
    sprintf(out,
            "TS: Failed to set IPX protocol option IPX_FILTERTYPE - error code "
            "%d.\n",
            GetLastError());
    OutputDebugString(out);
    assert(err != INVALID_SOCKET);
