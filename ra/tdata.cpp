 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * TerrainTypeClass::As_Reference -- Fetches a reference to the
 *terrain type object specified* TerrainTypeClass::Create_And_Place -- Creates
 *and places terrain object on map.           * TerrainTypeClass::Create_On_Of
 *-- Creates a terrain object from type.                     *
 ...
 *   TerrainTypeClass::Occupy_List -- Returns with the occupy list for the
 *terrain object type.* TerrainTypeClass::One_Time -- Performs any special one
 *time processing for terrain types. * TerrainTypeClass::Overlap_List -- Fetches
 *the overlap list for the terrain type.          *
 *   TerrainTypeClass::Prep_For_Add -- Prepares to add terrain object. *
 *   TerrainTypeClass::TerrainTypeClass -- The general constructor for the
 *terrain type objects* TerrainTypeClass::operator delete -- Returns a terrain
 *type object back to the mem pool.  * TerrainTypeClass::operator new --
 *Allocates a terrain type object from special pool.      *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
static short const _List1111[] = {0, 1, MAP_CELL_W, MAP_CELL_W + 1,
                                  REFRESH_EOL};
...
static TerrainTypeClass const Mine(
    TERRAIN_MINE, THEATERF_TEMPERATE | THEATERF_SNOW,
...
static TerrainTypeClass const Tree1Class(
    ...
    false,              // Is based on the water?
    "T01", TXT_TREE, (short const *)_List0010, (short const *)_List1001);

static TerrainTypeClass const Tree2Class(
    TERRAIN_TREE2, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(11, 44),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T02", TXT_TREE, (short const *)_List0010, (short const *)_List1001);

static TerrainTypeClass const Tree3Class(
    TERRAIN_TREE3, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(12, 45),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T03", TXT_TREE, (short const *)_List0010, (short const *)_List1001);

static TerrainTypeClass const Tree5Class(
    TERRAIN_TREE5, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(15, 41),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T05", TXT_TREE, (short const *)_List0010, (short const *)_List1001);

static TerrainTypeClass const Tree6Class(
    TERRAIN_TREE6, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(16, 37),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T06", TXT_TREE, (short const *)_List0010, (short const *)_List1001);

static TerrainTypeClass const Tree7Class(
    TERRAIN_TREE7, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(15, 41),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T07", TXT_TREE, (short const *)_List0010, (short const *)_List1001);

static TerrainTypeClass const Tree8Class(
    TERRAIN_TREE8, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(14, 22),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T08", TXT_TREE, (short const *)_List10, (short const *)_List01);

static TerrainTypeClass const Tree10Class(
    TERRAIN_TREE10, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(25, 43),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T10", TXT_TREE, (short const *)_List0011, (short const *)_List1100);

static TerrainTypeClass const Tree11Class(
    TERRAIN_TREE11, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(23, 44),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T11", TXT_TREE, (short const *)_List0011, (short const *)_List1100);

static TerrainTypeClass const Tree12Class(
    TERRAIN_TREE12, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(14, 36),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T12", TXT_TREE, (short const *)_List0010, (short const *)_List1001);

static TerrainTypeClass const Tree13Class(
    TERRAIN_TREE13, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(19, 40),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T13", TXT_TREE, (short const *)_List0010, (short const *)_List1101);

static TerrainTypeClass const Tree14Class(
    TERRAIN_TREE14, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(19, 40),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T14", TXT_TREE, (short const *)_List0011, (short const *)_List1100);

static TerrainTypeClass const Tree15Class(
    TERRAIN_TREE15, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(19, 40),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T15", TXT_TREE, (short const *)_List0011, (short const *)_List1100);

static TerrainTypeClass const Tree16Class(
    TERRAIN_TREE16, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(13, 36),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T16", TXT_TREE, (short const *)_List0010, (short const *)_List1001);

static TerrainTypeClass const Tree17Class(
    TERRAIN_TREE17, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(18, 44),  // Center base coordinate offset.
    false,              // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "T17", TXT_TREE, (short const *)_List0010, (short const *)_List1001);

static TerrainTypeClass const Clump1Class(
    TERRAIN_CLUMP1, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(28, 41),  // Center base coordinate offset.
    true,               // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "TC01", TXT_TREE, (short const *)_List000110, (short const *)_List110001);

static TerrainTypeClass const Clump2Class(
    TERRAIN_CLUMP2, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(38, 41),  // Center base coordinate offset.
    true,               // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "TC02", TXT_TREE, (short const *)_List010110, (short const *)_List101001);

static TerrainTypeClass const Clump3Class(
    TERRAIN_CLUMP3, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(33, 35),  // Center base coordinate offset.
    true,               // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "TC03", TXT_TREE, (short const *)_List110110, (short const *)_List001);

static TerrainTypeClass const Clump4Class(
    TERRAIN_CLUMP4, THEATERF_TEMPERATE | THEATERF_SNOW,
    XYP_COORD(44, 49),  // Center base coordinate offset.
    true,               // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "TC04", TXT_TREE, (short const *)_List000011101000,
...
static TerrainTypeClass const Clump5Class(
    ...
    true,               // Is it immune to normal combat damage?
    false,              // Is based on the water?
    "TC05", TXT_TREE, (short const *)_List001011100110,
 ...
 *type objects. *
 *                                                                                             *
 *    This is the constructor for terrain type objects. It is only used to
 *construct the       * static (constant) terrain type objects. *
 *                                                                                             *
 * INPUT:   see below.. *
 *                                                                                             *
 * OUTPUT:  none *
...
/***********************************************************************************************
 * TerrainTypeClass::operator new -- Allocates a terrain type object from
 *special pool.        *
 *                                                                                             *
 *    This routine will allocated a terrain type class object from the memory
 *pool set up      * for that purpose. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the freshly allocated terrain type object
 *block. If      * there was insufficient memory, then NULL is returned. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * TerrainTypeClass::operator delete -- Returns a terrain type object back to
 *the mem pool.    *
 *                                                                                             *
 *    This routine will return the supplied terrain type object back to the
 *special memory     * pool for whence it was originally allocated. *
 *                                                                                             *
 * INPUT:   pointer  -- Pointer to the terrain type object to return to the
 *memory pool.       *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 * TerrainTypeClass::Init_Heap -- Initialize the terrain object heap. *
 *                                                                                             *
 *    This routine preallocates the terrain type objects in the memory pool. It
 *must be called * before the terrain type object data can be filled in. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This must be called only once and before the rules.ini file is
 *processed.       *
 *                                                                                             *
 * HISTORY: * 07/19/1996 JLB : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * TerrainTypeClass::One_Time -- Performs any special one time processing for
 *terrain types.   *
 *                                                                                             *
 *    This routine will perform any special one time processing needed for the
 *terrain         * object types. Typically, this would load up artwork for
 *terrain objects that have        * artwork independant of the theater they
 *appear in.                                       *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * TerrainTypeClass::Init -- Loads terrain object shape files. *
 *                                                                                             *
 *    This routine is used to load up the terrain object shape files. * The
 *shape files loaded depends on theater. *
 *                                                                                             *
 ...
 * TerrainTypeClass::From_Name -- Convert name to terrain type. *
 *                                                                                             *
 *    This routine is used to convert a text name into the matching * terrain
 *type number. This is used during scenario initialization. *
 *                                                                                             *
 * INPUT:   name  -- The name to convert. *
 *                                                                                             *
 * OUTPUT:  Returns the TerrainType that matches the name specified. If * no
 *match was found, then TERRAIN_NONE is returned. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/16/1994 JLB : Created. *
 ...
 * TerrainTypeClass::Display -- Display a generic terrain object. *
 *                                                                                             *
 *    This routine is used to display a generic terrain object. Typical * use is
 *during scenario editing. *
 *                                                                                             *
 ...
 * TerrainTypeClass::Prep_For_Add -- Prepares to add terrain object. *
 *                                                                                             *
 *    Submits all of the valid terrain objects to the scenario editor for
 *possible selection   * and subsequent placement on the map. All terrain
 *objects, that have a valid shape        * file available, are added. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *map.             *
 *                                                                                             *
 *    This support routine is used by the scenario editor to add a terrain
 *object to the map.  *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This is used to create a terrain object by using the terrain type as a
 *guide. This       * routine is typically used by the scenario editor in order
 *to place a terrain object      * onto the map. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the created terrain object or NULL if one
 *couldn't be    * created. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * TerrainTypeClass::Occupy_List -- Returns with the occupy list for the terrain
 *object type.  *
 *                                                                                             *
 *    This routine will return with the occupy list for the terrain object type.
 *If there is   * no occupy list for this terrain object type, then a special
 *zero length occupy list      * pointer is returned. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the terrain object's occupy list. A zero
 *length list is  * returned in the case of no occupy list. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * TerrainTypeClass::Overlap_List -- Fetches the overlap list for the terrain
 *type.            *
 *                                                                                             *
 *    This routine will return with the overlap list for the terrain object
 *type. If there     * is no overlap list for the terrain object, then a null
 *length list is returned in order  * to ensure that a non-null pointer is
 *returned.                                           *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the overlap list for this object type. A
 *special zero    * length list pointer is returned if there is no overlap list
...
/***********************************************************************************************
 * TerrainTypeClass::As_Reference -- Fetches a reference to the terrain type
 *object specified. *
 *                                                                                             *
 *    Use this routine to convert the terrain type number into a reference to a
 *terrain        * type class object. *
 *                                                                                             *
 * INPUT:   type  -- The terrain type number to convert. *
 *                                                                                             *
 * OUTPUT:  Returns with a reference to the terrain type class object that is
 *referred to by   * the terrain type number. *
 *                                                                                             *
 * WARNINGS:   Be sure that the terrain type number is valid. Using an invalid
