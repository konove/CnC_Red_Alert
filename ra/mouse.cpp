 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MouseClass::AI -- Process player input as it relates to the
 *mouse                         * MouseClass::Init_Clear -- Sets the mouse
 *system to a known state                          * MouseClass::MouseClass --
 *Default constructor for the mouse handler class.                *
 ...
 *system.         * MouseClass::Override_Mouse_Shape -- Alters the shape of the
 *mouse.                        * MouseClass::Revert_Mouse_Shape -- Reverts the
 *mouse shape to the non overridden shape.    * MouseClass::Set_Default_Mouse --
 *Sets the mouse to match the shape specified.             *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 *    This is the default constructor for the mouse handling class. It merely
 *sets up the      * mouse system to its default state. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 **
 *                                                                                             *
 *    This routine is used to inform the display system as to which mouse shape
 *is desired.    *
 *                                                                                             *
...
/***********************************************************************************************
 * MouseClass::Revert_Mouse_Shape -- Reverts the mouse shape to the non
 *overridden shape.      *
 *                                                                                             *
 *    Use this routine to cancel the effects of Override_Mouse_Shape(). It will
 *revert the     * mouse back to the original shape. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * MouseClass::Mouse_Small -- Controls the sizing of the mouse. *
 *                                                                                             *
 *    This routine is called to change the mouse sizing override. If the mouse
 *can change      * size to that specified, then the mouse imagery will be
 *changed. If a change of imagery   * cannot occur (due to lack of appropriate
 *artwork), then no action will be performed.     *
 *                                                                                             *
 * INPUT:   small -- Should the mouse be made small? If not, then it will be
 *made large.       *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine is used to alter the shape of the mouse as needed. * Typical
 *mouse shape change occurs when scrolling the map or * selecting targets. *
 *                                                                                             *
 * INPUT:   mouse -- The mouse shape number to use. *
 *                                                                                             *
 * OUTPUT:  bool; Was the mouse shape changed? *
 *                                                                                             *
 * WARNINGS:   This is not intended to be used as a means to hide the * mouse.
 *Nor will it work correctly if the mouse shape                            *
 *             file hasn't been loaded. *
 *                                                                                             *
 * HISTORY: * 03/10/1994 JLB : Created. * 06/03/1994 JLB : Made into member
...
bool MouseClass::Override_Mouse_Shape(MouseType mouse, bool wsmall) {
  ...
  *mouse *	shape is not one of these, then force the small size override
  *flag to false.
  */
  if (control->SmallFrame == -1) {
    wsmall = false;
 ...
 *                                                                                             *
 *    This routine will is to be called once per game tick and is passed the
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/24/1994 JLB : Created. * 12/31/1994 JLB : Uses mouse coordinate
 *parameters.                                        * 03/27/1995 JLB : New
 *animation control.                                                   *
 ...
 *system.           *
 *                                                                                             *
 *    Use this routine to load the mouse data file and perform any other
 *necessary one time    * preparations for the game. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    This routine will reset the mouse handling system. Typically, this routine
 *is called     * when preparing for the beginning of a new scenario. *
 *                                                                                             *
