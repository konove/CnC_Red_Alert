...
HPALETTE FAR CreateDIBPalette(HDIB hDIB) {
  ...
  if (wNumColors) {
    ...
    hLogPal = GlobalAlloc(
        GHND, sizeof(LOGPALETTE) + sizeof(PALETTEENTRY) * wNumColors);
...
HPALETTE FAR GetSystemPalette(void) {
  ...
  hLogPal =
      GlobalAlloc(GHND, sizeof(LOGPALETTE) + nColors * sizeof(PALETTEENTRY));
...
HANDLE AllocRoomForDIB(BITMAPINFOHEADER bi, HBITMAP hBitmap) {
  ...
  *lpbi = bi;
  ...
  if (lpbi->biSizeImage == 0)
    lpbi->biSizeImage =
        WIDTHBYTES((DWORD)lpbi->biWidth * lpbi->biBitCount) * lpbi->biHeight;
...
HDIB FAR ChangeDIBFormat(HDIB hDIB, WORD wBitCount, DWORD dwCompression) {
  ...
  if (wBitCount == 0) {
    ...
    if ((dwCompression == BI_RLE4 && NewBPP == 4) ||
        (dwCompression == BI_RLE8 && NewBPP == 8) || (dwCompression == BI_RGB))
  ...
  if (hNewDIB) /* Unlock the new DIB's memory block */
    GlobalUnlock(hNewDIB);
...
HDIB FAR ChangeBitmapFormat(HBITMAP hBitmap, WORD wBitCount,
                            DWORD dwCompression, HPALETTE hPal) {
  HDC hDC;                  // Screen DC
  HDIB hNewDIB = NULL;      // Handle to new DIB
  BITMAP Bitmap;            // BITMAP data structure
  BITMAPINFOHEADER bi;      // Bitmap info. header
