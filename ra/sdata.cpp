 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * SmudgeTypeClass::As_Reference -- Fetches a reference to the
 *smudge type specified.        * SmudgeTypeClass::Create_And_Place -- Creates
 *and places on map, a smudge object.          * SmudgeTypeClass::Create_One_Of
 *-- Creates a smudge object of this type.                   *
 *   SmudgeTypeClass::Display -- Draws a generic version of this smudge type. *
 *   SmudgeTypeClass::Draw_It -- Renders the smudge image at the coordinate
 *specified.         * SmudgeTypeClass::From_Name -- Converts an ASCII name into
 *a smudge type.                  * SmudgeTypeClass::Init -- Performs theater
 *specific initializations.                       * SmudgeTypeClass::Init_Heap
 *-- Initialize the smudge type class object heap.               *
 *   SmudgeTypeClass::One_Time -- Performs one-time initialization *
 *   SmudgeTypeClass::Prep_For_Add -- Prepares the scenario editor for adding a
 *smudge object. * SmudgeTypeClass::SmudgeTypeClass -- Constructor for smudge
 *type objects.                  * SmudgeTypeClass::operator delete -- Returns a
 *smudge type class object to the pool.       * SmudgeTypeClass::operator new --
 *Allocate a smudge type object from the memory pool.      *
 *   SmudgetypeClass::Occupy_List -- Determines occupation list for smudge
 *object.             *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
#include <cstddef>
...
static SmudgeTypeClass const Bibx3(SMUDGE_BIB3, "BIB3", TXT_BIB, 2,
                                   2,  // Width and height of smudge (in icons).
                                   true,  // Is this a building bib?
                                   false  // Is this a crater smudge?
);
 ...
 *                                                                                             *
 *    This constructor is used to create the smudge type objects. These type
 *objects contain   * static information about the various smudge types
...
/***********************************************************************************************
 * SmudgeTypeClass::operator new -- Allocate a smudge type object from the
 *memory pool.        *
 *                                                                                             *
 *    This will allocate a smudge type class object from the special memory pool
 *for that      * purpose. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the newly allocated smudge type class object. If there
 *is insufficient* memory in the pool to fulfill the request, then NULL is
 *returned.                  *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/***********************************************************************************************
 * SmudgeTypeClass::operator delete -- Returns a smudge type class object to the
 *pool.         *
 *                                                                                             *
 *    This will return the smudge type class object back to the memory pool from
 *whence it     * was originally allocated. *
 *                                                                                             *
 * INPUT:   pointer  -- Pointer to the smudge type class object to return the
 *pool.            *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This will initialize the special heap for smudge type class objects, by
 *pre-allocated    * all known smudge types. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   Call this routine only once and before the rules.ini file is
 *processed.         *
 *                                                                                             *
 * HISTORY: * 07/09/1996 JLB : Created. *
 *=============================================================================================*/
void SmudgeTypeClass::Init_Heap(void) {
  /*
  **	These smudge type class objects must be allocated in the exact order
  *that they *	are specified in the SmudgeType enumeration. This is necessary
 ...
 *                                                                                             *
 *    This converts an ASCII name into a smudge type number. This is typically
 *necessary       * when processing scenario INI files and not used otherwise. *
 *                                                                                             *
 * INPUT:   name  -- Pointer to the name to convert. *
 *                                                                                             *
 * OUTPUT:  Returns with the SmudgeType number that matches the name supplied.
 *If no match     * was found, then SMUDGE_NONE is returned. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 08/12/1994 JLB : Created. *
 ...
 **
 *                                                                                             *
 *    Smudges are always only one icon in dimension, so this routine always
 *returns a cell     * occupation offset list of the center cell. *
 *                                                                                             *
 * INPUT:   placement   -- Is this for placement legality checking only? The
 *normal condition  * is for marking occupation flags. *
 *                                                                                             *
 * OUTPUT:  Returns occupation list specifying all the cells that the overlay
 *occupies. This   * is just the center cell. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 08/12/1994 JLB : Created. *
 ...
 *                                                                                             *
 *    Smudge object imagery varies between theaters. This routine will load the
 *appropriate    * imagery for the theater specified. *
 *                                                                                             *
 ...
 * SmudgeTypeClass::Display -- Draws a generic version of this smudge type. *
 *                                                                                             *
 *    The scenario object editor will call this routine to display a typical
 *imagery of this   * smudge object for graphical identification purposes. *
 *                                                                                             *
 ...
 *smudge object.   *
 *                                                                                             *
 *    This routine adds smudge objects to the list of objects that the scenario
 *editor can     * place upon the ground. It is only called from the scenario
 *editor.                       *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *object.            *
 *                                                                                             *
 *    This routine will, in one motion, create a smudge object and place it upon
 *the map.      * Since placing a smudge on the map will destroy the object,
 *this routine will leave the   * smudge object count unchanged. Typically, this
 *routine is used by the scenario editor    * for creating smudges and placing
 *them on the map.                                        *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will create a smudge object of the appropriate type. Smudge
 *objects are     * transitory in nature. They exist only from the point of
 *creation until they are given    * a spot on the map to reside. At that time
 *the map data is updated and the smudge         * object is destroyed. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to a created smudge object. If none could be
 ...
 *specified.           *
 *                                                                                             *
 *    This routine will draw the smudge overlay image at the coordinate (upper
 *left)           * specified. The underlying terrain icon is presumed to have
 *already been rendered.        *
 *                                                                                             *
 * INPUT:   x,y   -- Coordinate of the upper left corner of icon to render the
 *smudge object.  *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
...
/***********************************************************************************************
 * SmudgeTypeClass::As_Reference -- Fetches a reference to the smudge type
 *specified.          *
 *                                                                                             *
 *    Use this routine to get a reference to the smudge type class object when
 *given just      * the smudge type identifier. *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   Be sure that the smudge type specified is legal. An illegal type
 *value will     * produce undefined results. *
 *                                                                                             *
 * HISTORY: * 07/09/1996 JLB : Created. *
 *=============================================================================================*/
