...
/***********************************************************************************************
 * DriveClass::Response_Attack -- Voice feedback when ordering the unit to
 *attack a target.    *
 *                                                                                             *
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 09/25/1994 JLB : Created. * 09/27/1995 JLB : Revised to never
 *scatter if already moving.                              * 07/09/1996 JLB :
 *Moved to DriveClass so that ships will scatter too.                      *
...
/***********************************************************************************************
 * DriveClass::Stop_Driver -- Handles removing occupation bits when driving
 *stops.             *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This will teleport the object to the specified location or as close as
 *possible to it    * if the destination is blocked. *
 *                                                                                             *
 * INPUT:   cell  -- The desired destination cell to teleport to. *
 *                                                                                             *
 * OUTPUT:  bool; Was the teleport successful? *
 *                                                                                             *
 * WARNINGS:   All current activity of this object will be terminated by the
 *teleport. It will * arrive at the destination in static guard mode. *
 *                                                                                             *
 * HISTORY: * 10/21/1996 JLB : Created. * 10/31/1996 JLB : Handles flag teleport
 *case.                                              *
 *=============================================================================================*/
    ...
    */
    ...
    if (!Basic_Path()) {
      ...
      */
      ...
      /*
      **	Since the path was blocked, check to make sure that it was
      *completely *	blocked. If so and it has a valid TarCom and it is out
      *of range of the *	TarCom, then give this unit a range limit so
      *that it might not pick *	a "can't reach" target again.
      */
      ...
      /*
      **	Stop the movement, for now, and let the subsequent logic in
      *later game *	frames resume movement as appropriate.
      */
      Stop_Driver();
      TrackNumber = -1;
      IsTurretLockedDown = false;
...
DriveClass::TrackType const DriveClass::Track13[] = {
    {XYP_COORD(0, -35), DIR_S}, {XYP_COORD(0, -34), DIR_S},
    {XYP_COORD(0, -33), DIR_S}, {XYP_COORD(0, -32), DIR_S},
    {XYP_COORD(0, -31), DIR_S}, {XYP_COORD(0, -30), DIR_S},
    {XYP_COORD(0, -29), DIR_S}, {XYP_COORD(0, -28), DIR_S},
    {XYP_COORD(0, -27), DIR_S}, {XYP_COORD(0, -26), DIR_S},
    {XYP_COORD(0, -25), DIR_S}, {XYP_COORD(0, -24), DIR_S},
    {XYP_COORD(0, -23), DIR_S}, {XYP_COORD(0, -22), DIR_S},
    {XYP_COORD(0, -21), DIR_S}, {XYP_COORD(0, -20), DIR_S},
    {XYP_COORD(0, -19), DIR_S}, {XYP_COORD(0, -18), DIR_S},
    {XYP_COORD(0, -17), DIR_S}, {XYP_COORD(0, -16), DIR_S},
    {XYP_COORD(0, -15), DIR_S}, {XYP_COORD(0, -14), DIR_S},
    {XYP_COORD(0, -13), DIR_S}, {XYP_COORD(0, -12), DIR_S},
    {XYP_COORD(0, -11), DIR_S}, {XYP_COORD(0, -10), DIR_S},
    {XYP_COORD(0, -9), DIR_S},  {XYP_COORD(0, -8), DIR_S},
    {XYP_COORD(0, -7), DIR_S},  {XYP_COORD(0, -6), DIR_S},
    {XYP_COORD(0, -5), DIR_S},  {XYP_COORD(0, -4), DIR_S},
    {XYP_COORD(0, -3), DIR_S},  {XYP_COORD(0, -2), DIR_S},
    {XYP_COORD(0, -1), DIR_S},
...
DriveClass::TurnTrackType const DriveClass::TrackControl[67] = {
    ...
    {6, 8, DIR_N,
     (DriveClass::TrackControlType)(F_T | F_X | F_Y | F_D)},  //	1-0
    ...
    {0, 0, DIR_W, F_},                                        //	1-6 !
    {5, 10, DIR_NW,
     (DriveClass::TrackControlType)(F_T | F_X | F_Y | F_D)},  //	1-7
    {4, 9, DIR_N,
     (DriveClass::TrackControlType)(F_T | F_X | F_Y | F_D)},  //	2-0
    {3, 7, DIR_NE,
     (DriveClass::TrackControlType)(F_T | F_X | F_Y | F_D)},   //	2-1
    {1, 0, DIR_E, (DriveClass::TrackControlType)(F_T | F_X)},  //	2-2
    {3, 7, DIR_SE,
     (DriveClass::TrackControlType)(F_T | F_X | F_D)},               //	2-3
    ...
    {5, 10, DIR_SW,
     (DriveClass::TrackControlType)(F_T | F_X | F_D)},          //	3-5
    {0, 0, DIR_W, F_},                                          //	3-6 !
    {0, 0, DIR_NW, F_},                                         //	3-7 !
    {0, 0, DIR_N, F_},                                          //	4-0 !
    {0, 0, DIR_NE, F_},                                         //	4-1 !
    ...
    {1, 0, DIR_S, F_Y},                                         //	4-4
    {3, 7, DIR_SW,
     (DriveClass::TrackControlType)(F_X | F_Y | F_D)},               //	4-5
    ...
    {5, 10, DIR_NW,
     (DriveClass::TrackControlType)(F_X | F_Y | F_D)},               //	5-7
    ...
    {1, 0, DIR_W, F_T},                                              //	6-6
    {3, 7, DIR_NW,
     (DriveClass::TrackControlType)(F_T | F_Y | F_D)},               //	6-7
    {6, 8, DIR_N, (DriveClass::TrackControlType)(F_T | F_Y | F_D)},  //	7-0
    {5, 10, DIR_NE,
     (DriveClass::TrackControlType)(F_T | F_Y | F_D)},           //	7-1
    {0, 0, DIR_E, F_},                                           //	7-2 !
    {0, 0, DIR_SE, F_},                                          //	7-3 !
    {0, 0, DIR_S, F_},                                           //	7-4 !
