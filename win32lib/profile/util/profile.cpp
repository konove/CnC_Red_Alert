 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Overview: * Uses a map file to match addresses of functions in the sample
 *file with their names       *
 *                                                                                             *
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 *                                                                                             *
...

{
  ...
  */
  if (argc != 3 || !strcmpi(argv[1], "/?") || !strcmpi(argv[1], "/h") ||
      !strcmpi(argv[1], "/help") || !strcmpi(argv[1], "-?") ||
      !strcmpi(argv[1], "-h") || !strcmpi(argv[1], "-help") ||
      !strcmpi(argv[1], "?") || !strcmpi(argv[1], "h") ||
      !strcmpi(argv[1], "help")) {
    Print_Usage();
    return (0);
  }
  ...
  */
  ...
  /*
  ** Sort the functions into address order to make it easier to map the
  *functions
  */
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    File name * address to load at * read mode (text or binary) *
 *                                                                                             *
...
/***********************************************************************************************
 * Map_Profiler_Hits -- map function hits from sample file to functions in map
 *file            *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
  ...
  if (TotalFunctions > 1) {
    for (int outer = 0; outer < TotalFunctions; outer++) {
      ...
      for (int inner = 0; inner < TotalFunctions - 1; inner++) {
        if (FunctionList[inner].FunctionAddress >
            FunctionList[inner + 1].FunctionAddress) {
          memcpy(&address_swap, &FunctionList[inner], sizeof(Function));
          memcpy(&FunctionList[inner], &FunctionList[inner + 1],
                 sizeof(Function));
...
void Output_Profile(void) {
  ...
  for (int i = 0; i < TotalFunctions; i++) {
    if (FunctionList[i].Hits) {
      percentage =
          ((double)FunctionList[i].Hits * (double)100) /
          ((double)SampleFileLength / (double)4 - (double)SAMPLE_START);

      printf("%-6d %-3.3f%% %s\n", FunctionList[i].Hits, percentage,
...
/***********************************************************************************************
 * Extract_Function_Addresses -- gets the addresses of all global functions from
 *a map file    *
 *                                                                                             *
 *                                                                                             *
 * INPUT:    Nothing *
...
bool Extract_Function_Addresses(void) {
  ...
  */
  ...
  chars_left =
      MapFileLength - ((unsigned)segment_ptr - (unsigned)MapFileBuffer);
  ...
  /*
  ** Get the length of the segment section by searching for the start of the
  *next section
  */
  ...
  */
  do {
    ...
    */
    ...
    if (!segment_ptr)
      return (false);  // Couldnt find the segment address - must be a problem
                       // so abort
  ...
  */
  ...
  /*
  ** Get the length of the memory map segment by searching for the start of the
  *next segment
  */
  end_str_ptr = Search_For_String("+-----", map_ptr, chars_left);
  if (end_str_ptr) {
    MapFileLength =
        ((unsigned)MapFileBuffer + MapFileLength) - (unsigned)end_str_ptr;
  }
  chars_left = MapFileLength - ((unsigned)map_ptr - (unsigned)MapFileBuffer);
  ...
  */
  ...
  /*
  **
  ** Find each occurrence of 0001: as all the functions we want are in the 1st
  *segment
  **
  */
  do {
    ...
    */
    ...
    /*
    ** Skip the '0001:' portion of the address and get the hext offset of the
    *function
    */
    map_ptr += 5;
    ...
    /*
    ** Skip to the function name and get its length by searching for the end of
    *the line
    */
    map_ptr += 10;
    chars_left = MapFileLength - ((unsigned)map_ptr - (unsigned)MapFileBuffer);
    ...
    */
    ...
    FunctionNamePtr += function_name_length +
                       1;  // Leave an extra 0 on the end as a terminator
    FunctionList[TotalFunctions].Hits =
        0;  // We dont yet know how many times we hit it
    TotalFunctions++;

  } while (1);

  /*
  ** Add in a dummy function at the highest address to represent unknown code
  *hits
  */
 ...
 *            I'm sure there must be a lovely 'c' way of doing this but I dont
 *know what it is *
 *                                                                                             *
 *                                                                                             *
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 11/20/95 5:39PM ST : Created *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    character * ptr to buffer * length of buffer *
 *                                                                                             *
