 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * WinMain 		-- Program entry point * Set_Mode
 *-- Set up direct draw system                                               *
 *  WndProc			-- Callback procedure for main window *
 *  Font_Test		-- Test GVPC::Print * Rect_Test		-- Test
 *GVPC::Draw_Rect and GVPC::Draw_Line                                *
 *  Clear_Test		-- Test GVPC::Clear * Pixel_Test		-- Test
 *GVPC::Put_Pixel                                                    *
 *  Read_Pixel_Test	-- Test GVPC::Read_Pixel * Scale_Test		-- Test
 *GVPC::Scale                                                        *
 *  Blit_Test1		-- Test GVPC::Blit from video mem to video mem *
 *  Blit_Test1		-- Test GVPC::Blit from system mem to video mem *
 *  Fill_Rect_Test	-- Test GVPC::Fill_Rect * Remap_Test		-- Test
 *GVPC::Remap                                                        * Quad_Test
 *-- Test GVPC::Fill_Quad                                                    *
...
int WindowList[][8] = {
    {20 >> 3, 20, (MODE_WIDTH - 20) >> 3, MODE_HEIGHT - 20, 0, 0, 0,
     0} /* screen window */
};
...
int PASCAL WinMain(HANDLE hInstance, HANDLE hPrevInstance, LPSTR lpszCmdParam,
  ...
  //
  ScreenBuffer = new GraphicBufferClass(MODE_WIDTH, MODE_HEIGHT,
                                        (GBC_Enum)(GBC_VIDEOMEM | GBC_VISIBLE));

  //
  // Set the palette
  //
  DirectDrawObject->CreatePalette(DDPCAPS_8BIT | DDPCAPS_ALLOW256, &pe[0],
                                  &PalettePtr, NULL);
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    int - mode width * int - mode height * int - bits per pixel *
 *                                                                                             *
 * OUTPUT:   Nothing *
 *                                                                                             *
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 9/27/95 1:51PM ST : Created *
 *=============================================================================================*/
...
void Set_Mode(int width, int height, int bpp) {
  ...
  if (DirectDrawObject == NULL) {
    ...
    DirectDrawObject->SetCooperativeLevel(
        MainWindow, DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN | DDSCL_ALLOWMODEX);
...
void Stamp_Test(void) {
  ...
  for (y = 0; y < 5 * 24; y += 24) {
    for (x = 0; x < 6 * 24; x += 24) {
      ScreenBuffer->Draw_Stamp(IconPointer, icon++, x + StampX, y + StampY,
                               NULL, 0);
    }
  }
  Colour_Debug(0);
...
void Scale_Test(void) {
  ...
  BackBuffer->Scale(*ScreenBuffer, 0, 0, 10, 10, 320, 200, 320 + BlitStretchX,
                    200 + BlitStretchY);
...
void Blit_Test1(void) {
  ...
  BackBuffer->Blit(*ScreenBuffer, 0, 0, BlitXAdd, BlitYAdd, 320, 200,
                   (BOOL)FALSE);
...
void Fill_Rect_Test(void) {
  ...
  ScreenBuffer->Fill_Rect(min(x1, x2), min(y1, y2), max(x1, x2), max(y1, y2),
                          rand() & 255);
}
