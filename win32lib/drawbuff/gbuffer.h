...
class GraphicViewPortClass {
 public:
  ...
  HRESULT Blit(GraphicViewPortClass& dest, int x_pixel, int y_pixel,
               int dx_pixel, int dy_pixel, int pixel_width, int pixel_height,
               BOOL trans = FALSE);
  ...
  HRESULT DD_Linear_Blit_To_Linear(GraphicViewPortClass& dest, int source_x,
...
inline long GraphicViewPortClass::To_Buffer(int x, int y, int w, int h,
  ...
  long return_code = 0;
  if (Lock()) {
    return_code = (Buffer_To_Buffer(this, x, y, w, h, buff->Get_Buffer(),
                                    buff->Get_Size()));
  }
  Unlock();
  return (return_code);
...
inline long GraphicViewPortClass::To_Buffer(BufferClass* buff) {
  long return_code = 0;
  if (Lock()) {
    return_code = (Buffer_To_Buffer(this, 0, 0, Width, Height,
                                    buff->Get_Buffer(), buff->Get_Size()));
  }
  Unlock();
  return (return_code);
}
...
inline HRESULT GraphicViewPortClass::Blit(GraphicViewPortClass& dest,
  ...
  if (IsDirectDraw && dest.IsDirectDraw) {
    return (DD_Linear_Blit_To_Linear(
        dest, XPos + x_pixel, YPos + y_pixel, dest.Get_XPos() + dx_pixel,
 ...
 *   01/06/1995 PWG : Created.                                             *
 *=========================================================================*/
inline HRESULT GraphicViewPortClass::Blit(GraphicViewPortClass& dest, int dx,
                                          int dy, BOOL trans) {
  HRESULT return_code = 0;

  if (IsDirectDraw && dest.IsDirectDraw) {
    return (DD_Linear_Blit_To_Linear(dest, XPos, YPos, dest.Get_XPos() + dx,
                                     dest.Get_YPos() + dy, Width, Height,
                                     trans));
  } else {
    if (Lock()) {
      if (dest.Lock()) {
        return_code = (Linear_Blit_To_Linear(this, &dest, 0, 0, dx, dy, Width,
                                             Height, trans));
      }
      dest.Unlock();
    }
...
inline HRESULT GraphicViewPortClass::Blit(GraphicViewPortClass& dest,
  ...
  if (IsDirectDraw && dest.IsDirectDraw) {
    return (DD_Linear_Blit_To_Linear(
        dest, XPos, YPos, dest.Get_XPos(), dest.Get_YPos(),
        MAX(Width, dest.Get_Width()), MAX(Height, dest.Get_Height()), trans));
  } else {
    if (Lock()) {
      if (dest.Lock()) {
        return_code = (Linear_Blit_To_Linear(this, &dest, 0, 0, 0, 0, Width,
                                             Height, trans));
      }
      dest.Unlock();
    }
...
inline BOOL GraphicViewPortClass::Scale(GraphicViewPortClass& dest, int src_x,
  ...
  if (Lock()) {
    if (dest.Lock()) {
      return_code =
          (Linear_Scale_To_Linear(this, &dest, src_x, src_y, dst_x, dst_y,
                                  src_w, src_h, dst_w, dst_h, FALSE, remap));
    }
    dest.Unlock();
  }
  Unlock();
...
inline BOOL GraphicViewPortClass::Scale(GraphicViewPortClass& dest, BOOL trans,
  ...
  if (Lock()) {
    if (dest.Lock()) {
      return_code = (Linear_Scale_To_Linear(this, &dest, 0, 0, 0, 0, Width,
                                            Height, dest.Get_Width(),
 ...
 * HISTORY:                                                                *
 *=========================================================================*/
inline unsigned long GraphicViewPortClass::Print(short num, int x, int y,
                                                 int fcol, int bcol) {
  char str[17];
 ...
 * HISTORY:                                                                *
 *=========================================================================*/
inline unsigned long GraphicViewPortClass::Print(long num, int x, int y,
                                                 int fcol, int bcol) {
  char str[33];
      ...
      if (cache_index != -1) {
        if (CachedIcons[cache_index].Get_Is_Cached()) {
          CachedIcons[cache_index].Draw_It(
              GraphicBuff->Get_DD_Surface(), x_pixel, y_pixel,
...
inline VOID GraphicViewPortClass::Fill_Rect(int sx, int sy, int dx, int dy,
                                            unsigned char color) {
  if (AllowHardwareBlitFills && IsDirectDraw &&
      ((dx - sx) * (dy - sy) >= (32 * 32)) &&
    ...
    GraphicBuff->Get_DD_Surface()->Blt(
        &dest_rectangle, NULL, NULL, DDBLT_WAIT | DDBLT_ASYNC | DDBLT_COLORFILL,
        &blit_effects);
  } else {
    if (Lock()) {
...
inline long BufferClass::To_Page(GraphicViewPortClass& view) {
  long return_code = 0;
  if (view.Lock()) {
    return_code = (Buffer_To_Page(0, 0, view.Get_Width(), view.Get_Height(),
                                  Buffer, &view));
  }
  view.Unlock();
  return (return_code);
