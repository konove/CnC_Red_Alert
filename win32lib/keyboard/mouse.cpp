 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * WWMouseClass::WWMouseClass -- Constructor for the Mouse Class *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 * INPUT:		GraphicViewPortClass * screen - pointer to screen mouse
 *is created for				 *
 *                                                                                             *
 * OUTPUT:     none *
 *                                                                                             *
...
WWMouseClass::WWMouseClass(GraphicViewPortClass* scr, int mouse_max_width,
  ...
  /*
  ** Force the windows mouse pointer to stay withing the graphic view port
  *region
  */
  Set_Cursor_Clip();
}
...
void WWMouseClass::Process_Mouse(void) {
  ...
  //
  if (!Screen || !_Mouse || State > 0 || MouseUpdate || EraseFlags ||
      !GameInFocus)
    return;
  ...
  if (pt.x != MouseBuffX || pt.y != MouseBuffY) {
    ...
    if (Screen->Lock()) {
      ...
      //
      if (MCFlags & CONDHIDE && pt.x >= MouseCXLeft && pt.x <= MouseCXRight &&
          pt.y >= MouseCYUpper && pt.y <= MouseCYLower) {
        MCFlags |= CONDHIDDEN;
      }
...
void* WWMouseClass::Set_Cursor(int xhotspot, int yhotspot, void* cursor) {
  ...
  if (!cursor || cursor == PrevCursor) return (cursor);
...
void WWMouseClass::Low_Hide_Mouse() {
  if (!State) {
    if (MouseBuffX != -1 || MouseBuffY != -1) {
      if (Screen->Lock()) {
        Mouse_Shadow_Buffer(this, Screen, MouseBuffer, MouseBuffX, MouseBuffY,
                            MouseXHot, MouseYHot, 0);
        Screen->Unlock();
      }
    }
    MouseBuffX = -1;
...
void WWMouseClass::Low_Show_Mouse(int x, int y) {
  ...
  if (!State) {
    ...
    if (Screen->Lock()) {
      ...
      //
      Mouse_Shadow_Buffer(this, Screen, MouseBuffer, x, y, MouseXHot, MouseYHot,
                          1);
      //
...
void WWMouseClass::Conditional_Hide_Mouse(int x1, int y1, int x2, int y2) {
  ...
  if (!(MCFlags & CONDHIDDEN)) {
    GetCursorPos(&pt);
    if (MouseBuffX >= MouseCXLeft && MouseBuffX <= MouseCXRight &&
        MouseBuffY >= MouseCYUpper && MouseBuffY <= MouseCYLower) {
      Low_Hide_Mouse();
      MCFlags |= CONDHIDDEN;
    }
  }
...
void WWMouseClass::Draw_Mouse(GraphicViewPortClass* scr) {
  ...
  if (MCFlags & CONDHIDE && pt.x >= MouseCXLeft && pt.x <= MouseCXRight &&
    ...
    if (scr->Lock()) {
      ...
      //
      Mouse_Shadow_Buffer(this, scr, EraseBuffer, pt.x, pt.y, MouseXHot,
                          MouseYHot, 1);
...
void WWMouseClass::Erase_Mouse(GraphicViewPortClass* scr, int forced) {
  ...
  if (!forced) {
    ...
    if (scr->Lock()) {
      //
      // If the surface has not already been restore then restore it and erase
      // the restoration coordinates so we don't accidentally do it twice.
      //
    ...
    if (EraseBuffX != -1 || EraseBuffY != -1) {
      if (scr->Lock()) {
        Mouse_Shadow_Buffer(this, scr, EraseBuffer, EraseBuffX, EraseBuffY,
                            EraseBuffHotX, EraseBuffHotY, 0);
        scr->Unlock();
      }
      EraseBuffX = -1;
      EraseBuffY = -1;
 ...
 * INPUT:		none *
 *                                                                                             *
 * OUTPUT:     int		- returns the mouses current x position in
 *pixels                      *
 *                                                                                             *
 ...
 * INPUT:		none *
 *                                                                                             *
 * OUTPUT:     int		- returns the mouses current y position in
 *pixels                      *
 *                                                                                             *
 * HISTORY: * 10/17/1995 PWG : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * WWKeyboardClass::Get_Mouse_XY -- Returns the mouses x,y position via
 *reference vars         *
 *                                                                                             *
 * INPUT:		int &x		- variable to return the mouses x
 *position in pixels                * int &y		- variable to return the
 *mouses y position in pixels					  *
 *                                                                                             *
