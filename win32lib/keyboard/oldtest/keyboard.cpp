 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * WWKeyboardClass::Put -- Logic to insert a key into the
 *keybuffer]                         * WWKeyboardClass::Get -- Logic to get a
 *metakey from the buffer                            * WWKeyboardClass::Check --
 *Checks to see if a key is in the buffer                         *
 *   WWKeyboardClass::Put_Key_Message -- Translates and inserts wParam into
 *Keyboard Buffer    * WWKeyboardClass::Buff_Get -- Lowlevel function to get a
 *key from key buffer               * WWKeyboardClass::Get_Mouse_X -- Returns
 *the mouses current x position in pixels           *
...
WWKeyboardClass::WWKeyboardClass() {
  ...
  //
  // Build a remap table of the different keys which are affected by the caps
  // lock and the num lock.
  //
...
WWKeyboardClass::Put_Key_Message(UINT vk_key, BOOL release, BOOL dbl) {
  ...
  int caps =
      ((GetKeyState(VK_CAPITAL) & 0x00FF) != 0) && (ToggleKeys[vk_key] == 1);
  int nums =
      ((GetKeyState(VK_NUMLOCK) & 0x00FF) != 0) && (ToggleKeys[vk_key] == 2);
...
#pragma argsused
void WWKeyboardClass::Message_Handler(HWND hwnd, UINT message, UINT wParam,
                                      LONG lParam) {
  switch (message) {
    case WM_SYSKEYDOWN:
    case WM_KEYDOWN:
