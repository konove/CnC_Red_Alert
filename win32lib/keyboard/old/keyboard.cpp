 ...
 *position via reference vars       * Check_Key -- compatability routine for old
 *32 bit library             *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
int WWKeyboardClass::To_ASCII(int key) { return (key); }
...
void WWKeyboardClass::Message_Handler(HWND, UINT message, UINT wParam,
                                      LONG lParam) {
  switch (message) {
    case WM_SYSKEYDOWN:
    case WM_KEYDOWN:
