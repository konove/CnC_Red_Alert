 ...
 *                                                                                             *
 * INPUT:		GraphicBufferClass * screen - pointer to screen mouse is
 *created for				  * *
 *                                                                                             *
 * OUTPUT:     none *
 *                                                                                             *
...
void WWMouseClass::Process_Mouse() {
  ...
  //
  if (!_Mouse || State > 0 || MouseUpdate || EraseFlags) return;
...
void WWMouseClass::Conditional_Hide_Mouse(int x1, int y1, int x2, int y2) {
  ...
  if (!(MCFlags & CONDHIDDEN)) {
    GetCursorPos(&pt);
    if (pt.x >= MouseCXLeft && pt.x <= MouseCXRight && pt.y >= MouseCYUpper &&
        pt.y <= MouseCYLower) {
      Hide_Mouse();
      MCFlags |= CONDHIDDEN;
    }
...
void WWMouseClass::Erase_Mouse(GraphicBufferClass* scr, int forced) {
  ...
  if (!forced) {
    ...
    //
    // If this is a forced call then wait til we are allowed to update the
    // buffer
    //
    while (!scr->Lock()) {
    }
