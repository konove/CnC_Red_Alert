 ...
 ** around these functions must be locked or they will cause a read to * be
 *generated while in an interrupt.
 **
 *                                                                         *
 *-------------------------------------------------------------------------*
 * Functions:                                                              *
 ...
 * Sample_Copy -- Copies sound data from source format to raw format. *
 *                                                                                             *
 *    This routine is used to copy the sound data (possibly compressed) to the
 *destination     * buffer in raw format. *
 *                                                                                             *
 ...
 *                                                                                             *
 * OUTPUT:  Returns with the number of bytes placed into the output buffer. This
 *is usually   * the number of bytes requested except in the case when the
 *source is exhausted.     *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
        ...
        */
        {
          ...
          s = Simple_Copy(source, ssize, alternate, altsize, &fptr,
                          sizeof(fsize));
          if (s < sizeof(fsize)) {
            return datasize;
          }
          s = Simple_Copy(source, ssize, alternate, altsize, &dptr,
                          sizeof(dsize));
          if (s < sizeof(dsize) || size < dsize) {
            return datasize;
          }

          s = Simple_Copy(source, ssize, alternate, altsize, &mptr,
                          sizeof(magic));
          ...
          **	indicates that the frame is not compressed.  Just copy it
          *directly *	to the destination buffer in this case.
          */
          if (fsize == dsize) {
            ...
            /*
            **	The frame was compressed, so copy it to the staging buffer, and
            *then *	uncompress it into the final destination buffer.
            */
    ...
    if (st->Active) {
      ...
      */
      if (st->Service && !st->DontTouch) {
        ...
        /*
        ** Get the current position of the direct sound play cursor within the
        *buffer
        */
        return_code =
            st->PlayBuffer->GetCurrentPosition(&play_cursor, &write_cursor);
        ...
        */
        if (return_code != DS_OK) {
          ...
          return;  // Our app has lost focus or something else nasty has
                   // happened
        ...
        if (st->MoreSource) {
          ...
          */
          ...
          if (play_cursor < (unsigned)st->DestPtr) {
            if ((unsigned)st->DestPtr - (unsigned)play_cursor <=
            ...
            */
            if (((int)play_cursor > SECONDARY_BUFFER_SIZE * 3 / 4) &&
                st->DestPtr == 0) {
              write_more = TRUE;
            }
          }
          ...
          if (write_more) {
            ...
            */
            if (DS_OK == st->PlayBuffer->Lock((DWORD)st->DestPtr,
                                              ...
                                              0)) {
              bytes_copied = Sample_Copy(
                  st, &st->Source, &st->Remainder, &st->QueueBuffer,
                  &st->QueueSize, play_buffer_ptr, SECONDARY_BUFFER_SIZE / 4,
              ...
              if (bytes_copied != (SECONDARY_BUFFER_SIZE / 4)) {
                ...
                */
                ...
                /*
                ** Clear out an extra area in the buffer ahead of the play
                *cursor
                ** to give us a quiet period of grace in which to stop the
                *buffer playing
                */
              ...
              */
              st->DestPtr =
                  Audio_Add_Long_To_Pointer(st->DestPtr, bytes_copied);

              if ((unsigned)st->DestPtr >= (unsigned)SECONDARY_BUFFER_SIZE) {
                st->DestPtr = Audio_Add_Long_To_Pointer(
                    st->DestPtr, (long)-SECONDARY_BUFFER_SIZE);
              }

              /*
              ** Unlock the direct sound buffer
              */
              st->PlayBuffer->Unlock(play_buffer_ptr, lock_length1,
                                     dummy_buffer_ptr, lock_length2);
            }
          ...
          */
          if (((play_cursor >= (unsigned)st->DestPtr) &&
               (((unsigned)play_cursor - (unsigned)st->DestPtr) <
                SECONDARY_BUFFER_SIZE / 4)) ||
              (!st->OneShot &&
               ((play_cursor < (unsigned)st->DestPtr) &&
                (((unsigned)st->DestPtr - (unsigned)play_cursor) >
                 (SECONDARY_BUFFER_SIZE * 3 / 4))))) {
            st->PlayBuffer->Stop();
            st->Service = FALSE;
            Stop_Sample(index);
          }
      ...
      */
      if (!st->DontTouch && !st->QueueBuffer && st->FilePending) {
        st->QueueBuffer = Audio_Add_Long_To_Pointer(
      ...
      */
      if (st->Active && st->Reducer && st->Volume) {
        ...
        if (st->IsScore) {
          ...
          st->PlayBuffer->SetVolume(Convert_HMI_To_Direct_Sound_Volume(
              (LockedData.SoundVolume * (st->Volume >> 7)) / 256));
        }
 ...
 *                                                                         *
 * INPUT:		void * ptr - the pointer to add to * long size  - the
 *size to add to it *
 *                                                                         *
 ...
 *					unsigned char - the value to set the
 *memory to				  * long size     - how big an area to
 *set							  *
 *                                                                         *
 * OUTPUT:		none *
 *                                                                         *
