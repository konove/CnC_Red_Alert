...
/***********************************************************************************************
 **             C O N F I D E N T I A L --- W E S T W O O D   A S S O C I A T E
 *S             **
 ***********************************************************************************************
 *                                                                                             *
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Load_Long_Sample -- Loads a sample into XMS for double buffer
 *system.                     * Read_Long_Sample --
 *Loads/Processes/Formats/Builds offset. * Save_Table_Entry -- Put an entry in
 *the offset table.                                     * Play_Long_Sample --
 *Calls Init_Long_Sample and Start_Long_Sample.                         *
 *   Start_Long_Sample -- Starts a sample playing that has be initialized. *
 *   Get_Table_Entry -- Gets next entry in table. * Long_Sample_Ticks -- Gets
 *number of ticks in sample if in header.                         *
 *   Long_Sample_Status -- Returns the status of the sample. * Find_Table_Entry
 *-- Finds next entry in table that matches mask.                          *
 ...
 *   Sound_Callback -- Audio driver callback function. * DigiCallback -- Low
 *level double buffering handler.                                       *
 *   Load_Sample_Into_Buffer -- Loads a digitized sample into a buffer. *
 *   Stream_Sample -- Streams a sample directly from a file. * Sample_Read --
 *Reads sample data from an openned file.                                    *
...
/*
**      This is the number of times per sec that the maintenance callback gets
*called.
*/
...
WAVEFORMATEX PrimaryBuffFormat;  // Copy of format of direct sound primary
                                 // buffer
DSBUFFERDESC PrimaryBufferDesc;  // Copy of buffer description for re-creating
                                 // primary buffer
...
static int __cdecl Stream_Sample_Vol(void *buffer, long size,
                                     BOOL (*callback)(short id, short *odd,
                                                      void **buffer,
                                                      long *size),
                                     int volume, int handle);
void CALLBACK Sound_Timer_Callback(UINT, UINT, DWORD, DWORD, DWORD);
// static int __cdecl Stream_Sample(void *buffer, long size, BOOL
// (*callback)(short id, short *odd, void **buffer, long *size));
    ...
    if (ptr) {
      ...
      */
      ...
      if (!*buffer && st->FilePending) {
        *buffer = Add_Long_To_Pointer(
            ptr, (long)(*odd % LockedData.StreamBufferCount) *
                     (long)LockedData.StreamBufferSize);
        st->FilePending--;
        *odd = (short)(*odd + 1);
        if (!st->FilePending) {
      ...
      Sound_Timer_Callback(
          0, 0, 0, 0,
          0);  // Shouldnt block as we are calling it from the same thread
      ...
      */
      if (st->FilePending < (StreamLowImpact
        ...
        num_empty_buffers =
            (LockedData.StreamBufferCount - 2) - st->FilePending;
#else
        ...
        if (StreamLowImpact) {
          num_empty_buffers =
              MIN((LockedData.StreamBufferCount >> 1) + STREAM_CUSHION_BLOCKS,
                  (LockedData.StreamBufferCount - 2) - st->FilePending);
        } else {
          num_empty_buffers =
              (LockedData.StreamBufferCount - 2) - st->FilePending;
        ...
        while (num_empty_buffers && (st->FileHandle != WW_ERROR)) {
          ...
          ptr = Add_Long_To_Pointer(
              st->FileBuffer, (long)tofill * (long)LockedData.StreamBufferSize);
         ...
         **  is empty.  If so, then assign the first available pending buffer to
         *the
         **  queue.
         */
        if (!AudioDone) {
        ...
        if (!st->QueueBuffer && st->FilePending) {
          st->QueueBuffer = Add_Long_To_Pointer(
              st->FileBuffer, (long)(st->Odd % LockedData.StreamBufferCount) *
                                  (long)LockedData.StreamBufferSize);
          st->FilePending--;
          st->Odd++;
          if (!st->FilePending) {
...
#if (0)
static int __cdecl Stream_Sample(void *buffer, long size,
                                 BOOL (*callback)(short id, short *odd,
 ...
 *                                                                                             *
 *    This will take the file specified and play it directly from disk. * It
 *performs this by allocating a temporary buffer in XMS/EMS and * then keeping
 *this buffer filled by the Sound_Callback() routine.                         *
 *                                                                                             *
 * INPUT:   filename -- The name of the file to play. *
 *                                                                                             *
 * OUTPUT:  Returns the handle to the sound -- just like Play_Sample(). *
 *                                                                                             *
 * WARNINGS:   The temporary buffer is allocated when this routine is * called
 *and then freed when the sound is finished.  Keep                         *
 *             this in mind. *
 *                                                                                             *
...
void File_Stream_Preload(int handle) {
  ...
  /*
   ** Figure just how much we need to load.  If we are doing the load in
   *progress
  ...
  */
  for (int index = st->FilePending; index < num; index++) {
    long s =
        Read_File(fh,
                  Add_Long_To_Pointer(
                      buffer, (long)index * (long)LockedData.StreamBufferSize),
  ...
  /*
  ** If the last block was incomplete (ie. it didn't completely fill the buffer)
  *or
  ** we have now filled up as much of the Streaming Buffer as we need to, then
  *now is
  ** the time to kick off the sample.
  */
  if (st->FilePendingSize < LockedData.StreamBufferSize || index == maxnum) {
    /*
    ** Actually start the sample playing, and don't worry about the file
    *callback
    ** it won't be called for a while.
    */
    int old = LockedData.SoundVolume;
    int size = (st->FilePending == 1) ? st->FilePendingSize
                                      : LockedData.StreamBufferSize;
    ...
    /*
    ** The Sample is finished loading (if it was loading in small pieces) so
    *record that
    ...
    */
    ...
    /*
    ** Decrement the file pending because the first block is already playing
    *thanks
    ...
    */
    if (!st->FilePending) {
      ...
      /*
      ** The QueueBuffer counts as an already played block so remove it from the
      *total.
      ...
      */
      ...
      /*
      ** When we start loading we need to start past the first two blocks.  Why
      *this
      ...
      */
      ...
      /*
      ** If the file pending size is less than the stream buffer, then the last
      *block
      ** we loaded was the last block period.  So close the file and reset the
      *handle.
      */
      ...
      /*
      ** The Queue buffer needs to point at the next block to be processed.  The
      *size
      ** of the queue is dependant on how many more blocks there are.
      */
      st->QueueBuffer =
          Add_Long_To_Pointer(buffer, LockedData.StreamBufferSize);
      if (!st->FilePending) {
        st->QueueSize = st->FilePendingSize;
      } else {
        st->QueueSize = LockedData.StreamBufferSize;
...
int File_Stream_Sample_Vol(char const *filename, int volume,
  ...
  if (!AudioDone && LockedData.DigiHandle != -1 && filename &&
      Find_File(filename)) {
    /*
    ...
    */
    if (!FileStreamBuffer) {
      FileStreamBuffer =
          Alloc(LockedData.StreamBufferSize * LockedData.StreamBufferCount,
                (MemoryFlagType)(StreamBufferFlag | MEM_TEMP | MEM_LOCK));
 ...
 *                                                                                             *
 *    Maintains the audio buffers.  This routine must be called at least * 11
 *times per second or else audio glitches will occur. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   If this routine is not called often enough then audio * glitches
 *will occur.                                                            *
 *                                                                                             *
 * HISTORY: * 01/06/1994 JLB : Created. *
 *=============================================================================================*/
 ...
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the loaded sound file.  This pointer * is
 *passed to Play_Sample when playback is desired. *
 *                                                                                             *
 * WARNINGS:   If there is insufficient memory to load the sample, then * NULL
 *will be returned.                                                          *
 *                                                                                             *
 ...
 * Load_Sample_Into_Buffer -- Loads a digitized sample into a buffer. *
 *                                                                                             *
 *    This routine is used to load a digitized sample into a buffer * provided
 *by the programmer.  This buffer can be in XMS or EMS. *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   This routine will not overflow the buffer provided.  This * means
 *that the buffer must be big enough to hold the data                       * or
 *else the sound will be cut short.                                            *
 *                                                                                             *
 * HISTORY: * 01/06/1994 JLB : Created. *
 *=============================================================================================*/
long Load_Sample_Into_Buffer(char const *filename, void *buffer, long size) {
  ...
  */
  if (!buffer || !size || LockedData.DigiHandle == -1 || !filename ||
      !Find_File(filename)) {
    return (NULL);
  }
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 11/2/95 4:01PM ST : Created *
 *=============================================================================================*/
...
void Sound_Thread(void *) {
  DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(),
                  &SoundThreadHandle, THREAD_ALL_ACCESS, TRUE, 0);
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    error text * handle to window *
 *                                                                                             *
 * OUTPUT:   Nothing *
 *                                                                                             *
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 2/7/96 10:17AM ST : Created *
 *=============================================================================================*/

void Print_Sound_Error(char *sound_error, HWND window) {
  MessageBox(window, sound_error, "Command & Conquer",
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    window - window to send callback messages to * maximum
 *bits_per_sample - 8 or 16                                                 *
 *           stereo - will stereo samples be played * rate - maximum sample rate
 *required                                               * reverse_channels *
 *                                                                                             *
 * OUTPUT:   TRUE if correctly initialised *
 *                                                                                             *
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * Unknown.... * 08-24-95 10:01am ST : Modified for Windows 95 Direct
 *Sound                                *
 *=============================================================================================*/
  ...
  if (!SoundObject) {
    ...
    */
    if (DirectSoundCreate(NULL, &SoundObject, NULL) != DS_OK) {
      Print_Sound_Error("Warning - Unable to create Direct Sound Object",
                        window);
      return (FALSE);
    }
    ...
    */
    if (SoundObject->SetCooperativeLevel(window, DSSCL_PRIORITY) != DS_OK) {
      Print_Sound_Error(
          "Warning - Unable to set Direct Sound cooperative level", window);
      SoundObject->Release();
      SoundObject = NULL;
      return (FALSE);
    }
    ...
    */
    ...
    DsBuffFormat.nBlockAlign =
        (unsigned short)((DsBuffFormat.wBitsPerSample / 8) *
                         DsBuffFormat.nChannels);
    DsBuffFormat.nAvgBytesPerSec =
        DsBuffFormat.nSamplesPerSec * DsBuffFormat.nBlockAlign;
    DsBuffFormat.cbSize = 0;

    /*
    ** Make a copy of the primary buffer description so we can reset its format
    *later
    */
    ...
    */
    if (SoundObject->CreateSoundBuffer(&PrimaryBufferDesc, &PrimaryBufferPtr,
                                       NULL) != DS_OK) {
      Print_Sound_Error(
          "Warning - Unable to create Direct Sound primary buffer", window);
      SoundObject->Release();
      SoundObject = NULL;
      return (FALSE);
    }
      ...
      */
      if (DsBuffFormat.wBitsPerSample == 16) {
        ...
        */
        ...
        /*
        ** Make a copy of the primary buffer description so we can reset its
        *format later
        */
        memcpy(&PrimaryBufferDesc, &BufferDesc, sizeof(DSBUFFERDESC));
        memcpy(&PrimaryBuffFormat, &DsBuffFormat, sizeof(WAVEFORMATEX));
      ...
      if (!Set_Primary_Buffer_Format()) {
        /*
        ** We failed to set any useful format so print up an error message and
        *give up
        */
        ...
        Print_Sound_Error(
            "Warning - Your sound card does not meet the products audio "
            "requirements",
            window);
    ...
    */
    ...
    /*
    ** Initialise the global critical section object for sound thread
    *syncronisation
    */
    ...
    **
    */
    SoundTimerHandle = timeSetEvent(1000 / MAINTENANCE_RATE, 1,
                                    Sound_Timer_Callback, 0, TIME_PERIODIC);
    AudioDone = FALSE;
    ...
    */
     ...
     ** block.
     */
    LockedData.UncompBuffer =
        Alloc(LARGEST_SONARC_BLOCK + 50,
              (MemoryFlagType)(MEM_NORMAL | MEM_CLEAR | MEM_LOCK));

    /*
    ** Allocate once secondary direct sound buffer for each simultaneous sound
    *effect
    **
    */
    for (index = 0; index < MAX_SFX; index++) {
      SoundObject->CreateSoundBuffer(
          &BufferDesc, &LockedData.SampleTracker[index].PlayBuffer, NULL);
 ...
 *                                                                                             *
 *    This routine will uninitialize the sound driver (if any was * installed).
 *This routine must be called at program termination                          *
 *    time. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
void Sound_End(void) {
  ...
  */
  ...
  /*
  ** Since the timer has stopped, we are finished with our global critical
  *section.
  */
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/02/1992 JLB : Created. * 11/2/95 4:09PM ST : Modified for
 *Direct Sound                                             *
 *=============================================================================================*/
void Stop_Sample(int handle) {
  if (LockedData.DigiHandle != -1 && (unsigned)handle < MAX_SFX) {
    ...
    if (LockedData.SampleTracker[handle].Active ||
        LockedData.SampleTracker[handle].Loading) {
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/02/1992 JLB : Created. *
 *=============================================================================================*/
BOOL Sample_Status(int handle) {
  ...
  */
  ...
  if (LockedData.SampleTracker[handle].PlayBuffer->GetStatus(&status) ==
      DS_OK) {
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 11/2/95 4:11PM ST : Commented *
 *=============================================================================================*/
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 11/2/95 4:13PM ST : Commented *
 *=============================================================================================*/
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 11/2/95 4:14PM ST : Added function header *
 *=============================================================================================*/
...
int Get_Free_Sample_Handle(int priority) {
  ...
  */
  for (id = MAX_SFX - 1; id >= 0; id--) {
    if (!LockedData.SampleTracker[id].Active &&
        !LockedData.SampleTracker[id].Loading) {
      if (!StartingFileStream && LockedData.SampleTracker[id].IsScore) {
        StartingFileStream =
            TRUE;  // Ensures only one channel is kept free for scores.
        continue;
      }
      break;
    }
  ...
  if (LockedData.SampleTracker[id].Original &&
      !LockedData.SampleTracker[id].IsScore) {
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/20/96 9:47AM ST : Created *
 *=============================================================================================*/
...
/***********************************************************************************************
 * Convert_HMI_To_Direct_Sound_Volume -- Converts a linear volume value into an
 *expotential    * value                                                *
 *                                                                                             *
 * This function converts a linear C&C volume in the range 0-255 (255 loudest)
 *to a direct     * sound volume in the range 0 to -10000 (with 0 being the
 *loadest)                           *
 *                                                                                             *
 ...
 *                                                                                             *
 * HISTORY: * 9/18/96 11:36AM ST : Created *
 *=============================================================================================*/
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 04/17/1992 JLB : Created. * 05/24/1992 JLB : Volume support --
...
int Play_Sample_Handle(void const *sample, int priority, int volume,
  ...
  LPVOID dummy_buffer_ptr;  // dummy pointer to second area of locked direct
                            // sound buffer
  DWORD lock_length1;
  DWORD lock_length2;
  DWORD play_status;
  ...
  */
  ...
  /*
  ** If the sample rate , bits per sample or stereo capabilities of the buffer
  *do not
  ** match the sample then reallocate the direct sound buffer with the required
  *capabilities
  */
  if ((RawHeader.Rate != st->PlaybackRate) ||
    ...
    */
    ...
    DsBuffFormat.nBlockAlign =
        (short)((DsBuffFormat.wBitsPerSample / 8) * DsBuffFormat.nChannels);
    DsBuffFormat.nAvgBytesPerSec =
        DsBuffFormat.nSamplesPerSec * DsBuffFormat.nBlockAlign;
    ...
    **
    ** If we failed then flag the buffer as having an impossible format so it
    *wont match
    ** any sample. This will ensure that we try and create the buffer again next
    *time its used.
    */
  ...
  */
  ...
  //
  do {
    return_code = st->PlayBuffer->Lock(0, SECONDARY_BUFFER_SIZE,
                                       &play_buffer_ptr, &lock_length1,
                                       &dummy_buffer_ptr, &lock_length2, 0);
    if (return_code == DSERR_BUFFERLOST) {
      if (!Attempt_Audio_Restore(st->PlayBuffer)) return (-1);
    }
  } while (return_code == DSERR_BUFFERLOST);
  ...
  //
  st->DestPtr = (void *)Sample_Copy(
      st, &st->Source, &st->Remainder, &st->QueueBuffer, &st->QueueSize,
      play_buffer_ptr, SECONDARY_BUFFER_SIZE * 1 / 4, st->Compression,
      &st->Trailer[0], &st->TrailerLen);
  ...
  if (st->DestPtr == (void *)(SECONDARY_BUFFER_SIZE * 1 / 4)) {
    ...
    st->OneShot = TRUE;
    st->Service =
        TRUE;  // We still need to service it so that we can stop it when
               //  it plays past the end of the sample data
    // memset ( (char*)( (unsigned)play_buffer_ptr + (unsigned)st->DestPtr ), 0
    // , SECONDARY_BUFFER_SIZE - (unsigned)st->DestPtr );
    memset((char *)((unsigned)play_buffer_ptr + (unsigned)st->DestPtr), 0,
           SECONDARY_BUFFER_SIZE / 4);
  }

  st->PlayBuffer->Unlock(play_buffer_ptr, lock_length1, dummy_buffer_ptr,
                         lock_length2);
  ...
  st->Volume = (volume << 7);
  do {
    // return_code = st->PlayBuffer->SetVolume (
    // Convert_HMI_To_Direct_Sound_Volume(volume));
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 11/2/95 4:19PM ST : Added function header *
 *=============================================================================================*/
...
int Set_Score_Vol(int volume) {
  ...
  for (int index = 0; index < MAX_SFX; index++) {
    ...
    if (st->IsScore && st->Active) {
      ...
      // st->Volume >>7 ) );
      st->PlayBuffer->SetVolume(Convert_HMI_To_Direct_Sound_Volume(
          (LockedData.ScoreVolume * (st->Volume >> 7)) / 256));
    }
  }
  return (old);
}
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    Sample handle * fade rate *
 *                                                                                             *
 * OUTPUT:   Nothing *
 *                                                                                             *
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 11/2/95 4:21PM ST : Added function header *
 *=============================================================================================*/
