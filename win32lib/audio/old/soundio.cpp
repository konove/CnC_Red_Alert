      ...
      */
      ...
      if (!Set_Primary_Buffer_Format()) {
        ...
        */
        ...
        Print_Sound_Error(
            "Warning - Your sound card does not match C&C's audio requirements",
            window);
  ...
  */
  ...
  do {
    return_code = st->PlayBuffer->SetVolume(-(
        ((32768 - ((st->Volume >> 8) * LockedData.SoundVolume)) * 1000) >> 15));
    if (return_code == DSERR_BUFFERLOST) {
      if (!Attempt_Audio_Restore(st->PlayBuffer)) return (-1);
    }
  } while (return_code == DSERR_BUFFERLOST);
  ...
  for (int index = 0; index < MAX_SFX; index++) {
    ...
    if (st->IsScore && st->Active) {
      ...
      st->PlayBuffer->SetVolume(
          -(((32768 - ((st->Volume >> 8) * LockedData.ScoreVolume)) * 1000) >>
            15));
