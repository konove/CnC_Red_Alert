...
/* $Header:   F:\projects\c&c\vcs\code\rawfile.cpv   2.17   06 Sep 1995 16:38:30
 * JOE_BOSTIC  $ */
/***********************************************************************************************
...
void RawFileClass::Error(int error, int, char const *) {
...
#ifdef GERMAN
    strcat(message, " Beliebige Taste dr�cken f�r erneuten Versuch.");
    if (GraphicMode == TXT_MODE) strcat(message, "\n");
    strcat(message, " <ESC> dr�cken, um das Programm zu verlassen.");
#else
#ifdef FRENCH
  ...
  */
  if (GraphicMode == TXT_MODE) {
    ...
    */
    if (background) {
      SeenPage.To_Buffer(WinX << 3, WinY, WinW << 3, WinH, background,
...
long RawFileClass::Read(void *buffer, long size) {
  ...
  */
  while (size) {
    ...
    **	either a bad file number or invalid access. These are fatal conditions,
    *so *	display the error and then abort.
    */
    if (readresult != 0) {
      Error(errno, false, Filename);
    }  // else {
...
long RawFileClass::Write(void const *buffer, long size) {
  ...
  */
  while (size) {
    ...
    *file *	handle or invalid access. Either condition is fatal -- display
    *error condition *	and abort.
    */
    if (writeresult != 0) {
      Error(errno, false, Filename);
    } else {
  ...
  */
  for (;;) {
    ...
    */
    ...
    /*
    **	A negative one indicates a fatal error with the seek operation. Display
    *error *	condition and then abort.
    */
    if (pos == -1) {
      Error(errno, false, Filename);
    }
