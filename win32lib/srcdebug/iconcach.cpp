 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Overview: This file cantains members of the IconCacheClass and associated non
 *member        * functions. All functions are to do with caching individual
 *icons from icon sets   * into video memory to improve the speed of subsequent
 *drawing                      *
 *                                                                                             *
 * Functions: * Cache_New_Icon -- Call the Cache_It member to cache a registered
 *icon to video memory      * Invalidate_Cached_Icons -- Uncache all the icons *
 *  Restore_Cached_Icons -- restore cached icons after a focus loss *
 *  Register_Icon_Set -- register an icon set as cachable * Get_Free_Cache_Slot
...
BOOL Optimize_Video_Memory_Cache(void) {
  if (CacheMemoryExhausted && (UnCachedIconsDrawn + CachedIconsDrawn > 1000) &&
    ...
    if (total_cache_hits > 1) {
      for (i = 0; i < total_cache_hits; i++) {
        ...
        for (j = 0; j < total_cache_hits - 1; j++) {
          if (CachedIcons[cache_hits[j]].TimesDrawn >
              CachedIcons[cache_hits[j + 1]].TimesDrawn) {
            temp = cache_hits[j];
    ...
    */
    ...
    for (counter = 0;
         counter < total_cache_misses && counter < total_cache_hits;
         counter++) {
...
void Invalidate_Cached_Icons(void) {
  ...
  memset(&IconCacheLookup[0], -1,
         MAX_LOOKUP_ENTRIES * sizeof(IconCacheLookup[0]));
...
/***********************************************************************************************
 * Restore_Cached_Icons -- re-cache icons into video memory after a loss of
 *focus              *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 ...
 * OUTPUT:   Nothing *
 *                                                                                             *
 * WARNINGS: Assumes that the pointers that were originally used to cache the
 *icons            * are still valid. *
 *                                                                                             *
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    icon_data - ptr to icon set * pre_cache -- should we pre-cache the
 *icon data?                                   *
 *                                                                                             *
 * OUTPUT:   Nothing *
 *                                                                                             *
...
/***********************************************************************************************
 * ICC::Restore -- Restores the icons video surface memory and reloads it based
 *on the original* icon pointer *
 *                                                                                             *
 *                                                                                             *
 * INPUT:    Nothing *
 *                                                                                             *
...
BOOL IconCacheClass::Cache_It(void* icon_ptr) {
  ...
  */
  if (!CacheSurface) {
    if (DD_OK != DirectDrawObject->CreateSurface(&VideoSurfaceDescription,
                                                 &CacheSurface, NULL)) {
  ...
  */
  ...
  if ((DDSCAPS_SYSTEMMEMORY & surface_capabilities.dwCaps) ==
  ...
  */
  if (DD_OK ==
      CacheSurface->Lock(NULL, &(VideoSurfaceDescription), DDLOCK_WAIT, NULL)) {
    /*
    ** Copy the icon to the surface and flag that icon is cached
    */
    Cache_Copy_Icon(icon_ptr, VideoSurfaceDescription.lpSurface,
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    surface to draw to * x coord to draw to (relative to window) * y
 *coord to draw to (relative to window) * window left coord * window top coord *
 ...
 * HISTORY: * 11/13/95 9:48AM ST : Created *
 *=============================================================================================*/
void IconCacheClass::Draw_It(LPDIRECTDRAWSURFACE dest_surface, int x_pixel,
                             int y_pixel, int window_left, int window_top,
  ...
  */
  return_code =
      dest_surface->Blt(&dest_rectangle, CacheSurface, &source_rectangle,
