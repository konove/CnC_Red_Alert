 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Overview: * The profiler works by using the function prologue and epilogue
 *hooks available in Watcom * to register the current functions address in a
 *global variable and then sampling the * contents of the variable using a
 ...
 *disk in the PROFILE.BIN file. * Use *
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 *                                                                                             *
...
void Start_Profiler(void) {
  ...
  ProfilerEvent = timeSetEvent(
      1000 / PROFILE_RATE, 1,
      (void CALLBACK(UINT, UINT, DWORD, DWORD, DWORD))Profiler_Callback, 0,
      TIME_PERIODIC);
}
