...
void Vesa_Info(int vesa_mode) {
    ...
    for (temp = 0; temp < 3; temp++)
      if (*ptr == mode_table[temp][0]) {
        sprintf(buff, "%d\t%d x %d x %d\n", mode_table[temp][0],
                mode_table[temp][1], mode_table[temp][2], mode_table[temp][3]);
        cout << buff;
      }
  ...
  regs.x.eax &= 0xffff;
  if (regs.x.eax != 0x004F) {
    cout << "\nGraphic mode " << vesa_mode
         << " is not supported by this video card\n";
    DPMI_real_free(ModeInfoSel);
    DPMI_real_free(VInfoSel);
    return;
  }
  ...
  if (temp & 0x01)
    cout << "\tMode supported in hardware\n";
  else
    cout << "\tMode is not supported in hardware\n";
  if (temp & 0x20)
    cout << "\tMode is not VGA Windowed memory compatible\n";
  else
    cout << "\tMode is VGA Windowed memory compatible\n";

  cout << "Window A attributes\n";
  temp = (unsigned)ModeInfo->WinA_Attributes;
  ;
  if (temp & 0x02)
    cout << "\tWindow A is Readable\n";
  else
    cout << "\tWindow A is not Readable\n";
  if (temp & 0x04)
    cout << "\tWindow A is Writeable\n";
  else
    cout << "\tWindow A is not Writeable\n";
  ...
  ;
  if (temp & 0x02)
    cout << "\tWindow B is Readable\n";
  else
    cout << "\tWindow B is not Readable\n";
  if (temp & 0x04)
    cout << "\tWindow B is Writeable\n";
  else
    cout << "\tWindow B is not Writeable\n";
  ...
  /*
          cout	<< "Bttributes:                    " <<
     (long)ModeInfo.Bttributes			<< "\n"
                          << "Win B Bttributes:              " <<
     (long)ModeInfo.WinB_Bttributes	<< "\n"
                          << "Win B Bttributes:              " <<
     (long)ModeInfo.WinB_Bttributes	<< "\n"
                          << "Win Granularity                " <<
     (long)ModeInfo.WinGranularity   << "\n"
                          << "Win Size:                      " <<
     (long)ModeInfo.WinSize				<< "\n"
                          << "Win B Segment:                 " << hex <<
     (unsigned short)ModeInfo.WinB_Segment		<< "\n"
                          << "Win B Segment:                 " << (unsigned
     short)ModeInfo.WinB_Segment		<< "\n"
                          << "Bytes per scan line:           " << dec <<
     (unsigned short)ModeInfo.BytesPerScanline	<< "\n"
                          << "X resolution:                  " <<
     (long)ModeInfo.XRes					<< "\n"
                          << "Y resolution:                  " <<
     (long)ModeInfo.YRes             << "\n"
                          << "X Char Size:                   " <<
     (long)ModeInfo.XCharSize        << "\n"
                          << "Y Char Size:                   " <<
     (long)ModeInfo.YCharSize        << "\n"
                          << "Number of planes:              " <<
     (long)ModeInfo.NumPlanes			<< "\n"
                          << "Bits per pixels:               " <<
     (long)ModeInfo.BitsPerPixel		<< "\n"
                          << "Number of Banks:               " <<
     (long)ModeInfo.NumBanks			<< "\n"
                          << "Memory Model:                  " <<
     (long)ModeInfo.MemoryModel		<< "\n"
                          << "Bank Size:                     " <<
     (long)ModeInfo.BankSize         << "\n";
  */
  DPMI_real_free(ModeInfoSel);
  DPMI_real_free(VInfoSel);
}
