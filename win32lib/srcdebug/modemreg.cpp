 ...
 *value          * MREC::ModemRegistryEntryClass -- Constructor for
 *ModemRegistryEntryClass                    * MREC::~ModemRegistryEntryClass --
 *Destructor.Free all the memory we allocated for modem info*
 *                                                                                             *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    handle to key to search in * name of key to search for * value
 *expected in key                                                             *
 *                                                                                             *
...
HKEY Search_Registry_Key(HKEY key_in, char *value_name, char *search_string) {
  ...
  /*
  ** Scan through and enumerate all subkeys of this key. Exit the loop when
  *there are
  ...
  */
  do {
    ...
    ** Get the next key
    */
    retval = RegEnumKeyEx(key_in, top_key_index++, subkey_name,
                          &subkey_name_length, NULL, NULL, NULL, &filetime);
    ...
    if (retval == ERROR_SUCCESS) {
      ...
      */
      ...
      if (next_key) {
        ...
        if (RegQueryValueEx(next_key, value_name, NULL, &value_type, key_value,
                            &key_value_length) == ERROR_SUCCESS) {
          /*
          ** If this value is type string then do a compare with the value we
          *are looking for
          */
        ...
        /*
        ** We didnt find our search value so search this key for more sub keys
        *by reentering
        ...
        */
        ...
        /*
        ** If the value was found in a subkey then just return with the key
        *handle.
        */
        if (next_search) {
          delete[] subkey_name;
 ...
 *                                                                                             *
 *  This function does all the work in the class. All the registry searching is
 *done here      *
 *                                                                                             *
 * INPUT:    Modem number *
 *                                                                                             *
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 10/18/96 4:12AM ST : Created *
 *=============================================================================================*/
ModemRegistryEntryClass::ModemRegistryEntryClass(int modem_number) {
  ...
  */
  ...
  /*
  ** Get the name of the modem. This is what will be displayed in the modem list
  *presented
  ** to the user.
  */
  if (RegQueryValueEx(key, "Model", NULL, NULL, return_buf, &retbuf_size) !=
      ERROR_SUCCESS) {
    RegCloseKey(key);
    return;
  }
  ...
  /*
  ** Find out what COM port the modem is attached to. If this info isnt here,
  *then its a
  ** Plug n Pray modem. Set the flag so we know to do the pnp search later.
  */
  retbuf_size = sizeof(return_buf);
  if (RegQueryValueEx(key, "AttachedTo", NULL, NULL, return_buf,
                      &retbuf_size) != ERROR_SUCCESS) {
    /*
    ** Must be a plug n pray modem. Set the flag. We will look for the port
    *later.
    */
    pnp = 1;
  ...
  */
  retbuf_size = sizeof(return_buf);
  if (RegQueryValueEx(key, "ErrorControl_On", NULL, NULL, return_buf,
                      &retbuf_size) == ERROR_SUCCESS) {
  ...
  retbuf_size = sizeof(return_buf);
  if (RegQueryValueEx(key, "ErrorControl_Off", NULL, NULL, return_buf,
                      &retbuf_size) == ERROR_SUCCESS) {
  ...
  */
  retbuf_size = sizeof(return_buf);
  if (RegQueryValueEx(key, "Compression_On", NULL, NULL, return_buf,
                      &retbuf_size) == ERROR_SUCCESS) {
  ...
  retbuf_size = sizeof(return_buf);
  if (RegQueryValueEx(key, "Compression_Off", NULL, NULL, return_buf,
                      &retbuf_size) == ERROR_SUCCESS) {
  ...
  */
  retbuf_size = sizeof(return_buf);
  if (RegQueryValueEx(key, "FlowControl_Hard", NULL, NULL, return_buf,
                      &retbuf_size) == ERROR_SUCCESS) {
  ...
  */
  retbuf_size = sizeof(return_buf);
  if (RegQueryValueEx(key, "FlowControl_Off", NULL, NULL, return_buf,
                      &retbuf_size) == ERROR_SUCCESS) {
  ...
  /*
  ** If this is a plug n pray modem then we need to search for the COM port it
  *is
  ** attached to.
  */
  if (pnp) {
    /*
    ** The driver name in the HKEY_LOCAL_MACHINE / Enum section will be
    *Modem\nnnn where nnnn
    ...
    */
    ...
    if (newkey) {
      ...
      /*
      ** Extract the PORTNAME value. This is the name of the port to use to
      *communicate
      ** with the modem.
      */
      retbuf_size = sizeof(return_buf);
      if (RegQueryValueEx(newkey, "PORTNAME", NULL, NULL, return_buf,
                          &retbuf_size) == ERROR_SUCCESS) {
...
/***********************************************************************************************
 * MREC::~ModemRegistryEntryClass -- Destructor.Free all the memory we allocated
 *for modem info*
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
