 ...
 ** int x								- x
 *offset into buffer			* int y
 *- y offset into buffer			* int w
 *- view port width in pixels   * int h
 ...
 *                                                                         *
 * INPUT:		GraphicBufferClass *g_buff	- pointer to gbuff to
 *attach to  * int x                     - x position to attach to
 ** int y 							- y position to
 *attach to			* int w
 *- width of the view port			* int h
...
void GraphicViewPortClass::Attach(GraphicBufferClass* gbuffer, int x, int y,
  ...
  Offset = gbuffer->Get_Offset() +
           ((gbuffer->Get_Width() + gbuffer->Get_Pitch()) * y) + x;
 ...
 *					int the new y pixel position of the
 *graphic view port		* int the new width of the viewport in pixels
 ...
 **
 *                                                                         *
 * WARNINGS:   You may not resize a Graphic View Port which is derived * from a
 *Graphic View Port Buffer,
 **
 *                                                                         *
 * HISTORY:                                                                *
...
void GraphicBufferClass::DD_Init(GBC_Enum flags) {
  ...
  // Call CreateSurface
  //
  DirectDrawObject->CreateSurface(&VideoSurfaceDescription, &VideoSurfacePtr,
                                  NULL);
 ...
 * GBC::INIT -- Core function responsible for initing a GBC                *
 *                                                                         *
 * INPUT:		int 		- the width in pixels of the
 *GraphicBufferClass    * int		- the heigh in pixels of the
 *GraphicBufferClass		* void *	- pointer to user supplied
 *buffer (system will		* allocate space if buffer is NULL)
 ** long		- size of the user provided buffer
 ** GBC_Enum	- flags if this is defined as a direct draw * surface
 **
 *                                                                         *
 * OUTPUT:		none *
 *                                                                         *
 ...
 *					int h			- height of
 *buffer in pixels (default = 200) * void *buffer	- a pointer to the
 *buffer if any (optional)	*
 *                                                                         *
 * OUTPUT:                                                                 *
 *                                                                         *
 * WARNINGS:                                                               *
 ...
 *(default = 320)  * int h			- height of buffer in pixels
 *(default = 200) * void *buffer	- a pointer to the buffer if any
 *(optional)	*
 *                                                                         *
 * OUTPUT:     none                                                        *
 *                                                                         *
...
/*====================================================================================*
 * GBC::GRAPHICBUFFERCLASS -- contructor for GraphicsBufferClass with special
 *flags   *
 *                                                                                    *
 * INPUT:		int w			- width of buffer in pixels
 *(default = 320)           * int h			- height of buffer in
 *pixels (default = 200)      * void *buffer	- unused * unsigned flags -
 *flags for creation of special buffer types          * GBC_VISIBLE - buffer is
 *a visible screen surface    * GBC_VIDEOMEM - buffer resides in video memory *
 *                                                                                    *
 * OUTPUT:     none *
 *                                                                                    *
  ...
  */
  ...
  if (VideoSurfacePtr) {
    while (!LockCount && restore_attempts < 2) {
      result = VideoSurfacePtr->Lock(NULL, &(VideoSurfaceDescription),
                                     DDLOCK_WAIT, NULL);
  ...
  // Colour_Debug(1);
  Unblock_Mouse(this);
  return (FALSE);  // Return false because we couldnt lock or restore the
                   // surface
}
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    destination vvpc * x coord to blit from * y coord to blit from * x
 *coord to blit to * y coord to blit to * width to blit * height to blit *
 *                                                                                             *
 * OUTPUT:   DD_OK if successful *
 *                                                                                             *
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 09-22-95 11:05am ST : Created *
 *=============================================================================================*/
...

{
  ...
  return (dest.GraphicBuff->Get_DD_Surface()->Blt(
      &dest_rectangle, GraphicBuff->Get_DD_Surface(), &source_rectangle,
      key_source | DDBLT_WAIT | DDBLT_ASYNC, NULL));
}
