...
HKEY Get_Registry_Sub_Key(HKEY base_key, char* search_key, BOOL close) {
  ...
  if (RegQueryInfoKey(
          ...
          &longest_sub_key_name, &longest_class_string, &num_value_entries,
          &longest_value_name_length, &longest_value_data_length,
          &security_descriptor_length, &last_write_time) != ERROR_SUCCESS)
    return (0);
  ...
  for (int key_num = 0; key_num < num_sub_keys; key_num++) {
    *sub_key_buffer = 0;
    ...
    RegEnumKeyEx(base_key, key_num, sub_key_buffer, &longest_sub_key_name, NULL,
                 sub_key_class, &longest_class_string, &last_write_time);

    if (!stricmp(search_key, sub_key_buffer)) {
      if (RegOpenKeyEx(base_key, sub_key_buffer, NULL, KEY_READ, &result_key) ==
          ERROR_SUCCESS) {
        if (close) {
...
/***********************************************************************************************
 * Get_Modem_Name_From_Registry -- retrieve the name of the installed modem from
 *the registry  *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 * INPUT:    buffer to put the name in * length of buffer *
 *                                                                                             *
...
/***********************************************************************************************
 * WMC::Serial_Port_Open -- opens a com port for asyncronous read/write and gets
 *a handle to it*
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
HANDLE WinModemClass::Serial_Port_Open(int com, int baud, int parity,
  ...
  */
  static char com_ids[8][5] = {"COM1", "COM2", "COM3", "COM4",
                               "COM5", "COM6", "COM7", "COM8"};

  HLINEAPP app_line_handle;
  DWORD num_devices;

  lineInitialise(&app_line_handle, ProgramInstance, &lineCallbackFunc, NULL,
                 &num_devices);
  ...
  */
  com_handle = CreateFile(device_name, GENERIC_READ | GENERIC_WRITE, 0, NULL,
  ...
  */
  ...
  if (!found_modem) {
    /*
    ** Get the current state of the com port as a basis for our device control
    *block
    */
    ...
    */
    ...
    device_control.fErrorChar =
        FALSE;  // Dont put an error char into our input stream
    device_control.fOutxDsrFlow = FALSE;  // No DSR flow control
    device_control.fDtrControl =
        DTR_CONTROL_ENABLE;        // Enable control of DTR line
    device_control.fOutX = FALSE;  // No XON/XOF flow control
    device_control.fInX = FALSE;   // No XON/XOF flow control
    device_control.fAbortOnError =
        FALSE;  // Device continues to send after an error
    ...
    */
    ...
    /*
    ** If we are talking to a modem device then turn off compression and error
    *correction
    */
...
HANDLE WinModemClass::Serial_Port_Open(char* device_name, int baud, int parity,
  ...
  ** Open the com port for asyncronous reads/writes
  */
  // com_handle = CreateFile (&com_ids[com][0], GENERIC_READ | GENERIC_WRITE, 0,
  // NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
  com_handle = CreateFile(devname, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                          OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
  ...
  ** Set the size of the windows communication buffers
  */
  SetupComm(com_handle, SIZE_OF_WINDOWS_SERIAL_BUFFER,
            SIZE_OF_WINDOWS_SERIAL_BUFFER);
  ...
  */
  timeouts.ReadIntervalTimeout =
      1000;  // 1 second between incoming bytes will time-out the read
  timeouts.ReadTotalTimeoutMultiplier = 0;  // disable per byte timeouts
  timeouts.ReadTotalTimeoutConstant =
      3000;  // Read operations time out after 3 secs if no data received
  timeouts.WriteTotalTimeoutMultiplier =
      500;  // Allow 1/2 ms between each char write
  timeouts.WriteTotalTimeoutConstant =
      1000;  // Write operations time out after 1 sec + 1/2 sec per char if data
             // wasnt sent
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 1/10/96 2:22PM ST : Created *
 *=============================================================================================*/
 ...
 * INPUT:    Nothing *
 *                                                                                             *
 * OUTPUT:   Modem status. Any of the following bits CTS_SET, DSR_SET, RI_SET or
 *CD_SET        *
 *                                                                                             *
 * WARNINGS: None *
 *                                                                                             *
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 1/10/96 2:25PM ST : Created *
 *=============================================================================================*/
...
/***********************************************************************************************
 * WMC::Read_Serial_Chars -- copys chars from the windows serial buffer to the
 *class buffer    *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
BOOL WinModemClass::Read_Serial_Chars(void) {
  ...
  */
  if (WaitingForSerialCharRead) {
    ...
    */
    bytes_read = 0;
    overlap_result =
        GetOverlappedResult(PortHandle, &ReadOverlap, &bytes_read, FALSE);

    /*
    ** If we got a good result from GetOverlappedResult and data was read then
    *move it
    **  to our circular buffer
    */
    if (overlap_result) {
      WaitingForSerialCharRead =
          FALSE;  // Flag that we are no longer waiting for a read
  ...
  */
  ...
  /*
  **
  ** Clear any communications errors and get the number of bytes in the in
  *buffer
  **
  */
  DWORD error;
  ...
  */
  ...
  do {
    ...
    */
    bytes_read = 0;
    read_result = ReadFile(PortHandle, TempSerialBuffer, bytes_to_read,
                           &bytes_read, &ReadOverlap);
    ...
    if (!read_result) {
      ...
      */
      if (GetLastError() == ERROR_IO_PENDING) {
        /*
        ** But it threaded in the background OK so flag that we must wait for it
        *to finish
        */
...
/***********************************************************************************************
 * WMC::Read_From_Serial_Port -- retrieves chars from the internal class
 *circular buffer which * is filled from the windows serial buffer *
 *                                                                                             *
 *                                                                                             *
 * INPUT:    buffer to copy to * size of buffer *
 *                                                                                             *
  ...
  if (WaitingForSerialCharWrite) {
    ...
    */
    do {
      wait_send = FALSE;
      overlap_result =
          GetOverlappedResult(PortHandle, &WriteOverlap, &bytes_written, FALSE);
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    ptr to data * bytes to write *
 *                                                                                             *
 * OUTPUT:   Nothing *
 *                                                                                             *
 * WARNINGS: None *
...
void WinModemClass::Write_To_Serial_Port(unsigned char* buffer, int length) {
  ...
  */
  write_result =
      WriteFile(PortHandle, buffer, length, &bytes_written, &WriteOverlap);
...
/***********************************************************************************************
 * WMC::Get_Modem_Result -- gets the result code from the modem after issuing an
 *'AT' command  *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 * INPUT:    delay for result time-out * ptr to buffer to receive modem result *
 *           length of buffer *
 *                                                                                             *
...
int WinModemClass::Get_Modem_Result(int delay, char* buffer, int buffer_len) {
  ...
  /*
  ** Loop to parse data from the modem and discard any echoed 'AT' commands or
  *spurious LF
  ** and CR characters.
  **
  */
    ...
    do {
      ...
      */
      ...
      sprintf(abuffer, "Wincomm - End of inner do loop. Time is %d.\n",
              timer.Time());
      // OutputDebugString (abuffer);
    } while (timer.Time() && dest_ptr < buffer_len && !strchr(buffer, 13));

    // OutputDebugString ("Wincomm - Exited inner do loop.\n");

    /*
    ** We need to discard this result if it is just an echo of the 'AT' command
    *we sent
    */
  ...
  /*
  ** Terminate the string at the first CR character as this is what Greenleaf
  *does
  */
  if (strchr(buffer, 13)) {
  ...
  // timer.Time()); OutputDebugString (abuffer);
  return (timer.Time());
}
...
/***********************************************************************************************
 * WMC::Send_Command_To_Modem -- send an 'AT' command to the modem and await a
 *response        *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 * INPUT:    command string * terminator byte (usually "\r") * ptr to buffer to
 *receive modem result code                                        * length of
 *buffer                                                                  *
 *           timeout delay for waiting for result * number of times to retry the
 *command                                              *
 *                                                                                             *
 * OUTPUT:   result code *
 *                                                                                             *
  ...
  */
  ...
  for (times = 0; times < retries; times++) {
    ...
    */
    ...
    /*
    ** If the result was a 3 digit number then copy it to the users buffer and
    *return OK
    */
    ...
    */
    ...
    /*
    ** Spurious write for no apparent reason. Well it was there in the DOS
    *version so...
    */
    Sleep(100);
