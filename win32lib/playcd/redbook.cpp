...
RedBookClass::~RedBookClass(VOID) {
  ...
  if (Stat_addrp.seg) DPMI_real_free(Stat_addrp);

  if (Volm_addrp.seg) DPMI_real_free(Volm_addrp);

  if (Play_addrp.seg) DPMI_real_free(Play_addrp);

  if (Stop_addrp.seg) DPMI_real_free(Stop_addrp);
}
...
VOID RedBookClass::Play_CD_MSL(UWORD min_sec, UWORD len) {
  ...
  PlayMSF((UBYTE)startM, (UBYTE)startS, (UBYTE)startF, (UBYTE)endM, (UBYTE)endS,
          (UBYTE)endF, 2 /* chan */);
}
...
VOID RedBookClass::PlayMSF(UBYTE startM, UBYTE startS, UBYTE startF, UBYTE endM,
                           UBYTE endS, UBYTE endF, UBYTE chan) {
 ...
 ** UBYTE	endF
 ** UBYTE	chan
 ** OUTPUT: * UWORD	TRUE if playing else FALSE
 ** WARNINGS:                                                               *
 *                                                                         *
 * HISTORY:                                                                *
 *   05/27/1994 SW : Created.                                              *
