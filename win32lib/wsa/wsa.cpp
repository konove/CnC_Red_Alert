...
/* $Header: g:/library/source/rcs/./wsa.c 1.16 1994/05/20 15:35:27 joe_bostic
 * Exp $ */
/***************************************************************************
 ...
 *                                                                         *
 *   Direct to Page vs Use of a user buffer -- Noramally Direct to page * is the
 *best method both in speed and in RAM need to hold anim.    * One may want to
 *use the write to user buffer method if they       * are using the animation in
 *a non sequencial order.                *
 *                                                                         *
 *-------------------------------------------------------------------------*
 * Functions:                                                              *
...
#define EXTRA_charS_ANIMATE_NOT_KNOW_ABOUT \
...
#define WSA_FILE_HEADER_SIZE \
  (sizeof(WSA_FileHeaderType) - (2 * sizeof(unsigned long)))
...
PRIVATE unsigned long Get_File_Frame_Offset(int file_handle, int frame,
                                            int palette_adjust);
PRIVATE BOOL Apply_Delta(SysAnimHeaderType *sys_header, int curr_frame,
 ...
 *          unsigned long user_buffer_size if known  (NULL ok) * WSAOpenType
 *user_flags - flags on how to open.                 * unsigned char *palette -
 *pointer to palette space for return (NULL ok) *
 *                                                                         *
...
void *__cdecl Open_Animation(char const *file_name, char *user_buffer,
  ...
  if (file_header.flags & 1) {
    ...
    if (palette != NULL) {
      Seek_File(fh, sizeof(unsigned long) * (file_header.total_frames),
                SEEK_CUR);
  ...
  if (user_flags & WSA_OPEN_DIRECT) {
    ...
    target_buffer_size =
        (unsigned long)file_header.pixel_width * file_header.pixel_height;
  }
  ...
  // largest_frame_size BEFORE SAVING IT TO THE FILE.  THIS MEANS I HAVE TO ADD
  // THESE charS ON NOW FOR IT TO WORK.
  delta_buffer_size = (unsigned long)file_header.largest_frame_size +
                      EXTRA_charS_ANIMATE_NOT_KNOW_ABOUT;
...
BOOL __cdecl Animate_Frame(void *handle, GraphicViewPortClass &view,
  ...
  if (sys_header->flags & WSA_TARGET_IN_BUFFER) {
    ...
    Buffer_Bitblit_To_LogicPage(x_pixel, y_pixel, sys_header->pixel_width,
                                sys_header->pixel_height, 0, flags,
                                frame_buffer, pri, magic_cols, magic);
#endif
  }
 ...
 * INPUT:		void * to the animation that we are processing *
 *                                                                         *
 * OUTPUT:		int True if the animation has a set palette.  False if
...
PRIVATE unsigned long Get_File_Frame_Offset(int file_handle, int frame,
  ...
  if (Read_File(file_handle, (char *)&offset, sizeof(unsigned long)) !=
