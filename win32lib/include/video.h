 ...
 ** bit 2: 1 = std BIOS output funcs valid in this mode	* bit 3: 0 = monochrome,
 *1 = color							* bit 4: 0 =
 *text mode, 1 = graphics * WinA_Attributes  - bit 0 = win exists, bit
 *1=readable, bit 2= writable	* WinB_Attributes  - bit 0 = win exists, bit
 *1=readable, bit 2= writable	* WinGranularity   - smallest address boundary
 *window can be placed upon;	* size is in KB (ie 64, 32, 4)
 ...
 ** WinFunc          - address of window-setting function (This is provided * as
 *an alternative to Int 10 for speed.)					*
 ...
 ** 04h = packed pixel mode (1 byte/pixel) * 05h = non-chain 4, 256-color mode
 ** 06-0Fh =
 ** 10-FFh = OEM-specific
