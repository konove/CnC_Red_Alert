...
void CALLBACK TimerCallback(UINT event_id, UINT res1, DWORD user, DWORD res2,
                            DWORD res3);
...
void VQA_StopTimerInt(VQAHandleP *vqap) {
   ...
   */
  if (((AudioFlags & VQAAUDF_TIMERINIT) ==
       (HMI_VQAINIT << VQAAUDB_TIMERINIT)) &&
      (TimerSysCount == 0)) {
...
long VQA_OpenAudio(VQAHandleP *vqap) {
  ...
  if (config->AudioRate != -1) {
    ...
    audio->sSOSInitDriver.wSampleRate =
        ((audio->SampleRate * config->FrameRate) / (unsigned long)header->FPS);
  ...
  if ((config->OptionFlags & VQAOPTF_HMIINIT) ||
    ...
    if (config->DigiCard == -1) {
      ...
      if (header->Version == VQAHD_VER1) {
        ...
        config->DigiCard =
            AutoDetect(&audio->DigiCaps, audio->BitsPerSample, audio->Channels);
    ...
    if ((rc = sosDIGIInitDriver(config->DigiCard, &audio->DigiHardware,
                                &audio->sSOSInitDriver, &audio->DigiHandle)) !=
        0) {
      return (rc);
    }
    ...
    /* Register the event */
    rc = sosTIMERRegisterEvent(
        VQA_TIMETICKS, audio->sSOSInitDriver.lpFillHandler, &audio->DigiTimer);
...
long VQA_StartAudio(VQAHandleP *vqap) {
  ...
  if (audio->sSOSInitDriver.wSampleRate != audio->SampleRate) {
    ...
    audio->sSOSSampleData.dwSamplePitchAdd = (long)MAKE_LONG(
        (short)result.quot, (short)(((long)result.rem * 0x10000L) /
                                    audio->sSOSInitDriver.wSampleRate));
  ...
  /* Start playback */
  audio->SampleHandle =
      sosDIGIStartSample(audio->DigiHandle, &audio->sSOSSampleData);
...
long VQA_StartTimerInt(VQAHandleP *vqap, long init) {
  ...
  if ((AudioFlags & VQAAUDF_HMITIMER) == (HMI_UNINIT << VQAAUDB_HMITIMER)) {
    VQATimer =
        timeSetEvent(1000 / VQA_TIMETICKS, 1, TimerCallback, 0, TIME_PERIODIC);
...
void CALLBACK TimerCallback(UINT, UINT, DWORD, DWORD, DWORD) { VQATickCount++; }
  ...
  */
  if (!config->SoundObject) {
    if (DirectSoundCreate(NULL, &config->SoundObject, NULL) != DS_OK) {
      ...
      */
      if (config->SoundObject->SetCooperativeLevel(window, DSSCL_EXCLUSIVE) !=
          DS_OK) {
    ...
    if (!config->PrimaryBufferPtr) {
      ...
      */
      ...
      if (config->AudioRate != -1) {
        ...
        audio->DsBuffFormat.nSamplesPerSec =
            ((audio->SampleRate * config->FrameRate) /
      ...
      audio->DsBuffFormat.nBlockAlign =
          (unsigned short)((audio->DsBuffFormat.wBitsPerSample / 8) *
                           audio->DsBuffFormat.nChannels);
      audio->DsBuffFormat.nAvgBytesPerSec =
          audio->DsBuffFormat.nSamplesPerSec * audio->DsBuffFormat.nBlockAlign;
      audio->DsBuffFormat.cbSize = 0;

      /*
      **  Create the direct sound primary sound buffer object
      */
      if (config->SoundObject->CreateSoundBuffer(
              &audio->BufferDesc, &config->PrimaryBufferPtr, NULL) != DS_OK) {
...
long VQA_StartAudio(VQAHandleP *vqap) {
  ...
  */
  ...
  audio->DsBuffFormat.nBlockAlign =
      (short)((audio->DsBuffFormat.wBitsPerSample / 8) *
              audio->DsBuffFormat.nChannels);
  audio->DsBuffFormat.nAvgBytesPerSec =
      audio->DsBuffFormat.nSamplesPerSec * audio->DsBuffFormat.nBlockAlign;
  ...
  */
  config->SoundObject->CreateSoundBuffer(&audio->BufferDesc,
                                         &audio->SecondaryBufferPtr, NULL);
  ...
  */
  ...
  // Set orf 60hz timer
  audio->TimerHandle =
      timeSetEvent(1000 / 60, 1, AudioCallback, 0, TIME_PERIODIC);
...

{
  ...
  /*
  ** See if we are nearing the end of the meaningful data in the direct sound
  *buffer
  */
  return_code = audio->SecondaryBufferPtr->GetCurrentPosition(&play_cursor,
                                                              &write_cursor);
  ...
  if (play_cursor < audio->EndLastAudioChunk) {
    if (audio->EndLastAudioChunk - play_cursor <=
        audio->SecondaryBufferSize / 4) {
      write_more = TRUE;
    }
  } else {
...
/***********************************************************************************************
 * Move_HMI_Audio_Block_To_Direct_Sound_Buffer -- moves an audio block which
 *would have been   * played by HMI into a direct sound            * secondary
 *buffer                             *
 *                                                                                             *
 * INPUT:    Nothing *
 *                                                                                             *
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 12/21/95 9:51AM ST : Created *
 *=============================================================================================*/
extern int VQAMovieDone;
BOOL Move_HMI_Audio_Block_To_Direct_Sound_Buffer(void) {
  VQAAudio *audio;
  ...
  */
  return_code = audio->SecondaryBufferPtr->Lock(
      (DWORD)next_fill_pos, (DWORD)config->HMIBufSize, &play_buffer_ptr,
      &lock_length1, &dummy_buffer_ptr, &lock_length2, 0);

  if (return_code != DS_OK) return (FALSE);
  if (config->PrimaryBufferPtr->GetStatus(&status) == DSERR_BUFFERLOST)
    return (FALSE);

  /*
  ** Copy the HMI audio buffer to the direct sound buffer
  */
  memcpy((char *)play_buffer_ptr,
         (char *)((unsigned)audio->Buffer + (unsigned)audio->PlayPosition),
         config->HMIBufSize);

  /*
  ** Unlock the direct sound buffer
  */
  audio->SecondaryBufferPtr->Unlock(play_buffer_ptr, lock_length1,
                                    dummy_buffer_ptr, lock_length2);
  /*
...
unsigned long VQA_GetTime(VQAHandleP *vqap) {
  ...
  switch (TimerMethod) {
    ...
    case VQA_TMETHOD_AUDIO:
      ...
      if (audio->SecondaryBufferPtr &&
          audio->SecondaryBufferPtr->GetCurrentPosition(
      ...
      lastfill = _lpSOSSampleList[audio->DigiHandle][audio->SampleHandle]
                     ->sampleLastFill;
      totalbytes = _lpSOSSampleList[audio->DigiHandle][audio->SampleHandle]
                       ->sampleTotalBytes;
    ...
    case VQA_TMETHOD_DOS: {
      ...
      ftime(&mytime);
      ticks =
          (unsigned long)mytime.time * 1000L + (unsigned long)mytime.millitm;
