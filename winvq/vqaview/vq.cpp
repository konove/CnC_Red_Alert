...
#if (DEBUG_CODE)
BOOL Set_Frame_Rate_Dialog_Procedure(WindowHandle window_handle, UINT message,
...
VQAClass::VQAClass(char *filename, char *buffer, short media_src,
  ...
  //-------------------------------------------------------------------------
  // Create a VQ instance for playing from memory - does not actually allocate
  // buffers.
  //-------------------------------------------------------------------------
...
BOOL VQAClass::Update_Palette(unsigned char *newpalette) {
  ...
  */
  if (PalettesRead) {
    memcpy(&Palette_Interpolation_Table[0][0],
           InterpolatedPalettes[NumPalettes], 65536);
    NumPalettes++;
  } else {
...
/***********************************************************************************************
 * Strip_Interpolated_Palette -- strip the redundancy out of an interpolated
 *palette table     *
 *                                                                                             *
 * This just replaces the redundant entries with zeros so the palette will
 *compress better     *
 *                                                                                             *
...
/***********************************************************************************************
 * Rebuild_Interpolated_Palette -- rebuilds the entries stripped by
 *Strip_Interpolated_Palette *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 * INPUT:    ptr to palette *
...
/***********************************************************************************************
 * VQAC::Read_Palettes -- read the associated palette interpolation tables from
 *disk           *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
  ...
  if (PaletteFile->Is_Available()) {
    ...
    */
    for (i = 0; i < NumPalettes; i++) {
      ...
      // LCW_Uncompress((char
      // const*)comp_buff,(char*)InterpolatedPalettes[i],65536);
...
void VQAClass::Write_Palettes(void) {
  ...
  /*
  ** If the palette file wasnt there when we started playing the VQ then create
  *one
  */
  if (NumPalettes && !PalettesRead) {
    ...
    */
    for (int i = 0; i < NumPalettes; i++) {
      ...
      // comp_size = LCW_Compress ((char const*)InterpolatedPalettes[i] ,
      // (char*)comp_buff , 65536);
...
int VQAClass::Play_Generic_VQA(int last_frame_to_play) {
  ...
  while ((errval == VQAERR_NOT_TIME || errval >= 0 ||
          errval == VQAERR_NOBUFFER || errval == VQAERR_SLEEPING) &&
  ...
  switch (message) {
    case WM_COMMAND:
      ...
      if (LOWORD(w_param) == IDOK) {
        GetDlgItemText(window_handle, FRAME_RATE, frame_rate_string,
                       sizeof(frame_rate_string));
