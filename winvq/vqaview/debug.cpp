...
VOID Debug_Mono_Message(BYTE* message) {
  ...
  if (!_errored_once && !danger) {
    if (*(Executable_Path + 0) != '\0') {
      sprintf(name_path_string, "%s%s", Executable_Path, Debug_Log_File_Name);
      fh = Open_File(name_path_string, READ | WRITE);
      if (fh == FILEOPENERROR || fh == -1) {
        sprintf(Text_String, "Unable to open %s for WRITE.\r\n",
                Debug_Log_File_Name);
#if (DEBUG_CODE)
...
#define PIXEL_SIZE 19
  ...
  if (Game_Is_Now_Currently_Interrupted) {
    ...
    if (Current_Interrupting_Player > NO_PLAYER) {
      ...
      sprintf(Text_String, "Current_Interrupting_Player = unknown\r\n");
    }
  ...
  for (i = 0; i < 16; i++) {
    for (j = 0; j < 16; j++) {
      ...
      Monopoly_Fill_Rect(x, y, x + PIXEL_SIZE - 1, y + PIXEL_SIZE - 1,
                         COLOR_BLACK);
      Monopoly_Fill_Rect(x + 1, y + 1, x + PIXEL_SIZE - 2, y + PIXEL_SIZE - 2,
                         color++);
    }
  }
  ...
  WinGBitBlt(screen_dc, VQ_MOVIE_X, VQ_MOVIE_Y, VQ_MOVIE_WIDTH, VQ_MOVIE_HEIGHT,
             WinG_DC, VQ_MOVIE_X, VQ_MOVIE_Y);
...
VOID Debug_Draw_Line_Test() {
  ...
  for (x1 = 0; x1 <= 638; x1 += 10) {
    ...
    WinGBitBlt(Screen_DC, min(x1, x2), min(y1, y2), abs(x2 - x1) + 1,
               abs(y2 - y1) + 1, WinG_DC, min(x1, x2), min(y1, y2));
    x2 -= 10;
  }
  x1 = 0;
  //	y1=0;
...
VOID Debug_Put_Pixel_Test() {
  ...
  for (i = 0; i < 100000; i++) {
    ...
    WinGBitBlt(Screen_DC, x, y, 1, 1, WinG_DC, x, y);
...
VOID Debug_Print_Game_Stats() {
  ...
  for (i = PLAYER_1; i < TOTAL_PLAYERS; i++) {
    if (Players[i].Is_Active()) {
      ...
      if (Players[i].Get_AI_Type() >= AI_FIRST) {
        LoadString(Global_Instance_Handle,
                   STRING_AI_FIRST + Players[i].Get_AI_Type(), temp_ai_string,
                   50);
      } else {
        *temp_ai_string = '\0';
      }

      sprintf(Text_String, "%s%s, $%d, %s ", Text_String_2,
              Players[i].Get_Name(), Players[i].Get_Cash(), temp_token_string);
      ...
      */
      ...
      TextOut(Screen_DC, x, y, Text_String, strlen(Text_String));
      y += DEBUG_TEXT_SPACING;
      sprintf(Text_String, "     Liquid Assets: $%u       ",
              Players[i].Get_Liquid_Assets());
      TextOut(Screen_DC, x, y, Text_String, strlen(Text_String));
      y += DEBUG_TEXT_SPACING;
      sprintf(Text_String, "     Available Assets: $%u       ",
              Players[i].Get_Available_Assets());
      TextOut(Screen_DC, x, y, Text_String, strlen(Text_String));
      //			y += DEBUG_TEXT_SPACING;
  ...
  */
  ...
  sprintf(Text_String, "Free Parking: $%d, %s      ",
          Game_Status.Get_Free_Parking_Money(), Text_String_2);
  ...
  */
  y += DEBUG_TEXT_SPACING;
  sprintf(Text_String, "Bank: %d houses, %d hotels, $infinite      ",
          Bank.Get_Houses(), Bank.Get_Hotels());
  TextOut(Screen_DC, x, y, Text_String, strlen(Text_String));
}
#endif
...
VOID Debug_Adjust_Players_Cash(WindowHandle window_handle) {
  ...
  DialogBox(Global_Instance_Handle, "DebugSetCashDlgBox", window_handle,
  ...
  switch (message) {
    case WM_COMMAND:
      ...
      if (LOWORD(w_param) == IDOK) {
        GetDlgItemText(window_handle, CASH_EDIT1, Text_String,
                       SIZE_OF_TEXT_STRING);
        ...
        sprintf(Text_String, "%s now has $%d.\r\n", Players[player].Get_Name(),
                cash);
        Debug_Mono_Message(Text_String);
    ...
    case WM_INITDIALOG:
      ...
      */
      ...
      SetDlgItemText(window_handle, CASH_EDIT1, Text_String);
      SendDlgItemMessage(window_handle, CASH_EDIT1, EM_SETSEL, (WPARAM)0,
                         (LPARAM)-1);
      return (TRUE);
...
VOID Debug_Show_Chance_Deck() {
  ...
  for (i = 0; i < TOTAL_CHANCE_CARDS_IN_MAX_DECK; i++) {
    ...
    if (card > CHANCE_NO_CARD) {
      sprintf(Text_String, "Chance card[ %d ] = %s\r\n", i,
              Debug_Chance_Card_Text[card]);
    } else {
      sprintf(Text_String, "Chance card[ %d ] = -1\r\n", i);
    }
    Debug_Mono_Message(Text_String);
  }
}
...
VOID Debug_Show_CC_Deck() {
  ...
  for (i = 0; i < TOTAL_COMMUNITY_CHEST_CARDS_IN_MAX_DECK; i++) {
    ...
    if (card > CC_NO_CARD) {
      sprintf(Text_String, "C. Chest card[ %d ] = %s\r\n", i,
              Debug_CC_Card_Text[card]);
    } else {
      sprintf(Text_String, "C. Chest card[ %d ] = -1\r\n", i);
    }
    Debug_Mono_Message(Text_String);
  }
}
...
VOID Debug_Show_Player_Info() {
  ...
  for (i = FIRST_PLAYER; i <= LAST_PLAYER; i++) {
    if (Players[i].Get_Active_Status() != PLAYER_INACTIVE) {
      sprintf(Text_String, "Player[ %d ] - %s | %s | %s | %s | %s\r\n", i,
              tokens[Players[i].Get_Token()], ais[Players[i].Get_AI_Type()],
              Players[i].Get_IP_Address(),
              status[Players[i].Get_Active_Status()], Players[i].Get_Name());
    } else {
      sprintf(Text_String, "Player[ %d ] INACTIVE\r\n", i);
    }
    Debug_Mono_Message(Text_String);
  }
  Debug_Mono_Message(
...
VOID Debug_Broadcast_Save_Game_Checksum_Other_Sites() {
  ...
  */
  ...
  sprintf(file_name, "%s%s", Executable_Path, "TEMPSEND.TMP");
  ...
  */
  ...
  if (fh != -1) {
    ...
    if (buffer == NULL) {
      Close_File(fh);
#if (DEBUG_CODE)
      Debug_Error_Window_Message(
          "Unable to alloc buffer in "
  ...
  */
  ...
  sprintf(Text_String,
          ...
          file_name, check_sum, low_int, high_int);
  Debug_Mono_Message(Text_String);
#endif
  ...
  */
  WinSock_Broadcast_Command(CMD_SENDING_CHECKSUM_DATA, 3, (INT)file_size,
                            low_int, high_int);
...
#if (DEBUG_CHECKSUM_ALL_SITES_DATA)
#if (COMPILE_WINSOCK)
VOID Debug_Compare_Local_Checksum_With_Received_Checksum(INT received_file_size,
                                                         INT low_int,
                                                         INT high_int) {
  INT fh;
  ...
  */
...
#if (DEBUG_CODE)
  sprintf(Text_String,
          "Getting ready to compare other sites file, size = %d, checksum = "
          "%ld\r\n",
  ...
  sprintf(file_name, "%s%s", Executable_Path, "TEMPSAVE.TMP");
  ...
  */
  if (local_file_size != received_file_size) {
    sprintf(Text_String,
            "ERROR! TEMPSAVE.TMP (local, %d) & TEMPSEND.TMP (remote, %d) file "
            "sizes don't match!\r\n",
...
#if (DEBUG_CODE)
    sprintf(Text_String,
            "ERROR! TEMPSAVE.TMP (local, %ld) & TEMPSEND.TMP (remote, %ld) "
...
#if (DEBUG_CODE)
  sprintf(Text_String,
          "TEMPSAVE.TMP (local, %ld) & TEMPSEND.TMP (remote, %ld) checksums "
          "match!\r\n",
          local_check_sum, received_check_sum);
  Debug_Mono_Message(Text_String);
#endif
...
PRIVATE VOID Give_To_The_Mikes() {
  ...
  Real_Error_Window_Message(
      "...and then give it to the Mike's for error checking!");
}
#endif
...
VOID Build_Debug_Log_File_Name(BYTE* name_string_buffer,
  ...
  */
  ...
  //				data.wMinute,
  //				data.wSecond );
  sprintf(name_string_buffer, "%2d%2d%2d%2d.%3d", data.wMonth, data.wDay,
          data.wHour, data.wMinute, 0);
  ...
  */
  sprintf(Text_String_2, "%sMONOPOLY.INI", Executable_Path);

  /*
          now get the system id from "monopoly.ini" in Text_String_3, error if
     not in there
  */
  if (!Find_File(Text_String_2)) {
    sprintf(Text_String, "%s not found!", Text_String_2);
    Debug_Error_Window_Message(Text_String);
    return;
  }
  ...
  */
  GetPrivateProfileString("Monopoly", "DebugSystemName",
                          "No DebugSystemName found!", Text_String_3,
                          SIZE_OF_TEXT_STRING, Text_String_2);
...
*/
  ...
  */
  ...
  sprintf(Text_String,
          ...
          "\r\n---System: %s \r\n",
          name_string_buffer, data.wMonth, data.wDay, data.wYear, data.wHour,
          data.wMinute, Text_String_3);
...
VOID Debug_Print_Log_File_Name_On_Board_Hid() {
  ...
  sprintf(Text_String, "Debug file: %s", Debug_Log_File_Name);

  TextOut(WinG_DC, VQ_MOVIE_X + 2, VQ_MOVIE_Y - 2, Text_String,
          strlen(Text_String));

  sprintf(Text_String, "System name: %s", Debug_System_ID_Name);

  TextOut(WinG_DC, VQ_MOVIE_X + 2, VQ_MOVIE_Y - 2 + 10, Text_String,
          strlen(Text_String));

  sprintf(Text_String, "Vers: %s", VERSION_NUMBER_STRING);

  TextOut(WinG_DC, VQ_MOVIE_X + 2, VQ_MOVIE_Y - 2 + 20, Text_String,
          strlen(Text_String));
#endif
}
#endif  // DEBUG_CODE
...
BOOL Get_Windows_System_Info() {
  ...
  if (GetVersionEx(&version_info)) {
    sprintf(Text_String, "Windows OS version: %ld.%ld, build #%ld, %s\r\n",
            version_info.dwMajorVersion, version_info.dwMinorVersion,
            version_info.dwBuildNumber, version_info.szCSDVersion);
    Debug_Mono_Message(Text_String);
    ...
    if (version_info.dwPlatformId == VER_PLATFORM_WIN32s) {  // 0
      ...
      */
      ...
      sprintf(Text_String, "%ld.%ld", major, minor);
      sscanf(Text_String, "%f", &float_version);
      //			if ( major < REQUIRED_WIN32S_MAJOR_VERSION ||
      //				( major == REQUIRED_WIN32S_MAJOR_VERSION
      ...
      if (float_version < REQUIRED_WIN32S_FLOAT_VERSION) {
        ...
        */
        Real_Error_Window_Message_String(
            "Win32s version 1.25 or greater is required. Run SETUP.EXE from "
            "the Monopoly CD-ROM CD.");
  ...
  */
  ...
  sprintf(Text_String, "Executable_Path[] = %s\r\n", Executable_Path);
  Debug_Mono_Message(Text_String);

  sprintf(Text_String, "CD_Root_Path[] = %s\r\n", CD_Root_Path);
  Debug_Mono_Message(Text_String);
...
BYTE* Debug_Get_Time_String() {
  ...
  */
  ...
  //				data.wMinute,
  //				data.wSecond );
  sprintf(_time_string, "%2d:%2d", data.wHour, data.wMinute);
      ...
      if (j != i) {
        if (Players[j].Get_Token() == token) {
          ...
          */
          ...
          Debug_Error_Window_Message(
              "Emergency! The annoying Double-token bug has been "
              "encountered!\r\n");
          Debug_Error_Window_Message(
              "Give the .000 log file from this system/game to the Mikes!");
          Debug_Error_Window_Message(
              "Get the .000 log files from the other systems in this game, "
              "too.");
          Debug_Error_Window_Message(
              "Other data may be hosed, so this game is probably now invalid.");
          Debug_Error_Window_Message(
              "With the log file, we should be able to find the stinkin' bug.");
...
VOID Debug_Rig_Roll(WindowHandle window_handle) {
  ...
  DialogBox(Global_Instance_Handle, "DebugDiceDlgBox", window_handle,
...
PRIVATE BOOL Debug_Rig_Roll_Dialog_Procedure(WindowHandle window_handle,
  ...
  switch (message) {
    case WM_COMMAND:
      ...
      if (LOWORD(w_param) == IDOK) {
        GetDlgItemText(window_handle, DIE_EDIT1, Text_String,
                       SIZE_OF_TEXT_STRING);
        die1 = atoi(Text_String);

        GetDlgItemText(window_handle, DIE_EDIT2, Text_String,
                       SIZE_OF_TEXT_STRING);
...
#if (DEBUG_CODE)
          Debug_Error_Window_Message(
              "Illegal values, defaulting to 1 & 1.\r\n");
#endif
          die1 = 1;
          die2 = 1;
        }
