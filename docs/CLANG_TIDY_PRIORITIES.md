...
This tracks **all 89 currently excluded check names** and completed entries, in recommended work
...
| `modernize-use-using`                                               | Enabled | Commit `Replace legacy typedefs and enable modernize-use-using`: replace 143 declarations with named type definitions and equivalent aliases, preserving linkage names and layouts; initialize 44 locals exposed by the named-record checks. Plan and validation in [USE_USING_PLAN.md](USE_USING_PLAN.md). |
...
The original pass left `modernize-use-using` excluded (resolved by the 2026-09-16
[manual conversion](USE_USING_PLAN.md)). Its fix-its rewrite `typedef struct {...} Name;` as
