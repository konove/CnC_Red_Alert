...
/* $Header:   F:\projects\c&c\vcs\code\credits.cpv   2.17   16 Oct 1995 16:51:28
 * JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * CreditClass::AI -- Handles updating the credit display. *
 *   CreditClass::Graphic_Logic -- Handles the credit redraw logic. *
 *   CreditClass::CreditClass -- Default constructor for the credit class
 *object.              *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 **
 *                                                                                             *
 *    This is the constructor for the credit class object. It merely sets the
 *credit display   * state to null. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * CreditClass::Graphic_Logic -- Handles the credit redraw logic. *
 *                                                                                             *
 *    This routine should be called whenever the main game screen is to be
 *updated. It will    * check to see if the credit display should be redrawn. If
 *so, it will redraw it.          *
 *                                                                                             *
 * INPUT:   forced   -- Should the credit display be redrawn regardless of
 *whether the redraw  * flag is set? This is typically the case when the screen
 *needs to be    * redrawn from scratch. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
 ...
 *                                                                                             *
 *    This routine handles the logic that controls the rate of credit change in
 *the credit     * display. It doesn't actually redraw the credit display, but
 *will flag it to be redrawn   * if it detects that a change is to occur. *
 *                                                                                             *
 * INPUT:   forced   -- Should the credit display immediately reflect the
 *current credit       * total for the player? This is usually desired when
 *initially loading   * a scenario or saved game. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
