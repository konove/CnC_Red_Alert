 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Overview: * This file contains stubs for undefined externals when linked
 *under Watcom for Win 95      *
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 *                                                                                             *
 * Functions: *
...
long FAR PASCAL _export Windows_Procedure(HWND hwnd, UINT message, UINT wParam,
  ...
  switch (message) {
    ...
    case WM_DESTROY:
      ...
      PostQuitMessage(0);
      /*
      ** If we are shutting down gracefully than flag that the message loop has
      *finished.
      ** If this is a forced shutdown (ReadyToQuit == 0) then try and close down
      *everything
      ** before we exit.
      */
      if (ReadyToQuit) {
    ...
    case WM_SYSCOMMAND:
      switch (wParam) {
        case SC_CLOSE:
          /*
          ** Windows sent us a close message. Probably in response to Alt-F4.
          *Ignore it by
        ...
        case SC_SCREENSAVE:
          /*
          ** Windoze is about to start the screen saver. If we just return
          *without passing
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 10/10/95 4:08PM ST : Created *
 *=============================================================================================*/
...

{
  ...
  //
  hwnd = CreateWindowEx(WS_EX_TOPMOST, "Command & Conquer", "Command & Conquer",
                        WS_POPUP | WS_MAXIMIZE, 0, 0, width, height, NULL, NULL,
                        instance, NULL);
...
void Window_Dialog_Box(HANDLE hinst, LPCTSTR lpszTemplate, HWND hwndOwner,
                       DLGPROC dlgprc) {
  MSG msg;
  /*
...
// return ((char*)0); } int IPXAddressClass::Is_Broadcast() { return (0); } int
// IPXManagerClass::Send_Global_Message( void *, int, int, IPXAddressClass * ) {
// return (0); } int IPXManagerClass::Service() { return (0); } int
// IPXManagerClass::Get_Global_Message( void  *, int  *, IPXAddressClass  *,
// short unsigned  * ) { return (0); } int IPXAddressClass::operator ==(
// IPXAddressClass  & ) { return (0); } IPXManagerClass::IPXManagerClass( int,
// int, int, int, short unsigned, short unsigned ) {}
...
// int ) { return (0); } int  IPXManagerClass::Get_Private_Message( void  *, int
// *, int  * ) { return (0); } int  IPXManagerClass::Connection_Index( int ) {
// return (0); } void  IPXManagerClass::Reset_Response_Time(){} long unsigned
...
// void VQA_Init(VQAHandle *, long(*)(VQAHandle *vqa, long action,	void
// *buffer, long nbytes)){}
