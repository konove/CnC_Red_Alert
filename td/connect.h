...
/* $Header:   F:\projects\c&c\vcs\code\connect.h_v   1.12   16 Oct 1995 16:46:04
 * JOE_BOSTIC  $ */
/***************************************************************************
 ...
 *transmitted packets.  The ConnectionClass has virtual functions to * handle
 *adding packets to the queue, reading them from the queue, * a Send routine for
 *actually sending data, and a Receive_Packet function * which is used to tell
 *the connection that a new packet has come in.		*
 *																									*
 ...
 * The header is stored with each packet in the send & receive Queues; * it's
 *removed before it's passed back to the application, via
 ** Get_Packet()
 **
 *                                                                         *
 ...
 ** incoming packets & distribute them to the connections that need to * store
 *them (for multi-connection protocols).
 **
 *                                                                         *
 * NOTES ON ACK/RETRY:
 ...
 **
 *                                                                         *
 * If the protocol being used already guarantees delivery of packets, * no ACK
 *is required for the packets.  In this case, the connection * class for this
 *protocol can overload the Service routine to avoid			*
 ...
 **
 * - Init						a version of Init that
 *gives the connection 		* access to any hardware-specific values it
 *needs	* Must chain to the parent's Init routine.			*
 * - Send_Packet				adds the CommHeaderType header,
 *adds the packet	* to the out-going queue
 **
 * - Receive_Packet			processes incoming ACK packets, detects
 *resends,* adds new packets to the in-coming queue			*
 * - Get_Packet				reads the next-available packet from the
 ** receive queue
 **
 * - Send						the hardware-dependent
 *data-sending routine		*
 * - Service_Send_Queue		services the send queue; handles re-sends,
 ** detects when outgoing packets have been ACK'd;	* cleans out the queue
 *of old packets					*
 * - Service_Receive_Queue	services the receive queue; handles sending
 ** ACK's for new or re-sent packets; cleans out		* the queue of
 *old packets								*
 *																									*
 ...
 *|                      |			* |
 *|                      |			* IPXGlobalConnClass
 *NullModemConnClass       ModemConnClass	*
 *																									*
 *																									*
 *	ConnectionClass:
 ** Abstract base class.
 ...
 **
 *					 * Send_Packet(): adds header to packet,
 *adds it to Queue	*
 *					 * Receive_Packet(): adds incoming
 *packet to receive Queue, * handles incoming ACK's & resends
 **
 *					 * Get_Packet(): gets packet from the
 *receive queue			*
 *																									*
 ...
 *					   Service_Send_Queue() &
 *Service_Receive_Queue() to do		* their job
 **
 *					 * Ability to associate an IPX Address,
 *a numerical ID, and	* a character-string Name with a connection
 ** Inherits: * Sequenced ACK/Retry logic, Service routines, Queue & *
 *						PacketBuf, timeout variables
 **
 *																									*
 *	IPXGlobalConnClass:
 ** Special type of IPX Connection; supports receiving packets from * multiple
 *systems at once, and sending packets via Broadcast or			* to a
 *specific address.
 ** Provides: * Specialized Receive_Packet() routine, which handles * receiving
 *packets from multiple systems						*
 *					 * Specialized Send_Packet() &
 *Get_Packet() routines,			* which pass IPX address of
 *destination through to			* the application, giving the
 *application control over		* whether the packet will be Broadcast
 *or sent to a 			* specific destination (embeds destination
 *address within	* the packet itself)
 **
 *					 * Specialized Send routine, which
 *extracts the destination	* address from the packet
 ** Inherits: * Sequenced ACK/Retry logic, Service routines, Queue & *
 *						PacketBuf, timeout variables,
 *IPX-specific routines		*
 *																									*
 *	NullModemConnClass:
 ** Provides: * Hardware-dependent Serial-communication routines, which	* allow
 *Service_Send_Queue() & Service_Receive_Queue() to 	* do their job
 ** Inherits: * Non-Sequenced ACK/Retry logic, Service routines, Queue &
 ** PacketBuf, timeout variables
 **
 *																									*
 *	ModemConnClass:
 ** Provides: * Hardware-dependent Modem-communication routines, which	* allow
 *Service_Send_Queue() & Service_Receive_Queue() to 	* do their job
 ** Inherits: * Non-Sequenced ACK/Retry logic, Service routines, Queue &
 ** PacketBuf, timeout variables
 **
...
MagicNumber:	This is a number unique to the application; it's up to the
                                        Receive_Packet routine to check this
value, to be sure we're not getting data from some other product.  This value
should be unique for each application. Code:				This
will be one of the below-defined codes. PacketID:		This is a unique
...
class ConnectionClass {
 ...
 protected:
  ...
  .....................................................................*/
  virtual int Service_Send_Queue() = 0;
  virtual int Service_Receive_Queue() = 0;
