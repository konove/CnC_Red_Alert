    ...
    - up help text handler
       ...
       ÚÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄ¿
          AnimClass  ³ TemplateClass    ³        ÃÄ FuseClass     ³ TerrainClass
              ³                   ³        ÃÄ FlyClass      ³
              ³                   ³ BulletClass            ³ OverlayClass
          MissionClass SmudgeClass
                                  ³ RadioClass
                                  ³
                                  ÃÄ CrewClass
                                  ÃÄ FlasherClass
                                  ...
                                  ³
         ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿ FootClass
          BuildingClass
         ³
         ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄ¿ DriveClass
          InfantryClass         ÃÄ FlyClass
...
// inline CELL Coord_Cell(COORD coord){return (CELL)(((*(((unsigned short
// *)&coord)+1) & 0xFF00) >> 2) | *(((unsigned char *)&coord)+1));}
...
void Conquer_Clip_Text_Print(char const *, unsigned x, unsigned y,
                             ...
                             unsigned width = -1, int const *tabs = 0);
...
**	FINDPATH.CPP
*/
// PathType * Find_Path(CELL source, CELL dest, FacingType *final_moves, int
// maxlen, int (*callback)(CELL, FacingType), int threshhold);
...
void Bit_It_In_Scale(int x, int y, int w, int h, GraphicBufferClass *src,
                     GraphicBufferClass *dest, GraphicBufferClass *seen,
...
void *__cdecl Conquer_Build_Fading_Table(void const *palette, void *dest,
                                         int color, int frac);
void __cdecl Fat_Put_Pixel(int x, int y, int color, int size,
