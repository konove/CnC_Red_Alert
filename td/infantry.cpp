...
/* $Header:   F:\projects\c&c\vcs\code\infantry.cpv   2.19   16 Oct 1995
 * 16:50:30   JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 *once each      * cell travelled. This includes radioing a transport that is is
...
void InfantryClass::Per_Cell_Process(bool center) {
  ...
  */
  ...
  if (center && Mission == MISSION_ENTER && techno &&
  ...
  */
  if (IsActive && !IsInLimbo && center &&
      cellptr->Land_Type() == LAND_TIBERIUM && *this != INFANTRY_E5) {
    int damage = 2;
 ...
 *                                                                                             *
 *    This support routine is used to convert the infantry object (as a pointer)
 *into a target * value (which is a number). *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * InfantryClass::Look -- The infantry performs a look operation. *
 *                                                                                             *
 *    This routine will cause the infantry unit to "look". For player owned
 *infantry, this     * causes the dark shroud to be pushed back. *
 *                                                                                             *
 * INPUT:   incremental -- If it is known that the infantry performed a look in
 *the last cell  * it was in AND it has only moved one cell, then setting this *
 *                         parameter to true will perform a faster "incremental"
 *look          * operation. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
  ...
  */
  ...
  if (Doing == DO_NOTHING || Fetch_Stage() >= Class->DoControls[Doing].Count) {
    switch (Doing) {
      ...
      **	decay animation since there is no custom animation available -
      *yet.
      */
      case DO_PUNCH_DEATH:
      case DO_KICK_DEATH:
...
short const *InfantryClass::Overlap_List(void) const {
  ...
  return (Coord_Spillage_List(Coord,
                              24 + ((Doing > DO_WALK || IsSelected) ? 12 : 0)));
...
void InfantryClass::Random_Animate(void) {
  ...
  if (!IsDriving && !IsProne &&
    ...
    */
    ...
    /*
    **	If Nikoomba is not scared, then he will be doing his thing with random
    *animations.
    */
    if (*this == INFANTRY_C10) {
    ...
    switch (Random_Picky((int)0, (int)55, (char *)NULL, (int)0)) {
      ...
      case 2:
        Do_Action(DO_IDLE2);
        PrimaryFacing.Set(Facing_Dir(Random_Pick(FACING_N, FACING_NW)));
        if (!IsSelected && IsOwnedByPlayer && *this == INFANTRY_RAMBO &&
...
#ifdef NEVER
/***********************************************************************************************
 * InfantryClass::Set_Primary_Facing -- Change infantry primary facing -- always
 *and instantly *
 *                                                                                             *
 *    This routine ensures that when the infantry primary facing is changes, it
 *is changed     * instantly and always. There is no provision for infantry
 *facing changing slowly over     * time as the other vehicles usually do. *
 *                                                                                             *
...
BulletClass *InfantryClass::Fire_At(TARGET target, int which) {
  ...
  if (IsBoxing) {
    ...
    /*
    **	When fighting, verify that the target is legal to proceed. If there is
    *some *	error, then abort fightning mode. Otherwise, tell the target
    *that it has *	just been hit.
    */
    if (In_Radio_Contact() && Target_Legal(target) &&
        Transmit_Message(hitaction) == RADIO_ROGER) {
      Arm = Rearm_Delay(true);
    } else {
      /*
...
TARGET InfantryClass::Greatest_Threat(ThreatType threat) const {
  Validate();
  /*
  **	Engineers consider only buildings that can be captures as being a
  *threat. All others *	are ignored.
  */
 ...
 *audio repsonse    * generated by the infantry unit. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
...
/***********************************************************************************************
 * InfantryClass::Fire_Coord -- Calculates the origin point for projectiles
 *fired.             *
 *                                                                                             *
 *    This routine will return with the point of origin for any firing
 *projectiles. Typically, * this only includes the rocket launcher and the
 *grenade thrower. The other infantry       * have either invisible projectiles
 *or special animations.                                 *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the coordinate where the projectile will appear as it
 *is fired.       *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
RadioMessageType InfantryClass::Receive_Message(RadioClass *from,
  ...
  switch (message) {
    ...
    /*
    **	Request a fisticuff fight. If this infantry is already involved in a
    *fight, *	then refuse the offer.
    */
...
ActionType InfantryClass::What_Action(ObjectClass *object) const {
  ...
  /*
  **	First see if it's a commando, and if he's attacking a building, have him
  *return ACTION_SABOTAGE instead
  */
    ...
    if (object->Owner() != Owner() &&
        ((object->What_Am_I() == RTTI_AIRCRAFT &&
         ...
         (object->What_Am_I() == RTTI_BUILDING &&
          ...
          (GameToPlay != GAME_NORMAL ||
           *((BuildingClass *)object) != STRUCT_EYE || Scenario < 13)))) {
 ...
 *                                                                                             *
 *    This routine writes all of the infantry in the game out to an INI file.
 *This is used     * in the scenario editor when the game needs to be saved. *
 *                                                                                             *
 *    INI entry format: * Housename, Typename, Strength, Cellnum,
 *CellSublocation, Missionname,                  * Facingnum, Triggername *
 *                                                                                             *
 ...
 * InfantryClass::Made_A_Kill -- Marks a kill caused by this infantry soldier. *
 *                                                                                             *
 *    When the infantry soldier is responsible for a kill, this routine is
 *called. It checks   * to see if the soldier should make some comment or
 *perform some action. The commando      * infantry is most likely to respond. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
