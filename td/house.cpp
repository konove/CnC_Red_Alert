...
/* $Header:   F:\projects\c&c\vcs\code\house.cpv   2.13   02 Aug 1995 17:03:50
 * JOE_BOSTIC  $ */
/***********************************************************************************************
...
HouseClass::HouseClass(HousesType house)
    : Class(&HouseTypeClass::As_Reference(house)),
      IonCannon(ION_CANNON_GONE_TIME, VOX_ION_READY, VOX_ION_CHARGING,
                VOX_ION_CHARGING, VOX_NO_POWER),
      AirStrike(AIR_CANNON_GONE_TIME, VOX_AIRSTRIKE_READY, VOX_NONE,
...
bool HouseClass::Can_Build(TechnoTypeClass const *type,
  ...
  */
  ...
  /*
  **	GDI has a special second training mission. Adjust the scenario level so
  *that *	scenario two will still feel like scenario #1.
  */
 ...
 * HouseClass::Can_Build -- Determines if the building type can be built. *
 *                                                                                             *
 *    This routine is used by the construction preparation code to building a
 *list of building * types that can be built. It determines if a building can be
 *built by checking if the     * prerequisite buildings have been built (and
 *still exist) as well as checking to see if   * the house can build the
 *specified structure.                                             *
 *                                                                                             *
 * INPUT:   s     -- The structure type number that is being checked. *
 *                                                                                             *
 *          house -- The house number to use when determining if the object can
 *be built.      * This is necessary because the current owner of the factory
 *doesn't        * control what the factory can produce. Rather, the original
 *builder of     * the factory controls this. *
 *                                                                                             *
 * OUTPUT:  bool; Can this structure type be built at this time? *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 06/08/1994 JLB : Created. * 05/31/1995 JLB : Handles specified
 *ownership override.                                    *
 *=============================================================================================*/
...
/***********************************************************************************************
 * HouseClass::Can_Build -- Determines if the infantry unit can be built by this
 *house.        *
 *                                                                                             *
 *    Use this routine to determine if the infantry type specified can be built
 *by this        * house. It determines this by checking the ownership allowed
 *bits in the infantry         * type class. *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   It does not check to see if there is a functional barracks
 *available, but       * merely checks to see if it is legal for this house to
 *build that infantry       * type. *
 *                                                                                             *
 * HISTORY: * 12/09/1994 JLB : Created. * 05/31/1995 JLB : Handles specified
 *ownership override.                                    *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    This routine is used to determine if the unit type specified can in fact
 *be built by     * this house. It checks the ownable bits in the unit's type to
 *determine this.             *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   This doesn't check to see if there is a functional factory that
 *can build       * this unit, but merely if the unit can be built according to
 *ownership rules.    *
 *                                                                                             *
 * HISTORY: * 12/09/1994 JLB : Created. * 05/31/1995 JLB : Handles specified
 *ownership override.                                    *
 ...
 * HouseClass::Can_Build -- Determines if the aircraft type can be built. *
 *                                                                                             *
 *    Use this routine to determine if the specified aircraft type can be built.
 *This routine  * is used by the sidebar and factory to determine what can be
 *built.                       *
 *                                                                                             *
 * INPUT:   aircraft -- The aircraft type to check for build legality. *
 *                                                                                             *
 *          house    -- The house that is performing the check. This is
 *typically the house    * of the original building of the factory rather than
 *the current        * owner. *
 *                                                                                             *
  ...
  if (IonCannon.Is_Present()) {
    if (!(ActiveBScan & STRUCTF_EYE) && !IonCannon.Is_One_Time()) {
      ...
      */
      ...
      /*
      **	Turn the ion cannon suspension on or off depending on the
      *available *	power. Note that one time ion cannon will not be
      *affected by this.
      */
  ...
  if (NukeStrike.Is_Present()) {
    if (!(ActiveBScan & STRUCTF_TEMPLE) &&
      ...
      */
      ...
      /*
      **	Turn the nuke strike suspension on or off depending on the
      *available *	power. Note that one time nuke strike will not be
      *affected by this.
      */
 ...
 * HouseClass::Write_INI -- Writes house specific data into INI file. *
 *                                                                                             *
 *    Use this routine to write the house specific data (for all houses) into
 *the INI file.    * It is used by the scenario editor when saving the scenario.
 **
 *                                                                                             *
  ...
  for (HousesType i = HOUSE_FIRST; i < HOUSE_COUNT; i++) {
    ...
    if (p) {
      ...
      WWWritePrivateProfileInt(p->Class->IniName, "MaxUnit", p->MaxUnit,
                               buffer);
      WWWritePrivateProfileInt(p->Class->IniName, "MaxBuilding", p->MaxBuilding,
                               buffer);
 ...
 *                                                                                             *
 *    This routine will initiate the ion cannon charging countdown. It will add
 *the ion        * cannon to the sidebar if it isn't there and it is specified
 *to be added.                 *
 *                                                                                             *
 * INPUT:   first_time  -- Set to true if the ion cannon must be added to the
 *sidebar.         *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will disable the ion cannon. It is called when the ion cannon
 *cannot        * establish a command link to the ground (usually when there is
 *insufficient power).       *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *sidebar.             *
 *                                                                                             *
 *    Use this routine whenever a piece of atom bomb has been discovered (also
 *at scenario     * start). It will add the nuclear bomb button to the sidebar
 *if necessary.                 *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * HouseClass::Remove_Nuke_Bomb -- Removes the nuclear bomb from the sidebar. *
 *                                                                                             *
 *    This routine will remove the nuclear bomb from the sidebar. It should be
 *called when     * the nuclear strike has been launched. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 **
 *                                                                                             *
 *    This routine will activate (add if so indicated) the air strike button to
 *the sidebar.   * Call this routine when events indicate that a special air
 *strike is available.           *
 *                                                                                             *
 * INPUT:   first_time  -- If the air strike button is to be added, then this
 *will be true.    *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 *sidebar.            *
 *                                                                                             *
 *    This routine will remove the air strike button from the sidebar. Call this
 *routine when  * the air strike has been launched. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * HouseClass::Make_Air_Strike_Available -- Make the airstrike available. *
 *                                                                                             *
 *    This routine will make the airstrike available. Typically, this results in
 *a new icon    * added to the sidebar. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will a the specified nuclear piece to the house collection of
 *parts. When   * all the pieces have been added, a nuclear strike ability is
 *made available.              *
 *                                                                                             *
 * INPUT:   piece -- The nuclear piece to add. If equal to "-1", then the next
 *possible piece  * is added. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   none *
    ...
    case RTTI_UNITTYPE:
      if (CurUnits < MaxUnit) {
        ...
        */
        ...
        /*
        **	Team types that are flagged as prebuilt, will always try to
        ...
        */
        ...
        /*
        **	Pick to build the most needed object but don't consider those
        *object that *	can't be built because of scenario restrictions or
        *insufficient cash.
        */
        int bestval = -1;
        int bestcount = 0;
        ...
        /*
        **	The unit type to build is now known. Fetch a pointer to the
        *techno type class.
        */
        if (bestcount) {
    ...
    case RTTI_INFANTRYTYPE:
      if (CurUnits < MaxUnit) {
          ...
          if (team) {
            if (team->House == Class->House && team->IsPrebuilt &&
              ...
              for (int subindex = 0; subindex < team->ClassCount; subindex++) {
                if (team->Class[subindex]->What_Am_I() == RTTI_INFANTRYTYPE) {
                  int subtype =
                      ((InfantryTypeClass const *)(team->Class[subindex]))
                          ->Type;
                  //									counter[subtype]
                  //= 1;
                  counter[subtype] = std::max<int>(counter[subtype],
        ...
        */
        ...
        /*
        **	The infantry type to build is now known. Fetch a pointer to the
        *techno type class.
        */
        if (bestcount) {
 ...
 **
 *                                                                                             *
 *    This routine checks to see if the house has a nuclear device to launch. A
 *nuclear        * device is available when the necessary parts have been
 *retrieved in earlier scenarios    * or if this is the multiplayer version. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Does the house have a nuclear device? *
 *                                                                                             *
 * WARNINGS:   This does not check to see if there is a suitable launch facility
 *(i.e., the    * Temple of Nod), only that there is a nuclear device potential.
 **
 *                                                                                             *
      ...
      if (optr.IsWall) {
        ...
        switch (overlay) {
          case OVERLAY_SANDBAG_WALL:
            cost =
                BuildingTypeClass::As_Reference(STRUCT_SANDBAG_WALL).Cost_Of();
            break;

          case OVERLAY_CYCLONE_WALL:
            cost =
                BuildingTypeClass::As_Reference(STRUCT_CYCLONE_WALL).Cost_Of();
            break;
          ...
          case OVERLAY_BARBWIRE_WALL:
            cost =
                BuildingTypeClass::As_Reference(STRUCT_BARBWIRE_WALL).Cost_Of();
            break;
