...
/* $Header:   F:\projects\c&c\vcs\code\team.cpv   2.18   16 Oct 1995 16:48:34
 * JOE_BOSTIC  $ */
/***********************************************************************************************
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Assign_Mission_Target -- Sets teams mission target and clears
 *old target                  * TeamClass::Add -- Adds specified object to team.
 ** TeamClass::AI -- Process team logic. * TeamClass::As_Target -- Converts this
 *team object into a target number.                   * TeamClass::Calc_Center
 *-- Determines average location of team members.                    *
 ...
 *index ID.     * TeamClass::Remove -- Removes the specified object from the
 *team.                          * TeamClass::Suspend_Teams -- Suspends activity
 *for low priority teams                      * TeamClass::Took_Damage --
 *Informs the team when the team member takes damage.             *
  ...
  */
  if (IsAltered) {
    ...
    if (Total) {
      ...
      *ensures *	that no new team members will be recruited and the team
      *won't go into *	regroup logic.
      */
      if (House->IsHuman) {
        IsUnderStrength = false;
      } else {
        /*
        **	Reinforcable teams will revert (or snap out of) the under
 ...
 *team.             *
 *                                                                                             *
 *          typeindex-- Optional value that specifies the index in the team type
 *class array   * that this object belongs. *
 *                                                                                             *
...
bool TeamClass::Add(FootClass* obj, int typeindex) {
  ...
  *class. *	On the chance that a match could not be found, then it is
 ...
 *object type has either       * been exhausted or if the team's requirment for
 ...
 *                                                                                             *
 *    This routine is used by the save/load code to produce a persistant
 *identifier for this   * team object. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the cell number of the team's center point. If the team
 *contains      * no members, then the return value will be zero. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/29/1994 JLB : Created. *
  ...
  while (unit) {
    ...
    if (unit->IsInitiated && !unit->IsInLimbo) {
      if (unit->Distance(Center) > STRAY_DISTANCE &&
        ...
        *is *	now considered intiated. An initiated unit is considered when
      ...
      while (unit) {
        ...
        if (unit->IsInitiated && !unit->IsInLimbo) {
          if (unit->What_Am_I() != RTTI_AIRCRAFT &&
              unit->Distance(Center) > STRAY_DISTANCE) {
            IsLagging = true;
            finished = false;
          } else {
    ...
    if (!unit->IsInLimbo) {
      ...
      */
      if (unit->Distance(ObjectiveCenter) > STRAY_DISTANCE) {
        if (unit->Mission != MISSION_MOVE || !Target_Legal(unit->NavCom) ||
            ::Distance(As_Cell(unit->NavCom), ObjectiveCenter) >
                STRAY_DISTANCE) {
          unit->Assign_Mission(MISSION_MOVE);
