...
/* $Header:   F:\projects\c&c\vcs\code\audio.cpv   2.17   16 Oct 1995 16:50:20
 * JOE_BOSTIC  $ */
/***********************************************************************************************
...
void Sound_Effect(VocType voc, COORDINATE coord, int variation) {
  ...
  if (coord && !Map.In_View(cell_pos)) {
    ...
    distance = (unsigned int)std::min((int)distance, MAP_CELL_W);
  ...
  Sound_Effect(voc, (VolType)Fixed_To_Cardinal(distance, Options.Volume),
               variation, pan_value);
}
