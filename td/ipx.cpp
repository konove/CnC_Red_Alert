...
/* $Header:   F:\projects\c&c\vcs\code\ipx.cpv   2.17   16 Oct 1995 16:49:34
 * JOE_BOSTIC  $ */
/***************************************************************************
 ...
 ** from Client/Server LAN Programming			* Westwood-ized by Bill
 *Randolph				*
 *                                                                         *
 ...
 * - It turns out that IPX is pretty bad about saving registers, so if * you
 *have any register variables in your program, they may get * trashed.  To
 *circumvent this, all functions in this module save &		* restore the
 *registers before invoking any IPX or NETX function.			*
 *                                                                         *
 *-------------------------------------------------------------------------*
 * Functions:                                                              *
 ...
 ** values from 0x4000 to 0x8000 for custom socket numbers.  The app * must know
 *its own socket number as well as the socket number of		* a destination
 *workstation.
 **
 *                                                                         *
 * HISTORY:                                                                *
 ...
 *                                                                         *
 * OUTPUT:                                                                 *
 *		first-found Connection Number for that user, 0 if user not
 *logged in	*
 *                                                                         *
 * WARNINGS:                                                               *
 *                                                                         *
 ...
 ** a Network Number and Node Address; then, place those numbers in the * IPX
...
int IPX_Get_Internet_Address(int connection_number,
  ...
  replybuf->len = 12;
  replybuf->network_number[0] =
      replybuf->network_number[0];                          // suppress warning
 ...
 *                                                                         *
 * INPUT:                                                                  *
 *		connection_number		Connection Number to get User ID
 *for				* user_id
 *ptr to buffer to put User ID into; 					* size
 *must be >= 48 chars *
 *                                                                         *
 * OUTPUT:                                                                 *
 *		0 = OK, -1 = error
 ...
 ** SocketNumber:				must contain the socket you've
 *opened,			* and are "listening" on
 ** Event_Service_Routine:	optionally points to a callback routine *
 *		PacketCount:				set to 2, to tell IPX
 *there are 2 areas to	* store the incoming data in
 ...
 *                                                                         *
 * INPUT:                                                                  *
 *		ecb_ptr		pointer to a filled-in ECB; MUST be real-mode
 *memory		*
 *                                                                         *
 * OUTPUT:                                                                 *
 ...
 *                                                                         *
 * WARNINGS:                                                               *
 *		The ECB must be located in real-mode memory, as well as the
 *values	* pointed to in the ECB (the IPX Header, the buffer to send,
 *etc.)		*
 *                                                                         *
 * HISTORY:                                                                *
 *   12/15/1994 BR : Created.                                              *
 ...
 * Before calling this function, you must fill in an ECB:
 ** SocketNumber:				must contain the socket you've
 *opened,			* and are sending on
 ** Event_Service_Routine:	optionally points to a callback routine *
 *		PacketCount:				set to 2, to tell IPX
 *there are 2 areas the	* outgoing data is stored in
 ** Packet[0].Address:		set to the address of an IPXHeaderType
 ** Packet[0].Length:			sizeof(IPXHeaderType)
 ** Packet[1].Address:		address of buffer containing data to send
 ** Packet[1].Length:			size of the data buffer
 ** ImmediateAddress:			must be filled in with the node address
 *of	* the bridge that will route the message;		* fill this in
 *by calling IPX_Get_Local_Target	*
 *                                                                         *
 ...
 ** this doesn't have to be the same as the		* socket opened on the
 *local machine.				*
 *                                                                         *
 ...
 *                                                                         *
 * OUTPUT:                                                                 *
 *		none.  This function doesn't return anything; the caller must
 *check	* its ECB.CompletionCode for: 0 = OK, IPX Error otherwise.
 **
 *                                                                         *
 * WARNINGS:                                                               *
 *		The ECB must be located in real-mode memory, as well as the
 ...
 * IPX_Get_Local_Target -- fills in ImmediateAddress field of ECB          *
 *                                                                         *
 * Use this function to fill in the ECB's ImmediateAddress field when * sending
 *a packet.  The Immediate Address is the node address of the		* bridge
 *that must route the message.  If there is no bridge, it's * filled in with the
 *destination Node Address.  In either case, it			* will contain
 *the proper value for sending.
 **
 *                                                                         *
 * INPUT:                                                                  *
